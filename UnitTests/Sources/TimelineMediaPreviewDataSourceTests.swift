...
struct TimelineMediaPreviewDataSourceTests {
  ...
  init() {
    ...
    initialMediaViewStates = initialMediaItems.map {
      RoomTimelineItemViewState(item: $0, groupStyle: .single)
    }
  }
  ...
  func currentUpdateItem() throws {
    ...
    let dataSource = TimelineMediaPreviewDataSource(
      itemViewStates: initialMediaViewStates,
    ...
    let previewItem = try #require(
      dataSource.previewController(previewController, previewItemAt: 1 + initialPadding)
        as? TimelineMediaPreviewItem.Media,
    ...
    #expect(
      dataSource.currentMediaItemID == previewItem.id,
      "The displayed item should be the initial item.")

    // When a loading item is displayed.
    guard
      let loadingItem = dataSource.previewController(
        previewController, previewItemAt: initialPadding - 1) as? TimelineMediaPreviewItem.Loading
    else {
    ...
    #expect(
      dataSource.currentItem == .loading(loadingItem),
      "The displayed item should be the loading item.")
  }
  ...
  func updatedItems() async throws {
    ...
    let deferred = deferFailure(dataSource.previewItemsPaginationPublisher, timeout: .seconds(1)) {
      _ in true
    }
    ...
    let displayedItem = try #require(
      dataSource.previewController(previewController, previewItemAt: dataSource.initialItemIndex)
        as? TimelineMediaPreviewItem.Media)
    #expect(
      displayedItem.id == initialMediaItems[initialItemIndex].id.eventOrTransactionID,
      "The displayed item should not change.")
    #expect(
      dataSource.currentMediaItemID == initialMediaItems[initialItemIndex].id.eventOrTransactionID,
      "The current item should not change.")

    #expect(
      dataSource.previewItems.count == initialMediaViewStates.count,
      "The number of items should not change.")
    #expect(
      previewItemCount == initialMediaViewStates.count + (2 * initialPadding),
      "The padded number of items should not change.")
  }
  ...
  func pagination() async throws {
    ...
    let backPaginationChunk = newChunk().map {
      RoomTimelineItemViewState(item: $0, groupStyle: .single)
    }
    ...
    let forwardPaginationChunk = newChunk().map {
      RoomTimelineItemViewState(item: $0, groupStyle: .single)
    }
  ...
  func emptyTimeline() async throws {
    ...
    #expect(
      dataSource.previewItems.count == 1, "The initial item should be in the preview items array.")
    #expect(
      previewItemCount == 1 + (2 * initialPadding),
      "The initial item count should be padded for the preview controller.")
    #expect(
      dataSource.initialItemIndex == initialPadding,
      "The initial item index should be padded for the preview controller.")

    #expect(
      displayedItem.id == initialItem.id.eventOrTransactionID,
      "The displayed item should be the initial item.")
    #expect(
      dataSource.currentMediaItemID == initialItem.id.eventOrTransactionID,
      "The current item should also be the initial item.")
    ...
    let loadedItems = initialMediaItems.map {
      RoomTimelineItemViewState(item: $0, groupStyle: .single)
    }
    ...
    #expect(
      dataSource.previewItems.count == initialMediaViewStates.count,
      "The preview items should now be loaded.")
    #expect(
      previewItemCount == 1 + (2 * initialPadding),
      "The item count should not change as the padding will be reduced.")
  ...
  func timelineUpdateWithoutInitialItem() async throws {
    ...
    let failure = deferFailure(dataSource.previewItemsPaginationPublisher, timeout: .seconds(1)) {
      _ in true
    }
    ...
    #expect(
      previewItemCount == 1 + (2 * initialPadding), "The initial item count should not change.")
    #expect(
      dataSource.initialItemIndex == initialPadding, "The initial item index should not change.")
  ...
  private func assertInitialDataSource() throws -> TimelineMediaPreviewDataSource {
    ...
    #expect(
      dataSource.initialItemIndex == initialItemIndex + initialPadding,
      "The initial item index should be padded for the preview controller.")
    #expect(
      displayedItem.id == initialMediaItems[initialItemIndex].id.eventOrTransactionID,
      "The displayed item should be the initial item.")
    ...
    #expect(
      dataSource.previewItems.count == initialMediaViewStates.count,
      "The initial count of preview items should be correct.")
    #expect(
      previewItemCount == initialMediaViewStates.count + (2 * initialPadding),
      "The initial item count should be padded for the preview controller.")
...
extension TimelineMediaPreviewDataSource {
  fileprivate var currentMediaItemID: TimelineItemIdentifier.EventOrTransactionID? {
    switch currentItem {

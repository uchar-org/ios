...
class SpaceRoomListProxy: SpaceRoomListProxyProtocol {
  ...
  init(_ spaceRoomList: SpaceRoomListProtocol) throws {
    ...
    let paginationStateSubject = CurrentValueSubject<SpaceRoomListPaginationState, Never>(
      spaceRoomList.paginationState())
    paginationStatePublisher = paginationStateSubject.asCurrentValuePublisher()

    paginationStateHandle = spaceRoomList.subscribeToPaginationStateUpdates(
      listener: SDKListener { paginationState in
        paginationStateSubject.send(paginationState)
      })

    spaceRoomsHandle = spaceRoomList.subscribeToRoomUpdate(
      listener: SDKListener { [weak self] updates in
        self?.handleUpdates(updates)
      })

    spaceServiceRoomHandle = spaceRoomList.subscribeToSpaceUpdates(
      listener: SDKListener { [weak self] spaceRoom in

...
struct TimelineItemBubbledStylerView<Content: View>: View {
  ...
  private var isPinned: Bool {
    guard context.viewState.timelineKind != .pinned,
      let eventID = timelineItem.id.eventID
    else {
      return false
    }
  ...
  var messageBubbleWithActions: some View {
    messageBubble
      ...
      .swipeRightAction {
        ...
        context.send(
          viewAction: .handleTimelineItemMenuAction(
            itemID: timelineItem.id,
            action: .reply(isThread: timelineItem.properties.isThreaded)))
      }
      .contextMenu {
        let provider = TimelineItemMenuActionProvider(
        ...
        TimelineItemMacContextMenu(item: timelineItem, actionProvider: provider) { action in
          context.send(
            viewAction: .handleTimelineItemMenuAction(itemID: timelineItem.id, action: action))
        }
      }
  ...
  var messageBubble: some View {
    contentWithReply
      .timelineItemSendInfo(
        timelineItem: timelineItem, adjustedDeliveryStatus: adjustedDeliveryStatus, context: context
      )
      .bubbleBackground(
  ...
  var contentWithReply: some View {
    TimelineBubbleLayout(spacing: 8) {
      ...
      if let replyDetails = timelineItem.properties.replyDetails {
        ...
        TimelineReplyView(
          placement: .timeline, timelineItemReplyDetails: replyDetails, maxWidth: .infinity
        )
...
extension EventBasedTimelineItemProtocol {
  ...
  fileprivate var bubbleInsets: EdgeInsets {
    ...
    case let locationTimelineItem as LocationRoomTimelineItem:
      return locationTimelineItem.content.geoURI == nil || properties.replyDetails != nil
        || properties.isThreaded ? defaultInsets : .zero
    case is LiveLocationRoomTimelineItem:
      return properties.replyDetails != nil || properties.isThreaded ? defaultInsets : .zero
    default:
      return defaultInsets
    }
  }

  fileprivate var contentCornerRadius: CGFloat {
    switch self {
    case is ImageRoomTimelineItem, is VideoRoomTimelineItem, is LocationRoomTimelineItem,
      is LiveLocationRoomTimelineItem:
...
extension EdgeInsets {
  fileprivate init(around: CGFloat) {
    self.init(top: around, leading: around, bottom: around, trailing: around)
  }

  fileprivate static var zero: Self = .init(around: 0)
}
...
extension View {
  fileprivate func pinnedIndicator(isPinned: Bool, isOutgoing: Bool) -> some View {
...
struct TimelineItemBubbledStylerView_Previews: PreviewProvider, TestablePreview {
        ...
        replyDetails: .loaded(
          ...
          eventContent: .message(
            .text(
              .init(
                body:
                  "A long message that should be on more than 2 lines and so will be clipped by the layout."
              )))))
      RoomTimelineItemView(
        viewState: .init(
          item: TextRoomTimelineItem(
  ...
  static var threadSummary: some View {
    ScrollView {
      let threadSummary = TimelineItemThreadSummary.loaded(
        senderID: "@alice:matrix.org",
        sender: .init(id: "@alice:matrix.org", displayName: "Alice"),
        latestEventContent: .message(
          .text(.init(body: "This is a very long, multi-lined, threaded message"))),
        numberOfReplies: 42)
    ...
    VStack(spacing: 0) {
      ...
      RoomTimelineItemView(
        viewState: .init(
          item: TextRoomTimelineItem(
            ...
            content: .init(body: "Short message"),
            properties: RoomTimelineItemProperties(
              encryptionAuthenticity: .unknownDevice(color: .red))),
          groupStyle: .first))
      ...
      VoiceMessageRoomTimelineView(
        ...
        playerState: AudioPlayerState(
          id: .timelineItemIdentifier(.randomEvent),

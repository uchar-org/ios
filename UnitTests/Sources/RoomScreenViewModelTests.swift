...
final class RoomScreenViewModelTests {
  ...
  func pinnedEventsBanner() async throws {
    ...
    let providerUpdateSubject = PassthroughSubject<
      ([TimelineItemProxy], TimelinePaginationState), Never
    >()
    pinnedTimelineItemProviderMock.underlyingUpdatePublisher =
      providerUpdateSubject.eraseToAnyPublisher()
    pinnedTimelineMock.timelineItemProvider = pinnedTimelineItemProviderMock
    pinnedTimelineItemProviderMock.itemProxies = [
      .event(
        .init(item: EventTimelineItem(configuration: .init(eventID: "test1")), uniqueID: .init("1"))
      ),
      .event(
        .init(item: EventTimelineItem(configuration: .init(eventID: "test2")), uniqueID: .init("2"))
      ),
    ]
    ...
    providerUpdateSubject.send(
      (
        [
          .event(
            .init(
              item: EventTimelineItem(configuration: .init(eventID: "test1")), uniqueID: .init("1"))
          ),
          .event(
            .init(
              item: EventTimelineItem(configuration: .init(eventID: "test2")), uniqueID: .init("2"))
          ),
          .event(
            .init(
              item: EventTimelineItem(configuration: .init(eventID: "test3")), uniqueID: .init("3"))
          ),
        ], .initial
      ))
    try await deferred.fulfill()
  ...
  func pinnedEventsBannerSelection() async throws {
    ...
    let pinnedTimelineItemProviderMock = TimelineItemProviderMock()
    pinnedTimelineMock.timelineItemProvider = pinnedTimelineItemProviderMock
    pinnedTimelineItemProviderMock.underlyingUpdatePublisher = Empty<
      ([TimelineItemProxy], TimelinePaginationState), Never
    >().eraseToAnyPublisher()
    pinnedTimelineItemProviderMock.itemProxies = [
      .event(
      ...
      .event(
        .init(item: EventTimelineItem(configuration: .init(eventID: "test3")), uniqueID: .init("3"))
      ),
    ]
    ...
    let deferredAction = deferFulfillment(viewModel.actions) { action in
      if case .focusEvent(let eventID) = action {
  ...
  func pinnedEventsBannerThreadedSelection() async throws {
    ...
    let deferredAction1 = deferFulfillment(viewModel.actions) { action in
      if case .focusEvent(let threadRootEventID) = action {
        return threadRootEventID == "thread"
      }
      return false
    }
    let deferredAction2 = deferFulfillment(viewModel.actions) { action in
      if case .displayThread(let threadRootEventID, let focussedEventID) = action {
  ...
  func roomInfoUpdate() async throws {
    var configuration = JoinedRoomProxyMockConfiguration(
      id: "TestID", name: "StartingName", avatarURL: nil, hasOngoingCall: false)
    let roomProxyMock = JoinedRoomProxyMock(configuration)
    ...
    let deferred = deferFulfillment(viewModel.context.$viewState) { viewState in
      viewState.roomTitle == "NewName"
        ...
        && viewState.canJoinCall && viewState.hasOngoingCall
    }
  ...
  func callButtonVisibility() async throws {
    ...
    let deferredFailure = deferFailure(viewModel.context.$viewState, timeout: .seconds(1)) {
      !$0.shouldShowCallButton
    }
  ...
  func knockRequestBanner() async throws {
    ...
    let roomProxyMock = JoinedRoomProxyMock(
      .init(
        knockRequestsState: .loaded([
          KnockRequestProxyMock(
            .init(
              eventID: "1", userID: "@alice:matrix.org", displayName: "Alice",
              reason: "Hello World!")),
    ...
    var deferred = deferFulfillment(viewModel.context.$viewState) { state in
      state.shouldSeeKnockRequests
        && state.unseenKnockRequests == [
          .init(
            displayName: "Alice", avatarURL: nil, userID: "@alice:matrix.org",
            reason: "Hello World!", eventID: "1")
        ]
    }
    try await deferred.fulfill()
    ...
    deferred = deferFulfillment(viewModel.context.$viewState) { state in
      state.handledEventIDs == ["1"] && !state.shouldSeeKnockRequests
    }
      ...
      state.shouldSeeKnockRequests
        && state.unseenKnockRequests == [
          .init(
            ...
            reason: "Hello World!", eventID: "1"),
          .init(
            displayName: nil, avatarURL: nil, userID: "@bob:matrix.org", reason: nil, eventID: "2"),
        ]
    }
    try await deferred.fulfill()

    deferred = deferFulfillment(viewModel.context.$viewState) { state in
      state.handledEventIDs == ["1", "2"] && !state.shouldSeeKnockRequests
    }
  ...
  func knockRequestsBannerDoesNotAppearIfUserHasNoPermission() async throws {
    ...
    let deferred = deferFulfillment(viewModel.context.$viewState) { state in
      state.unseenKnockRequests == [
        .init(
          displayName: "Alice", avatarURL: nil, userID: "@alice:matrix.org", reason: "Hello World!",
          eventID: "1")
      ] && !state.shouldSeeKnockRequests
    }
    try await deferred.fulfill()
  ...
  func roomWithSharedHistoryDisplaysBadge() async throws {
    var configuration = JoinedRoomProxyMockConfiguration(
      isEncrypted: false, historyVisibility: .joined)
    let infoSubject = CurrentValueSubject<RoomInfoProxyProtocol, Never>(
      RoomInfoProxyMock(configuration))
    let roomProxyMock = JoinedRoomProxyMock(configuration)
    ...
    let deferredInvisible = deferFailure(
      viewModel.context.$viewState,
      timeout: .seconds(1),
      message:
        "The icon should be hidden when the room history visibility is not .shared or .worldReadable"
    ) { viewState in
    ...
    infoSubject.send(RoomInfoProxyMock(configuration))
    let deferredInvisibleUnencrypted = deferFailure(
      viewModel.context.$viewState,
      timeout: .seconds(1),
      message: "The icon should not be shown when the room is unencrypted"
    ) { viewState in
      viewState.roomHistorySharingState != nil
    }
    ...
    let deferredShared = deferFulfillment(
      viewModel.context.$viewState,
      message: "The icon should be shown when the room history visibility is .shared"
    ) { viewState in
    ...
    infoSubject.send(RoomInfoProxyMock(configuration))
    let deferredWorldReadable = deferFulfillment(
      viewModel.context.$viewState,
      message: "The icon should be shown when the room history visibility is .worldReadable"
    ) { viewState in

...
final class ServerConfirmationScreenCoordinator: CoordinatorProtocol {
  ...
  private let actionsSubject: PassthroughSubject<ServerConfirmationScreenCoordinatorAction, Never> =
    .init()
  ...
  init(parameters: ServerConfirmationScreenCoordinatorParameters) {
    let mode =
      if parameters.appSettings.allowOtherAccountProviders {
        ServerConfirmationScreenMode.confirmation(
          parameters.authenticationService.homeserver.value.address)
      } else {

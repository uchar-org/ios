...
struct HomeScreenContent: View {
    ...
    GeometryReader { geometry in
      ScrollView {
        ...
        case .skeletons:
          LazyVStack(spacing: 0) {
            ForEach(context.viewState.visibleRooms) { room in
              HomeScreenRoomCell(
                room: room, isSelected: false, mediaProvider: context.mediaProvider,
                action: context.send
              )
        ...
        case .rooms:
          ...
          .searchable(
            text: $context.searchQuery, placement: .navigationBarDrawer(displayMode: .always)
          )
          .compoundSearchField()
      ...
      .onChange(of: context.viewState.visibleRooms) {
        ...
        DispatchQueue.main.async {
          guard !scrollViewAdapter.isScrolling.value, let scrollView = scrollViewAdapter.scrollView
          else {
            return
          }
  ...
  private var topSection: some View {
    ...
    if context.viewState.shouldShowFilters || context.viewState.shouldShowBanner {
      VStack(spacing: 0) {
        ...
        if case .show(let state) = context.viewState.securityBannerMode {
  ...
  private func delayedUpdateVisibleRange() {
    guard let scrollView = scrollViewAdapter.scrollView,
      ...
      context.viewState.visibleRooms.count > 0
    else {
      return
    }
    ...
    let firstIndex = Int(
      max(0.0, scrollView.contentOffset.y + scrollView.contentInset.top) / cellHeight)
    let lastIndex = Int(
      max(0.0, scrollView.contentOffset.y + scrollView.bounds.height) / cellHeight)

...
struct AppLockSetupSettingsScreen: View {
  ...
  var body: some View {
    Form {
      Section {
        ListRow(
          label: .plain(title: L10n.screenAppLockSettingsChangePin),
        ...
        if !context.viewState.isMandatory {
          ListRow(
            label: .plain(title: L10n.screenAppLockSettingsRemovePin, role: .destructive),
      ...
      if context.viewState.supportsBiometrics {
        Section {
          ListRow(
            label: .plain(title: context.viewState.enableBiometricsTitle),
...
struct AppLockSetupSettingsScreen_Previews: PreviewProvider, TestablePreview {
  static let faceIDViewModel = AppLockSetupSettingsScreenViewModel(
    appLockService: AppLockServiceMock.mock(biometryType: .faceID))
  static let touchIDViewModel = AppLockSetupSettingsScreenViewModel(
    appLockService: AppLockServiceMock.mock(isMandatory: true, biometryType: .touchID))
  static let biometricsUnavailableViewModel = AppLockSetupSettingsScreenViewModel(
    appLockService: AppLockServiceMock.mock(biometryType: .none))

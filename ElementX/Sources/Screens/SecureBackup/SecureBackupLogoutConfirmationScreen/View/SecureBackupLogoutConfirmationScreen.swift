...
struct SecureBackupLogoutConfirmationScreen: View {
  ...
  private var content: some View {
    ...
    if case .waitingToStart(let hasStalled) = context.viewState.mode {
      Spacer()
      ProgressView()
    ...
    } else if case .backupOngoing(let progress) = context.viewState.mode {
      Spacer()
...
struct SecureBackupLogoutConfirmationScreen_Previews: PreviewProvider, TestablePreview {
  ...
  static var previews: some View {
    ...
    .snapshotPreferences(
      expect: waitingViewModel.context.observe(\.viewState.mode).map {
    ...
    .snapshotPreferences(
      expect: ongoingViewModel.context.observe(\.viewState.mode).map {
  ...
  static func makeViewModel(mode: SecureBackupLogoutConfirmationScreenViewMode)
    -> SecureBackupLogoutConfirmationScreenViewModel
  {
    ...
    let viewModel = SecureBackupLogoutConfirmationScreenViewModel(

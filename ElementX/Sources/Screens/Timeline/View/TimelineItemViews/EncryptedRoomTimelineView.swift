...
struct EncryptedRoomTimelineView_Previews: PreviewProvider, TestablePreview {
  ...
  static var body: some View {
    VStack(alignment: .leading, spacing: 20.0) {
      EncryptedRoomTimelineView(
        timelineItem: itemWith(
          text: L10n.commonWaitingForDecryptionKey,
          timestamp: .mock,
          isOutgoing: false,
          senderId: "Bob"))
      ...
      EncryptedRoomTimelineView(
        timelineItem: itemWith(
          text: "Some other text that is very long and will wrap onto multiple lines.",
          timestamp: .mock,
          isOutgoing: true,
          senderId: "Anne"))

      EncryptedRoomTimelineView(
        timelineItem: expectedItemWith(
          timestamp: .mock,
          isOutgoing: false,
          senderId: "Bob"))
    }
  ...
  private static func expectedItemWith(timestamp: Date, isOutgoing: Bool, senderId: String)
    -> EncryptedRoomTimelineItem
  {
    EncryptedRoomTimelineItem(
      id: .randomEvent,

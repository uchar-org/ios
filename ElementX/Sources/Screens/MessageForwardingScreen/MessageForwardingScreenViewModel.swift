...
typealias MessageForwardingScreenViewModelType = StateStoreViewModel<
  MessageForwardingScreenViewState, MessageForwardingScreenViewAction
>

class MessageForwardingScreenViewModel: MessageForwardingScreenViewModelType,
  MessageForwardingScreenViewModelProtocol
{
  ...
  private var actionsSubject: PassthroughSubject<MessageForwardingScreenViewModelAction, Never> =
    .init()
  ...
  init(
    forwardingItem: MessageForwardingItem,
    ...
    super.init(
      initialViewState: MessageForwardingScreenViewState(), mediaProvider: userSession.mediaProvider
    )
  ...
  private func forward() async {
    ...
    guard case .joined(let targetRoomProxy) = await clientProxy.roomForIdentifier(roomID) else {
      MXLog.error("Failed retrieving room to forward to with id: \(roomID)")
      userIndicatorController.submitIndicator(UserIndicator(title: L10n.errorUnknown))
      return
    }

    if case .failure(let error) = await targetRoomProxy.timeline.sendMessageEventContent(

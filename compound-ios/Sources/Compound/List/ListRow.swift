...
public enum ListRowPadding {
  ...
  public static let insets = EdgeInsets(
    top: vertical,
    leading: horizontal,
    bottom: vertical,
    trailing: horizontal)

  public static let textFieldInsets = EdgeInsets(
    top: 11,
    leading: horizontal,
    bottom: 11,
    trailing: horizontal)
...
public struct ListRow<Icon: View, DetailsIcon: View, CustomContent: View, SelectionValue: Hashable>:
  View
{
...
extension ListRow where CustomContent == EmptyView {
  public init(
    label: ListRowLabel<Icon>,
...
extension ListRow where DetailsIcon == EmptyView, CustomContent == EmptyView {
  public init(
    label: ListRowLabel<Icon>,
    details: ListRowDetails<DetailsIcon>? = nil,
    kind: Kind<CustomContent, SelectionValue>
...
extension ListRow where Icon == EmptyView, DetailsIcon == EmptyView {
  public init(kind: Kind<CustomContent, SelectionValue>) {
    label = ListRowLabel()
    details = nil
    self.kind = kind
  }

  public init(kind: Kind<CustomContent, SelectionValue>) where SelectionValue == String {
    label = ListRowLabel()
    details = nil
    self.kind = kind
  }
...
extension TextField {
  ...
  fileprivate init(text: Binding<String>, axis: Axis?, label: () -> Label) {
    if let axis {
...
public struct ListRow_Previews: PreviewProvider, TestablePreview {
  ...
  @ViewBuilder static var pickers: some View {
    ...
    ListRow(
      label: .default(title: "Title", systemIcon: .squareDashed),
      kind: .picker(
        selection: .constant("Item 1"),
        items: [
          (title: "Item 1", tag: "Item 1"),
  ...
  static var descriptionLabelSection: some View {
    Section {
      ListRow(
        label: .description(
          "This is a row in the list, with a multiline description but it doesn't have either an icon or a title, just this text here."
        ),
        kind: .label)
    }
  }
    ...
    Form {
      ListRow(
        label: .plain(
          title: "Selected",
          description:
            "This is a long multiline description which shows what happens when wrapping with a details view and selection, specifically, an activity indicator in the details."
        ),

...
typealias EditRoomAddressScreenViewModelType = StateStoreViewModel<
  EditRoomAddressScreenViewState, EditRoomAddressScreenViewAction
>

class EditRoomAddressScreenViewModel: EditRoomAddressScreenViewModelType,
  EditRoomAddressScreenViewModelProtocol
{
  ...
  private let actionsSubject: PassthroughSubject<EditRoomAddressScreenViewModelAction, Never> =
    .init()
  ...
  init(
    initialViewState: EditRoomAddressScreenViewState? = nil,
    ...
    if let initialViewState {
      ...
      super.init(
        initialViewState: EditRoomAddressScreenViewState(
      ...
      state.bindings.desiredAliasLocalPart =
        localPartForMatchingAlias(computeFromDisplayName: true) ?? ""
    }
  ...
  private func localPartForMatchingAlias(computeFromDisplayName: Bool) -> String? {
    if let matchingAlias = roomProxy.infoPublisher.value.firstAliasMatching(
      serverName: clientProxy.userIDServerName, useFallback: false)
    {
  ...
  private func setupSubscriptions() {
    context.$viewState
      ...
      .sink { [weak self] aliasLocalPart in
        ...
        guard
          let canonicalAlias = String.makeCanonicalAlias(
  ...
  private func save() async {
    ...
    guard
      let desiredCanonicalAlias = String.makeCanonicalAlias(
        aliasLocalPart: state.bindings.desiredAliasLocalPart, serverName: state.serverName),
      isRoomAliasFormatValid(alias: desiredCanonicalAlias)
    else {
    ...
    let savedAliasFromHomeserver = roomProxy.infoPublisher.value.firstAliasMatching(
      serverName: state.serverName, useFallback: false)
    ...
    if savedCanonicalAlias == nil || savedCanonicalAlias?.hasSuffix(state.serverName) == true {
      ...
      if case .failure = await roomProxy.updateCanonicalAlias(
        desiredCanonicalAlias, altAliases: newAlternativeAliases)
      {
        userIndicatorController.submitIndicator(.init(title: L10n.errorUnknown))
        return
      }
      ...
      if case .failure = await roomProxy.updateCanonicalAlias(
        savedCanonicalAlias, altAliases: newAlternativeAliases)
      {
        userIndicatorController.submitIndicator(.init(title: L10n.errorUnknown))
        return
      }
...
extension String {
  fileprivate var aliasLocalPart: String {

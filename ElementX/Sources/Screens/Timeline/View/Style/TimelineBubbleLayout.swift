...
struct TimelineBubbleLayout: Layout {
  ...
  func sizeThatFits(proposal: ProposedViewSize, subviews: Subviews, cache: inout Cache) -> CGSize {
    ...
    let subviewSizes = layoutSubviews.map {
      size(for: $0, subviews: subviews, proposedSize: proposal, cache: &cache)
    }
  ...
  func placeSubviews(
    ...
    let layoutSubviews = subviews.filter { $0[Size.self].shouldLayout }
    let maxWidth = layoutSubviews.map {
      size(for: $0, subviews: subviews, proposedSize: proposal, cache: &cache).width
    }.reduce(0, max)
    ...
    let subviewSizes = visibleSubviews.map {
      size(
        for: $0, subviews: subviews,
        proposedSize: ProposedViewSize(width: maxWidth, height: proposal.height), cache: &cache)
    }

    var y = bounds.minY
    for index in visibleSubviews.indices {
      let height = subviewSizes[index].height
      visibleSubviews[index].place(
  ...
  private func size(
    for subview: LayoutSubview, subviews: LayoutSubviews, proposedSize: ProposedViewSize,
    cache: inout Cache
  ) -> CGSize {

...
struct AuthenticationStartScreen: View {
  ...
  var body: some View {
    if case .welcomeBack(let classicAppAccount) = context.viewState.classicAppMode,
    ...
    VStack(spacing: 0) {
      ...
      if verticalSizeClass == .regular {
        ...
        AuthenticationStartLogo(
          hideBrandChrome: context.viewState.hideBrandChrome,
  ...
  var versionText: Text {
    ...
    let shortVersionString =
      ProcessInfo.isRunningTests ? "0.0.0" : InfoPlistReader.main.bundleShortVersionString
  ...
  var toolbar: some ToolbarContent {
    ToolbarItem(placement: .primaryAction) {
      if case .otherOptions(let classicAppAccount) = context.viewState.classicAppMode {
...
struct AuthenticationStartScreen_Previews: PreviewProvider, TestablePreview {
  ...
  static func makeViewModel(
    provisionedServerName: String? = nil, hasClassicAppAccount: Bool = false
    ...
    let classicAppManager: ClassicAppManagerMock? =
      hasClassicAppAccount ? .init(.init(accounts: [classicAppAccount])) : nil

    return AuthenticationStartScreenViewModel(
      authenticationService: AuthenticationService.mock(classicAppManager: classicAppManager),
      provisioningParameters: provisionedServerName.map {
        .init(accountProvider: $0, loginHint: nil)
      },

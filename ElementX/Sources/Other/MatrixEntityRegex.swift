...
enum MatrixEntityRegex: String {
  ...
  var rawValue: String {
    ...
    case .uri:
      return
        "matrix:(r|u|roomid)\\/[A-Z0-9\\-._~:/?#\\[\\]@!$&'()*+,;=%]*(?:\\?[A-Z0-9\\-._~:/?#\\[\\]@!$&'()*+,;=%]*)?"
    case .allUsers:
  ...
  static let homeserverRegex = try! NSRegularExpression(
    pattern: MatrixEntityRegex.homeserver.rawValue, options: .caseInsensitive)
  static let userIdentifierRegex = try! NSRegularExpression(
    pattern: MatrixEntityRegex.userID.rawValue, options: .caseInsensitive)
  static let roomAliasRegex = try! NSRegularExpression(
    pattern: MatrixEntityRegex.roomAlias.rawValue, options: .caseInsensitive)
  static let uriRegex = try! NSRegularExpression(
    pattern: MatrixEntityRegex.uri.rawValue, options: .caseInsensitive)
  ...
  static let legacyRoomIDRegex = try! NSRegularExpression(
    pattern: MatrixEntityRegex.legacyRoomID.rawValue, options: .caseInsensitive)

...
struct RoomTimelineItemView: View {
  ...
  @ViewBuilder private var timelineView: some View {
    ...
    case .voice(let item):
      let playerState =
        context?.viewState.audioPlayerStateProvider?(item.id)
        ?? AudioPlayerState(
    ...
    case .liveLocation(let item):
      LiveLocationRoomTimelineView(
        timelineItem: item,
        isStopped: context?.viewState.stoppedLiveLocationIDs.contains(item.id) ?? false)
    }
  }

  private func linkMetadataForItem(_ item: TextRoomTimelineItem) -> OrderedDictionary<
    URL, LinkMetadataProviderItem
  > {

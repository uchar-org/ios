...
struct VoiceMessageMediaManagerTests {
  ...
  func loadVoiceMessageFromSourceUnsupportedMedia() async throws {
    ...
    do {
      _ = try await voiceMessageMediaManager.loadVoiceMessageFromSource(
        unsupportedMediaSource, body: nil)
  ...
  mutating func loadVoiceMessageFromSourceMimeTypeWithParameters() async throws {
    ...
    mediaProvider.loadFileFromSourceFilenameReturnValue = .success(
      MediaFileHandleProxy.unmanaged(url: loadedFile))
  ...
  mutating func loadVoiceMessageFromSourceSingleCall() async throws {
    ...
    #expect(voiceMessageCache.cacheMediaSourceUsingMoveCalled)
    #expect(
      voiceMessageCache.cacheMediaSourceUsingMoveReceivedArguments?.mediaSource == mediaSource)
    #expect(
      voiceMessageCache.cacheMediaSourceUsingMoveReceivedArguments?.fileURL.pathExtension == "m4a")
  ...
  mutating func loadVoiceMessageFromSourceMultipleCalls() async throws {
    ...
    let mediaSource = try MediaSourceProxy(url: someURL, mimeType: audioOGGMimeType)
    for _ in 0..<10 {
      let url = try await voiceMessageMediaManager.loadVoiceMessageFromSource(
        mediaSource, body: nil)
      #expect(url == cachedConvertedFileURL)
    }

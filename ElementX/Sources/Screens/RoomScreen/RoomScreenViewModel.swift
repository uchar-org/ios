...
class RoomScreenViewModel: RoomScreenViewModelType, RoomScreenViewModelProtocol {
  ...
  init(
    ...
    let viewState = RoomScreenViewState(
      roomTitle: roomProxy.infoPublisher.value.displayName ?? roomProxy.id,
    ...
    super.init(
      initialViewState: appHooks.roomScreenHook.update(viewState),
  ...
  private func updateVerificationBadge() async {
    guard roomProxy.isDirectOneToOneRoom,
      ...
      case .success(let userIdentity) = await clientProxy.userIdentity(
        for: dmRecipient.userID, fallBackToServer: true)
    else {
  ...
  private func buildPinnedEventContents(timelineItems: [TimelineItemProxy]) {
    ...
    for item in timelineItems {
      ...
      {
        pinnedEventContents.updateValue(
          pinnedEventStringBuilder.buildAttributedString(for: event)
  ...
  private func acceptKnock(eventID: String) async {
    ...
    case .failure:
      userIndicatorController.submitIndicator(
        .init(id: Self.errorIndicatorIdentifier, type: .toast, title: L10n.errorUnknown))
  ...
  private func markAllKnocksAsSeen() async {
    ...
    let failedIDs = await withTaskGroup(of: (String, Result<Void, KnockRequestProxyError>).self) {
      group in
      ...
      Task {
        ...
        case .success(let event):
            ...
            let threadRootEventID = event.threadRootEventId()
          {
            ...
            actionsSubject.send(
              .displayThread(threadRootEventID: threadRootEventID, focussedEventID: eventID))
          } else {
  ...
  private func showLoadingIndicator() {
    userIndicatorController.submitIndicator(
      .init(id: Self.loadingIndicatorIdentifier, type: .toast, title: L10n.commonLoading))
  }
...
extension KnockRequestInfo {
  fileprivate init(from proxy: KnockRequestProxyProtocol) {
    self.init(

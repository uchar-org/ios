...
typealias SpaceScreenViewModelType = StateStoreViewModelV2<
  SpaceScreenViewState, SpaceScreenViewAction
>
...
class SpaceScreenViewModel: SpaceScreenViewModelType, SpaceScreenViewModelProtocol {
  ...
  init(
    spaceRoomListProxy: SpaceRoomListProxyProtocol,
    ...
    super.init(
      initialViewState: SpaceScreenViewState(
        space: spaceRoomListProxy.spaceServiceRoomPublisher.value,
    ...
    Task {
      if case .joined(let roomProxy) = await userSession.clientProxy.roomForIdentifier(
  ...
  private func join(_ spaceServiceRoom: SpaceServiceRoom) async {
    ...
    guard case .success = await clientProxy.joinRoom(spaceServiceRoom.id, via: spaceServiceRoom.via)
    else {
  ...
  private func removeSelectedChildren() async {
    ...
    MXLog.info(
      "Removing \(state.editModeSelectedIDs.count) children from space \(spaceRoomListProxy.id)")
    ...
    MXLog.info(
      "\(state.editModeSelectedIDs.count) children removed from space \(spaceRoomListProxy.id)")
  ...
  private func showLeaveSpaceConfirmation() async {
    guard
      case .success(let leaveHandle) = await spaceServiceProxy.leaveSpace(

...
enum RoomFlowCoordinatorAction: Equatable {
  ...
  static func == (lhs: RoomFlowCoordinatorAction, rhs: RoomFlowCoordinatorAction) -> Bool {
    switch (lhs, rhs) {
    case (
      .presentCallScreen(let lhsRoomProxy, let lhsIsVoiceCall),
      .presentCallScreen(let rhsRoomProxy, let rhsIsVoiceCall)
    ):
...
class RoomFlowCoordinator: FlowCoordinatorProtocol {
  ...
  func handleAppRoute(_ appRoute: AppRoute, animated: Bool) {
    ...
    case .childRoom(let roomID, let via):
      if case .membersFlow = stateMachine.state, let membersFlowCoordinator {
        ...
        stateMachine.tryEvent(
          .startChildFlow(roomID: roomID, via: via, entryPoint: .room),
    ...
    case .roomDetails(let roomID):
      ...
      Task {
        if roomProxy == nil {
          guard
            case .joined(let roomProxy) = await userSession.clientProxy.roomForIdentifier(roomID)
          else {
            return
          }
    ...
    case .roomMemberDetails(let userID):
      ...
      if case .membersFlow = stateMachine.state, let membersFlowCoordinator {
        membersFlowCoordinator.handleAppRoute(
          .roomMemberDetails(userID: userID), animated: animated)
      } else if case .presentingChild = stateMachine.state, let childRoomFlowCoordinator {
        childRoomFlowCoordinator.handleAppRoute(appRoute, animated: animated)
      } else {
        stateMachine.tryEvent(
          .startMembersFlow(entryPoint: .roomMember(userID: userID)),
          userInfo: EventUserInfo(animated: animated))
      }
    ...
    case .accountProvisioningLink, .roomList, .userProfile, .call, .settings, .chatBackupSettings,
      .globalSearch:
  ...
  private func handleChildEventRoute(eventID: String, roomID: String, via: [String], animated: Bool)
  {
    if case .membersFlow = stateMachine.state, let membersFlowCoordinator {
      membersFlowCoordinator.handleAppRoute(
        .childEvent(eventID: eventID, roomID: roomID, via: via), animated: animated)
    } else if case .presentingChild = stateMachine.state, let childRoomFlowCoordinator {
      childRoomFlowCoordinator.handleAppRoute(
        .childEvent(eventID: eventID, roomID: roomID, via: via), animated: animated)
    } else if roomID != roomProxy.id {
      stateMachine.tryEvent(
        .startChildFlow(roomID: roomID, via: via, entryPoint: .eventID(eventID)),
        userInfo: EventUserInfo(animated: animated))
    } else {
      ...
      Task {
        ...
        case .success(let event):
          ...
          {
            if case .thread(threadRootEventID: threadRootEventID, _) = stateMachine.state,
              let threadCoordinator = childThreadScreenCoordinators.last
            {
              ...
              if childThreadScreenCoordinators.isEmpty {
                roomScreenCoordinator?.focusOnEvent(
                  .init(eventID: threadRootEventID, shouldSetPin: false))
              }
              stateMachine.tryEvent(
                .presentThread(threadRootEventID: threadRootEventID, focusEventID: eventID))
            }
  ...
  private func presentTransferOwnershipScreen() {
    let parameters = RoomChangeRolesScreenCoordinatorParameters(
      mode: .owner,
      roomProxy: roomProxy,
  ...
  private func handleRoomRoute(
    roomID: String, via: [String], presentationAction: PresentationAction? = nil, animated: Bool
  ) async {
    guard roomID == self.roomID else {
      fatalError("Navigation route doesn't belong to this room flow.")
    }
    ...
    guard let room = await userSession.clientProxy.roomForIdentifier(roomID) else {
      stateMachine.tryEvent(
        .presentJoinRoomScreen(via: via), userInfo: EventUserInfo(animated: animated))
      return
    }
    ...
    case .joined(let roomProxy):
      if roomProxy.infoPublisher.value.isSpace {
        ...
        guard case .eventFocus(let focusEvent) = presentationAction else {
          // If is not a focus event just handle the presentation action directly in `presentRoom`
          stateMachine.tryEvent(
            .presentRoom(presentationAction: presentationAction),
            userInfo: EventUserInfo(animated: animated))
          return
        }
        ...
        case .failure:
          showErrorIndicator()
          stateMachine.tryEvent(
            .presentRoom(presentationAction: nil), userInfo: EventUserInfo(animated: animated))
        }
      }
    default:
      stateMachine.tryEvent(
  ...
  private func setupStateMachine() {
    ...
    stateMachine.addAnyHandler(.any => .any) { [weak self] context in
      ...
      case (_, .presentThread(let threadRootEventID, let focusEventID), .thread):
        Task {
          await self.presentThread(
            threadRootEventID: threadRootEventID, focusEventID: focusEventID, animated: animated)
        }

      // Thread + Room

      case (_, .startSpaceFlow, .spaceFlow):
        guard let spaceRoomListProxy = (context.userInfo as? EventUserInfo)?.spaceRoomListProxy
        else {
      ...
      case (_, .presentMediaUploadPicker, .mediaUploadPicker(let mode, _)):
        guard let timelineController = (context.userInfo as? EventUserInfo)?.timelineController
        else {
          fatalError("Missing required TimelineController")
        }
        presentMediaUploadPickerWithMode(
          mode, timelineController: timelineController, animated: animated)
      ...
      case (_, .presentMapNavigator(let mode), .mapNavigator):
        ...
        presentMapNavigator(
          interactionMode: mode, timelineController: timelineController, animated: animated)
      ...
      case (
        .securityAndPrivacy, .presentManageAuthorizedSpacesScreen, .manageAuthorizedSpacesScreen
      ):
      ...
      case (
        .notificationSettings, .presentGlobalNotificationSettingsScreen, .globalNotificationSettings
      ):
      ...
      case (_, .presentMediaUploadPreview, .mediaUploadPreview(let mediaURLs, _)):
        ...
        presentMediaUploadPreviewScreen(
          for: mediaURLs, timelineController: timelineController, animated: animated)
    ...
    stateMachine.addAnyHandler(.any => .any) { context in
      if let event = context.event {
        MXLog.info(
          "Transitioning from `\(context.fromState)` to `\(context.toState)` with event `\(event)`")
      } else {
  ...
  private func presentRoom(
    ...
    if !isChildFlow {
      ...
      default:
        ...
        case .share(.mediaFiles(_, let mediaFiles)):
          stateMachine.tryEvent(
            .presentMediaUploadPreview(mediaURLs: mediaFiles.map(\.url)),
        ...
        case .thread(let rootEventID, let focusEvent):
          ...
          stateMachine.tryEvent(
            .presentThread(threadRootEventID: rootEventID, focusEventID: focusEvent?.eventID))
        case .none:
          break
        }
    ...
    flowParameters.analytics.trackViewRoom(
      isDM: roomProxy.infoPublisher.value.isDirect, isSpace: roomProxy.infoPublisher.value.isSpace)

    let coordinator = makeRoomScreenCoordinator(
      presentationAction: presentationAction, animated: animated)
  ...
  private func makeRoomScreenCoordinator(presentationAction: PresentationAction?, animated: Bool)
    -> RoomScreenCoordinator
  {
    let userID = userSession.clientProxy.userID
    let timelineItemFactory = RoomTimelineItemFactory(
      userID: userID,
      attributedStringBuilder: AttributedStringBuilder(mentionBuilder: MentionBuilder()),
      stateEventStringBuilder: RoomStateEventStringBuilder(userID: userID))
    let timelineController = flowParameters.timelineControllerFactory.buildTimelineController(
      roomProxy: roomProxy,
    ...
    let completionSuggestionService = CompletionSuggestionService(
      roomProxy: roomProxy,
      roomListPublisher: userSession.clientProxy.staticRoomSummaryProvider.roomListPublisher
        .eraseToAnyPublisher())
    let composerDraftService = ComposerDraftService(
      roomProxy: roomProxy,
      timelineItemfactory: timelineItemFactory,
    ...
    coordinator.actions
      .sink { [weak self] action in
        ...
        case .presentMediaUploadPreviewScreen(let url):
          stateMachine.tryEvent(
            .presentMediaUploadPreview(mediaURLs: url),
            userInfo: EventUserInfo(animated: animated, timelineController: timelineController))
        case .presentEmojiPicker(let itemID, let selectedEmojis):
          stateMachine.tryEvent(
            .presentEmojiPicker(itemID: itemID, selectedEmojis: selectedEmojis),
            userInfo: EventUserInfo(animated: animated, timelineController: timelineController))
        case .presentLocationPicker:
          stateMachine.tryEvent(
            .presentMapNavigator(interactionMode: .picker),
            userInfo: EventUserInfo(animated: animated, timelineController: timelineController))
        ...
        case .presentLiveLocationViewer(let sender, let initialLiveLocationShare):
          stateMachine.tryEvent(
            .presentMapNavigator(
              interactionMode: .viewLive(
                sender: sender, initialLiveLocationShare: initialLiveLocationShare)),
            userInfo: EventUserInfo(animated: animated, timelineController: timelineController))
        ...
        case .presentThread(let threadRootEventID, let focussedEventID):
          stateMachine.tryEvent(
            .presentThread(threadRootEventID: threadRootEventID, focusEventID: focussedEventID))
  ...
  private func presentThreadList(animated: Bool) async {
    let coordinator = RoomThreadListScreenCoordinator(
      parameters: .init(
        threadListServiceProxy: roomProxy.threadListService(),
        mediaProvider: userSession.mediaProvider))
    ...
    coordinator.actionsPublisher.sink { [weak self] action in
      ...
      case .presentThread(let threadRootEventID):
        stateMachine.tryEvent(
          .presentThread(threadRootEventID: threadRootEventID, focusEventID: nil))
      }
    }.store(in: &cancellables)
  ...
  private func presentThread(threadRootEventID: String, focusEventID: String?, animated: Bool) async
  {
    ...
    guard
      case .success(let timelineController) = await flowParameters.timelineControllerFactory
        .buildThreadTimelineController(
    ...
    let coordinator = ThreadTimelineScreenCoordinator(
      parameters: .init(
        userSession: userSession,
        roomProxy: roomProxy,
    ...
    coordinator.actions.sink { [weak self] action in
      ...
      case .presentMediaUploadPreviewScreen(let mediaURLs):
        stateMachine.tryEvent(
          .presentMediaUploadPreview(mediaURLs: mediaURLs),
          userInfo: EventUserInfo(animated: animated, timelineController: timelineController))
      case .presentLocationPicker:
        stateMachine.tryEvent(
          .presentMapNavigator(interactionMode: .picker),
  ...
  private func presentJoinRoomScreen(via: [String], animated: Bool) {
    let coordinator = JoinRoomScreenCoordinator(
      parameters: .init(
        source: .generic(roomID: roomID, via: via),
        userSession: userSession,
        userIndicatorController: flowParameters.userIndicatorController,
        appSettings: flowParameters.appSettings))
      ...
      .sink { [weak self] action in
        ...
        case .joined(.roomID(let roomID)):
          Task { [weak self] in
            ...
            if case .joined(let roomProxy) = await userSession.clientProxy.roomForIdentifier(roomID)
            {
              await storeAndSubscribeToRoomProxy(roomProxy)
              stateMachine.tryEvent(
                .presentRoom(presentationAction: nil), userInfo: EventUserInfo(animated: animated))

              flowParameters.analytics.trackJoinedRoom(
                isDM: roomProxy.infoPublisher.value.isDirect,
        ...
        case .joined(.space(let spaceRoomListProxy)):
          stateMachine.tryEvent(
            .joinedSpace,
            userInfo: EventUserInfo(animated: true, spaceRoomListProxy: spaceRoomListProxy))
        case .cancelled:
  ...
  private func dismissFlow(
    animated: Bool, continuingWith spaceRoomListProxy: SpaceRoomListProxyProtocol? = nil
  ) {
  ...
  private func presentRoomDetails(isRoot: Bool, animated: Bool) async {
    let params = RoomDetailsScreenCoordinatorParameters(
      roomProxy: roomProxy,
      userSession: userSession,
    ...
    coordinator.actions.sink { [weak self] action in
      ...
      case .presentCall(let isVoiceCall):
        actionsSubject.send(.presentCallScreen(roomProxy: roomProxy, isVoiceCall: isVoiceCall))
      case .presentPinnedEventsTimeline:
        stateMachine.tryEvent(.presentPinnedEventsTimeline)
  ...
  private func presentRoomDetailsEditScreen() {
    ...
    let roomDetailsEditParameters = RoomDetailsEditScreenCoordinatorParameters(
      ...
      appSettings: flowParameters.appSettings)
    let roomDetailsEditCoordinator = RoomDetailsEditScreenCoordinator(
      parameters: roomDetailsEditParameters)
  ...
  private func presentReportContent(for itemID: TimelineItemIdentifier, from senderID: String) {
    ...
    let stackCoordinator = NavigationStackCoordinator()
    let parameters = ReportContentScreenCoordinatorParameters(
      eventID: eventID,
      senderID: senderID,
      roomProxy: roomProxy,
    ...
    coordinator.actions
      .sink { [weak self] action in
        ...
        case .cancel:
          break
        case .finish:
          flowParameters.userIndicatorController.submitIndicator(
            UserIndicator(title: L10n.commonReportSubmitted, iconName: "checkmark"))
        }
      }
      .store(in: &cancellables)
  ...
  private func presentMediaUploadPickerWithMode(
    _ mode: MediaPickerScreenMode,
    ...
    let mediaPickerCoordinator = MediaPickerScreenCoordinator(
      ...
      case .selectedMediaAtURLs(let urls):
        stateMachine.tryEvent(
          .presentMediaUploadPreview(mediaURLs: urls),
          userInfo: EventUserInfo(animated: animated, timelineController: timelineController))
      }
    }
  ...
  private func presentMediaUploadPreviewScreen(
    for mediaURLs: [URL],
    timelineController: TimelineControllerProtocol,
    animated: Bool
  ) {
    ...
    let mediaUploadPreviewScreenCoordinator = MediaUploadPreviewScreenCoordinator(
    ...
    navigationStackCoordinator.setSheetCoordinator(stackCoordinator, animated: animated) {
      [weak self] in
  ...
  private func presentEmojiPicker(
    for itemID: TimelineItemIdentifier,
  ...
  private func presentMapNavigator(
    interactionMode: LocationSharingInteractionMode,
    timelineController: TimelineControllerProtocol,
    animated: Bool
  ) {
    let stackCoordinator = NavigationStackCoordinator()

    let params = LocationSharingScreenCoordinatorParameters(
  ...
  private func presentPollForm(mode: PollFormMode, timelineController: TimelineControllerProtocol) {
    let stackCoordinator = NavigationStackCoordinator()
    let coordinator = PollFormScreenCoordinator(
      parameters: .init(
        mode: mode,
        timelineController: timelineController,
        analytics: flowParameters.analytics,
  ...
  private func presentRoomPollsHistory(animated: Bool) async {
    ...
    let parameters = RoomPollsHistoryScreenCoordinatorParameters(
      pollInteractionHandler: PollInteractionHandler(
        analyticsService: flowParameters.analytics,
        timelineController: timelineController),
      timelineController: timelineController,
      userIndicatorController: flowParameters.userIndicatorController)
    ...
    coordinator.actions
      .sink { [weak self] action in
        ...
        case .editPoll(let pollStartID, let poll):
          stateMachine.tryEvent(
            .presentPollForm(mode: .edit(eventID: pollStartID, poll: poll)),
            userInfo: EventUserInfo(animated: animated, timelineController: timelineController))
        }
      }
      .store(in: &cancellables)
  ...
  private func presentMessageForwarding(with forwardingItem: MessageForwardingItem) {
    ...
    let parameters = MessageForwardingScreenCoordinatorParameters(
  ...
  private func presentNotificationSettingsScreen() {
    let parameters = RoomNotificationSettingsScreenCoordinatorParameters(
  ...
  private func presentGlobalNotificationSettingsScreen() {
    let stackCoordinator = NavigationStackCoordinator()
    let parameters = NotificationSettingsScreenCoordinatorParameters(
      navigationStackCoordinator: stackCoordinator,
      userSession: userSession,
  ...
  private func presentInviteUsersScreen() {
    let stackCoordinator = NavigationStackCoordinator()
    let inviteParameters = InviteUsersScreenCoordinatorParameters(
      userSession: userSession,
      roomProxy: roomProxy,
      isSkippable: false,
  ...
  private func presentRolesAndPermissionsScreen() {
    let parameters = RoomRolesAndPermissionsFlowCoordinatorParameters(
      roomProxy: roomProxy,
      mediaProvider: userSession.mediaProvider,
      navigationStackCoordinator: navigationStackCoordinator,
      userIndicatorController: flowParameters.userIndicatorController,
  ...
  private func presentResolveSendFailure(
    failure: TimelineItemSendFailure.VerifiedUser, sendHandle: SendHandleProxy
  ) {
    let coordinator = ResolveVerifiedUserSendFailureScreenCoordinator(
      parameters: .init(
        failure: failure,
        sendHandle: sendHandle,
        roomProxy: roomProxy,
  ...
  private func presentSecurityAndPrivacyScreen() {
    let coordinator = SecurityAndPrivacyScreenCoordinator(
      parameters: .init(
        roomProxy: roomProxy,
        clientProxy: userSession.clientProxy,
        userIndicatorController: flowParameters.userIndicatorController,
    ...
    coordinator.actionsPublisher.sink { [weak self] action in
      ...
      case .displayManageAuthorizedSpacesScreen(let selection):
        stateMachine.tryEvent(
          .presentManageAuthorizedSpacesScreen,
          userInfo: EventUserInfo(animated: true, authorizedSpacesSelection: selection))
      }
    }
    .store(in: &cancellables)
  ...
  private func presentManageAuthorizedSpacesScreen(selection: AuthorizedSpacesSelection) {
    ...
    let coordinator = ManageAuthorizedSpacesScreenCoordinator(
      parameters: .init(
        authorizedSpacesSelection: selection,
        mediaProvider: flowParameters.userSession.mediaProvider))
    coordinator.actionsPublisher.sink { [weak self] action in
      guard let self else { return }
      switch action {
  ...
  private func presentEditAddressScreen() {
    let stackCoordinator = NavigationStackCoordinator()
    let coordinator = EditRoomAddressScreenCoordinator(
      parameters: .init(
        roomProxy: roomProxy,
        clientProxy: userSession.clientProxy,
        userIndicatorController: flowParameters.userIndicatorController))
  ...
  private func presentReportRoom() {
    let stackCoordinator = NavigationStackCoordinator()
    let coordinator = ReportRoomScreenCoordinator(
      parameters: .init(
        roomProxy: roomProxy,
        userIndicatorController: flowParameters.userIndicatorController))
  ...
  private func presentDeclineAndBlockScreen(userID: String) {
    let stackCoordinator = NavigationStackCoordinator()
    let coordinator = DeclineAndBlockScreenCoordinator(
      parameters: .init(
        userID: userID,
        roomID: roomID,
        clientProxy: userSession.clientProxy,
  ...
  private func startChildFlow(
    for roomID: String, via: [String], entryPoint: RoomFlowCoordinatorEntryPoint
  ) {
    let coordinator = RoomFlowCoordinator(
      roomID: roomID,
      isChildFlow: true,
      navigationStackCoordinator: navigationStackCoordinator,
  ...
  private func startPinnedEventsTimelineFlow() {
    ...
    let flowCoordinator = PinnedEventsTimelineFlowCoordinator(
      roomProxy: roomProxy,
      navigationStackCoordinator: stackCoordinator,
      flowParameters: flowParameters)
        ...
        if let threadRootEventID {
          stateMachine.tryEvent(
            .presentRoom(
              presentationAction: .thread(
                rootEventID: threadRootEventID,
                focusEvent: .init(eventID: eventID, shouldSetPin: true))))
        } else {
          stateMachine.tryEvent(
            .presentRoom(
              presentationAction: .eventFocus(.init(eventID: eventID, shouldSetPin: true))))
        }
      }
    }
    .store(in: &cancellables)
  ...
  private func startMediaEventsTimelineFlow() async {
    let flowCoordinator = MediaEventsTimelineFlowCoordinator(
      roomProxy: roomProxy,
      navigationStackCoordinator: navigationStackCoordinator,
      flowParameters: flowParameters)
  ...
  private func startSpaceFlow(spaceRoomListProxy: SpaceRoomListProxyProtocol, animated: Bool) {
    let coordinator = SpaceFlowCoordinator(
  ...
  private func startMembersFlow(entryPoint: RoomMembersFlowCoordinatorEntryPoint, animated: Bool) {
    let flowCoordinator = RoomMembersFlowCoordinator(

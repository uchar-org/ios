...
typealias CreateRoomScreenViewModelType = StateStoreViewModel<
  CreateRoomScreenViewState, CreateRoomScreenViewAction
>
...
class CreateRoomScreenViewModel: CreateRoomScreenViewModelType, CreateRoomScreenViewModelProtocol {
  ...
  init(
    ...
    let bindings = CreateRoomScreenViewStateBindings(
      roomTopic: "",
      selectedAccessType: selectedAccessType,
      selectedSpace: selectedSpace)

    super.init(
      initialViewState: CreateRoomScreenViewState(
        isSpace: isSpace,
  ...
  func updateAvatar(fileURL: URL) {
    ...
    Task { [weak self] in
      ...
      do {
        guard case .success(let maxUploadSize) = await userSession.clientProxy.maxMediaUploadSize
        else {
          MXLog.error("Failed to get max upload size")
          state.bindings.alertInfo = .init(id: .unknown)
          return
        }
        let mediaInfo = try await mediaUploadingPreprocessor.processMedia(
          at: fileURL, maxUploadSize: maxUploadSize
        ).get()
    ...
    context.$viewState
      ...
      .sink { [weak self] aliasLocalPart in
        ...
        checkAliasAvailabilityTask = Task { [weak self] in
          ...
          if case .success(false) = await self.userSession.clientProxy.isAliasAvailable(
            canonicalAlias)
          {
  ...
  private func createRoom() async {
    ...
    if !state.roomAccessType.isVisibilityPrivate {
      guard
        let canonicalAlias = String.makeCanonicalAlias(
          aliasLocalPart: state.aliasLocalPart,
          serverName: state.serverName),
        isRoomAliasFormatValid(alias: canonicalAlias)
      else {
    ...
    switch await userSession.clientProxy.createRoom(
      name: state.roomName,
  ...
  private func addRoomToSpace(roomProxy: JoinedRoomProxyProtocol, spaceID: String) async {
    ...
    do {
      ...
      if case .failure = await userSession.clientProxy.spaceService.addChild(
        roomProxy.id, to: spaceID)
      {
        MXLog.error(
          "Failed to add the created room with id: \(roomProxy.id) to the space with id: \(spaceID)"
        )
        userIndicatorController.submitIndicator(.init(title: L10n.errorUnknown))
      }
    } catch {

...
struct AppLockTimerTests {
  ...
  mutating func timerLockedOnStartup() {
    ...
    #expect(
      timer.computeLockState(didBecomeActiveAt: now),
      "The app should be locked on a fresh launch.")

    setupTimer(unlocked: false)
    #expect(
      timer.computeLockState(didBecomeActiveAt: now + 1),
      "The app should be locked after a fresh launch.")

    setupTimer(unlocked: false)
    #expect(
      timer.computeLockState(didBecomeActiveAt: now + halfGracePeriod),
      "The app should be locked after a fresh launch.")

    setupTimer(unlocked: false)
    #expect(
      timer.computeLockState(didBecomeActiveAt: now + gracePeriod),
      "The app should be locked after a fresh launch.")

    setupTimer(unlocked: false)
    #expect(
      timer.computeLockState(didBecomeActiveAt: now + gracePeriodX10),
      "The app should be locked after a fresh launch.")
  }
  ...
  mutating func timerRepeatingWithinGracePeriod() {
    ...
    #expect(
      !timer.computeLockState(didBecomeActiveAt: nextCheck),
      "The app should remain unlocked when it was unlocked and backgrounded for less then the grace period."
    )
  ...
  mutating func timerWithLongForeground() {
    ...
    #expect(
      !timer.computeLockState(didBecomeActiveAt: backgroundDate + 1),
  ...
  private mutating func setupTimer(
    gracePeriod: TimeInterval = 180, unlocked: Bool, backgroundedAt backgroundedDate: Date? = nil
  ) {

...
struct BumpCalendarVersion: ParsableCommand {
  static let configuration = CommandConfiguration(
    abstract: "A tool that bumps the CalVer every month (if needed), setting the patch back to 0.",
  ...
  private func updateProjectYAML() throws {
    ...
    yamlString.enumerateLines { line, _ in
      let processedLine =
        if let match = line.firstMatch(of: marketingVersionRegex),
  ...
  private func generateNewVersion(from currentVersion: String) throws -> String? {
    ...
    guard versionComponents.count == 3 else {
      fatalError("Unexpected version format: \(currentVersion)")
    }

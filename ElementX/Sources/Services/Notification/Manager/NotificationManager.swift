...
final class NotificationManager: NSObject, NotificationManagerProtocol {
  ...
  func requestAuthorization() {
    ...
    Task {
      do {
        let permissionGranted = try await notificationCenter.requestAuthorization(options: [
          .alert, .sound, .badge,
        ])
    ...
    let notificationsIdentifiers =
      ...
      .filter { notification in
        guard let roomID = notification.request.content.roomID,
          let lastMessageDate = roomsToLastMessageDates[roomID]
        else {
          return false
        }
  ...
  private func removeReceivedWhileOfflineNotification() {
    notificationCenter.removeDeliveredNotifications(withIdentifiers: [
      NotificationServiceExtension.receivedWhileOfflineNotificationID
    ])
  }
...
extension NotificationManager: UNUserNotificationCenterDelegate {
  ...
  func userNotificationCenter(
    ...
    case NotificationConstants.Action.inlineReply:
      ...
      await delegate?.handleInlineReply(
        self,

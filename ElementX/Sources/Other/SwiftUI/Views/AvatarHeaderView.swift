...
struct AvatarHeaderView<Footer: View>: View {
  ...
  init(
    ...
    if let roomAlias = room.canonicalAlias {
      subtitle = roomAlias
    } else if room.isDirect, case .heroes(let heroes) = room.avatar, heroes.count == 1 {
  ...
  init(
    accountOwner: RoomMemberDetails,
  ...
  private var badgesStack: some View {
    HFlow(
      ...
      ForEach(badges, id: \.self) { badge in
        ...
        case .encrypted(true):
          BadgeLabel(
            title: L10n.screenRoomDetailsBadgeEncrypted,
            icon: \.lockSolid,
            style: .accent)
        case .encrypted(false):
          BadgeLabel(
            title: L10n.screenRoomDetailsBadgeNotEncrypted,
            icon: \.lockOff,
            style: .info)
        case .public:
          BadgeLabel(
            title: L10n.screenRoomDetailsBadgePublic,
            icon: \.public,
            style: .info)
        case .verified:
          BadgeLabel(
            title: L10n.commonVerified,
            icon: \.verified,
            style: .accent)
        case .historySharingState(.hidden):
          BadgeLabel(
            title: L10n.cryptoHistorySharingRoomInfoHiddenBadgeContent,
            icon: \.visibilityOff,
            style: .info)
        case .historySharingState(.shared):
          BadgeLabel(
            title: L10n.cryptoHistorySharingRoomInfoSharedBadgeContent,
            icon: \.history,
            style: .info)
        case .historySharingState(.worldReadable):
          BadgeLabel(
            title: L10n.cryptoHistorySharingRoomInfoWorldReadableBadgeContent,
...
struct AvatarHeaderView_Previews: PreviewProvider, TestablePreview {
  static var previews: some View {
    ...
    Form {
      AvatarHeaderView(
        accountOwner: RoomMemberDetails(withProxy: RoomMemberProxyMock.mockMe),
        dmRecipient: RoomMemberDetails(withProxy: RoomMemberProxyMock.mockAlice),
        mediaProvider: MediaProviderMock(configuration: .init())
      ) {
        HStack(spacing: 32) {
          ShareLink(item: "test") {
    ...
    VStack(spacing: 16) {
      AvatarHeaderView(
        member: RoomMemberDetails(withProxy: RoomMemberProxyMock.mockAlice),
        avatarSize: .room(on: .details),
        mediaProvider: MediaProviderMock(configuration: .init())
      ) { Text("") }

      AvatarHeaderView(
        member: RoomMemberDetails(withProxy: RoomMemberProxyMock.mockBob),
        isVerified: true,
        avatarSize: .room(on: .details),
        mediaProvider: MediaProviderMock(configuration: .init())
      ) { Text("") }
      ...
      AvatarHeaderView(
        member: RoomMemberDetails(withProxy: RoomMemberProxyMock.mockBanned[3]),
        avatarSize: .room(on: .details),
        mediaProvider: MediaProviderMock(configuration: .init())
      ) { Text("") }
    }
    ...
    makeHistorySharingPreview(state: .worldReadable).previewDisplayName(
      "History Sharing - World Readable")
  }

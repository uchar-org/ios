...
struct SpaceAddRoomsScreenViewModelTests {
  ...
  init() {
    ...
    spaceRoomListProxy = SpaceRoomListProxyMock(
      .init(spaceServiceRoom: SpaceServiceRoom.mock(isSpace: true)))

    let clientProxy = ClientProxyMock(.init())
    clientProxy.recentlyVisitedRoomsFilterReturnValue = .init(
      repeating: JoinedRoomProxyMock(.init()), count: 5)
    spaceServiceProxy =
      clientProxy.underlyingSpaceService as? SpaceServiceProxyMock ?? SpaceServiceProxyMock(.init())
  ...
  func addingChildRoom() async throws {
    var deferred = deferFulfillment(
      context.observe(\.viewState.roomsSection),
      message: "The screen should start with some suggestions."
    ) { section in
    ...
    deferred = deferFulfillment(
      context.observe(\.viewState.roomsSection),
      message: "The screen should show search results when there's a query."
    ) { section in
    ...
    let room = try #require(
      context.viewState.roomsSection.rooms.first, "Expected a room in the section")
  ...
  func failureWithMultipleRoomsSelected() async throws {
    ...
    var deferred = deferFulfillment(
      context.observe(\.viewState.roomsSection),
      message: "There should be 4 search results."
    ) { section in
    ...
    deferred = deferFulfillment(
      context.observe(\.viewState.roomsSection),
      message: "The search results should update."
    ) { section in
    ...
    #expect(
      spaceServiceProxy.addChildToCallsCount == 3,
      "The remaining calls to the service should stop after a failure.")
    #expect(
      !context.viewState.selectedRooms.contains { successfulIDs.contains($0.id) },
      "The added rooms should no longer show as selected.")
    #expect(
      !context.viewState.roomsSection.rooms.contains { successfulIDs.contains($0.id) },

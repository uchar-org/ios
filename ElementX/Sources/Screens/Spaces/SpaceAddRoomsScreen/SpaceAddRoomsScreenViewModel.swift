...
typealias SpaceAddRoomsScreenViewModelType = StateStoreViewModelV2<
  SpaceAddRoomsScreenViewState, SpaceAddRoomsScreenViewAction
>

class SpaceAddRoomsScreenViewModel: SpaceAddRoomsScreenViewModelType,
  SpaceAddRoomsScreenViewModelProtocol
{
  ...
  private var actionsSubject: PassthroughSubject<SpaceAddRoomsScreenViewModelAction, Never> =
    .init()
  ...
  init(
    ...
    super.init(
      initialViewState: SpaceAddRoomsScreenViewState(
        roomsSection: .init(type: .suggestions, rooms: [])),
  ...
  private func updateRooms() {
    ...
    let rooms = roomSummaryProvider.roomListPublisher.value
      ...
      .filter { [addedRoomIDs] summary in
        !summary.isDirect && !existingRooms.contains { $0.id == summary.id }
          && !addedRoomIDs.contains(summary.id)
      }

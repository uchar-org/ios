...
enum MXLog {
  ...
  static func configure(currentTarget: String) {
    ...
    rootSpan = Span(
      file: #file, line: #line, level: .info, target: self.currentTarget, name: "root",
      bridgeTraceId: nil)
    rootSpan.enter()
  }
  ...
  static func failure(
    ...
    static func dev(
      ...
      log(
        "[\(devPrefix)] \(message)", level: .info, file: file, function: function, line: line,
        column: column)
    }
  #endif
  ...
  private static func createSpan(
    ...
    return Span(
      file: file, line: UInt32(line), level: level.rustLogLevel, target: currentTarget, name: name,
      bridgeTraceId: nil)
  }
  ...
  private static func log(
    ...
    logEvent(
      file: (file as NSString).lastPathComponent, line: UInt32(line), level: level.rustLogLevel,
      target: currentTarget, message: "\(message)")
  }
}

...
struct SecurityAndPrivacyScreenViewState: BindableState {
  ...
  var isSaveDisabled: Bool {
    !hasChanges
      || (currentSettings.isVisibileInRoomDirectory == nil
        && bindings.desiredSettings.accessType != .inviteOnly && canonicalAlias != nil)
  }
  ...
  var isAskToJoinWithSpaceMembersOptionAvailable: Bool {
    currentSettings.accessType.isAskToJoinWithSpaceMembers
  ...
  var spaceSelection: SpaceSelection {
    if selectableSpacesCount == 0 {
      ...
      if currentSettings.accessType.isSpaceMembers
        || currentSettings.accessType.isAskToJoinWithSpaceMembers
      {
  ...
  init(
    ...
    let settings = SecurityAndPrivacySettings(
...
enum SecurityAndPrivacyHistoryVisibility: Int, Comparable {
  ...
  static func < (lhs: SecurityAndPrivacyHistoryVisibility, rhs: SecurityAndPrivacyHistoryVisibility)
    -> Bool
  {
...
struct SecurityAndPrivacyScreenStrings {
  ...
  init(historySharingDetailsURL: URL) {
    ...
    var accessFooterString = AttributedString(
      L10n.screenSecurityAndPrivacyRoomAccessFooter(linkPlaceholder))
    var accessLinkString = AttributedString(
      L10n.screenSecurityAndPrivacyRoomAccessFooterManageSpacesAction)
    ...
    var historyFooterString = AttributedString(
      L10n.screenSecurityAndPrivacyRoomHistorySectionFooter(linkPlaceholder))

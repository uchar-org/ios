...
final class ComposerToolbarViewModelTests {
  ...
  func composerFocus() {
    viewModel.process(
      timelineAction: .setMode(
        mode: .edit(originalEventOrTransactionID: .eventID("mock"), type: .default)))
  ...
  func composerModeIsPublished() async throws {
    ...
    let deferred = deferFulfillment(
      viewModel.context.$viewState.map(\.composerMode).removeDuplicates().dropFirst()
    ) { $0 == mode }
    viewModel.process(timelineAction: .setMode(mode: mode))
  ...
  func suggestions() {
    let suggestions: [SuggestionItem] = [
      ...
      .init(
        suggestionType: .user(
          .init(id: "@user_mention_2:matrix.org", displayName: "User 2", avatarURL: nil)),
        range: .init(), rawSuggestionText: ""),
    ]
  ...
  func suggestionTrigger() async throws {
    let deferred = deferFulfillment(wysiwygViewModel.$attributedContent) {
      $0.plainText == "#room-alias-test"
    }
    ...
    #expect(
      completionSuggestionServiceMock.setSuggestionTriggerReceivedInvocations == [
        nil,
        .init(type: .user, text: "user-test", range: .init(location: 0, length: 10)),
        .init(
          type: .room, text: "room-alias-test",
  ...
  func selectedUserSuggestion() {
    let suggestion = SuggestionItem(
      suggestionType: .user(.init(id: "@test:matrix.org", displayName: "Test", avatarURL: nil)),
      range: .init(), rawSuggestionText: "")
    ...
    #expect(
      wysiwygViewModel.content.html
        == "<a href=\"https://matrix.to/#/@test:matrix.org\">@test:matrix.org</a> ")
  }
  ...
  func selectedRoomSuggestion() {
    ...
    #expect(
      wysiwygViewModel.content.html
        == "<a href=\"https://matrix.to/#/%23room-alias:matrix.org\">#room-alias:matrix.org</a> ")
  }

  @Test
  func allUsersSuggestion() throws {
    let suggestion = SuggestionItem(
      suggestionType: .allUsers(.room(id: "", name: nil, avatarURL: nil)), range: .init(),
      rawSuggestionText: "")
    viewModel.context.send(viewAction: .selectedSuggestion(suggestion))
  ...
  func userMentionPillInRTE() async {
    ...
    let suggestion = SuggestionItem(
      suggestionType: .user(.init(id: userID, displayName: "Test", avatarURL: nil)), range: .init(),
      rawSuggestionText: "")
    viewModel.context.send(viewAction: .selectedSuggestion(suggestion))

    let attachment =
      wysiwygViewModel.textView.attributedText.attribute(.attachment, at: 0, effectiveRange: nil)
      as? PillTextAttachment
  ...
  func roomMentionPillInRTE() async {
    ...
    let suggestion = SuggestionItem(
      suggestionType: .room(
        .init(
          id: "room-id", canonicalAlias: roomAlias, name: "Room",
          avatar: .room(id: "room-id", name: "Room", avatarURL: nil))), range: .init(),
      rawSuggestionText: "")
    viewModel.context.send(viewAction: .selectedSuggestion(suggestion))
  ...
  func intentionalMentions() async throws {
    ...
    let deferred = deferFulfillment(viewModel.actions) { action in
      switch action {
      case .sendMessage(_, _, _, let intentionalMentions):
        return intentionalMentions
          == IntentionalMentions(userIDs: ["@test:matrix.org"], atRoom: true)
      default:
        return false
      }
    }
  ...
  func saveDraftEdit() async throws {
    ...
    viewModel.process(
      timelineAction: .setMode(
        mode: .edit(originalEventOrTransactionID: .eventID("testID"), type: .default)))
    viewModel.context.plainComposerText = .init(string: "Hello world!")
    ...
    viewModel.process(
      timelineAction: .setMode(
        mode: .previewVoiceMessage(
          state: AudioPlayerState(id: .recorderPreview, title: "", duration: 10.0),
  ...
  func restoreEdit() async {
    ...
    #expect(
      viewModel.state.composerMode
        == .edit(originalEventOrTransactionID: .eventID("testID"), type: .default))
    #expect(viewModel.context.plainComposerText == NSAttributedString(string: "Hello world!"))
  }
  ...
  func restoreReply() async throws {
    ...
    let loadedReply = TimelineItemReplyDetails.loaded(
      sender: .init(
        id: "userID",
  ...
  func restoreReplyAndCancelReplyMode() async throws {
    ...
    let text = "Hello world!"
    let loadedReply = TimelineItemReplyDetails.loaded(
      sender: .init(id: "userID", displayName: "Username"),
      eventID: testEventID,
      eventContent: .message(.text(.init(body: "Reply text"))))
  ...
  func restoreVolatileDraftWhenClearing() async {
    await waitForConfirmation("Volatile draft loaded and cleared", expectedCount: 2) {
      confirmation in
      draftServiceMock.loadVolatileDraftClosure = {
        defer { confirmation() }
        return .init(
  ...
  func restoreUserMentionInPlainText() async throws {
    ...
    let deferred = deferFulfillment(viewModel.actions) { action in
      switch action {
      case .sendMessage(let plainText, _, _, let intentionalMentions):
        ...
        return plainText == "Hello [@test:matrix.org](https://matrix.to/#/@test:matrix.org)!"
          && intentionalMentions
            == IntentionalMentions(userIDs: ["@test:matrix.org"], atRoom: false)
      default:
        return false
      }
    }
  ...
  func restoreMixedMentionsInPlainText() async throws {
    viewModel.context.composerFormattingEnabled = false
    let text =
      "Hello [User1](https://matrix.to/#/@user1:matrix.org), [User2](https://matrix.to/#/@user2:matrix.org) and @room"
    viewModel.process(timelineAction: .setText(plainText: text, htmlText: nil))
    ...
    let deferred = deferFulfillment(viewModel.actions) { action in
      ...
      case .sendMessage(let plainText, _, _, let intentionalMentions):
        // As of right now the markdown loses the display name when restored
        return plainText
          == "Hello [@user1:matrix.org](https://matrix.to/#/@user1:matrix.org), [@user2:matrix.org](https://matrix.to/#/@user2:matrix.org) and @room"
          && intentionalMentions
            == IntentionalMentions(
              userIDs: ["@user1:matrix.org", "@user2:matrix.org"], atRoom: true)
      default:
        return false
      }
    }
    ...
    let deferred = deferFulfillment(viewModel.actions) { action in
      ...
      case .sendMessage(let plainText, _, _, let intentionalMentions):
        ...
        return plainText == "Hello [@roomuser:matrix.org](https://matrix.to/#/@roomuser:matrix.org)"
          && intentionalMentions
            == IntentionalMentions(userIDs: ["@roomuser:matrix.org"], atRoom: false)
      default:
        return false
      }
    }
  ...
  func verificationViolationDisablesComposer() async throws {
    ...
    let roomMemberProxyMock = RoomMemberProxyMock(
    ...
    var fulfillment = deferFulfillment(
      viewModel.context.$viewState, message: "Composer is disabled"
    ) { $0.canSend == false }
    mockSubject.send([
    ...
    fulfillment = deferFulfillment(viewModel.context.$viewState, message: "Composer is enabled") {
      $0.canSend == true
    }
  ...
  func multipleViolation() async throws {
    ...
    let aliceRoomMemberProxyMock = RoomMemberProxyMock(
      with: .init(userID: "@alice:localhost", membership: .join))
    let bobRoomMemberProxyMock = RoomMemberProxyMock(
      with: .init(userID: "@bob:localhost", membership: .join))
    ...
    fulfillment = deferFulfillment(
      viewModel.context.$viewState, message: "Composer is still disabled"
    ) { $0.canSend == false }
    mockSubject.send([IdentityStatusChange(userId: "@alice:localhost", changedTo: .pinned)])
    try await fulfillment.fulfill()

    fulfillment = deferFulfillment(viewModel.context.$viewState, message: "Composer is now enabled")
    { $0.canSend == true }
  ...
  func pinViolationDoesNotDisableComposer() async throws {
    ...
    roomProxyMock.underlyingIdentityStatusChangesPublisher = CurrentValueSubject([
      IdentityStatusChange(userId: "@alice:localhost", changedTo: .pinViolation)
    ...
    let deferred = deferFulfillment(
      viewModel.context.$viewState, message: "Composer should be enabled"
    ) { $0.canSend == true }
    try await deferred.fulfill()
  }

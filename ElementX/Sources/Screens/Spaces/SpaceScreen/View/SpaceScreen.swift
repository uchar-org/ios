...
struct SpaceScreen: View {
  ...
  var body: some View {
    ScrollView {
      LazyVStack(spacing: 0) {
        if !isEditModeActive {
          SpaceHeaderView(
            spaceServiceRoom: context.viewState.space,
  ...
  var rooms: some View {
    ForEach(context.viewState.visibleRooms, id: \.id) { spaceServiceRoom in
      SpaceRoomCell(
        spaceServiceRoom: spaceServiceRoom,
  ...
  var emptyState: some View {
    VStack(spacing: 24) {
      TitleAndIcon(
        title: L10n.screenSpaceEmptyStateTitle,
        icon: \.room,
      ...
      VStack(spacing: 16) {
        Button {
          context.send(viewAction: .addExistingRooms)
        } label: {
  ...
  var toolbar: some ToolbarContent {
    ...
    ToolbarItem(placement: .principal) {
      RoomHeaderView(
        roomName: context.viewState.space.name,
        ...
        Menu {
          ...
          Section {
            if let roomProxy = context.viewState.roomProxy {
              Button {
                context.send(viewAction: .displayMembers(roomProxy: roomProxy))
              } label: {
...
struct SpaceScreen_Previews: PreviewProvider, TestablePreview {
  ...
  static var previews: some View {
    ...
    .snapshotPreferences(
      expect: newSpaceViewModel.context.observe(\.viewState).map(\.canEditChildren))
  }

  static func makeViewModel(isManagingRooms: Bool = false, isNewSpace: Bool = false)
    -> SpaceScreenViewModel
  {
    let spaceServiceRoom = SpaceServiceRoom.mock(
      id: "!eng-space:matrix.org",
      ...
      topic:
        "Description of the space goes right here. Lorem ipsum dolor sit amet consectetur. Leo viverra morbi habitant in.",
      canonicalAlias: "#engineering-team:element.io",
      joinRule: .knockRestricted(rules: [.roomMembership(roomID: "")]))
    let spaceRoomListProxy = SpaceRoomListProxyMock(
      .init(
        spaceServiceRoom: spaceServiceRoom,
    ...
    let viewModel = SpaceScreenViewModel(

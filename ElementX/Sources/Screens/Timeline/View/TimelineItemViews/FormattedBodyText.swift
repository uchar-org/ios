...
struct FormattedBodyText: View {
  ...
  private var attributedComponents: [AttributedStringBuilderComponent] {
    ...
    if !String(attributedString.characters).starts(with: "\t") {
      adjustedAttributedString =
        AttributedString(layoutDirection.isolateLayoutUnicodeString) + adjustedAttributedString
    }
  ...
  private var additionalWhitespacesSuffix: String {
    .generateBreakableWhitespaceEnd(
      whitespaceCount: additionalWhitespacesCount, layoutDirection: layoutDirection)
  }
  ...
  var layout: some View {
    TimelineBubbleLayout(spacing: 8) {
      ForEach(attributedComponents) { component in
        ...
        if String(component.attributedString.characters)
          == layoutDirection.isolateLayoutUnicodeString
        {
          EmptyView()
        } else {
...
struct FormattedBodyText_Previews: PreviewProvider, TestablePreview {
  static let attributedStringBuilder = AttributedStringBuilder(
    cacheKey: "FormattedBodyText", mentionBuilder: MentionBuilder())
  ...
  static var basicText: some View {
    VStack(alignment: .leading, spacing: 4.0) {
      FormattedBodyText(
        attributedString: AttributedString("Some plain text wrapped in an AttributedString.")
      )
      .bubbleBackground()

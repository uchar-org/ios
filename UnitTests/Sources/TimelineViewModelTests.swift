...
final class TimelineViewModelTests {
  ...
  func messageGrouping() {
    ...
    #expect(
      viewModel.state.timelineState.itemViewStates[0].groupStyle == .first,
      "Nothing should prevent the first message from being grouped.")
    #expect(
      viewModel.state.timelineState.itemViewStates[1].groupStyle == .middle,
      "Nothing should prevent the middle message from being grouped.")
    #expect(
      viewModel.state.timelineState.itemViewStates[2].groupStyle == .last,
      "Nothing should prevent the last message from being grouped.")
  }
  ...
  func messageGroupingMultipleSenders() {
    ...
    #expect(
      viewModel.state.timelineState.itemViewStates[0].groupStyle == .single,
      "A message should not be grouped when the sender changes.")
    #expect(
      viewModel.state.timelineState.itemViewStates[1].groupStyle == .single,
      "A message should not be grouped when the sender changes.")
    #expect(
      viewModel.state.timelineState.itemViewStates[2].groupStyle == .first,
      "A group should start with a new sender if there are more messages from that sender.")
    #expect(
      viewModel.state.timelineState.itemViewStates[3].groupStyle == .last,
      "A group should be ended when the sender changes in the next message.")
    #expect(
      viewModel.state.timelineState.itemViewStates[4].groupStyle == .first,
      "A group should start with a new sender if there are more messages from that sender.")
    #expect(
      viewModel.state.timelineState.itemViewStates[5].groupStyle == .last,
      "A group should be ended when the sender changes in the next message.")
  }
  ...
  func messageGroupingWithLeadingReactions() {
    ...
    #expect(
      viewModel.state.timelineState.itemViewStates[0].groupStyle == .single,
      "When the first message has reactions it should not be grouped.")
    #expect(
      viewModel.state.timelineState.itemViewStates[1].groupStyle == .first,
      "A new group should be made when the preceding message has reactions.")
    #expect(
      viewModel.state.timelineState.itemViewStates[2].groupStyle == .last,
      "Nothing should prevent the last message from being grouped.")
  }
  ...
  func messageGroupingWithInnerReactions() {
    ...
    #expect(
      ...
      "Nothing should prevent the first message from being grouped.")
    #expect(
      viewModel.state.timelineState.itemViewStates[1].groupStyle == .last,
      "When the message has reactions, the group should end here.")
    #expect(
      viewModel.state.timelineState.itemViewStates[2].groupStyle == .single,
      "The last message should not be grouped when the preceding message has reactions.")
  }
  ...
  func messageGroupingWithTrailingReactions() {
    ...
    #expect(
      viewModel.state.timelineState.itemViewStates[1].groupStyle == .middle,
      "Nothing should prevent the second message from being grouped.")
    #expect(
      viewModel.state.timelineState.itemViewStates[2].groupStyle == .last,
      "Reactions on the last message should not prevent it from being grouped.")
  }
  ...
  func focusItem() async throws {
    ...
    #expect(!viewModel.context.viewState.timelineState.isLive)
    #expect(
      viewModel.context.viewState.timelineState.focussedEvent
        == .init(eventID: "t4", appearance: .immediate))
  }
  ...
  func focusLoadedItem() async throws {
    ...
    let deferred = deferFailure(viewModel.context.$viewState, timeout: .seconds(1)) {
      !$0.timelineState.isLive
    }
    ...
    #expect(
      viewModel.context.viewState.timelineState.focussedEvent
        == .init(eventID: "t1", appearance: .animated))
  }
  ...
  func initialFocusViewState() {
    ...
    #expect(
      viewModel.context.viewState.timelineState.focussedEvent
        == .init(eventID: "t10", appearance: .immediate))
  }
  ...
  func sendReadReceiptWithoutEvents() async throws {
    ...
    let items = [
      SeparatorRoomTimelineItem(uniqueID: .init("v1")),
  ...
  private func readReceiptsConfiguration(with items: [RoomTimelineItemProtocol]) -> (
    TimelineViewModel,
    JoinedRoomProxyMock,
    TimelineProxyMock,
  ...
  func showReadReceipts() async throws {
    let receipts: [ReadReceipt] = [
      .init(userID: "@alice:matrix.org", formattedTimestamp: "12:00"),
      .init(userID: "@charlie:matrix.org", formattedTimestamp: "11:00"),
    ]
    // Given 3 messages from Bob where the middle message has a reaction.
    let message = TextRoomTimelineItem(
      text: "Test",
      sender: "bob",
    ...
    let viewModel = TimelineViewModel(
      roomProxy: JoinedRoomProxyMock(
        .init(name: "", members: [RoomMemberProxyMock.mockAlice, RoomMemberProxyMock.mockCharlie])),
      timelineController: timelineController,
      userSession: UserSessionMock(.init()),
      mediaPlayerProvider: MediaPlayerProviderMock(),
      userIndicatorController: userIndicatorControllerMock,
  ...
  func showManageUserAsAdmin() async throws {
    ...
    viewModel.context.send(
      viewAction: .tappedOnSenderDetails(sender: .init(with: RoomMemberProxyMock.mockAlice)))
    try await deferred.fulfill()
  ...
  func showDetailsForAnAdmin() async throws {
    ...
    viewModel.context.send(
      viewAction: .tappedOnSenderDetails(sender: .init(with: RoomMemberProxyMock.mockAdmin)))
    try await deferredState.fulfill()
  ...
  func showDetailsForABannedUser() async throws {
    ...
    viewModel.context.send(
      viewAction: .tappedOnSenderDetails(sender: .init(with: RoomMemberProxyMock.mockBanned[0])))
    try await deferredState.fulfill()
  ...
  func pinnedEvents() async throws {
    ...
    let infoSubject = CurrentValueSubject<RoomInfoProxyProtocol, Never>(
  ...
  func tapSendInfoEncryptionAuthentictyDisplaysAlert() {
    ...
    let items = [
      TextRoomTimelineItem(
        eventID: "t1", encryptionAuthenticity: .verificationViolation(color: .red))
    ]
    let timelineController = MockTimelineController()
    timelineController.timelineItems = items
    let viewModel = makeViewModel(timelineController: timelineController)
  ...
  func tapSendInfoEncryptionForwarderDisplaysAlert() {
    ...
    #expect(
      viewModel.state.bindings.alertInfo?.title
        == "alice (@alice:matrix.org) shared this message since you were not in the room when it was sent."
    )
  }
  ...
  private func makeViewModel(
    ...
    TimelineViewModel(
      roomProxy: roomProxy ?? JoinedRoomProxyMock(.init(name: "")),
...
extension TextRoomTimelineItem {
  fileprivate init(
    text: String, sender: String, addReactions: Bool = false, addReadReceipts: [ReadReceipt] = []
  ) {
    let reactions =
      addReactions
      ? [
        AggregatedReaction(
          accountOwnerID: "bob", key: "🦄", senders: [ReactionSender(id: sender, timestamp: Date())])
      ] : []
    self.init(
      id: .randomEvent,
      timestamp: .mock,
      ...
      properties: RoomTimelineItemProperties(
        reactions: reactions, orderedReadReceipts: addReadReceipts))
  }
}

extension SeparatorRoomTimelineItem {
  fileprivate init(uniqueID: TimelineItemIdentifier.UniqueID) {
    self.init(id: .virtual(uniqueID: uniqueID), timestamp: .mock)
  }
}

extension TextRoomTimelineItem {
  fileprivate init(eventID: String) {
    self.init(
      id: .event(uniqueID: .init(UUID().uuidString), eventOrTransactionID: .eventID(eventID)),
      timestamp: .mock,
      isOutgoing: false,
      isEditable: false,
      canBeRepliedTo: true,
...
extension TextRoomTimelineItem {
  fileprivate init(eventID: String, keyForwarder: TimelineItemKeyForwarder) {
    self.init(
      id: .event(uniqueID: .init(UUID().uuidString), eventOrTransactionID: .eventID(eventID)),
      timestamp: .mock,
      isOutgoing: false,
...
extension TextRoomTimelineItem {
  fileprivate init(eventID: String, encryptionAuthenticity: EncryptionAuthenticity) {
    self.init(
      id: .event(uniqueID: .init(UUID().uuidString), eventOrTransactionID: .eventID(eventID)),
      timestamp: .mock,
      isOutgoing: false,
...
extension TimelineItemSender {
  fileprivate init(with proxy: RoomMemberProxyMock) {
    self.init(
      id: proxy.userID,
...
extension TimelineItemKeyForwarder {
  fileprivate static var test: TimelineItemKeyForwarder {

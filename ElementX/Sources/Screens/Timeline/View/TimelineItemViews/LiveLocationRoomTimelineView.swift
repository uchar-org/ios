...
struct LiveLocationRoomTimelineView: View {
  ...
  init(
    currentDate: Date = .now, timelineItem: LiveLocationRoomTimelineItem, isStopped: Bool = false
  ) {
  ...
  private var liveContent: some View {
    if let geoURI = timelineItem.content.lastGeoURI {
      MapLibreStaticMapView(
        ...
        LocationMarkerView(
          kind: .liveUser(.init(sender: timelineItem.sender)),
...
extension Date {
  ...
  fileprivate static var mockToday410: Date {
  ...
  fileprivate static var mockToday420: Date {
    // swiftlint:disable:next force_unwrapping
  ...
  fileprivate static var mockToday430: Date {
    // swiftlint:disable:next force_unwrapping
...
struct LiveLocationRoomTimelineView_Previews: PreviewProvider, TestablePreview {
  ...
  static var states: some View {
    ...
    LiveLocationRoomTimelineView(
      currentDate: .mockToday410,
      timelineItem: .init(
        id: .randomEvent,
        timestamp: .mock,
        isOutgoing: false,

...
struct NotificationItemProxyMockConfiguration {
  var event: NotificationEvent? = {
    ...
    let messageType = MessageType.text(
      content: TextMessageContent(body: "Hello world!", formatted: nil))
    let messageLikeContent = MessageLikeEventContent.roomMessage(
      messageType: messageType, inReplyToEventId: nil)

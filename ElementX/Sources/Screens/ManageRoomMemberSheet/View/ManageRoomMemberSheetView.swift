  ...
  var body: some View {
    Form {
      ...
      Section {
        ListRow(
          label: .default(
            title: L10n.screenBottomSheetManageRoomMemberMemberUserInfo,
      ...
      Section {
        if context.viewState.permissions.canKick, !context.viewState.isMemberBanned {
          ListRow(
            label: .default(
              title: L10n.screenBottomSheetManageRoomMemberRemove,
              icon: \.close,
              role: .destructive),
            kind: .button {
        ...
        if context.viewState.permissions.canBan {
          if !context.viewState.isMemberBanned {
            ListRow(
              label: .default(
                title: L10n.screenBottomSheetManageRoomMemberBan,
                icon: \.block,
                role: .destructive),
              kind: .button {
            ...
            ListRow(
              label: .default(
                title: L10n.screenBottomSheetManageRoomMemberUnban,
                icon: \.restart,
                role: .destructive),
              kind: .button {
...
struct ManageRoomMemberSheetView_Previews: PreviewProvider, TestablePreview {
  ...
  static let allActionsDisabledViewModel = ManageRoomMemberSheetViewModel.mock(
    powerLevel: .init(value: 0))
  ...
  static let unbanOnlyViewModel = ManageRoomMemberSheetViewModel.mock(
    canKick: true, memberIsBanned: true)
...
extension ManageRoomMemberSheetViewModel {
  fileprivate static func mock(
    canKick: Bool = true,
    canBan: Bool = true,
    memberIsBanned: Bool = false,
    powerLevel: RoomPowerLevel = .init(value: 100)
    ...
    return ManageRoomMemberSheetViewModel(
      memberDetails: .memberDetails(roomMember: member),
      permissions: .init(
        canKick: canKick,
        canBan: canBan,

...
struct KeychainControllerTests {
  ...
  func addRestorationToken() {
    ...
    #expect(
      keychain.restorationTokenForUsername(username) == restorationToken,
      "The retrieved restoration token should match the value that was stored.")
  }
  ...
  func removingRestorationToken() {
    ...
    keychain.setRestorationToken(restorationToken, forUsername: username)
    #expect(
      keychain.restorationTokens().count == 1, "The keychain should have 1 restoration token.")
    #expect(
      keychain.restorationTokenForUsername(username) == restorationToken,
      "The initial restoration token should match the value that was stored.")
    ...
    #expect(
      keychain.restorationTokens().isEmpty, "The keychain should be empty after deleting the token."
    )
    #expect(
      keychain.restorationTokenForUsername(username) == nil,
      "There restoration token should not be returned after removal.")
  }
  ...
  func removingAllRestorationTokens() {
    ...
    #expect(
      keychain.restorationTokens().count == 5, "The keychain should have 5 restoration tokens.")
  ...
  func removingSingleRestorationTokens() {
    ...
    #expect(
      keychain.restorationTokens().count == 4, "The keychain have 4 remaining restoration tokens.")
    #expect(
      keychain.restorationTokenForUsername("@test0:example.com") != nil,
      "The restoration token should not have been deleted.")
    #expect(
      keychain.restorationTokenForUsername("@test1:example.com") != nil,
      "The restoration token should not have been deleted.")
    #expect(
      keychain.restorationTokenForUsername("@test2:example.com") == nil,
      "The restoration token should have been deleted.")
    #expect(
      keychain.restorationTokenForUsername("@test3:example.com") != nil,
      "The restoration token should not have been deleted.")
    #expect(
      keychain.restorationTokenForUsername("@test4:example.com") != nil,
      "The restoration token should not have been deleted.")
  }
  ...
  func unsupportedRestorationToken() throws {
    ...
    let underlyingKeychain = Keychain(
      service: KeychainControllerService.tests.restorationTokenID,
      accessGroup: InfoPlistReader.main.keychainAccessGroupIdentifier)
    ...
    let unsupportedToken = RestorationTokenV4(
      session: SessionV1(
        accessToken: "1234",
        refreshToken: nil,
  ...
  func addPINCodeBiometricState() throws {
    ...
    #expect(
      !keychain.containsPINCodeBiometricState(), "A new keychain shouldn't contain biometric state."
    )
    #expect(
      keychain.pinCodeBiometricState() == nil, "A new keychain shouldn't return biometric state.")
    ...
    #expect(
      keychain.containsPINCodeBiometricState(), "The keychain should contain the biometric state.")
    #expect(
      keychain.pinCodeBiometricState() == data,
      "The stored biometric state should match what was set.")
  }
  ...
  func updatePINCodeBiometricState() throws {
    ...
    #expect(
      keychain.containsPINCodeBiometricState(), "The keychain should still contain biometric state."
    )
    #expect(
      keychain.pinCodeBiometricState() != data,
      "The stored biometric state shouldn't match the old value.")
    #expect(
      keychain.pinCodeBiometricState() == newData,
      "The stored biometric state should match the new value.")
  }
  ...
  func removePINCodeBiometricState() throws {
    ...
    #expect(
      !keychain.containsPINCodeBiometricState(),
      "The keychain should no longer contain the biometric state.")
    #expect(
      keychain.pinCodeBiometricState() == nil,
      "There shouldn't be any stored biometric state after removing it.")
  }
}

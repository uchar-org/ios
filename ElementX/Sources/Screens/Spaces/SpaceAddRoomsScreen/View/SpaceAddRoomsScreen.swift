...
struct SpaceAddRoomsScreen_Previews: PreviewProvider, TestablePreview {
  ...
  static var previews: some View {
    ...
    .snapshotPreferences(
      expect: viewModel.context.observe(\.viewState.roomsSection).map {
  ...
  static func makeViewModel(searchQuery: String? = nil, hasSelection: Bool = false)
    -> SpaceAddRoomsScreenViewModel
  {
    ...
    let viewModel = SpaceAddRoomsScreenViewModel(
    ...
    if hasSelection {
      viewModel.state.selectedRooms = Array(summaryProvider.roomListPublisher.value.prefix(2)).map(
        SpaceAddRoomsScreenRoom.init)
    }

...
typealias StartChatScreenViewModelType = StateStoreViewModel<
  StartChatScreenViewState, StartChatScreenViewAction
>
...
class StartChatScreenViewModel: StartChatScreenViewModelType, StartChatScreenViewModelProtocol {
    ...
    case .selectUser(let user):
      ...
      case .success:
        Task {
          ...
          let isUnknown =
            if case .success(let identity) = await self.userSession.clientProxy.userIdentity(
              for: user.userID, fallBackToServer: false)
            {
              identity == nil
            } else {
              true
  ...
  private func resolveRoomAddress(_ roomAddress: String) {
    guard !roomAddress.isEmpty,
      isRoomAliasFormatValid(alias: roomAddress)
    else {
    ...
    resolveAliasTask = Task { [weak self] in
      ...
      guard
        case .success(let resolved) = await userSession.clientProxy.resolveRoomAlias(roomAddress)
      else {
        if Task.isCancelled {
          return
        }
  ...
  private func createDirectRoom(user: UserProfileProxy) async {
    ...
    switch await userSession.clientProxy.createDirectRoom(
      with: user.userID, expectedRoomName: user.displayName)
    {
  ...
  private func joinRoomByAddress() {
    if case .addressFound(let lastTestedAddress, let roomID) = internalRoomAddressState,

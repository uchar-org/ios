...
struct CollapsibleReactionLayout: Layout {
  ...
  func sizeThatFits(proposal: ProposedViewSize, subviews: some FlowLayoutSubviews, cache: inout ())
    -> CGSize
  {
    guard let subviewsByType = getSubviewsByItemType(subviews: Array(subviews)),
      subviewsByType.reactions.count > 0
    else {
      return .zero
    }

    // Calculate the layout of the rows with the reactions button and add more button
    let reactionsAndAddMore = calculateRows(
      proposal: proposal, subviews: Array(subviewsByType.reactions + [subviewsByType.addMoreButton])
    )
    ...
    if let rowsBeforeCollapsible, reactionsAndAddMore.count > rowsBeforeCollapsible {
      if collapsed {
        ...
        let (collapsedRowsWithButtons, _) = replaceTrailingItemsWithButtons(
          rowWidth: proposal.width ?? 0,
          rows: collapsedRows,
  ...
  func placeSubviews(
    in bounds: CGRect, proposal: ProposedViewSize, subviews: some FlowLayoutSubviews,
    cache: inout ()
  ) {
    guard let subviewsByType = getSubviewsByItemType(subviews: Array(subviews)),
      subviewsByType.reactions.count > 0
    ...
    let reactionsAndAddMore = calculateRows(
      ...
      subviews: Array(subviewsByType.reactions + [subviewsByType.addMoreButton]))
    // If we have extended beyond the defined number of rows we are showing the expand/collapse ui
    if let rowsBeforeCollapsible, reactionsAndAddMore.count > rowsBeforeCollapsible {
      if collapsed {
        // Truncate to `rowsBeforeCollapsible` number of rows and replace the item at the end of the last row with the button
        let collapsedRows = Array(reactionsAndAddMore.prefix(rowsBeforeCollapsible))
        let (collapsedRowsWithButtons, subviewsToHide) = replaceTrailingItemsWithButtons(
          rowWidth: bounds.width,
          rows: collapsedRows,
          collapseButton: subviewsByType.collapseButton,
          addMoreButton: subviewsByType.addMoreButton)

        var remainingSubviews =
          subviewsToHide
          + Array(reactionsAndAddMore.suffix(reactionsAndAddMore.count - rowsBeforeCollapsible))
          .joined()
        ...
        var rowsWithButtons = calculateRows(
          proposal: ProposedViewSize(bounds.size), subviews: Array(subviews))
        ensureCollapseAndAddMoreButtonsAreOnTheSameRow(&rowsWithButtons)
      ...
      subviewsByType.collapseButton.place(
        at: Self.pointOffscreen, anchor: .leading, proposal: .zero)
    }
  }
  ...
  private func calculateRows(proposal: ProposedViewSize, subviews: [FlowLayoutSubview])
    -> [[FlowLayoutSubview]]
  {
  ...
  private func sizeThatFits(rows: [[FlowLayoutSubview]]) -> CGSize {
    ...
    return sizes.enumerated().reduce(CGSize.zero) { partialResult, rowItem in
      ...
      let rowSize = row.enumerated().reduce(CGSize.zero) { partialResult, subviewItem in
        ...
        return CGSize(
          width: partialResult.width + size.width + horizontalSpacing, height: maxHeight)
      }
      let verticalSpacing = rowIndex == 0 ? 0 : rowSpacing
      return CGSize(
        width: max(partialResult.width, rowSize.width),
        height: partialResult.height + rowSize.height + verticalSpacing)
    }
  }
  ...
  private func replaceTrailingItemsWithButtons(
    rowWidth: CGFloat, rows: [[FlowLayoutSubview]], collapseButton: FlowLayoutSubview,
    addMoreButton: FlowLayoutSubview
  ) -> ([[FlowLayoutSubview]], [FlowLayoutSubview]) {
    var rows = rows
  ...
  private func placeSubviews(in bounds: CGRect, rows: [[FlowLayoutSubview]]) {
    ...
    for (i, row) in sizes.enumerated() {
      ...
      for (j, size) in row.enumerated() {
        ...
        let point = CGPoint(
          x: rowX + horizontalSpacing, y: rowY + verticalSpacing + (maxHeight / 2))
        subview.place(
          at: point, anchor: .leading,
          proposal: ProposedViewSize(CGSize(width: size.width, height: maxHeight)))
  ...
  private func getSubviewsByItemType(subviews: [FlowLayoutSubview]) -> ReactionSubviews? {
    ...
    return ReactionSubviews(
      reactions: reactions, collapseButton: collapseButton, addMoreButton: addMoreButton)
  }
}
...
protocol FlowLayoutSubviews: RandomAccessCollection
where Element: FlowLayoutSubview, Index == Int, SubSequence == Self {}

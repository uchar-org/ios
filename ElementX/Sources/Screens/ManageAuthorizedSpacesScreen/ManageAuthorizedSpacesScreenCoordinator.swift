...
final class ManageAuthorizedSpacesScreenCoordinator: CoordinatorProtocol {
  ...
  private let actionsSubject:
    PassthroughSubject<ManageAuthorizedSpacesScreenCoordinatorAction, Never> = .init()
  ...
  init(parameters: ManageAuthorizedSpacesScreenCoordinatorParameters) {
    viewModel = ManageAuthorizedSpacesScreenViewModel(
      authorizedSpacesSelection: parameters.authorizedSpacesSelection,

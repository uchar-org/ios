...
struct PollFormScreen: View {
  ...
  private var questionSection: some View {
    Section {
      ListRow(
        label: .plain(title: L10n.screenCreatePollQuestionHint),
    ...
    Section {
      ForEach(context.options) { option in
        if let index = context.options.firstIndex(of: option) {
          PollFormOptionRow(
            text: $context.options[index].text.limited(to: 240),
      ...
      if context.options.count < context.viewState.maxNumberOfOptions {
        ListRow(
          label: .plain(title: L10n.screenCreatePollAddOptionBtn),
          kind: .button {
  ...
  private var showResultsSection: some View {
    Section {
      ListRow(
        label: .plain(title: L10n.screenCreatePollAnonymousDesc),
  ...
  private var deletePollSection: some View {
    ...
    case .edit:
      Section {
        ListRow(
          label: .plain(title: L10n.actionDeletePoll, role: .destructive),
...
extension View {
  ...
  fileprivate func trackAnalyticsIfNeeded(context: PollFormScreenViewModel.Context) -> some View {
    switch context.viewState.mode {
    case .edit:
      self
    case .new:
      ...
      kind: .custom {
        HStack(spacing: 16) {
          if editMode?.wrappedValue == .active {
            ...
            .accessibilityLabel(
              L10n.screenCreatePollRemoveAccessibilityLabel(
                L10n.screenCreatePollOptionAccessibilityLabel(placeholder, text)))
          }
          ...
          .accessibilityLabel(
            text.isEmpty ? "" : L10n.screenCreatePollOptionAccessibilityLabel(placeholder, text)
          )
...
extension Binding where Value == String {
  fileprivate func limited(to limit: Int) -> Self {
    .init {
      wrappedValue
    } set: { newValue in

...
extension SpaceRoomListProxyMock {
  class Configuration {
    ...
    init(
      ...
      paginationStateSubject: CurrentValueSubject<SpaceRoomListPaginationState, Never> = .init(
  ...
  convenience init(_ configuration: Configuration) {
    ...
    let spaceRoomsSubject: CurrentValueSubject<[SpaceServiceRoom], Never> = .init(
      configuration.initialSpaceRooms)
    ...
    paginateClosure = {
      ...
      configuration.paginationStateSubject.send(
        .idle(endReached: configuration.paginationResponses.isEmpty))
    }
  }
}

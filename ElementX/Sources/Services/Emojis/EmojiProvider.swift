...
class EmojiProvider: EmojiProviderProtocol {
  ...
  func categories(searchString: String? = nil) async -> [EmojiCategory] {
    ...
    let emojis =
      ...
      .sorted { first, second in
        guard let firstIndex = frequentlyUsedEmojis.firstIndex(of: first.unicode),
          let secondIndex = frequentlyUsedEmojis.firstIndex(of: second.unicode)
        else {
          return false
        }
    ...
    if !emojis.isEmpty {
      emojiCategories.insert(
        .init(id: EmojiCategory.frequentlyUsedCategoryIdentifier, emojis: emojis), at: 0)
    }
  ...
  func markEmojiAsFrequentlyUsed(_ emoji: String) {
    ...
    let frequentlyUsed =
      if !frequentlyUsedSystemEmojis().contains(emoji) {
    ...
    do {
      ...
      return EmojibaseCategory.allCases.map { category in
        ...
        return EmojiCategory(
          id: category.rawValue, emojis: emojis.compactMap(EmojiItem.init(from:)))
      }
    } catch {

...
struct GlobalSearchScreen: View {
  ...
  private var header: some View {
    GlobalSearchTextFieldRepresentable(placeholder: L10n.actionSearch, text: $context.searchQuery) {
      keyCode in
      switch keyCode {
  ...
  class Coordinator: NSObject, UITextFieldDelegate {
    ...
    func textField(
      ...
      DispatchQueue.main.async {
        self.text.wrappedValue = (currentText as NSString).replacingCharacters(
...
struct GlobalSearchScreen_Previews: PreviewProvider, TestablePreview {
  static let viewModel = GlobalSearchScreenViewModel(

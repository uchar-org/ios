...
struct VoiceMessageButton: View {
  ...
  private var buttonLabel: some View {
    ...
    case .playing, .paused:
      CompoundIcon(
        state == .playing ? \.pauseSolid : \.playSolid,
        size: iconSize,
...
private struct VoiceMessageButtonStyle: ButtonStyle {
  ...
  func makeBody(configuration: Configuration) -> some View {
    configuration.label
      .foregroundColor(
        isEnabled ? color.opacity(configuration.isPressed ? 0.6 : 1) : .compound.iconDisabled
      )
      .background(
        Circle()
          .foregroundColor(
            configuration.isPressed ? .compound.bgSubtlePrimary : .compound.bgCanvasDefault))
  }
}

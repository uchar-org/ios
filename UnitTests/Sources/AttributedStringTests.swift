...
struct AttributedStringTests {
  ...
  func replacingFontWithPresentationIntent() throws {
    ...
    let boldString = "Bold"
    let originalString = try #require(
      AttributedStringBuilder(mentionBuilder: MentionBuilder())
    ...
    for run in string.runs {
      ...
      if String(substring.characters) == boldString {
        #expect(
          run.inlinePresentationIntent == .stronglyEmphasized, "The bold string should be bold.")
      } else {

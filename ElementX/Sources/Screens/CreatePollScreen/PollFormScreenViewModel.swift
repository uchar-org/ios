...
typealias PollFormScreenViewModelType = StateStoreViewModelV2<
  PollFormScreenViewState, PollFormScreenViewAction
>
...
class PollFormScreenViewModel: PollFormScreenViewModelType, PollFormScreenViewModelProtocol {
    ...
    case .submit:
      ...
      Task {
        ...
        case .edit(let eventID, _):
          await editPoll(
            pollStartID: eventID, question: question, options: options, pollKind: pollKind)
        }
      }
    case .delete:
  ...
  private func createPoll(question: String, options: [String], pollKind: Poll.Kind) async {
    guard
      case .success = await timelineController.createPoll(
        question: question, answers: options, pollKind: pollKind)
    else {
    ...
    analytics.trackPollCreated(
      isUndisclosed: pollKind == .undisclosed, numberOfAnswers: options.count)
  }

  private func editPoll(
    pollStartID: String, question: String, options: [String], pollKind: Poll.Kind
  ) async {
    switch await timelineController.editPoll(
      original: pollStartID, question: question, answers: options, pollKind: pollKind)
    {
    case .success:
      actionsSubject.send(.close)
    case .failure:

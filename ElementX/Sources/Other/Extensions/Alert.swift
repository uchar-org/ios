...
extension View {
  func alert<Item: AlertProtocol, Actions: View, Message: View>(
    item: Binding<Item?>, @ViewBuilder actions: (Item) -> Actions,
    ...
    return alert(
      item.wrappedValue?.title ?? "", isPresented: binding, presenting: item.wrappedValue,
      actions: actions, message: message)
  }

  // periphery: ignore - not used yet but might be useful
  func alert<Item: AlertProtocol, Actions: View>(
    item: Binding<Item?>, @ViewBuilder actions: (Item) -> Actions
  ) -> some View {
    let binding = Binding<Bool>(
      get: {

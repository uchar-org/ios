...
struct UserProfileScreenViewModelTests {
  ...
  func initialState() async throws {
    let profile = UserProfileProxy(
      userID: "@alice:matrix.org", displayName: "Alice", avatarURL: .mockMXCAvatar)
    ...
    let waitForMemberToLoad = deferFulfillment(context.observe(\.viewState.userProfile)) {
      $0 != nil
    }
  ...
  func initialStateAccountOwner() async throws {
    let profile = UserProfileProxy(
      userID: RoomMemberProxyMock.mockMe.userID, displayName: "Me", avatarURL: .mockMXCAvatar)
    let clientProxy = ClientProxyMock(.init())
    clientProxy.profileForReturnValue = .success(profile)
  ...
  func startingDmWithUnknownUserFetchesIdentity() async throws {
    ...
    let deferred = deferFulfillment(
      context.observe(\.viewState.bindings).compactMap(\.inviteConfirmationUser),
      timeout: .seconds(5)
    ) { $0.isUnknown }

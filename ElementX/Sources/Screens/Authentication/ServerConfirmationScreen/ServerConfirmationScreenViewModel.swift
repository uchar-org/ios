...
typealias ServerConfirmationScreenViewModelType = StateStoreViewModelV2<
  ServerConfirmationScreenViewState, ServerConfirmationScreenViewAction
>

class ServerConfirmationScreenViewModel: ServerConfirmationScreenViewModelType,
  ServerConfirmationScreenViewModelProtocol
{
  ...
  private var actionsSubject: PassthroughSubject<ServerConfirmationScreenViewModelAction, Never> =
    .init()
  ...
  init(
    ...
    super.init(
      initialViewState: ServerConfirmationScreenViewState(
        mode: mode,
        authenticationFlow: authenticationFlow,
  ...
  private func confirmServer() async {
    ...
    guard homeserver.loginMode == .unknown || authenticationService.flow != authenticationFlow
    else {
  ...
  private func displayError(_ type: ServerConfirmationScreenAlert) {
    ...
    case .slidingSync:
      let nonBreakingAppName = InfoPlistReader.main.bundleDisplayName.replacingOccurrences(
        of: " ", with: "\u{00A0}")
      state.bindings.alertInfo = AlertInfo(
        id: .slidingSync,
        title: L10n.commonServerNotSupported,
    ...
    case .elementProRequired(let serverName):
      state.bindings.alertInfo = AlertInfo(
        id: .elementProRequired(serverName: serverName),

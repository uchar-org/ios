...
typealias EmojiPickerScreenViewModelType = StateStoreViewModelV2<
  EmojiPickerScreenViewState, EmojiPickerScreenViewAction
>
...
class EmojiPickerScreenViewModel: EmojiPickerScreenViewModelType, EmojiPickerScreenViewModelProtocol
{
  ...
  init(
    itemID: TimelineItemIdentifier, selectedEmojis: Set<String>,
    emojiProvider: EmojiProviderProtocol, timelineController: TimelineControllerProtocol
  ) {
    let initialViewState = EmojiPickerScreenViewState(
      categories: [], selectedEmojis: selectedEmojis)
    self.itemID = itemID
  ...
  override func process(viewAction: EmojiPickerScreenViewAction) {
    ...
    case .emojiTapped(emoji: let emoji):
  ...
  private func convert(emojiCategories: [EmojiCategory]) -> [EmojiPickerEmojiCategoryViewData] {
    emojiCategories.compactMap { emojiCategory in
      let emojisViewData: [EmojiPickerEmojiViewData] = emojiCategory.emojis.compactMap {
        emojiItem in
        EmojiPickerEmojiViewData(
          id: "\(emojiCategory.id)-\(emojiItem.id)", value: emojiItem.unicode)
      }
  ...
  private func selectEmoji(_ emoji: EmojiPickerEmojiViewData) async {
    ...
    guard case .event(_, let eventOrTransactionID) = itemID else {
      fatalError("Attempted to react to a virtual item.")
    }

...
typealias ResolveVerifiedUserSendFailureScreenViewModelType = StateStoreViewModelV2<
  ResolveVerifiedUserSendFailureScreenViewState, ResolveVerifiedUserSendFailureScreenViewAction
>

class ResolveVerifiedUserSendFailureScreenViewModel:
  ResolveVerifiedUserSendFailureScreenViewModelType,
  ResolveVerifiedUserSendFailureScreenViewModelProtocol
{
  ...
  private let actionsSubject:
    PassthroughSubject<ResolveVerifiedUserSendFailureScreenViewModelAction, Never> = .init()
  ...
  init(
    ...
    members = Dictionary(
      uniqueKeysWithValues: roomProxy.membersPublisher.value.map { ($0.userID, $0) })
    ...
    super.init(
      initialViewState: ResolveVerifiedUserSendFailureScreenViewState(
  ...
  private static let failureIndicatorIdentifier =
    "\(ResolveVerifiedUserSendFailureScreenViewModel.self)-Failure"
...
private class UnsignedDeviceFailureIterator: VerifiedUserSendFailureIterator {
  ...
  func next() -> (userID: String, failure: TimelineItemSendFailure.VerifiedUser)? {
    ...
    return (
      nextUserDevices.key, .hasUnsignedDevice(devices: [nextUserDevices.key: nextUserDevices.value])
    )
  }
}

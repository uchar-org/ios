...
class VoiceMessageMediaManager: VoiceMessageMediaManagerProtocol {
  ...
  func loadVoiceMessageFromSource(_ source: MediaSourceProxy, body: String?) async throws -> URL {
    guard let mimeType = source.mimeType, mimeType.starts(with: supportedVoiceMessageMimeType)
    else {
    ...
    guard
      case .success(let fileHandle) = await mediaProvider.loadFileFromSource(source, filename: body)
    else {
      throw MediaProviderError.failedRetrievingFile
    }

    return try await enqueueVoiceMessageConversionRequest(forSource: source) {
      [audioConverter, voiceMessageCache] in
      // Do we already have a converted version?
      if let fileURL = voiceMessageCache.fileURL(for: source) {
        return fileURL
      }
      ...
      let convertedFileURL = URL.temporaryDirectory.appendingPathComponent(
  ...
  private func enqueueVoiceMessageConversionRequest(
    forSource source: MediaSourceProxy, operation: @escaping () throws -> URL
  ) async throws -> URL {

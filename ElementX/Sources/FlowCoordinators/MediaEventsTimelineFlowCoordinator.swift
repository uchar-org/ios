...
class MediaEventsTimelineFlowCoordinator: FlowCoordinatorProtocol {
  ...
  private let actionsSubject: PassthroughSubject<MediaEventsTimelineFlowCoordinatorAction, Never> =
    .init()
  ...
  private func presentMediaEventsTimeline() async {
    ...
    guard
      case .success(let mediaTimelineController) = await flowParameters.timelineControllerFactory
        .buildMessageFilteredTimelineController(
          focus: .live,
    ...
    guard
      case .success(let filesTimelineController) = await flowParameters.timelineControllerFactory
        .buildMessageFilteredTimelineController(
          focus: .live,
    ...
    let parameters = MediaEventsTimelineScreenCoordinatorParameters(
      roomProxy: roomProxy,

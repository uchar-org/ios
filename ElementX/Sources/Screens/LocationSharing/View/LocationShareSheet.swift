...
struct LocationShareSheet: View {
  ...
  var body: some View {
    AppActivityView(
      activityItems: [
        ShareToMapsAppActivity.MapsAppType.apple.activityURL(for: location, senderName: senderName)
      ],
      applicationActivities: ShareToMapsAppActivity.MapsAppType.allCases.map {
        ShareToMapsAppActivity(type: $0, location: location, senderName: senderName)
      }
    )
...
struct LocationShareSheet_Previews: PreviewProvider {
  ...
  static let annotation = LocationAnnotation(
    id: profile.userID,

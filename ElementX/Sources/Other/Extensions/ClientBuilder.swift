...
extension ClientBuilder {
  ...
  static func baseBuilder(
    setupEncryption: Bool = true,
    ...
    var builder = ClientBuilder()
      .crossProcessLockConfig(
        crossProcessLockConfig: .multiProcess(holderName: InfoPlistReader.main.bundleIdentifier)
      )
      ...
      .requestConfig(
        config: .init(
          retryLimit: 3,  // Must be non-zero for the SDK to retry API calls when rate-limited.
    ...
    if enableOnlySignedDeviceIsolationMode {
      builder =
        ...
        .decryptionSettings(
          decryptionSettings: .init(senderDeviceTrustRequirement: .crossSignedOrLegacy))
    } else {
      builder =
        builder

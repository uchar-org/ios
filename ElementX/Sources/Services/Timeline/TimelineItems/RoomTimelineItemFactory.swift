...
struct RoomTimelineItemFactory: RoomTimelineItemFactoryProtocol {
    ...
    -> RoomTimelineItemProtocol?
  {
    ...
    case .msgLike(let messageLikeContent):
      ...
      case .message(let messageContent):
        return buildMessageTimelineItem(
          eventItemProxy, messageLikeContent, messageContent, isOutgoing)
      case .sticker(let body, let imageInfo, let mediaSource):
        return buildStickerTimelineItem(
          eventItemProxy, messageLikeContent, body, imageInfo, mediaSource, isOutgoing)
      case .poll(
        let question, let kind, let maxSelections, let answers, let votes, let endTime, let edited):
        return buildPollTimelineItem(
          eventItemProxy, messageLikeContent, question, kind, maxSelections, answers, votes,
      ...
      case .unableToDecrypt(let encryptedMessage):
        return buildEncryptedTimelineItem(
          eventItemProxy, messageLikeContent, encryptedMessage, isOutgoing)
      case .liveLocation(let content):
        return buildLiveLocationTimelineItem(
          eventItemProxy, messageLikeContent, content, isOutgoing)
      case .other:
    ...
    case .roomMembership(userId: let userID, let displayName, let change, let reason):
      ...
      return buildStateMembershipChangeTimelineItem(
        for: eventItemProxy, memberUserID: userID, memberDisplayName: displayName,
    ...
    case .profileChange(let displayName, let prevDisplayName, let avatarUrl, let prevAvatarUrl):
      return buildStateProfileChangeTimelineItem(
        for: eventItemProxy,
  ...
  private func buildMessageTimelineItem(
    ...
    case .text(content: let textMessageContent):
      return buildTextTimelineItem(
        for: eventItemProxy, messageLikeContent, messageContent, textMessageContent, isOutgoing)
    case .image(content: let imageMessageContent):
      return buildImageTimelineItem(
        for: eventItemProxy, messageLikeContent, messageContent, imageMessageContent, isOutgoing)
    case .video(let videoMessageContent):
      return buildVideoTimelineItem(
        for: eventItemProxy, messageLikeContent, messageContent, videoMessageContent, isOutgoing)
    case .file(let fileMessageContent):
      return buildFileTimelineItem(
        for: eventItemProxy, messageLikeContent, messageContent, fileMessageContent, isOutgoing)
    case .notice(content: let noticeMessageContent):
      return buildNoticeTimelineItem(
        for: eventItemProxy, messageLikeContent, messageContent, noticeMessageContent, isOutgoing)
    case .emote(content: let emoteMessageContent):
      return buildEmoteTimelineItem(
        for: eventItemProxy, messageLikeContent, messageContent, emoteMessageContent, isOutgoing)
    case .audio(let audioMessageContent):
      if audioMessageContent.voice != nil {
        return buildVoiceTimelineItem(
          for: eventItemProxy, messageLikeContent, messageContent, audioMessageContent, isOutgoing)
      } else {
        return buildAudioTimelineItem(
          for: eventItemProxy, messageLikeContent, messageContent, audioMessageContent, isOutgoing)
      }
    case .location(let locationMessageContent):
      return buildLocationTimelineItem(
        for: eventItemProxy, messageLikeContent, messageContent, locationMessageContent, isOutgoing)
    case .gallery(let galleryMessageContent):
      return buildGalleryTimelineItem(
        for: eventItemProxy, messageLikeContent, messageContent, galleryMessageContent, isOutgoing)
    case .other:
      return nil
    }
  }

  private func buildTextTimelineItem(
    for eventItemProxy: EventTimelineItemProxy,
    _ messageLikeContent: MsgLikeContent,
    _ messageContent: MessageContent,
  ...
  private func buildImageTimelineItem(
    for eventItemProxy: EventTimelineItemProxy,
    _ messageLikeContent: MsgLikeContent,
    _ messageContent: MessageContent,
  ...
  private func buildVideoTimelineItem(
    for eventItemProxy: EventTimelineItemProxy,
    _ messageLikeContent: MsgLikeContent,
    _ messageContent: MessageContent,
  ...
  private func buildFileTimelineItem(
    for eventItemProxy: EventTimelineItemProxy,
    _ messageLikeContent: MsgLikeContent,
    _ messageContent: MessageContent,
  ...
  private func buildEmoteTimelineItem(
    for eventItemProxy: EventTimelineItemProxy,
    _ messageLikeContent: MsgLikeContent,
    _ messageContent: MessageContent,
    ...
    EmoteRoomTimelineItem(
      ...
      canBeRepliedTo: eventItemProxy.canBeRepliedTo,
      sender: eventItemProxy.sender,
      content: buildEmoteTimelineItemContent(
        senderDisplayName: eventItemProxy.sender.displayName, senderID: eventItemProxy.sender.id,
  ...
  private func buildLocationTimelineItem(
    for eventItemProxy: EventTimelineItemProxy,
    _ messageLikeContent: MsgLikeContent,
    _ messageContent: MessageContent,
  ...
  private func buildGalleryTimelineItem(
    for eventItemProxy: EventTimelineItemProxy,
    _ messageLikeContent: MsgLikeContent,
    _ messageContent: MessageContent,
  ...
  private func buildStickerTimelineItem(
    _ eventItemProxy: EventTimelineItemProxy,
    _ messageLikeContent: MsgLikeContent,
    _ body: String,
    ...
    let imageInfo = ImageInfoProxy(
      source: mediaSource, width: info.width, height: info.height, mimeType: info.mimetype,
      fileSize: info.size.map(UInt.init))

    return StickerRoomTimelineItem(
      id: eventItemProxy.id,
      body: body,
      timestamp: eventItemProxy.timestamp,
      isOutgoing: isOutgoing,
  ...
  private func buildPollTimelineItem(
    _ eventItemProxy: EventTimelineItemProxy,
    _ messageLikeContent: MsgLikeContent,
    _ question: String,
    _ pollKind: PollKind,
  ...
  private func buildLiveLocationTimelineItem(
    _ eventItemProxy: EventTimelineItemProxy,
    _ messageLikeContent: MsgLikeContent,
    _ liveLocationContent: LiveLocationContent,
    _ isOutgoing: Bool
  ) -> RoomTimelineItemProtocol {
    LiveLocationRoomTimelineItem(
      id: eventItemProxy.id,
      timestamp: eventItemProxy.timestamp,
      isOutgoing: isOutgoing,
      isEditable: eventItemProxy.isEditable,
  ...
  private func buildRedactedTimelineItem(
    _ eventItemProxy: EventTimelineItemProxy,
    _ messageLikeContent: MsgLikeContent,
    _ isOutgoing: Bool
  ) -> RoomTimelineItemProtocol {
  ...
  private func buildEncryptedTimelineItem(
    _ eventItemProxy: EventTimelineItemProxy,
    _ messageLikeContent: MsgLikeContent,
    ...
    return EncryptedRoomTimelineItem(
      id: eventItemProxy.id,
      body: errorLabel,
    ...
    -> TextRoomTimelineItemContent
  {
    ...
    return .init(
      body: messageContent.body, formattedBody: formattedBody, formattedBodyHTMLString: htmlBody)
  }
    ...
    -> AudioRoomTimelineItemContent
  {
    ...
    return AudioRoomTimelineItemContent(
      ...
      source: MediaSourceProxy(
        source: messageContent.source, mimeType: messageContent.info?.mimetype),
      fileSize: messageContent.info?.size.map(UInt.init),
      contentType: UTType(
        mimeType: messageContent.info?.mimetype, fallbackFilename: messageContent.filename))
  }
    ...
    -> ImageRoomTimelineItemContent
  {
    ...
    let thumbnailInfo = ImageInfoProxy(
      source: messageContent.info?.thumbnailSource,
  ...
  private func buildEmoteTimelineItemContent(
    senderDisplayName: String?, senderID: String, messageContent: EmoteMessageContent
  ) -> EmoteRoomTimelineItemContent {
    ...
    case .ready(let content, let senderID, let senderProfile, _, _):
      ...
      let latestEventContent: TimelineEventContent =
        ...
        case .msgLike(let messageLikeContent):
          ...
          case .message(let messageContent):
            .message(
              buildMessageTimelineItemContent(
  ...
  private func buildUnsupportedTimelineItem(
    _ eventItemProxy: EventTimelineItemProxy,
    _ eventType: String,
    _ error: String,
    _ isOutgoing: Bool
  ...
  private func buildStateTimelineItem(
    for eventItemProxy: EventTimelineItemProxy,
    state: OtherState,
    isOutgoing: Bool
  ) -> RoomTimelineItemProtocol? {
    guard
      let text = stateEventStringBuilder.buildString(
        for: state, sender: eventItemProxy.sender, isOutgoing: isOutgoing)
    else { return nil }
    return buildStateTimelineItem(for: eventItemProxy, text: text, isOutgoing: isOutgoing)
  }

  private func buildStateMembershipChangeTimelineItem(
    for eventItemProxy: EventTimelineItemProxy,
    memberUserID: String,
  ...
  private func buildStateProfileChangeTimelineItem(
    for eventItemProxy: EventTimelineItemProxy,
    displayName: String?,
    ...
    guard
      let text = stateEventStringBuilder.buildProfileChangeString(
        displayName: displayName,
  ...
  private func buildStateTimelineItem(
    for eventItemProxy: EventTimelineItemProxy, text: String, isOutgoing: Bool
  ) -> RoomTimelineItemProtocol {
    StateRoomTimelineItem(
      id: eventItemProxy.id,
      body: text,
      timestamp: eventItemProxy.timestamp,
      isOutgoing: isOutgoing,
  ...
  func buildTimelineItemReply(_ details: MatrixRustSDK.InReplyToDetails) -> TimelineItemReply {
    ...
    case .ready(let timelineItem, let senderID, let senderProfile, _, _):
      ...
      case .msgLike(let messageLikeContent):
        switch messageLikeContent.kind {
        case .message(let messageContent):
          let replyContent = buildMessageTimelineItemContent(
            messageType: messageContent.msgType,
            senderID: sender.id,
            senderDisplayName: sender.displayName)
          return .init(
      ...
      return .init(
        details: .loaded(sender: sender, eventID: details.eventId(), eventContent: replyContent),
        isThreaded: isThreaded)
    case .error(let message):
      return .init(
        details: .error(eventID: details.eventId(), message: message), isThreaded: isThreaded)
    }
  }

  // MARK: - Helpers

  private func buildMessageTimelineItemContent(
    messageType: MessageType?, senderID: String, senderDisplayName: String?
    ...
    case .emote(let content):
      .emote(
        buildEmoteTimelineItemContent(
          senderDisplayName: senderDisplayName, senderID: senderID, messageContent: content))
...
extension EmbeddedEventDetails {
  fileprivate var isThreaded: Bool {
    switch self {
...
extension Receipt {
  fileprivate var dateTimestamp: Date? {

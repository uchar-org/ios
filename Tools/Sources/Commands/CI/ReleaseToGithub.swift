...
struct ReleaseToGitHub: AsyncParsableCommand {
    ...
    func run() async throws {
        ...
        try await CI.run(.name("sed"),
                         [
                             "-i", "", "s/MARKETING_VERSION: \(currentVersion)/MARKETING_VERSION: \(newVersion)/g",
                             targetFilePath
                         ])
    ...
    private func updateChangelog(version: String, generatedNotes: String) throws {
        ...
        let newContent =
            "## Changes in \(version) (\(releaseDate))\(cleanedNotes)\n\n\(existingContent)"
    ...
    private func rebaseMainOntoCurrentBranch() async throws {
        guard
            let currentBranch = try await CI.run(.name("git"), ["rev-parse", "--abbrev-ref", "HEAD"], output: .string(limit: 4096))
            .standardOutput.map({ $0.trimmingCharacters(in: .whitespacesAndNewlines) })
        else {

...
class UserSessionStore: UserSessionStoreProtocol {
  ...
  func userSession(
    ...
    do {
      ...
      keychainController.setRestorationToken(
        RestorationToken(
          session: session,
  ...
  private func buildUserSessionWithClient(_ clientProxy: ClientProxyProtocol) async
    -> UserSessionProtocol
  {
    let mediaProvider = MediaProvider(
      mediaLoader: clientProxy.mediaLoader,
  ...
  private func restorePreviousLogin(_ credentials: KeychainCredentials) async -> Result<
    ClientProxyProtocol, UserSessionStoreError
  > {
    ...
    await appHooks.remoteSettingsHook.loadCache(
      forHomeserver: homeserverURL, applyingTo: appSettings)
    ...
    let builder =
      ...
      .sqliteStore(
        config: .init(
          dataPath: credentials.restorationToken.sessionDirectories.dataPath,
    ...
    do {
      ...
      MXLog.info(
        "Set up session for user \(credentials.userID) at: \(credentials.restorationToken.sessionDirectories)"
      )

    ...
    if composerFormattingEnabled {
      Color.clear
        ...
        .frame(
          ...
          maxHeight: max(composerHeight, composerFrame.height),
          alignment: .top
        )
        .onAppear {
          onAppearAction()
...
private struct MessageComposerReplyHeader: View {
  ...
  var body: some View {
    TimelineReplyView(
      placement: .composer, timelineItemReplyDetails: replyDetails, maxWidth: .infinity
    )
      ...
      if isEnabled {
        mainContent(content: content)
          .snapshotableGlassEffect(
            .regular.interactive(),  // Doesn't need to be interactive but Apple does it 🤷‍♂️
...
struct MessageComposer_Previews: PreviewProvider, TestablePreview {
  ...
  static func messageComposer(
    _ content: NSAttributedString = .init(string: ""),
    mode: ComposerMode = .default,
    placeholder: String = L10n.richTextEditorComposerPlaceholder
  ) -> MessageComposer {
    ...
    return MessageComposer(
      plainComposerText: .constant(content),

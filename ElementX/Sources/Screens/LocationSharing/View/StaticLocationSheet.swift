  ...
  private var mainContent: some View {
    VStack(spacing: 0) {
      Text(L10n.screenStaticLocationSheetTitle)
        ...
        .padding(.bottom, 25)
        .padding(.top, 29)
      if case .viewStatic(let location) = context.viewState.interactionMode,
        ...
        Button {
          context.send(
            viewAction: .setMapCenter(
              .init(
                latitude: location.geoURI.latitude,
...
struct StaticLocationSheet_Previews: PreviewProvider, TestablePreview {
  static let viewModel = LocationSharingScreenViewModel.mock(
    type: .staticSenderLocation, senderID: RoomMemberProxyMock.mockMe.userID)

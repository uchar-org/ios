...
struct NotificationSettingsEditScreenViewModelTests {
  ...
  init() throws {
    ...
    clientProxy = ClientProxyMock(
      .init(userID: "@a:b.com", notificationSettings: notificationSettingsProxy))
  ...
  mutating func fetchSettings() async throws {
    ...
    viewModel = NotificationSettingsEditScreenViewModel(
      chatType: .groupChat, userSession: userSession)

    let deferred = deferFulfillment(viewModel.context.observe(\.viewState.defaultMode)) {
      $0 != nil
    }
    ...
    let invocations = notificationSettingsProxy
      .getDefaultRoomNotificationModeIsEncryptedIsOneToOneReceivedInvocations
  ...
  mutating func setModeAllMessages() async throws {
    ...
    var deferredViewState = deferFulfillment(
      viewModel.context.observe(\.viewState.pendingMode),
    ...
    deferredViewState = deferFulfillment(
      viewModel.context.observe(\.viewState.defaultMode),
  ...
  mutating func setModeDirectChats() async throws {
    ...
    viewModel = NotificationSettingsEditScreenViewModel(
      chatType: .oneToOneChat, userSession: userSession)
  ...
  mutating func setModeFailure() async throws {
    ...
    notificationSettingsProxy
      .setDefaultRoomNotificationModeIsEncryptedIsOneToOneModeThrowableError =
  ...
  mutating func selectRoom() async throws {
    ...
    let expectedAction =
      NotificationSettingsEditScreenViewModelAction.requestRoomNotificationSettingsPresentation(
        roomID: roomID)
    guard
      case .requestRoomNotificationSettingsPresentation(roomID: let receivedRoomID) = sentAction,

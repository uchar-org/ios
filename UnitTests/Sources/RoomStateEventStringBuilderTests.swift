...
struct RoomStateEventStringBuilderTests {
  ...
  func avatarChanges() {
    // Changes by you.
    validateAvatarChange(
      senderID: userID, oldAvatarURL: "mxc://1", newAvatarURL: "mxc://2",
      expectedString: L10n.stateEventAvatarUrlChangedByYou)
    validateAvatarChange(
      senderID: userID, oldAvatarURL: "mxc://1", newAvatarURL: nil,
      expectedString: L10n.stateEventAvatarUrlChangedByYou)
    validateAvatarChange(
      senderID: userID, oldAvatarURL: nil, newAvatarURL: "mxc://1",
      expectedString: L10n.stateEventAvatarUrlChangedByYou)
    ...
    let senderName = "Bob"
    validateAvatarChange(
      senderID: senderID, senderName: senderName, oldAvatarURL: "mxc://1", newAvatarURL: "mxc://2",
      expectedString: L10n.stateEventAvatarUrlChanged(senderName))
    validateAvatarChange(
      senderID: senderID, senderName: senderName, oldAvatarURL: "mxc://1", newAvatarURL: nil,
      expectedString: L10n.stateEventAvatarUrlChanged(senderName))
    validateAvatarChange(
      senderID: senderID, senderName: senderName, oldAvatarURL: nil, newAvatarURL: "mxc://1",
      expectedString: L10n.stateEventAvatarUrlChanged(senderName))
  }

  private func validateAvatarChange(
    senderID: String, senderName: String? = nil,
  ...
  func topicChanges() {
    ...
    var string = stringBuilder.buildString(
      for: .roomTopic(topic: newTopic), sender: you, isOutgoing: true)
    #expect(string == L10n.stateEventRoomTopicChangedByYou(newTopic))
    string = stringBuilder.buildString(
      for: .roomTopic(topic: newTopic), sender: other, isOutgoing: false)
    #expect(string == L10n.stateEventRoomTopicChanged(other.displayName ?? "", newTopic))

    let emptyTopic = ""
    string = stringBuilder.buildString(
      for: .roomTopic(topic: emptyTopic), sender: you, isOutgoing: true)
    #expect(string == L10n.stateEventRoomTopicRemovedByYou)
    string = stringBuilder.buildString(
      for: .roomTopic(topic: emptyTopic), sender: other, isOutgoing: false)
    ...
    #expect(string == L10n.stateEventRoomTopicRemovedByYou)
    string = stringBuilder.buildString(
      for: .roomTopic(topic: nil), sender: other, isOutgoing: false)
    #expect(string == L10n.stateEventRoomTopicRemoved(other.displayName ?? ""))
  }
  ...
  func kickMember() {
    ...
    var string = stringBuilder.buildString(
      ...
      isOutgoing: true)
    #expect(
      string == L10n.stateEventRoomRemoveByYouWithReason(banned.displayName ?? banned.id, reason))
    string = stringBuilder.buildString(
      for: .kicked,
      reason: nil,
      memberUserID: banned.id,
    ...
    #expect(
      string
        == L10n.stateEventRoomRemoveWithReason(
          other.displayName ?? other.id, banned.displayName ?? banned.id, reason))
    string = stringBuilder.buildString(
      for: .kicked,
      reason: nil,
      memberUserID: banned.id,
    ...
    #expect(
      string
        == L10n.stateEventRoomRemove(other.displayName ?? other.id, banned.displayName ?? banned.id)
    )
  }
  ...
  func banMember() {
    ...
    var string = stringBuilder.buildString(
      ...
      isOutgoing: true)
    #expect(
      string == L10n.stateEventRoomBanByYouWithReason(banned.displayName ?? banned.id, reason))
    string = stringBuilder.buildString(
      for: .banned,
      reason: nil,
      memberUserID: banned.id,
    ...
    #expect(
      string
        == L10n.stateEventRoomBanWithReason(
          other.displayName ?? other.id, banned.displayName ?? banned.id, reason))
    string = stringBuilder.buildString(
      for: .banned,
      reason: nil,
    ...
    #expect(
      string
        == L10n.stateEventRoomBan(other.displayName ?? other.id, banned.displayName ?? banned.id))
  }
}

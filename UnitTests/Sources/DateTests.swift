...
struct DateTests {
    ...
    func minimalDateFormatting() throws {
        ...
        #expect(yesterday.formattedMinimal()
            == yesterday.formatted(Date.RelativeFormatStyle(presentation: .named, capitalizationContext: .beginningOfSentence)))

        let nearYesterday = try #require(calendar.date(byAdding: DateComponents(hour: -10), to: today))
        #expect(nearYesterday.formattedMinimal()
            == yesterday.formatted(Date.RelativeFormatStyle(presentation: .named, capitalizationContext: .beginningOfSentence)))
        ...
        #expect(sometimeInTheLastYear.formattedMinimal()
    ...
    func dateSeparatorFormatting() throws {
        ...
        let nearYesterday = try #require(calendar.date(byAdding: DateComponents(hour: -10), to: today))
        #expect(nearYesterday.formattedDateSeparator()
            == yesterday.formatted(Date.RelativeFormatStyle(presentation: .named, capitalizationContext: .beginningOfSentence)))
        ...
        #expect(startOfTheYear.formattedDateSeparator()
            == startOfTheYear.formatted(.dateTime.weekday(.wide).day().month(.wide)))

        let theMillennium = try #require(calendar.date(from: DateComponents(year: 2000, month: 1, day: 1)))
        #expect(theMillennium.formattedDateSeparator()

...
extension Image {
  ...
  public static let compound = CompoundIcons()
}
...
extension Label {
  ...
  public init(_ title: some StringProtocol, icon: KeyPath<CompoundIcons, Image>)
  where Title == Text, Icon == CompoundIcon {
    self.init {
      Text(title)
    } icon: {
      CompoundIcon(icon)
...
struct CompoundIcon_Previews: PreviewProvider, TestablePreview {
  ...
  static var form: some View {
    Form {
      Section {
        ListRow(
          label: .action(title: "Plain Icon", icon: \.userProfile),
          kind: .label)
        ListRow(
          label: .default(title: "Plain Icon", icon: \.userProfile),
          kind: .label)
        ListRow(
          label: .default(title: "Plain Icon", systemIcon: .personCropCircle),
          kind: .label)
      }
    }
    .compoundList()

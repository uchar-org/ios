...
class ChatsTabFlowCoordinator: FlowCoordinatorProtocol {
    ...
    func asyncHandleAppRoute(_ appRoute: AppRoute, animated: Bool) async {
        ...
        case .room(let roomID, let via):
            stateMachine.processEvent(.selectRoom(roomID: roomID, via: via, entryPoint: .room),
        ...
        case .roomDetails(let roomID):
            if case .room(roomID) = stateMachine.state.detailState {
                ...
                stateMachine.processEvent(.selectRoom(roomID: roomID, via: [], entryPoint: .roomDetails),
                                          userInfo: .init(animated: animated))
            }
        case .roomList:
            roomFlowCoordinator?.clearRoute(animated: animated)
        ...
        case .event(let eventID, let roomID, let via):
            stateMachine.processEvent(.selectRoom(roomID: roomID, via: via, entryPoint: .eventID(eventID)),
                                      userInfo: .init(animated: animated))
        case .eventOnRoomAlias(let eventID, let alias):
            switch await userSession.clientProxy.resolveRoomAlias(alias) {
            case .success(let resolved):
                await asyncHandleAppRoute(.event(eventID: eventID, roomID: resolved.roomId, via: resolved.servers),
                                          animated: animated)
            case .failure: showFailureIndicator()
            }
        ...
        case .childEventOnRoomAlias(let eventID, let alias):
            switch await userSession.clientProxy.resolveRoomAlias(alias) {
            case .success(let resolved):
                await asyncHandleAppRoute(.childEvent(eventID: eventID, roomID: resolved.roomId, via: resolved.servers),
                                          animated: animated)
            case .failure: showFailureIndicator()
            }
    ...
    private func setupStateMachine() {
        stateMachine.addTransitionHandler { [weak self] context in
            ...
            case (.roomList(let detailState), .selectRoom(let roomID, let via, let entryPoint), .roomList):
                handleSelectRoomTransition(roomID: roomID, via: via, entryPoint: entryPoint, detailState: detailState,
                                           animated: animated)
            ...
            case (.roomList(let roomListSelectedRoomID), .showShareExtensionRoomList,
    ...
    private func handleSelectRoomTransition(roomID: String, via: [String], entryPoint: RoomFlowCoordinatorEntryPoint,
    ...
    private func startRoomFlow(roomID: String,
        ...
        case .thread(let rootEventID, let focusEventID):
            coordinator.handleAppRoute(.thread(roomID: roomID, threadRootEventID: rootEventID, focusEventID: focusEventID),
                                       animated: animated)
        }

...
private struct RoomThreadListCell: View {
  ...
  var body: some View {
    HStack(alignment: .center, spacing: 16) {
      LoadableAvatarImage(
        url: item.rootMessageDetails.sender.avatarURL,
  ...
  private var creatorDetails: some View {
    Text(
      item.rootMessageDetails.sender.disambiguatedDisplayName ?? item.rootMessageDetails.sender.id
    )
  ...
  private var latestMessageDetails: some View {
    if let latestMessageDetails = item.latestMessageDetails {
      HStack(alignment: .center, spacing: 8) {
        ...
        LoadableAvatarImage(
          url: latestMessageDetails.sender.avatarURL,
...
private struct ThreadCellButtonStyle: ButtonStyle {
  func makeBody(configuration: Configuration) -> some View {
    configuration.label
      .background(
        configuration.isPressed ? Color.compound.bgSubtleSecondary : Color.compound.bgCanvasDefault
      )
...
struct RoomThreadListScreen_Previews: PreviewProvider, TestablePreview {
  ...
  static func makeViewModel() -> RoomThreadListScreenViewModel {
    RoomThreadListScreenViewModel(
      threadListServiceProxy: RoomThreadListServiceProxyMock(.init()),

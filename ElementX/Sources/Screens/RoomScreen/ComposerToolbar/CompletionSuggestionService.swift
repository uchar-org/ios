...
final class CompletionSuggestionService: CompletionSuggestionServiceProtocol {
  ...
  init(
    ...
    suggestionsPublisher =
      ...
      .map {
        ...
        case .user:
          return membersSuggestions(
            suggestionTrigger: suggestionTrigger, members: members, ownUserID: ownUserID)
        case .room:
  ...
  private func membersSuggestions(
    ...
    var membersSuggestion =
      ...
      .compactMap { member -> SuggestionItem? in
        ...
        return .init(
          suggestionType: .user(
            .init(id: member.userID, displayName: member.displayName, avatarURL: member.avatarURL)),
    ...
    if canMentionAllUsers,
      ...
      Self.shouldIncludeMember(
        userID: PillUtilities.atRoom, displayName: PillUtilities.everyone,
        searchText: suggestionTrigger.text)
    {
      membersSuggestion
        .insert(
          SuggestionItem(
            suggestionType: .allUsers(roomProxy.details.avatar), range: suggestionTrigger.range,
    ...
    roomSummaries
      .compactMap { roomSummary -> SuggestionItem? in
        guard let canonicalAlias = roomSummary.canonicalAlias,
          Self.shouldIncludeRoom(
            roomName: roomSummary.name, roomAlias: canonicalAlias,
            searchText: suggestionTrigger.text)
        else {
          return nil
        }

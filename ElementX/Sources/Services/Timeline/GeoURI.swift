...
private typealias RegexGeoURI = Regex<
  (Substring, latitude: Substring, longitude: Substring, uncertainty: Substring?)
>

extension RegexGeoURI {
  fileprivate static let standard: Self =
...
extension GeoURI {
  init(coordinate: CLLocationCoordinate2D, uncertainty: CLLocationAccuracy?) {
    self.init(
      latitude: coordinate.latitude, longitude: coordinate.longitude, uncertainty: uncertainty)
  }
}

extension NumberFormatter {
  fileprivate static let decimal: NumberFormatter = {

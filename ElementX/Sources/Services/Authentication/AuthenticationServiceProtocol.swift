...
extension OAuthAuthorizationData: @retroactive Hashable {
  public static func == (
    lhs: MatrixRustSDK.OAuthAuthorizationData, rhs: MatrixRustSDK.OAuthAuthorizationData
  ) -> Bool {
...
protocol QRCodeLoginServiceProtocol {
  typealias QRLoginProgressPublisher = CurrentValuePublisher<
    QRLoginProgress, AuthenticationServiceError
  >
...
extension QRLoginProgress: Equatable, CustomStringConvertible {
  static func == (lhs: QRLoginProgress, rhs: QRLoginProgress) -> Bool {
    ...
    case (.starting, .starting):
      true
    case (
      .establishingSecureChannel(let lhsCheckCode, let lhsCheckCodeString),
      .establishingSecureChannel(let rhsCheckCode, let rhsCheckCodeString)
    ):
      lhsCheckCode == rhsCheckCode && lhsCheckCodeString == rhsCheckCodeString
    case (.waitingForToken(let lhsUserCode), .waitingForToken(let rhsUserCode)):

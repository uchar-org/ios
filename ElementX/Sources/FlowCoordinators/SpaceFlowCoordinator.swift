...
class SpaceFlowCoordinator: FlowCoordinatorProtocol {
  ...
  init(
    entryPoint: SpaceFlowCoordinatorEntryPoint,
  ...
  private func configureStateMachine() {
    stateMachine.addRouteMapping { event, fromState, userInfo in
      ...
      case (.space, .startChildFlow):
        guard let childEntryPoint = userInfo as? SpaceFlowCoordinatorEntryPoint else {
          fatalError("An entry point must be provided.")
        }
    ...
    stateMachine.addAnyHandler(.any => .any) { [weak self] context in
      ...
      case (.space, .startCreateChildRoomFlow, .createChildRoomFlow):
        guard let space = context.userInfo as? SpaceServiceRoom else {
          fatalError("The space is missing")
        }
  ...
  private func presentSpace() {
    guard case .space(let spaceRoomListProxy) = entryPoint else {
      fatalError("Attempting to show a space with the wrong entry point.")
    }

    let parameters = SpaceScreenCoordinatorParameters(
      ...
      .sink { [weak self] action in
        ...
        case .selectSpace(let spaceRoomListProxy):
          stateMachine.tryEvent(
            .startChildFlow, userInfo: SpaceFlowCoordinatorEntryPoint.space(spaceRoomListProxy))
        case .selectUnjoinedSpace(let spaceServiceRoom):
          stateMachine.tryEvent(
            .startChildFlow, userInfo: SpaceFlowCoordinatorEntryPoint.joinSpace(spaceServiceRoom))
  ...
  private func presentJoinSpaceScreen() {
    guard case .joinSpace(let spaceServiceRoom) = entryPoint else {
      fatalError("Attempting to join a space with the wrong entry point.")
    }

    let parameters = JoinRoomScreenCoordinatorParameters(
      source: .space(spaceServiceRoom),
      userSession: flowParameters.userSession,
      userIndicatorController: flowParameters.userIndicatorController,
  ...
  private func presentSpaceAddRoomsScreen() {
    ...
    let parameters = SpaceAddRoomsScreenCoordinatorParameters(
      spaceRoomListProxy: spaceRoomListProxy,
      userSession: flowParameters.userSession,
  ...
  private func startSettingsFlow(roomProxy: JoinedRoomProxyProtocol) {
    let flowCoordinator = SpaceSettingsFlowCoordinator(
      roomProxy: roomProxy,
      navigationStackCoordinator: navigationStackCoordinator,
      flowParameters: flowParameters)
  ...
  private func startRolesAndPermissionsFlow(roomProxy: JoinedRoomProxyProtocol) {
    let flowCoordinator = RoomRolesAndPermissionsFlowCoordinator(
      parameters: .init(
        roomProxy: roomProxy,
        mediaProvider: flowParameters.userSession.mediaProvider,
        navigationStackCoordinator: navigationStackCoordinator,
  ...
  private func startCreateChildFlow(space: SpaceServiceRoom) {
    ...
    let flowCoordinator = StartChatFlowCoordinator(
      entryPoint: .createRoomInSpace(space),

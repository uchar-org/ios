...
typealias AdvancedSettingsScreenViewModelType = StateStoreViewModelV2<
  AdvancedSettingsScreenViewState, AdvancedSettingsScreenViewAction
>

class AdvancedSettingsScreenViewModel: AdvancedSettingsScreenViewModelType,
  AdvancedSettingsScreenViewModelProtocol
{
  ...
  init(
    advancedSettings: AdvancedSettingsProtocol,
    ...
    let state = AdvancedSettingsScreenViewState(
  ...
  override func process(viewAction: AdvancedSettingsScreenViewAction) {
    ...
    case .optimizeMediaUploadsChanged:
      ...
      analytics.trackInteraction(
        name: state.bindings.optimizeMediaUploads
          ? .MobileSettingsOptimizeMediaUploadsEnabled : .MobileSettingsOptimizeMediaUploadsDisabled
      )
    case .updateHideInviteAvatars(let value):
      hideInviteAvatarsTask = Task { [weak self] in await self?.updateHideInviteAvatars(value) }
    case .updateTimelineMediaVisibility(let value):
      timelineMediaVisibilityTask = Task { [weak self] in
        await self?.updateTimelineMediaVisibility(value)
      }
    }
  }

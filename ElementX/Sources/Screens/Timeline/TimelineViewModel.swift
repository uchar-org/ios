...
class TimelineViewModel: TimelineViewModelType, TimelineViewModelProtocol {
  ...
  init(
    ...
    let voiceMessageRecorder = VoiceMessageRecorder(
      audioRecorder: AudioRecorder(), mediaPlayerProvider: mediaPlayerProvider)

    timelineInteractionHandler = TimelineInteractionHandler(
      roomProxy: roomProxy,
    ...
    super.init(
      initialViewState: TimelineViewState(
        timelineKind: timelineController.timelineKind,
        roomID: roomProxy.id,
        isDirectOneToOneRoom: roomProxy.isDirectOneToOneRoom,
        timelineState: TimelineState(
          focussedEvent: focussedEventID.map { .init(eventID: $0, appearance: .immediate) }),
  ...
  func focusOnEvent(eventID: String) async {
    ...
    switch await timelineController.focusOnEvent(
      eventID, timelineSize: Constants.detachedTimelineSize)
    {
    case .success:
  ...
  private func updateMembers(_ members: [RoomMemberProxyProtocol]) {
    state.members = members.reduce(into: [String: RoomMemberState]()) { dictionary, member in
      dictionary[member.userID] = RoomMemberState(
    ...
    timelineController.callbacks
      ...
      .sink { [weak self] callback in
        ...
        case .updatedTimelineItems(let updatedItems, let isSwitchingTimelines):
          buildTimelineViews(
            timelineItems: updatedItems, isSwitchingTimelines: isSwitchingTimelines)
  ...
  func viewInRoomTimeline(eventID: String) async {
    ...
    case .success(let event):
      ...
      actionsSubject.send(
        .viewInRoomTimeline(eventID: eventID, threadRootEventID: threadRootEventID))
    case .failure:
  ...
  private func handleMediaTapped(with itemID: TimelineItemIdentifier) async {
    ...
    case .displayMediaPreview(let item, let timelineViewModelKind):
      ...
      let mediaPreviewViewModel = makeMediaPreviewViewModel(
        item: item, timelineViewModelKind: timelineViewModelKind)
    ...
    case .displayLiveLocation(let sender, let initialLiveLocationShare):
      actionsSubject.send(
        .displayLiveLocation(sender: sender, initialLiveLocationShare: initialLiveLocationShare))
    case .none:
      break
    }
  ...
  private func sendCurrentMessage(
    _ message: String, html: String?, mode: ComposerMode, intentionalMentions: IntentionalMentions
  ) async {
  ...
  private func trackComposerMode(_ mode: ComposerMode) {
    ...
    analyticsService.trackComposer(
      inThread: false, isEditing: isEdit, isReply: isReply, startsThread: nil)
  }

  private func makeMediaPreviewViewModel(
    ...
    return TimelineMediaPreviewViewModel(
      initialItem: item,
  ...
  private func buildTimelineViews(
    timelineItems: [RoomTimelineItemProtocol], isSwitchingTimelines: Bool = false
  ) {
    var timelineItemsDictionary = OrderedDictionary<
      TimelineItemIdentifier.UniqueID, RoomTimelineItemViewState
    >()
    ...
    timelineItems.filter { $0 is RedactedRoomTimelineItem }.forEach { timelineItem in
      ...
      guard
        let playerState = mediaPlayerProvider.playerState(
          for: .timelineItemIdentifier(timelineItem.id))
      else {
        return
      }
    ...
    for itemGroup in itemsGroupedByTimelineDisplayStyle {
      ...
      if itemGroup.count == 1 {
        if let firstItem = itemGroup.first {
          timelineItemsDictionary.updateValue(
            updateViewState(item: firstItem, groupStyle: .single),
        ...
        for (index, item) in itemGroup.enumerated() {
          if index == 0 {
            timelineItemsDictionary.updateValue(
              updateViewState(
                item: item, groupStyle: state.timelineKind == .pinned ? .single : .first),
              forKey: item.id.uniqueID)
          } else if index == itemGroup.count - 1 {
            timelineItemsDictionary.updateValue(
              updateViewState(
                item: item, groupStyle: state.timelineKind == .pinned ? .single : .last),
              forKey: item.id.uniqueID)
          } else {
            timelineItemsDictionary.updateValue(
              updateViewState(
                item: item, groupStyle: state.timelineKind == .pinned ? .single : .middle),
              forKey: item.id.uniqueID)
          }
        }
      }
  ...
  private func canGroupItem(
    timelineItem: RoomTimelineItemProtocol, with otherTimelineItem: RoomTimelineItemProtocol
  ) -> Bool {
  ...
  private func inviteOtherDMUserBack() {
    ...
    Task {
      userIndicatorController.submitIndicator(
        .init(id: inviteLoadingIndicatorID, type: .toast, title: L10n.commonLoading))
      defer {
  ...
  private func displayReactionSummary(for itemID: TimelineItemIdentifier, selectedKey: String) {
    ...
    state.bindings.reactionSummaryInfo = .init(
      reactions: eventTimelineItem.properties.reactions, selectedKey: selectedKey)
  }
  ...
  private func displayReadReceipts(for itemID: TimelineItemIdentifier) {
    ...
    state.bindings.readReceiptsSummaryInfo = .init(
      orderedReceipts: eventTimelineItem.properties.orderedReadReceipts, id: eventTimelineItem.id)
  }
  ...
  private func pillContextUpdater(_ pillContext: PillContext) {
      ...
      if let profile = state.members[id] {
        ...
        pillContext.cancellable = context.$viewState
          ...
          .sink { [weak pillContext] profile in
            ...
            pillContext.viewState = .mention(
              isOwnMention: isOwnMention,
              displayText: PillUtilities.userPillDisplayText(
                username: profile.displayName, userID: id))
    ...
    case .event(let room):
      ...
      case .roomAlias(let alias):
        ...
        pillViewState = .reference(
          displayText: PillUtilities.eventPillDisplayText(
            roomName: roomSummary?.name, rawRoomText: alias))
      case .roomID(let id):
        let roomSummary = userSession.clientProxy.roomSummaryForIdentifier(id)
        pillViewState = .reference(
          displayText: PillUtilities.eventPillDisplayText(
            roomName: roomSummary?.name, rawRoomText: id))
      }
      pillContext.viewState = pillViewState
    case .roomAlias(let alias):
      let roomSummary = userSession.clientProxy.roomSummaryForAlias(alias)
      pillContext.viewState = .reference(
        displayText: PillUtilities.roomPillDisplayText(
          roomName: roomSummary?.name, rawRoomText: alias))
    case .roomID(let id):
      let roomSummary = userSession.clientProxy.roomSummaryForIdentifier(id)
      pillContext.viewState = .reference(
        displayText: PillUtilities.roomPillDisplayText(roomName: roomSummary?.name, rawRoomText: id)
      )
    }
  }
  ...
  private func showFocusLoadingIndicator() {
    userIndicatorController.submitIndicator(
      UserIndicator(
        id: Constants.focusTimelineToastIndicatorID,
...
extension TimelineViewModel {
  ...
  static func mock(
    timelineKind: TimelineKind = .live, timelineController: MockTimelineController? = nil,
    hasPredecessor: Bool = false
  ) -> TimelineViewModel {
    let clientProxyMock = ClientProxyMock(.init())
    clientProxyMock.roomSummaryForAliasReturnValue = .mock(id: "!room:matrix.org", name: "Room")
    clientProxyMock.roomSummaryForIdentifierReturnValue = .mock(
      id: "!room:matrix.org", name: "Room", canonicalAlias: "#room:matrix.org")
    let roomProxy = JoinedRoomProxyMock(
      .init(
        name: "Preview room", predecessor: hasPredecessor ? .init(roomId: UUID().uuidString) : nil))

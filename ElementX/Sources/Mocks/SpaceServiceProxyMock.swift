...
extension SpaceServiceProxyMock.Configuration {
  static var populated: SpaceServiceProxyMock.Configuration {
    let spaceFilters = [SpaceServiceRoom].mockJoinedSpaces.reduce(into: [SpaceServiceFilter]()) {
      partialResult, spaceRoom in
    ...
    let spaceRoomLists = [SpaceServiceRoom].mockJoinedSpaces.map {
      (
        $0.id,
        SpaceRoomListProxyMock(.init(spaceServiceRoom: $0, initialSpaceRooms: .mockSpaceList))
      )
    }
    ...
    let subSpaceRoomLists = [SpaceServiceRoom].mockSpaceList.map {
      (
        $0.id,
        SpaceRoomListProxyMock(.init(spaceServiceRoom: $0, initialSpaceRooms: .mockSingleRoom))
      )
    }

    return .init(
      topLevelSpaces: .mockJoinedSpaces,

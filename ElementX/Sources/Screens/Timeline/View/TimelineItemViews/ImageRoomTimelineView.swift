...
struct ImageRoomTimelineView: View {
    ...
    TimelineStyler(timelineItem: timelineItem) {
      VStack(alignment: .leading, spacing: 4) {
        ...
        if let attributedCaption = timelineItem.content.formattedCaption {
          FormattedBodyText(
            attributedString: attributedCaption,
  ...
  private var loadableImage: some View {
    if timelineItem.content.contentType == .gif {
      ...
      LoadableImage(
        mediaSource: timelineItem.content.thumbnailInfo?.source
          ?? timelineItem.content.imageInfo.source,
        mediaType: .timelineItem(uniqueID: timelineItem.id.uniqueID),
        blurhash: timelineItem.content.blurhash,
  ...
  private var placeholder: some View {
    Rectangle()
      .foregroundColor(
        timelineItem.isOutgoing ? .compound._bgBubbleOutgoing : .compound._bgBubbleIncoming
      )
      .opacity(0.3)
  }
}
...
struct ImageRoomTimelineView_Previews: PreviewProvider, TestablePreview {
    ...
    ScrollView {
      VStack(spacing: 20.0) {
        ...
        ImageRoomTimelineView(
          timelineItem: makeTimelineItem(
            caption: "This is a great image with a really long multiline caption.",
            isEdited: true))
      }
    }
  ...
  private static func makeTimelineItem(caption: String? = nil, isEdited: Bool = false)

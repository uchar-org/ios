...
struct InviteUsersScreen_Previews: PreviewProvider, TestablePreview {
  ...
  static var previews: some View {
    ...
    .snapshotPreferences(
      expect: viewModel.context.$viewState.map { !$0.usersSection.users.isEmpty })
    ...
    .snapshotPreferences(
      expect: searchingViewModel.context.$viewState.map {
    ...
    .snapshotPreferences(
      expect: selectedViewModel.context.$viewState.map { !$0.selectedUsers.isEmpty })
  ...
  static func makeViewModel(
    searchQuery: String? = nil, hasSelection: Bool = false, shouldConfirm: Bool = false
  ) -> InviteUsersScreenViewModel {
    ...
    let viewModel = InviteUsersScreenViewModel(

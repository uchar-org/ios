...
final class CreateRoomScreenViewModelTests {
  ...
  func createRoom() async throws {
    ...
    let deferred = deferFulfillment(viewModel.actions) { action in
      guard case .createdRoom(let roomProxy, nil) = action, roomProxy.id == "1" else {
        return false
      }
      return true
    }
    ...
    #expect(
      clientProxy
        .createRoomNameTopicAccessTypeIsSpaceUserIDsAvatarURLAliasLocalPartReceivedArguments?.name
        == "A")
    #expect(
      clientProxy
        .createRoomNameTopicAccessTypeIsSpaceUserIDsAvatarURLAliasLocalPartReceivedArguments?.topic
        == nil,
  ...
  func createSpace() async throws {
    ...
    let viewModel = CreateRoomScreenViewModel(
      isSpace: true,
  ...
  func createKnockingRoom() async {
    ...
    await waitForConfirmation("Wait for the room to be created") { confirmation in
      clientProxy.createRoomNameTopicAccessTypeIsSpaceUserIDsAvatarURLAliasLocalPartClosure = {
        _, _, accessType, _, _, _, localAliasPart in
  ...
  func createRoomInAnAlreadySelectedSpace() async throws {
    ...
    #expect(
      context.viewState.availableAccessTypes == [
        .spaceMembers, .askToJoinWithSpaceMembers, .private,
      ])
    #expect(context.viewState.canSelectSpace)
    ...
    #expect(
      clientProxy
        .createRoomNameTopicAccessTypeIsSpaceUserIDsAvatarURLAliasLocalPartReceivedArguments?
        .accessType == .spaceMembers(spaceID: space.id))
  }
  ...
  private func setup(
    isSpace: Bool = false,
    spacesSelectionMode: CreateRoomScreenSpaceSelectionMode = .editableSpacesList(
      preSelectedSpace: nil)
  ) {
    spaceService = SpaceServiceProxyMock(
      .init(
        editableSpaces: .mockJoinedSpaces2,

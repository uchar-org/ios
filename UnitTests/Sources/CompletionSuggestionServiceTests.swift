...
struct CompletionSuggestionServiceTests {
    ...
    func userSuggestions() async throws {
        ...
        deferred = deferFulfillment(service.suggestionsPublisher) { suggestions in
            suggestions == [
                .init(suggestionType: .user(.init(id: alice.userID, displayName: alice.displayName, avatarURL: alice.avatarURL)),
                      range: .init(), rawSuggestionText: "ali")
            ]
        }
    ...
    func userSuggestionsIncludingAllUsers() async throws {
        ...
        deferred = deferFulfillment(service.suggestionsPublisher) { suggestions in
            suggestions == [
                .init(suggestionType: .allUsers(.room(id: "roomID", name: "test", avatarURL: nil)),
                      range: .init(), rawSuggestionText: "ro")
            ]
        }
        ...
        deferred = deferFulfillment(service.suggestionsPublisher) { suggestions in
            suggestions == [
                .init(suggestionType: .allUsers(.room(id: "roomID", name: "test", avatarURL: nil)),
                      range: .init(), rawSuggestionText: "every")
            ]
        }
    ...
    func userSuggestionsWithEmptyText() async throws {
        ...
        deferred = deferFulfillment(service.suggestionsPublisher) { suggestions in
            suggestions == [
                ...
                .init(suggestionType: .user(.init(id: alice.userID, displayName: alice.displayName, avatarURL: alice.avatarURL)),
                      range: .init(), rawSuggestionText: ""),
                .init(suggestionType: .user(.init(id: bob.userID, displayName: bob.displayName, avatarURL: bob.avatarURL)),
                      range: .init(), rawSuggestionText: "")
            ]
        }
        ...
        deferred = deferFulfillment(service.suggestionsPublisher) { suggestions in
            suggestions == [
                .init(suggestionType: .allUsers(.room(id: "roomID", name: "test", avatarURL: nil)),
                      range: .init(location: 0, length: 1), rawSuggestionText: ""),
                .init(suggestionType: .user(.init(id: alice.userID, displayName: alice.displayName, avatarURL: alice.avatarURL)),
                      range: .init(location: 0, length: 1), rawSuggestionText: ""),
                .init(suggestionType: .user(.init(id: bob.userID, displayName: bob.displayName, avatarURL: bob.avatarURL)),
                      range: .init(location: 0, length: 1), rawSuggestionText: "")
    ...
    func userSuggestionInDifferentMessagePositions() async throws {
        ...
        var deferred = deferFulfillment(service.suggestionsPublisher) { suggestions in
            suggestions == [
                .init(suggestionType: .user(.init(id: alice.userID, displayName: alice.displayName, avatarURL: alice.avatarURL)),
                      range: .init(location: 0, length: 3), rawSuggestionText: "al")
            ]
        }
        ...
        deferred = deferFulfillment(service.suggestionsPublisher) { suggestions in
            suggestions == [
                .init(suggestionType: .user(.init(id: alice.userID, displayName: alice.displayName, avatarURL: alice.avatarURL)),
                      range: .init(location: 5, length: 3), rawSuggestionText: "al")
            ]
        }
    ...
    func userSuggestionWithMultipleMentionSymbol() async throws {
        ...
        deffered = deferFulfillment(service.suggestionsPublisher) { suggestions in
            suggestions == [
                .init(suggestionType: .user(.init(id: bob.userID, displayName: bob.displayName, avatarURL: bob.avatarURL)),
                      range: .init(location: 9, length: 3), rawSuggestionText: "bo")
            ]
        }
    ...
    func roomSuggestions() async throws {
        ...
        deferred = deferFulfillment(service.suggestionsPublisher) { suggestions in
            suggestions == [
                .init(suggestionType: .room(.init(id: "2",
    ...
    func suggestionsWithMultipleDifferentTriggers() async throws {
        ...
        deffered = deferFulfillment(service.suggestionsPublisher) { suggestions in
            suggestions == [
                .init(suggestionType: .user(.init(id: alice.userID, displayName: alice.displayName, avatarURL: alice.avatarURL)),
                      range: .init(location: 9, length: 3), rawSuggestionText: "al")
            ]
        }

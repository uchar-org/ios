...
struct NotificationContentBuilder {
  ...
  func process(
    notificationContent: inout UNMutableNotificationContent,
    ...
    case .invite:
      await processInvited(
        notificationContent: &notificationContent,
        notificationItem: notificationItem,
        mediaProvider: mediaProvider)
    case .timeline(let event):
      guard case .messageLike(let messageContent) = try? event.content() else {
        processEmpty(&notificationContent)
        return
      }

      await processMessageLike(
        notificationContent: &notificationContent,
        notificationItem: notificationItem,
        mediaProvider: mediaProvider)
  ...
  private func processMessageLike(
    notificationContent: inout UNMutableNotificationContent,
    notificationItem: NotificationItemProxyProtocol,
    mediaProvider: MediaProviderProtocol
  ) async {
    notificationContent.title =
      notificationItem.senderDisplayName ?? notificationItem.roomDisplayName
    ...
    let senderAvatarDisplayName =
      if let displayName = notificationItem.senderDisplayName {
        displayName
      } else {
    ...
    let senderDisplayName =
      notificationItem.hasMention
      ? L10n.notificationSenderMentionReply(senderAvatarDisplayName) : senderAvatarDisplayName
  ...
  private func icon(for notificationItem: NotificationItemProxyProtocol) -> NotificationIcon {
    if notificationItem.isDM {
      if userSession.threadsEnabled, let threadRootEventID = notificationItem.threadRootEventID {
        .init(
          mediaSource: notificationItem.senderAvatarMediaSource,
          groupInfo: .init(
      ...
      if userSession.threadsEnabled, let threadRootEventID = notificationItem.threadRootEventID {
        .init(
          mediaSource: notificationItem.roomAvatarMediaSource,
          groupInfo: .init(
            avatarDisplayName: notificationItem.roomDisplayName,
  ...
  private func processRoomMessage(
    notificationContent: inout UNMutableNotificationContent,
    notificationItem: NotificationItemProxyProtocol,
    messageType: MessageType,
    mediaProvider: MediaProviderProtocol
    ...
    notificationContent.body = String(
      messageEventStringBuilder.buildAttributedString(
        for: messageType, senderDisplayName: displayName, isOutgoing: false
      ).characters)

    let timelineMediaVisibility = await userSession.mediaPreviewVisibility
    guard
      timelineMediaVisibility == .on
  ...
  private func addMediaAttachment(
    notificationContent: inout UNMutableNotificationContent,
    ...
    case .success(let file):
      do {
        ...
        let newURL = try FileManager.default.copyFileToTemporaryDirectory(
          file: url, with: "\(identifier).\(url.pathExtension)")
  ...
  private func addCommunicationContext(
    notificationContent: inout UNMutableNotificationContent,
    senderID: String,
    ...
    {
      switch await mediaProvider.loadThumbnailForSource(
    ...
    if let fetchedImage {
      image = fetchedImage
    } else if let data = await getPlaceholderAvatarImageData(
      name: icon.groupInfo?.avatarDisplayName ?? senderAvatarDisplayName,
    ...
    if let groupInfo = icon.groupInfo {
      ...
      let me = INPerson(
        personHandle: meHandle, nameComponents: nil, displayName: nil, image: nil,
        contactIdentifier: nil, customIdentifier: nil, isMe: true)
  ...
  func getPlaceholderAvatarImageData(name: String, id: String) async -> Data? {
    ...
    let data = Avatars.generatePlaceholderAvatarImageData(
      name: name, id: id, size: .init(width: 50, height: 50))

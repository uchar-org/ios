...
struct RunTests: AsyncParsableCommand {
  static let configuration = CommandConfiguration(
    abstract: "Runs xcodebuild tests with simulator management, retries, and formatting.",
    discussion: """
    ...
    if let createName = createSimulatorName {
      guard let createType = createSimulatorType else {
        throw ValidationError(
          "--create-simulator-type must be provided when --create-simulator-name is set.")
      }
  ...
  private func createSimulatorIfNecessary(name: String, type: String) async throws {
    ...
    guard
      let simulators = try await CI.run(
        .path("/bin/zsh"), ["-cu", "xcrun simctl list devices \"iOS \(runtime)\" available"],
  ...
  private func createSimulator(name: String, type: String) async throws {
    let deviceID = try await CI.run(
      .path("/bin/zsh"), ["-cu", "xcrun simctl create '\(name)' \(type) \(simulatorRuntime)"],
      output: .string(limit: 4096)
    ).standardOutput
    logger.info(
      "Created simulator '\(name)' (\(deviceID?.trimmingCharacters(in: .whitespacesAndNewlines) ?? "unknown"))."
    )
  }

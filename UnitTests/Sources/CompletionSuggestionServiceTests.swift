...
struct CompletionSuggestionServiceTests {
  ...
  func userSuggestions() async throws {
    ...
    let service = CompletionSuggestionService(
    ...
    deferred = deferFulfillment(service.suggestionsPublisher) { suggestions in
      suggestions == [
        .init(
          suggestionType: .user(
            .init(id: alice.userID, displayName: alice.displayName, avatarURL: alice.avatarURL)),
  ...
  func userSuggestionsIncludingAllUsers() async throws {
    ...
    deferred = deferFulfillment(service.suggestionsPublisher) { suggestions in
      suggestions == [
        .init(
          suggestionType: .allUsers(.room(id: "roomID", name: "test", avatarURL: nil)),
  ...
  func userSuggestionsWithEmptyText() async throws {
    ...
    deferred = deferFulfillment(service.suggestionsPublisher) { suggestions in
      suggestions == [
        ...
        .init(
          suggestionType: .user(
            .init(id: bob.userID, displayName: bob.displayName, avatarURL: bob.avatarURL)),
          range: .init(), rawSuggestionText: ""),
      ]
    }

...
extension UserPreference {
  ...
  convenience init(
    key: String, defaultValue: T, storageType: StorageType, mode: Mode = .localOverRemote
  ) {
  ...
  convenience init<R: RawRepresentable>(
    key: R, defaultValue: T, storageType: StorageType, mode: Mode = .localOverRemote
  ) where R.RawValue == String {
    self.init(key: key.rawValue, defaultValue: defaultValue, storageType: storageType, mode: mode)
  }

  convenience init(key: String, storageType: StorageType, mode: Mode = .localOverRemote)
  where T: ExpressibleByNilLiteral {
    self.init(key: key, defaultValue: nil, storageType: storageType, mode: mode)
  }

  convenience init<R: RawRepresentable>(
    key: R, storageType: StorageType, mode: Mode = .localOverRemote
  ) where R: RawRepresentable, R.RawValue == String, T: ExpressibleByNilLiteral {

    ...
    if let settings = Binding($context.settings[group]) {
      Section {
        ForEach(settings) { $setting in
          ListRow(
            label: .plain(title: setting.title),
            kind: .picker(
              selection: $setting.roleValue,
...
struct RoomChangePermissionsScreen_Previews: PreviewProvider, TestablePreview {
  ...
  static let roomAsUserViewModel = makeViewModel(
    isSpace: false, ownPowerLevel: RoomRole.user.powerLevel)

...
struct RoomEventStringBuilderTests {
  ...
  init() {
    ...
    stringBuilder = RoomEventStringBuilder(
      stateEventStringBuilder: stateEventStringBuilder,
  ...
  func senderPrefix() {
    let ownMessageString = stringBuilder.buildAttributedString(
      for: makeMessageItem(senderID: ownUserID, senderDisplayName: "Alice"))
    #expect(
      ownMessageString?.string == "You: Hello, World!",
      "Your own messages should be prefixed with 'You'")

    let otherMessageString = stringBuilder.buildAttributedString(
      for: makeMessageItem(senderID: "@bob:matrix.org", senderDisplayName: "Bob"))
    #expect(
      otherMessageString?.string == "Bob: Hello, World!",
      "Everyone else's messages should be prefixed with their display name.")

    let ambiguousMessageString = stringBuilder.buildAttributedString(
      for: makeMessageItem(
        senderID: "@charlie:matrix.org",
        senderDisplayName: "Charlie",
        senderDisplayNameAmbiguous: true))
    #expect(
      ambiguousMessageString?.string == "Charlie (@charlie:matrix.org): Hello, World!",
      "Messages from senders with ambiguous display names should include their user ID in the prefix."
    )

    let ownEmoteString = stringBuilder.buildAttributedString(
      for: makeMessageItem(
        senderID: ownUserID,
    ...
    let otherEmoteString = stringBuilder.buildAttributedString(
      for: makeMessageItem(
        ...
        message: "sighs"))
    #expect(
      otherEmoteString?.string == "* Bob sighs",
      "Everyone else's emotes should contain their display name.")

    let ownPollString = stringBuilder.buildAttributedString(
      for: makePollItem(senderID: ownUserID, senderDisplayName: "Alice"))
    #expect(
      ownPollString?.string == "You: Poll: Which is better?",
      "Your own polls should be prefixed with 'You'")

    let otherPollString = stringBuilder.buildAttributedString(
      for: makePollItem(senderID: "@bob:matrix.org", senderDisplayName: "Bob"))
    #expect(
      otherPollString?.string == "Bob: Poll: Which is better?",
      "Everyone else's polls should be prefixed with their display name.")
  }
    ...
    return .init(
      item: .init(
        configuration: .init(
          ...
          sender: senderID,
          senderProfile: .ready(
            displayName: senderDisplayName, displayNameAmbiguous: senderDisplayNameAmbiguous,
            avatarUrl: nil),

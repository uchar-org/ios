...
final class VoiceMessageCacheTests {
  ...
  private let testTemporaryDirectory = URL.temporaryDirectory.appendingPathComponent(
    "test-voice-messsage-cache")
  ...
  func fileURL() throws {
    ...
    let temporaryFileURL = try createTemporaryFile(
      named: testFilename, withExtension: mpeg4aacFileExtension)
    guard
      case .success(let cachedURL) = voiceMessageCache.cache(
        mediaSource: mediaSource, using: temporaryFileURL, move: true)
    else {
  ...
  func cacheInvalidFileExtension() throws {
    ...
    guard
      case .failure(let error) = voiceMessageCache.cache(
        mediaSource: mediaSource, using: mpegFileURL, move: true)
    else {
  ...
  func cacheCopy() throws {
    ...
    guard
      case .success(let cacheURL) = voiceMessageCache.cache(
        mediaSource: mediaSource, using: fileURL, move: false)
    else {
      Issue.record("A success is expected")
      return
    }
  ...
  func cacheMove() throws {
    ...
    guard
      case .success(let cacheURL) = voiceMessageCache.cache(
        mediaSource: mediaSource, using: fileURL, move: true)
    else {
      Issue.record("A success is expected")
      return
    }
  ...
  private func createTemporaryFile(named filename: String, withExtension fileExtension: String)
    throws -> URL
  {
    let temporaryFileURL = testTemporaryDirectory.appendingPathComponent(filename)

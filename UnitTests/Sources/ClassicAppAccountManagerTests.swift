...
final class ClassicAppAccountManagerTests {
  ...
  init() throws {
    accountAESKey = try #require(
      Data(base64Encoded: "BzaSCm5i8QhJr6wGBPj7MDvqBwkwuHLqkRxprVV2zJE="))
    accountIV = try #require(Data(base64Encoded: "dMmg6H2dYTRBE8PwjfAbAQ=="))
    cryptoStorePassphrase = try #require(
      Data(base64Encoded: "ERE/ZXw8rlY3Lv3MBG9sV9g+euOuJnrJaSuvrAMWPrI="))

    classicAppAccountManager = ClassicAppAccountManager(
      cacheFolder: testDirectory,
  ...
  func activeAccount() throws {
    let account = ClassicAppAccount.mock(
      classicAppAccountManager: classicAppAccountManager,
      cryptoStorePassphrase: cryptoStorePassphrase)
  ...
  private func setupFixtures(for account: ClassicAppAccount) throws {
    ...
    try FileManager.default.createDirectory(
      at: accountFileDestination.deletingLastPathComponent(), withIntermediateDirectories: true)
    ...
    try FileManager.default.copyItem(
      at: userFileSource, to: usersDestination.appending(component: userFileName))
  }
}

extension ClassicAppAccount {
  /// Creates a mock account based on the fixtures used by this test.
  static func mock(classicAppAccountManager: ClassicAppAccountManager, cryptoStorePassphrase: Data)
    -> ClassicAppAccount
  {
    let userID = "@classicappaccount:matrix.org"

    return ClassicAppAccount(
      userID: userID,

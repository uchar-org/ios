...
final class NSEUserSession: NSEUserSessionProtocol {
  ...
  private(set) lazy var mediaProvider: MediaProviderProtocol = MediaProvider(
    mediaLoader: MediaLoader(client: baseClient),
  ...
  init(
    ...
    try await baseClient.restoreSessionWith(
      session: credentials.restorationToken.session,
  ...
  {
    do {
      let notificationStatus = try await notificationClient.getNotification(
        roomId: roomID, eventId: eventID)

      switch notificationStatus {
      case .event(let notification):
        return NotificationItemProxy(
      ...
      MXLog.error(
        "Could not get notification's content creating an empty notification instead, error: \(error)"
      )

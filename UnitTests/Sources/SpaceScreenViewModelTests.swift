...
struct SpaceScreenViewModelTests {
  ...
  var paginationStateSubject: CurrentValueSubject<SpaceRoomListPaginationState, Never> = .init(
  ...
  mutating func singlePagination() async throws {
    ...
    deferred = deferFulfillment(spaceRoomListProxy.paginationStatePublisher) {
      $0 == .idle(endReached: true)
    }
    try await deferred.fulfill()
  ...
  mutating func multiplePaginations() async throws {
    ...
    // When the pagination is triggered.
    let deferredIsPaginating = deferFulfillment(
      context.observe(\.viewState.paginationState),
      transitionValues: [.paginating, .idle, .paginating, .endReached])
    let deferredState = deferFulfillment(
      spaceRoomListProxy.paginationStatePublisher, keyPath: \.self,
      transitionValues: [
        .loading,
  ...
  mutating func selectingSpace() async throws {
    ...
    let selectedSpace = try #require(
      mockSpaceRooms.first { $0.isSpace && $0.state == .joined },
      "There should be a space to select.")
  ...
  mutating func selectingRoom() async throws {
    ...
    let selectedRoom = try #require(
      mockSpaceRooms.first { !$0.isSpace }, "There should be a room to select.")
    let deferred = deferFulfillment(viewModel.actionsPublisher) { _ in true }
  ...
  mutating func joiningSpace() async throws {
    ...
    let deferredState = deferFulfillment(
      viewModel.context.observe(\.viewState.joiningRoomIDs),
      transitionValues: [[selectedSpace.id], []])
  ...
  mutating func joiningRoom() async throws {
    ...
    let deferredState = deferFulfillment(
      viewModel.context.observe(\.viewState.joiningRoomIDs),
      transitionValues: [[selectedRoom.id], []])
  ...
  mutating func manageRoomsWithoutRemoving() throws {
    ...
    #expect(
      context.viewState.editModeSelectedIDs.isEmpty, "No rooms should be selected to begin with.")
    #expect(
      !context.viewState.visibleRooms.contains { $0.isSpace },
      "Spaces should be filtered out when managing rooms.")
    ...
    #expect(
      context.viewState.editModeSelectedIDs.count == 1, "The selected room should be included.")
    ...
    #expect(
      context.viewState.editModeSelectedIDs.isEmpty, "Cancelling should clear all selected rooms.")
    #expect(
      context.viewState.visibleRooms.contains { $0.isSpace },
      "Cancelling should restore the hidden spaces.")

    #expect(
      !spaceServiceProxy.removeChildFromCalled,
      "There should be no attempt to remove children when cancelling.")
  }
  ...
  mutating func manageRoomsRemovingChildren() async throws {
    ...
    let firstRoom = try #require(
      mockSpaceRooms.first { !$0.isSpace }, "There should be a room to select.")
    let lastRoom = try #require(
      mockSpaceRooms.last { !$0.isSpace }, "There should be a room to select.")
    ...
    #expect(
      context.viewState.editModeSelectedIDs.count == 2, "The selected rooms should be included.")

    context.send(viewAction: .removeSelectedChildren)
    #expect(
      context.isPresentingRemoveChildrenConfirmation,
      "A confirmation prompt should be shown before removing children.")
    #expect(
      !spaceServiceProxy.removeChildFromCalled,
      "There should be no attempt to remove children before confirming.")
    ...
    #expect(
      !context.isPresentingRemoveChildrenConfirmation,
      "Confirming should dismiss the confirmation prompt.")
    #expect(
      context.viewState.editMode == .inactive, "Confirming should disable edit mode when done.")
    #expect(
      context.viewState.editModeSelectedIDs.isEmpty,
      "Confirming should clear all selected rooms when done.")
    #expect(
      context.viewState.visibleRooms.contains { $0.isSpace },
      "Confirming should restore the hidden spaces when done.")

    #expect(
      spaceServiceProxy.removeChildFromCallsCount == 2,
      "Each selected room should have been removed.")
  ...
  mutating func manageRoomsRemovingChildrenWithFailure() async throws {
    ...
    #expect(
      context.viewState.editModeSelectedIDs.count == 3,
      "All of the visible rooms should be selected.")
    #expect(
      context.isPresentingRemoveChildrenConfirmation,
      "A confirmation prompt should be shown before removing children.")
    ...
    let deferredFailure = deferFailure(context.observe(\.viewState.editMode), timeout: .seconds(1))
    { $0 == .inactive }
    context.send(viewAction: .confirmRemoveSelectedChildren)
    try await deferred.fulfill()
    ...
    #expect(
      context.viewState.visibleRooms.count == 2,
      "The removed rooms should no longer be listed for selection.")
    #expect(
      context.viewState.editModeSelectedIDs.count == 2,
      "The removed rooms should no longer be selected.")

    #expect(
      spaceServiceProxy.removeChildFromCallsCount == 2,
      "Each selected room should have been removed.")
    #expect(
      !spaceRoomListProxy.resetCalled, "The room list should be reset to pick up the changes.")
  }
  ...
  mutating func leavingSpace() async throws {
    ...
    leaveSpaceViewModel.context.send(
      viewAction: .toggleRoom(roomID: firstSelectedRoom.spaceServiceRoom.id))
    ...
    #expect(
      rustLeaveHandle.leaveRoomIdsReceivedRoomIds == [
  ...
  private mutating func setupViewModel(
    initialSpaceRooms: [SpaceServiceRoom] = [], paginationResponses: [[SpaceServiceRoom]] = []
  ) {
    spaceRoomListProxy = SpaceRoomListProxyMock(
      .init(
        spaceServiceRoom: SpaceServiceRoom.mock(isSpace: true),
    ...
    spaceServiceProxy.spaceRoomListSpaceIDClosure = { [mockSpaceRooms] spaceID in
      guard let spaceServiceRoom = mockSpaceRooms.first(where: { $0.id == spaceID }) else {
        return .failure(.missingSpace)
      }
...
extension SpaceScreenViewModelAction {
  fileprivate var isLeftSpace: Bool {
    switch self {
    case .leftSpace: true
    default: false
    }

        ...
        HStack(spacing: 8) {
          ForEach(reactions) { reaction in
            ReactionSummaryButton(
              reaction: reaction, highlighted: selectedReactionKey == reaction.key
            ) { key in
      ...
      ScrollView {
        VStack(alignment: .leading, spacing: 8) {
          ForEach(selectedReaction.senders) { sender in
            ReactionSummarySenderView(
              sender: sender, member: members[sender.id], mediaProvider: mediaProvider
            )

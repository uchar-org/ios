...
final class LoggingTests {
  ...
  deinit {
    ...
    do {
      try reloadTracingFileWriter(
        configuration: .init(
          path: URL.appGroupLogsDirectory.path(percentEncoded: false),
  ...
  func timelineContentIsRedacted() throws {
    ...
    let textMessage = TextRoomTimelineItem(
      id: .randomEvent,
      timestamp: .mock,
      isOutgoing: false,
      isEditable: false,
    ...
    let noticeMessage = NoticeRoomTimelineItem(
      id: .randomEvent,
      timestamp: .mock,
      isOutgoing: false,
      isEditable: false,
    ...
    let emoteMessage = EmoteRoomTimelineItem(
      id: .randomEvent,
      timestamp: .mock,
      isOutgoing: false,
      isEditable: false,
    ...
    let imageMessage = ImageRoomTimelineItem(
      id: .randomEvent,
      timestamp: .mock,
      isOutgoing: false,
      isEditable: false,
      ...
      content: .init(
        ...
        imageInfo: .mockImage,
        thumbnailInfo: nil))
    let videoMessage = VideoRoomTimelineItem(
      id: .randomEvent,
      timestamp: .mock,
      isOutgoing: false,
      isEditable: false,
      ...
      content: .init(
        ...
        videoInfo: .mockVideo,
        thumbnailInfo: nil))
    let fileMessage = FileRoomTimelineItem(
      id: .randomEvent,
      timestamp: .mock,
      isOutgoing: false,
      isEditable: false,
  ...
  func rustMessageContentIsRedacted() throws {
    ...
    let rustVideoMessage = VideoMessageContent(
      filename: "VideoString",
      caption: "VideoString",
      formattedCaption: nil,
      source: MediaSource(noHandle: .init()),
    ...
    let rustFileMessage = FileMessageContent(
      filename: "FileString",
      caption: "FileString",
      formattedCaption: nil,
      source: MediaSource(noHandle: .init()),
  ...
  func logFileSorting() throws {
    ...
    for i in 1...5 {
      ...
      try "console".write(
        to: logsFileDirectory.appending(path: filename), atomically: true, encoding: .utf8)
    }

    for i in 1...5 {
      let nseFilename = "console-nse.\(i).log"
      try "nse".write(
        to: logsFileDirectory.appending(path: nseFilename), atomically: true, encoding: .utf8)
    }

    // Then the logs should be sorted chronologically (newest first) and not alphabetically.
    #expect(
      Tracing.logFiles.map(\.lastPathComponent) == [
        "console-nse.5.log",
        "console-nse.4.log",
        "console-nse.3.log",
        "console-nse.2.log",
  ...
  private func setupTest(name: String = #function, redirectTracingFileWriter: Bool = true) throws {
    let testDirectory = URL.appGroupLogsDirectory.appending(
    ...
    if redirectTracingFileWriter {
      try reloadTracingFileWriter(
        configuration: .init(
          path: testDirectory.path(percentEncoded: false),
          filePrefix: "console",
          fileSuffix: ".log",
          maxTotalSizeBytes: 1000,
          maxAgeSeconds: 1000))

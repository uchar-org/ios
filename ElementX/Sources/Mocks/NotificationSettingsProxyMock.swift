...
struct NotificationSettingsProxyMockConfiguration {
  ...
  init(
    defaultRoomMode: RoomNotificationModeProxy = .allMessages,
    roomMode: RoomNotificationModeProxy = .allMessages, canPushEncryptedEvents: Bool = false
  ) {
    self.defaultRoomMode = defaultRoomMode
    self.roomMode = RoomNotificationSettingsProxyMock(
      with: RoomNotificationSettingsProxyMockConfiguration(
        mode: roomMode, isDefault: defaultRoomMode == roomMode))
...
extension NotificationSettingsProxyMock {
  convenience init(with configuration: NotificationSettingsProxyMockConfiguration) {
    ...
    getUserDefinedRoomNotificationModeRoomIdReturnValue =
      configuration.roomMode.isDefault ? nil : configuration.roomMode.mode
    ...
    setNotificationModeRoomIdModeClosure = { [weak self] _, mode in
      ...
      self.getNotificationSettingsRoomIdIsEncryptedIsOneToOneReturnValue =
    ...
    restoreDefaultNotificationModeRoomIdClosure = { [weak self] _ in
      ...
      self.getNotificationSettingsRoomIdIsEncryptedIsOneToOneReturnValue =
        RoomNotificationSettingsProxyMock(
          with: .init(mode: configuration.defaultRoomMode, isDefault: true))
      Task {
        self.callbacks.send(.settingsDidChange)
      }
    }

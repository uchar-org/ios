...
typealias JoinRoomScreenViewModelType = StateStoreViewModel<
  JoinRoomScreenViewState, JoinRoomScreenViewAction
>
...
class JoinRoomScreenViewModel: JoinRoomScreenViewModelType, JoinRoomScreenViewModelProtocol {
  ...
  init(
    ...
    super.init(
      initialViewState: JoinRoomScreenViewState(roomID: roomID),
    ...
    case .generic(let roomID, let via):
      ...
      case .failure:
        ...
        state.bindings.alertInfo = .init(
          ...
          secondaryButton: .init(title: L10n.actionCancel, role: .cancel) { [weak self] in
      ...
      membershipStateChangeCancellable = clientProxy
        ...
        .compactMap { summaries -> Void? in
          guard let roomSummary = summaries.first(where: { $0.id == roomInfo?.id }),
            roomSummary.room.membership() != .knocked
          else {
            return nil
          }
          return ()
  ...
  private func updateGenericRoomDetails(
    roomID: String, roomInfo: BaseRoomInfoProxyProtocol?, inviter: RoomInviterDetails?
  ) async {
    ...
    state.roomDetails = JoinRoomScreenRoomDetails(
  ...
  private func updateSpaceRoomDetails(
    spaceServiceRoom: SpaceServiceRoom, inviter: RoomInviterDetails?
  ) async {
    state.roomDetails = JoinRoomScreenRoomDetails(
  ...
  private func updateMode() async {
    ...
    if case .space(let spaceServiceRoom) = source {
      ...
      case .invited:
        state.mode = .invited(
          isDM: spaceServiceRoom.isDirect == true && spaceServiceRoom.joinedMembersCount == 1)
      case .knocked:
        state.mode = .knocked
      case .banned:
      ...
      case .invited:
        state.mode = .invited(
          isDM: state.roomDetails?.isDirect == true && state.roomDetails?.memberCount == 1)
      case .knocked:
        state.mode = .knocked
      case .banned:
        state.mode = .banned(
          sender: membershipDetails?.senderRoomMember?.displayName
            ?? membershipDetails?.senderRoomMember?.userID,
    ...
    if let alias = state.roomDetails?.canonicalAlias {
      ...
      case .failure(let error):
        ...
        case .invalidInvite:
          ...
          state.bindings.alertInfo = .init(
            id: .invalidInvite, title: L10n.dialogTitleError, message: L10n.errorInvalidInvite)
        default:
  ...
  private func knockRoom() async {
    ...
    if let alias = state.roomDetails?.canonicalAlias {
      switch await clientProxy.knockRoomAlias(
        alias,
      ...
      let (roomID, via) = source.roomIDAndVia()
      switch await clientProxy.knockRoom(
        roomID,
        via: via,
        message: state.bindings.knockMessage.isBlank ? nil : state.bindings.knockMessage)
      {
  ...
  private func showDeclineInviteConfirmationAlert() {
    ...
    state.bindings.alertInfo = .init(
      ...
      secondaryButton: .init(title: L10n.actionDecline, role: .destructive) {
        Task { await self.declineInvite() }
      })
  }
  ...
  private func showCancelKnockConfirmationAlert() {
    state.bindings.alertInfo = .init(
      ...
      secondaryButton: .init(
        title: L10n.screenJoinRoomCancelKnockAlertConfirmation, role: .destructive
      ) { Task { await self.cancelKnock() } })
  }
  ...
  private func showDeclineAndBlockConfirmationAlert(userID: String) async {
    if await clientProxy.isReportRoomSupported {
      ...
      state.bindings.alertInfo = .init(
        id: .declineInviteAndBlock,
        title: L10n.screenJoinRoomDeclineAndBlockAlertTitle,
        message: L10n.screenJoinRoomDeclineAndBlockAlertMessage(userID),
        primaryButton: .init(title: L10n.actionCancel, role: .cancel, action: nil),
        secondaryButton: .init(
          title: L10n.screenJoinRoomDeclineAndBlockAlertConfirmation, role: .destructive
        ) { Task { await self.declineAndBlock(userID: userID) } })
    }
  }
  ...
  private func declineInvite() async -> Bool {
    ...
    guard case .invited(let roomProxy) = room else {
      userIndicatorController.submitIndicator(.init(title: L10n.errorUnknown))
      return false
    }
  ...
  private func cancelKnock() async {
    ...
    guard case .knocked(let roomProxy) = room else {
      userIndicatorController.submitIndicator(.init(title: L10n.errorUnknown))
      return
    }
  ...
  private func forgetRoom() async {
    ...
    guard case .banned(let roomProxy) = room else {
      userIndicatorController.submitIndicator(.init(title: L10n.errorUnknown))
      return
    }
  ...
  private func showLoadingIndicator() {
    userIndicatorController.submitIndicator(
      UserIndicator(
        ...
        type: .modal(
          progress: .indeterminate, interactiveDismissDisabled: false, allowsInteraction: true),

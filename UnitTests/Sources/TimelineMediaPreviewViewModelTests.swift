...
struct TimelineMediaPreviewViewModelTests {
  ...
  mutating func loadingItemFailure() async throws {
    // Given a fresh view model.
    setupViewModel()
    guard case .media(let mediaItem) = context.viewState.currentItem else {
    ...
    let failure = deferFailure(viewModel.state.previewControllerDriver, timeout: .seconds(1)) {
      $0.isItemLoaded
    }
    context.send(
      viewAction: .updateCurrentItem(.media(context.viewState.dataSource.previewItems[0])))
  ...
  mutating func swipingBetweenItems() async throws {
    ...
    context.send(
      viewAction: .updateCurrentItem(.media(context.viewState.dataSource.previewItems[1])))
  ...
  mutating func pagination() async throws {
    ...
    let deferred = deferFulfillment(context.viewState.dataSource.previewItemsPaginationPublisher) {
      _ in true
    }
    ...
    context.send(
      viewAction: .updateCurrentItem(.media(context.viewState.dataSource.previewItems[3])))
    try await failure.fulfill()
  ...
  mutating func viewInRoomTimeline() async throws {
    ...
    let deferred = deferFulfillment(viewModel.actions) {
      $0 == .viewInRoomTimeline(mediaItem.timelineItem.id)
    }
  ...
  mutating func redactConfirmation() async throws {
    ...
    let deferredDriver = deferFulfillment(context.viewState.previewControllerDriver) {
      $0.isShowItemDetails
    }
    ...
    guard case .showItemDetails(let mediaDetailsItem) = action else {
  ...
  mutating func saveImageWithoutAuthorization() async throws {
    ...
    // When choosing to save the image.
    let deferred = deferFulfillment(context.viewState.previewControllerDriver) {
      $0.isAuthorizationRequired
    }
    context.send(viewAction: .menuAction(.save, item: mediaItem))
  ...
  mutating func saveFile() async throws {
    ...
    guard case .exportFile(let file) = exportAction else {
  ...
  private func loadInitialItem() async throws {
    let deferred = deferFulfillment(viewModel.state.previewControllerDriver) { $0.isItemLoaded }
    let initialItem = context.viewState.dataSource.previewController(
      QLPreviewController(),
  ...
  private mutating func setupViewModel(
    initialItemIndex: Int = 0, photoLibraryAuthorizationDenied: Bool = false
  ) {
    ...
    photoLibraryManager = PhotoLibraryManagerMock(
      .init(authorizationDenied: photoLibraryAuthorizationDenied))

    viewModel = TimelineMediaPreviewViewModel(
      initialItem: initialItems[initialItemIndex],
      timelineViewModel: TimelineViewModel.mock(
        timelineKind: .media(.mediaFilesScreen),

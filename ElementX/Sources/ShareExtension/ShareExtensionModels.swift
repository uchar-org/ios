...
enum ShareExtensionPayload: Hashable, Codable {
    ...
    func withDefaultTemporaryDirectory() throws -> Self {
        ...
        case .mediaFiles(let roomID, let mediaFiles):
            return try .mediaFiles(roomID: roomID,
                                   mediaFiles: mediaFiles.map { mediaFile in
                                       let path = mediaFile.url.path.replacing(URL.appGroupTemporaryDirectory.path, with: "")
                                           .trimmingPrefix("/")

...
struct BuildSDK: AsyncParsableCommand {
  static let configuration = CommandConfiguration(
    abstract: "A tool to checkout and build MatrixRustSDK locally for development.")
  ...
  @Flag(
    help:
      "Build the SDK to run on an iPhone or iPad. This flag takes precedence over --simulator and --target."
  )
  var device
  ...
  @Option(
    help:
      "The target to build for such as aarch64-apple-ios. Omit this option to build for all targets."
  )
  var target: [Target] = []

  @Option(
    help: "The profile to use when building the SDK. Omit this option to build in debug mode.")
  ...
  func checkRustupTargets() throws {
    ...
    guard let output = try Zsh.run(command: "rustup target list --toolchain stable --installed")
    else { throw Error.rustupOutputFailure }
  ...
  func cloneSDKIfNeeded() throws {
    ...
    try Zsh.run(
      command: "git clone https://github.com/matrix-org/matrix-rust-sdk",
  ...
  func updateProjectYAML() throws {
    ...
    guard var projectConfig = try Yams.compose(yaml: yamlString) else {
      throw Error.failureParsingProjectYAML
    }

...
class VoiceMessageRecorder: VoiceMessageRecorderProtocol {
  ...
  func sendVoiceMessage(
    ...
    let oggFile = URL.temporaryDirectory.appendingPathComponent(sourceFilename)
    ...
    let result = await timelineController.sendVoiceMessage(
      url: oggFile,
      audioInfo: audioInfo,
      waveform: waveform
    ) { _ in }
      ...
      if !recordingCancelled {
        Task {
          guard case .success = await finalizeRecording() else {
            actionsSubject.send(
              .didFailWithError(error: VoiceMessageRecorderError.previewNotAvailable))
            return
          }
          ...
          actionsSubject.send(
            .didStopRecording(previewState: previewAudioPlayerState, url: recordingURL))
        }
      }
  ...
  private func finalizeRecording() async -> Result<Void, VoiceMessageRecorderError> {
    ...
    previewAudioPlayerState = await AudioPlayerState(
      id: .recorderPreview, title: L10n.commonVoiceMessage, duration: recordingDuration,
      waveform: EstimatedWaveform(data: []))

...
struct MediaProviderTests {
  ...
  func loadingRetriedOnReconnection() async throws {
    ...
    let loadTask = try mediaProvider.loadImageRetryingOnReconnection(
      MediaSourceProxy(url: .mockMXCImage, mimeType: "image/jpeg"))
  ...
  func whenImageFromSourceWithSourceNil_nilReturned() {
    let image = mediaProvider.imageFromSource(
      nil, size: Avatars.Size.room(on: .timeline).scaledSize)
    #expect(image == nil)
  }
  ...
  func whenImageFromSourceWithSourceNotNilAndImageCacheContainsImage_ImageIsReturned() throws {
    ...
    let image = try mediaProvider.imageFromSource(
      MediaSourceProxy(url: url, mimeType: "image/jpeg"),
  ...
  func whenLoadImageFromSourceAndImageCacheContainsImage_successIsReturned() async throws {
    ...
    let imageForKey = UIImage()
    imageCache.retrievedImagesInMemory[key] = imageForKey
    let result = try await mediaProvider.loadImageFromSource(
      MediaSourceProxy(url: url, mimeType: "image/jpeg"),
      size: avatarSize.scaledSize)
  ...
  @Test
  func whenLoadImageFromSourceAndImageNotCachedAndRetrieveImageSucceeds_successIsReturned()
    async throws
  {
    let avatarSize = Avatars.Size.room(on: .timeline)
    let url = URL.mockMXCImage
  ...
  @Test
  func whenLoadImageFromSourceAndImageNotCachedAndRetrieveImageFails_imageThumbnailIsLoaded()
    async throws
  {
    let avatarSize = Avatars.Size.room(on: .timeline)
  ...
  func whenLoadImageFromSourceAndImageNotCachedAndRetrieveImageFails_imageIsStored() async throws {
    ...
    _ = try await mediaProvider.loadImageFromSource(
      MediaSourceProxy(url: url, mimeType: "image/jpeg"),
      size: avatarSize.scaledSize)
  ...
  @Test
  func
    whenLoadImageFromSourceAndImageNotCachedAndRetrieveImageFailsAndNoAvatarSize_imageContentIsLoaded()
    async throws
  {
    let expectedImage = try loadTestImage()
  ...
  @Test
  func
    whenLoadImageFromSourceAndImageNotCachedAndRetrieveImageFailsAndLoadImageThumbnailFails_errorIsThrown()
    async throws
  {
  ...
  @Test
  func
    whenLoadImageFromSourceAndImageNotCachedAndRetrieveImageFailsAndNoAvatarSizeAndLoadImageContentFails_errorIsThrown()
    async throws
  {
    mediaLoader.loadMediaContentForSourceThrowableError = MediaProviderTestsError.error
  ...
  @Test
  func
    whenLoadImageFromSourceAndImageNotCachedAndRetrieveImageFailsAndImageThumbnailIsLoadedWithCorruptedData_errorIsThrown()
    async throws
  {
  ...
  private func loadTestImage() throws -> UIImage {
    guard
      let path = Bundle(for: UnitTestsAppCoordinator.self).path(
        forResource: "test_image", ofType: "png"),

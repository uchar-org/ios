...
class TimelineMediaPreviewController: QLPreviewController {
  ...
  init(context: TimelineMediaPreviewViewModel.Context) {
    ...
    downloadIndicatorHostingController = UIHostingController(
      rootView: DownloadIndicatorView(context: context))
        ...
        let bottomConstraint =
          if #available(iOS 26, *) {
            captionView.bottomAnchor.constraint(
              equalTo: bottomBarItemsContainer.safeAreaLayoutGuide.bottomAnchor, constant: -50)
          } else {
  ...
  private func presentMediaDetails(for mediaItem: TimelineMediaPreviewItem.Media) {
    ...
    let hostingController = UIHostingController(
      rootView: TimelineMediaPreviewDetailsView(
        item: mediaItem,
        context: context,
  ...
  private func exportFile(_ file: TimelineMediaPreviewFileExportPicker.File) {
    let hostingController = UIHostingController(
      rootView: TimelineMediaPreviewFileExportPicker(file: file))
    present(hostingController, animated: true)
  }
  ...
  private func presentAuthorizationRequiredAlert(appMediator: AppMediatorProtocol) {
    ...
    alertController.addAction(
      .init(title: L10n.commonSettings, style: .default) { _ in appMediator.openAppSettings() })
...
extension UISheetPresentationController.Detent {
  fileprivate static func height(_ height: CGFloat) -> UISheetPresentationController.Detent {
    .custom { _ in height }
  }
}

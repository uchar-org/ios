...
struct CI: ParsableCommand {
  static let configuration = CommandConfiguration(
    abstract: "CI workflow commands that can be run both locally and in CI environments.",
    subcommands: [
      PreviewTests.self,
  ...
  static func readMarketingVersion() throws -> String {
    ...
    guard let projectConfig = try Yams.compose(yaml: projectString),
      let version = projectConfig["settings"]?["MARKETING_VERSION"]?.string
    else {
  ...
  static func collectCoverage(resultBundle: String, target: String = "ElementX", outputName: String)
    async
  {
    ...
    do {
      try await run(
        ...
        [
          "-cu",
          "xcresultparser -q -o cobertura -t \(target) -p \(projectPath) \(resultBundlePath) > \(outputPath)",
        ])
  ...
  static func collectTestResults(resultBundle: String, outputName: String) async {
    ...
    guard FileManager.default.fileExists(atPath: resultBundlePath) else {
      logger.info(
        " Result bundle not found at \(resultBundlePath), skipping test result collection.")
      return
    }
    ...
    do {
      try await run(
        .path("/bin/zsh"),
        ["-cu", "xcresultparser -q -o junit -p \(projectPath) \(resultBundlePath) > \(outputPath)"])
  ...
  static func zipResults(bundles: [String], outputName: String) async {
    ...
    do {
      ...
      try await run(
        .path("/bin/zsh"),
        ["-cu", "cd \(testOutputDirectory) && zip -rq \(outputName) \(bundleArgs)"])
  ...
  @discardableResult
  static func run<Output: OutputProtocol, Error: ErrorOutputProtocol>(
  ...
  static func gitRepositoryURL() async throws -> String {
    guard
      let rawURL = try await CI.run(
        .name("git"), ["ls-remote", "--get-url", "origin"],

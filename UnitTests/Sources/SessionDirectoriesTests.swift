...
struct SessionDirectoriesTests {
  ...
  func initWithDataDirectory() {
    ...
    let sessionDirectory = URL.applicationSupportBaseDirectory.appending(
      component: sessionDirectoryName)
  ...
  func pathOutput() {
    ...
    let sessionDirectories = SessionDirectories(
      dataDirectory: dataDirectory, cacheDirectory: cacheDirectory)
  ...
  func deleteDirectories() throws {
    ...
    try fileManager.createDirectory(
      at: sessionDirectories.dataDirectory, withIntermediateDirectories: true)
    try fileManager.createDirectory(
      at: sessionDirectories.cacheDirectory, withIntermediateDirectories: true)
...
extension SessionDirectories {
  fileprivate var mockStateStorePath: String {
    dataDirectory.appending(component: "matrix-sdk-state.sqlite3").path(percentEncoded: false)
  }

  fileprivate var mockCryptoStorePath: String {
    dataDirectory.appending(component: "matrix-sdk-crypto.sqlite3").path(percentEncoded: false)
  }

  fileprivate var mockEventCachePath: String {
    cacheDirectory.appending(component: "matrix-sdk-event-cache.sqlite3").path(
      percentEncoded: false)
  }

  fileprivate func generateMockData() {

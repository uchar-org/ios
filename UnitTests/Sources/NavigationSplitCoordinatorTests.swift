...
struct NavigationSplitCoordinatorTests {
  ...
  init() {
    navigationSplitCoordinator = NavigationSplitCoordinator(
      placeholderCoordinator: SomeTestCoordinator())
  }
  ...
  func fullScreenCover() {
    ...
    assertCoordinatorsEqual(
      fullScreenCoordinator, navigationSplitCoordinator.fullScreenCoverCoordinator)
  ...
  func embeddedStackPresentsSheetThroughSplit() {
    let sidebarNavigationStackCoordinator = NavigationStackCoordinator(
      navigationSplitCoordinator: navigationSplitCoordinator)
  ...
  func splitTracksEmbeddedStackRootChanges() async {
    ...
    assertCoordinatorsEqual(
      sidebarNavigationStackCoordinator.rootCoordinator,
      navigationSplitCoordinator.compactLayoutRootCoordinator)
  ...
  func splitTracksEmbeddedStackChanges() async {
    ...
    await waitForConfirmation("Coordinators should match", timeout: .seconds(1)) { confirm in
      DispatchQueue.main.async {
        #expect(
          sidebarNavigationStackCoordinator.stackCoordinators.count
            == navigationSplitCoordinator.compactLayoutStackCoordinators.count)
        for index in sidebarNavigationStackCoordinator.stackCoordinators.indices {
          assertCoordinatorsEqual(
            sidebarNavigationStackCoordinator.stackCoordinators[index],
            navigationSplitCoordinator.compactLayoutStackCoordinators[index])
        }
        confirm()
      }
    }
  ...
  func compactStackCreation() async {
    ...
    await waitForConfirmation("Coordinators should match", timeout: .seconds(1)) { confirm in
      DispatchQueue.main.async {
        assertCoordinatorsEqual(
          navigationSplitCoordinator.compactLayoutRootCoordinator,
          sidebarCoordinator.rootCoordinator)
        assertCoordinatorsEqual(
          navigationSplitCoordinator.compactLayoutStackModules[0].coordinator,
          sidebarCoordinator.stackCoordinators.first)
        assertCoordinatorsEqual(
          navigationSplitCoordinator.compactLayoutStackModules[1].coordinator,
          detailCoordinator.rootCoordinator)
        assertCoordinatorsEqual(
          navigationSplitCoordinator.compactLayoutStackModules[2].coordinator,
          detailCoordinator.stackCoordinators.first)
        assertCoordinatorsEqual(
          navigationSplitCoordinator.compactLayoutStackModules[3].coordinator,
          detailCoordinator.stackCoordinators.last)
        confirm()
      }
    }
  }
  ...
  mutating func setRootDetailToNilAfterPoppingToRoot() async {
    ...
    await waitForConfirmation(
      "Details coordinator should be nil, and the compact layout revert to the sidebar root",
      timeout: .seconds(1)
    ) { [navigationSplitCoordinator] confirm in
      DispatchQueue.main.async {
...
private func assertCoordinatorsEqual(_ lhs: CoordinatorProtocol?, _ rhs: CoordinatorProtocol?) {
  ...
  guard let lhs = lhs as? SomeTestCoordinator,
    let rhs = rhs as? SomeTestCoordinator
  else {
    Issue.record(
      "Coordinators are not the same: \(String(describing: lhs)) != \(String(describing: rhs))")
    return
  }

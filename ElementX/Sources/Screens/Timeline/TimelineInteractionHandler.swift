...
class TimelineInteractionHandler {
  ...
  func handleTimelineItemMenuAction(
    _ action: TimelineItemMenuAction, itemID: TimelineItemIdentifier
  ) {
    guard let timelineItem = timelineController.timelineItems.firstUsingStableID(itemID),
      let eventTimelineItem = timelineItem as? EventBasedTimelineItemProtocol
    else {
    ...
    case .copyCaption:
      guard let messageTimelineItem = timelineItem as? EventBasedMessageTimelineItemProtocol,
        let caption = messageTimelineItem.mediaCaption
      else {
        return
      }
    ...
    case .edit, .addCaption, .editCaption, .editPoll:
      ...
      case let pollTimelineItem as PollRoomTimelineItem:
        ...
        actionsSubject.send(
          .displayPollForm(mode: .edit(eventID: eventID, poll: pollTimelineItem.poll)))
      default:
        MXLog.error("Cannot edit item with id: \(timelineItem.id)")
      }
    case .removeCaption:
      guard case .event(_, let eventOrTransactionID) = timelineItem.id else {
    ...
    case .copyPermalink:
      ...
      Task {
        guard case .success(let permalinkURL) = await roomProxy.matrixToEventPermalink(eventID)
        else {
          actionsSubject.send(.displayErrorToast(L10n.errorFailedCreatingThePermalink))
          return
        }
    ...
    case .redact:
      guard case .event(_, let eventOrTransactionID) = itemID else { fatalError() }
    ...
    case .reply:
      ...
      let replyDetails = TimelineItemReplyDetails.loaded(
        sender: eventTimelineItem.sender, eventID: eventID, eventContent: replyInfo.type)

      actionsSubject.send(
        .composer(
          action: .setMode(
            mode: .reply(eventID: eventID, replyDetails: replyDetails, isThread: replyInfo.isThread)
          )))
    case .replyInThread:
    ...
    case .report:
      actionsSubject.send(
        .displayReportContent(itemID: itemID, senderID: eventTimelineItem.sender.id))
    case .react:
    ...
    case .pin:
      analyticsService.trackPinUnpinEvent(
        .init(
          from: timelineController.timelineKind == .pinned ? .MessagePinningList : .Timeline,
          kind: .Pin))
  ...
  {
    guard case .event(_, let eventOrTransactionID) = messageTimelineItem.id else {
    ...
    actionsSubject.send(
      .composer(
        action: .setMode(
          mode: .edit(originalEventOrTransactionID: eventOrTransactionID, type: editType))))
  ...
  func sendPollResponse(pollStartID: String, optionID: String) {
    Task {
      let sendPollResponseResult = await pollInteractionHandler.sendPollResponse(
  ...
  func handlePasteOrDrop(_ providers: [NSItemProvider]) {
    Task {
      ...
      self.userIndicatorController.submitIndicator(
        UserIndicator(
          id: loadingIndicatorIdentifier, type: .modal, title: L10n.commonLoading, persistent: true)
      )
      defer {
  ...
  private func handleVoiceMessageRecorderAction(_ action: VoiceMessageRecorderAction) {
    ...
    case .didStartRecording(let audioRecorder):
      ...
      actionsSubject.send(
        .composer(action: .setMode(mode: .recordVoiceMessage(state: audioRecordState))))
    case .didStopRecording(let previewAudioPlayerState, let url):
      actionsSubject.send(
        .composer(
          action: .setMode(
            mode: .previewVoiceMessage(
              state: previewAudioPlayerState, waveform: .url(url), isUploading: false))))
  ...
  func sendCurrentVoiceMessage() async {
    guard let audioPlayerState = voiceMessageRecorder.previewAudioPlayerState,
      let recordingURL = voiceMessageRecorder.recordingURL
    else {
      actionsSubject.send(.displayErrorToast(L10n.errorFailedUploadingVoiceMessage))
      return
    }

    analyticsService.trackComposer(
      inThread: false,
      isEditing: false,
      isReply: false,
    ...
    actionsSubject.send(
      .composer(
        action: .setMode(
          mode: .previewVoiceMessage(
            state: audioPlayerState, waveform: .url(recordingURL), isUploading: true))))
    await voiceMessageRecorder.stopPlayback()

    switch await voiceMessageRecorder.sendVoiceMessage(
    ...
    case .failure(let error):
      ...
      actionsSubject.send(
        .composer(
          action: .setMode(
            mode: .previewVoiceMessage(
              state: audioPlayerState, waveform: .url(recordingURL), isUploading: false))))
      actionsSubject.send(.displayErrorToast(L10n.errorFailedUploadingVoiceMessage))
    }
  }
  ...
  func playPauseAudio(for itemID: TimelineItemIdentifier) async {
    ...
    guard let voiceMessageRoomTimelineItem = timelineItem as? VoiceMessageRoomTimelineItem else {
      fatalError(
        "Invalid TimelineItem type for itemID \(itemID) (expecting `VoiceMessageRoomTimelineItem` but found \(type(of: timelineItem)) instead"
      )
    }
    ...
    guard audioPlayer.sourceURL == source.url, audioPlayer.state != .error else {
      ...
      do {
        ...
        let url = try await userSession.voiceMessageMediaManager.loadVoiceMessageFromSource(
          source, body: nil)
  ...
  func seekAudio(for itemID: TimelineItemIdentifier, progress: Double) async {
    guard let playerState = mediaPlayerProvider.playerState(for: .timelineItemIdentifier(itemID))
    else {
      return
    }
  ...
  func audioPlayerState(for itemID: TimelineItemIdentifier) -> AudioPlayerState? {
    ...
    guard let voiceMessageRoomTimelineItem = timelineItem as? VoiceMessageRoomTimelineItem else {
      MXLog.error(
        "Invalid TimelineItem type (expecting `VoiceMessageRoomTimelineItem` but found \(type(of: timelineItem)) instead"
      )
      return nil
    }
    ...
    let playerState = AudioPlayerState(
      id: .timelineItemIdentifier(itemID),
  ...
  func displayEmojiPicker(for itemID: TimelineItemIdentifier) {
    ...
    let selectedEmojis = Set(
      eventTimelineItem.properties.reactions.compactMap { $0.isHighlighted ? $0.key : nil })
  ...
  func processItemTap(_ itemID: TimelineItemIdentifier) async -> TimelineControllerAction {
    guard
      let timelineItem = timelineController.timelineItems.firstUsingStableID(itemID)
        as? EventBasedTimelineItemProtocol
    else {
      return .none
    }
    ...
    case let item as LiveLocationRoomTimelineItem:
      ...
      let initialLiveLocationShare = LiveLocationShare(
        userID: item.sender.id,
        geoURI: geoURI,
        timestamp: item.timestamp,
        timeoutDate: item.content.timeoutDate)
      return .displayLiveLocation(
        sender: item.sender, initialLiveLocationShare: initialLiveLocationShare)
  ...
  private func buildReplyInfo(for item: EventBasedTimelineItemProtocol) -> ReplyInfo {
    ...
    case let messageItem as EventBasedMessageTimelineItemProtocol:
      return .init(
        type: .message(messageItem.contentType), isThread: messageItem.properties.isThreaded)
  ...
  private func mediaPreviewAction(
    for item: EventBasedMessageTimelineItemProtocol, messageTypes: [TimelineAllowedMessageType]
  ) async -> TimelineControllerAction {
    ...
    case .detached:
      guard case .event(_, eventOrTransactionID: .eventID(let eventID)) = item.id else {
    ...
    if let newTimelineFocus, let newTimelinePresentation {
      ...
      guard
        case .success(let timelineController) =
          await timelineControllerFactory.buildMessageFilteredTimelineController(

...
class LiveLocationManager: NSObject, LiveLocationManagerProtocol, CLLocationManagerDelegate {
  ...
  init(
    ...
    setupMinimumDistanceUpdatesAndAccuracy(
      minimumDistance: appSettings.liveLocationMinimumDistanceUpdate)
    setupSubscriptions()
  }
  ...
  > {
    ...
    startingLiveLocationSharingSessionsByRoomID[roomID] = LiveLocationSession(
      eventID: eventID, expirationDate: expirationDate)
  ...
  func stopLiveLocation(roomID: String) async {
    ...
    } else if case .joined(let fetchedRoomProxy) = await clientProxy.roomForIdentifier(roomID) {
  ...
  private func handleBeaconInfoUpdate(_ update: LiveLocationOwnInfoUpdate) {
    ...
    guard let session = startingLiveLocationSharingSessionsByRoomID[update.roomID],
      session.eventID == update.eventID
    else {
      return
    }
  ...
  private func stopAllSessions() {
    let roomIDs = Array(
      Set(appSettings.liveLocationSharingSessionsByRoomID.keys)

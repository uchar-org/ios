...
class AudioRecorder: AudioRecorderProtocol {
  ...
  private let dispatchQueue = DispatchQueue(
    label: "io.element.elementx.audio_recorder", qos: .userInitiated)
  ...
  private func startRecording(
    audioFileURL: URL, completion: @escaping (Result<Void, AudioRecorderError>) -> Void
  ) {
    dispatchQueue.async { [weak self] in
      ...
      guard let recordingFormat = AVAudioFormat(standardFormatWithSampleRate: 48000, channels: 1)
      else {
      ...
      if recordingFormat.sampleRate != hardwareSampleRate {
        MXLog.info(
          "Sample rate conversion is needed \(hardwareSampleRate) -> \(recordingFormat.sampleRate)")
    ...
    if let audioConverter {
      ...
      guard
        let convertedBuffer = AVAudioPCMBuffer(
          pcmFormat: recordingFormat, frameCapacity: frameCapacity)
      else {
      ...
      audioConverter.convert(
        to: convertedBuffer, error: &conversionError, withInputFrom: inputBlock)

...
class UserFlowTests: XCTestCase {
  ...
  private func checkRoomFlows() {
    ...
    // And open it
    let firstRoom = app.buttons.matching(
      NSPredicate(format: "identifier CONTAINS %@", Self.integrationTestsRoomName)
    ).firstMatch
  ...
  private func checkTimelineItemActionMenu() {
    ...
    let timelineItemActionMenu = app.scrollViews[A11yIdentifiers.roomScreen.timelineItemActionMenu]
      .firstMatch

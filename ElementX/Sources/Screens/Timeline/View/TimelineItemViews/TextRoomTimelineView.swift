...
struct TextRoomTimelineView: View, TextBasedRoomTimelineViewProtocol {
  ...
  init(
    timelineItem: TextRoomTimelineItem,
    linkMetadata: OrderedDictionary<URL, LinkMetadataProviderItem> = [:]
  ) {
          ...
          VStack(spacing: 8) {
            ForEach(linkMetadata.keys, id: \.absoluteString) { url in
              let metadata =
                ...
                ?? context?.viewState.linkMetadataProvider?.metadataItems[url]?.metadata
  ...
  private func fetchLinkPreviews() async {
    ...
    await withTaskGroup { taskGroup in
      for url in timelineItem.links.prefix(Self.maxLinkPreviewsToRender) {
        taskGroup.addTask {
          if case .success(let metadata) = await context?.viewState.linkMetadataProvider?
            .fetchMetadataFor(url: url)
          {
            await MainActor.run {
              linkMetadata[url] = metadata
            }
...
struct TextRoomTimelineView_Previews: PreviewProvider, TestablePreview {
    ...
    ScrollView {
      VStack(alignment: .leading, spacing: 20.0) {
        ...
        TextRoomTimelineView(
          timelineItem: itemWith(
            text:
              "Check out this cool website: https://www.apple.com and also https://github.com for some great projects!",
            timestamp: .mock,
            isOutgoing: true,
            senderId: "Anne"))
        ...
        TextRoomTimelineView(
          timelineItem: itemWith(
            html: "<ol><li>פריט ראשון</li><li>הפריט השני</li><li>פריט שלישי</li></ol>",
            timestamp: .mock,
            isOutgoing: true,
            senderId: "Anne"))
        ...
        TextRoomTimelineView(
          timelineItem: itemWith(
            html:
              "Check out <a href=\"https://www.apple.com\">Apple's website</a> and <a href=\"https://github.com\">GitHub</a>!",
            timestamp: .mock,
            isOutgoing: false,
            senderId: "Bob"))
      }
  ...
  private static func itemWith(html: String, timestamp: Date, isOutgoing: Bool, senderId: String)
    -> TextRoomTimelineItem
  {

...
class TimelineMediaPreviewDataSource: NSObject, QLPreviewControllerDataSource {
    ...
    init(itemViewStates: [RoomTimelineItemViewState],
        ...
        if let initialItemArrayIndex = previewItems.firstIndex(where: {
            $0.id == initialItem.id.eventOrTransactionID
        }) {
    ...
    func updatePreviewItems(itemViewStates: [RoomTimelineItemViewState]) {
        let newItems: [TimelineMediaPreviewItem.Media] = itemViewStates.compactMap { itemViewState in
            guard let newItem = TimelineMediaPreviewItem.Media(roomTimelineItemViewState: itemViewState)
            else { return nil }
    ...
    func previewController(_ controller: QLPreviewController, previewItemAt index: Int)
        ...
        if index < firstPreviewItemIndex {
            return paginationState.backward == .endReached
                ? TimelineMediaPreviewItem.Loading.timelineStart : .paginatingBackwards
        } else if index > lastPreviewItemIndex {
            return paginationState.forward == .endReached
                ? TimelineMediaPreviewItem.Loading.timelineEnd : .paginatingForwards
        } else {
...
enum TimelineMediaPreviewItem: Equatable {
    ...
    @Observable class Media: NSObject, QLPreviewItem, Identifiable {
        ...
        var id: TimelineItemIdentifier.EventOrTransactionID {
            guard let id = timelineItem.id.eventOrTransactionID else {
                fatalError("Virtual items cannot be previewed.")
            }
            return id
        }

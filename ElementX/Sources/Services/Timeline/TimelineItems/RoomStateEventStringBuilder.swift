...
struct RoomStateEventStringBuilder {
  ...
  func buildString(
    ...
    case .banned, .kickedAndBanned:
      ...
      case (false, .some(let reason)):
        L10n.stateEventRoomBanWithReason(senderDisplayName, member, reason)
      case (false, .none): L10n.stateEventRoomBan(senderDisplayName, member)
      }
    case .unbanned:
      return senderIsYou
        ? L10n.stateEventRoomUnbanByYou(member)
        : L10n.stateEventRoomUnban(senderDisplayName, member)
    case .kicked:
      return switch (senderIsYou, reason) {
      ...
      case (false, .some(let reason)):
        L10n.stateEventRoomRemoveWithReason(senderDisplayName, member, reason)
    ...
    case .invitationAccepted:
      return memberIsYou
        ? L10n.stateEventRoomInviteAcceptedByYou : L10n.stateEventRoomInviteAccepted(member)
    case .invitationRejected:
      return memberIsYou
        ? L10n.stateEventRoomRejectByYou : L10n.stateEventRoomReject(senderDisplayName)
    case .invitationRevoked:
      return senderIsYou
        ? L10n.stateEventRoomThirdPartyRevokedInviteByYou(member)
        : L10n.stateEventRoomThirdPartyRevokedInvite(senderDisplayName, member)
    case .knocked:
      return memberIsYou ? L10n.stateEventRoomKnockByYou : L10n.stateEventRoomKnock(member)
    case .knockAccepted:
      return senderIsYou
        ? L10n.stateEventRoomKnockAcceptedByYou(member)
        : L10n.stateEventRoomKnockAccepted(senderDisplayName, member)
    case .knockRetracted:
      return memberIsYou
        ? L10n.stateEventRoomKnockRetractedByYou : L10n.stateEventRoomKnockRetracted(member)
    case .knockDenied:
      if senderIsYou {
  ...
  func buildProfileChangeString(
    displayName: String?, previousDisplayName: String?,
    ...
    case (true, true, _):
      ...
      guard
        let string = buildProfileChangeString(
          displayName: displayName, previousDisplayName: previousDisplayName,
  ...
  func buildString(for state: OtherState, sender: TimelineItemSender, isOutgoing: Bool) -> String? {
    ...
    case .roomPinnedEvents(let change):
      ...
      case .added:
        return isOutgoing
          ? L10n.stateEventRoomPinnedEventsPinnedByYou
          : L10n.stateEventRoomPinnedEventsPinned(displayName)
      case .changed:
        return isOutgoing
          ? L10n.stateEventRoomPinnedEventsChangedByYou
          : L10n.stateEventRoomPinnedEventsChanged(displayName)
      case .removed:
        return isOutgoing
          ? L10n.stateEventRoomPinnedEventsUnpinnedByYou
          : L10n.stateEventRoomPinnedEventsUnpinned(displayName)
      }

...
class TimelineController: TimelineControllerProtocol {
  ...
  func focusOnEvent(_ eventID: String, timelineSize: UInt16) async -> Result<
    Void, TimelineControllerError
  > {
  ...
  func toggleReaction(
    _ reaction: String, to eventOrTransactionID: TimelineItemIdentifier.EventOrTransactionID
  ) async {
  ...
  func edit(
    _ eventOrTransactionID: TimelineItemIdentifier.EventOrTransactionID,
    message: String,
    html: String?,
    ...
    let messageContent = activeTimeline.buildMessageContentFor(
      message,
      html: html,
      intentionalMentions: intentionalMentions.toRustMentions())

    switch await activeTimeline.edit(
      eventOrTransactionID, newContent: .roomMessage(content: messageContent))
    {
    case .success:
  ...
  func editCaption(
    ...
    let newContent = createCaptionEdit(
      caption: message,
  ...
  func sendMessage(
    ...
    switch await activeTimeline.sendMessage(
      message,
      html: html,
  ...
  ) async -> Result<Void, TimelineControllerError> {
    await activeTimeline.sendAudio(
      url: url,
      audioInfo: audioInfo,
      caption: caption,
  ...
  func sendImage(
    ...
    await activeTimeline.sendImage(
      url: url,
  ...
  func sendVideo(
    ...
    await activeTimeline.sendVideo(
      url: url,
      thumbnailURL: thumbnailURL,
      videoInfo: videoInfo,
      caption: caption,
  ...
  func sendLocation(
    ...
    await activeTimeline.sendLocation(
  ...
  func sendVoiceMessage(
    ...
    await activeTimeline.sendVoiceMessage(
      url: url,
      audioInfo: audioInfo,
      waveform: waveform, requestHandle: requestHandle
    ).mapError(TimelineControllerError.timelineProxyError)
  }
  ...
  func createPoll(question: String, answers: [String], pollKind: Poll.Kind) async -> Result<
    Void, TimelineControllerError
  > {
    await activeTimeline.createPoll(question: question, answers: answers, pollKind: pollKind)
      .mapError(TimelineControllerError.timelineProxyError)
  }
  ...
  func editPoll(original eventID: String, question: String, answers: [String], pollKind: Poll.Kind)
    async -> Result<Void, TimelineControllerError>
  {
    await activeTimeline.editPoll(
      original: eventID, question: question, answers: answers, pollKind: pollKind
    ).mapError(TimelineControllerError.timelineProxyError)
  }
  ...
  func sendPollResponse(pollStartID: String, answers: [String]) async -> Result<
    Void, TimelineControllerError
  > {
    await activeTimeline.sendPollResponse(pollStartID: pollStartID, answers: answers).mapError(
      TimelineControllerError.timelineProxyError)
  }

  func endPoll(pollStartID: String, text: String) async -> Result<Void, TimelineControllerError> {
    await activeTimeline.endPoll(pollStartID: pollStartID, text: text).mapError(
      TimelineControllerError.timelineProxyError)
  }
  ...
  private func configureActiveTimelineItemProvider() {
    ...
    let contentSizeChangePublisher = NotificationCenter.default.publisher(
      for: UIContentSizeCategory.didChangeNotification)
    let timelineUpdates = activeTimelineItemProvider.updatePublisher.merge(
      with: contentSizeChangePublisher.compactMap { [weak self] _ in
  ...
  private func updateTimelineItems(
    itemProxies: [TimelineItemProxy], paginationState: TimelinePaginationState
  ) async {
    ...
    case .paginating:
      newTimelineItems.insert(
        PaginationIndicatorRoomTimelineItem(position: .end), at: newTimelineItems.count)
    case .idle, .endReached:
      break
    }
    ...
    callbacks.send(
      .updatedTimelineItems(timelineItems: newTimelineItems, isSwitchingTimelines: isNewTimeline))
    self.paginationState = paginationState
  }

  private nonisolated func buildTimelineItem(
    for itemProxy: TimelineItemProxy,
    isDM: Bool,
    hasPredecessor: Bool,
  ...
  private nonisolated func fetchEventDetails(
    for timelineItem: EventBasedMessageTimelineItemProtocol,
    refetchOnError: Bool,
    activeTimeline: TimelineProxyProtocol
  ) {
  ...
  private func donateSendMessageIntent() async {
    guard let displayName = roomProxy.details.name ?? roomProxy.details.canonicalAlias,
      !displayName.isEmpty
    else {
    ...
    let avatarURL: URL? =
      switch roomProxy.details.avatar {
    ...
    func addPlacehoder() {
      if let imageData = Avatars.generatePlaceholderAvatarImageData(
        name: displayName, id: roomProxy.id, size: .init(width: 100, height: 100))
      {
        sendMessageIntent.setImage(
          INImage(imageData: imageData), forParameterNamed: \.speakableGroupName)
      }
    }

    if let avatarURL, let mediaSource = try? MediaSourceProxy(url: avatarURL, mimeType: nil) {
      if case .success(let avatarData) = await mediaProvider.loadThumbnailForSource(
        source: mediaSource, size: .init(width: 100, height: 100))
      {
        sendMessageIntent.setImage(
          INImage(imageData: avatarData), forParameterNamed: \.speakableGroupName)
      } else {
        addPlacehoder()
      }
    } else {
...
extension TimelineItemProxy {
  fileprivate var isItemCollapsible: Bool {
    if case .event(let eventItem) = self {

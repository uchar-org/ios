...
struct NotificationPermissionsScreen: View {
  ...
  var body: some View {
    FullscreenDialog(
      topPadding: UIConstants.startScreenBreakerScreenTopPadding, background: .gradient
    ) {
      mainContent
    } bottomContent: {
      buttons
  ...
  private var buttons: some View {
    VStack(spacing: 16) {
      ...
      Button {
        context.send(viewAction: .notNow)
      } label: {
...
struct NotificationPermissionsScreen_Previews: PreviewProvider, TestablePreview {
  static let viewModel = NotificationPermissionsScreenViewModel(

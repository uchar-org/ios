    ...
    if items.isEmpty {
      ...
      ZStack {
        MentionSuggestionItemView(
          mediaProvider: nil,
          item: .init(
            suggestionType: .user(.init(id: "", displayName: nil, avatarURL: nil)), range: .init(),
            rawSuggestionText: "")
        )
...
private struct BackgroundView<Content: View>: View {
  ...
  var body: some View {
    content()
      ...
      .mask(
        Rectangle().padding(
          .init(top: -(shadowRadius * 2), leading: 0.0, bottom: 0.0, trailing: 0.0)))
  }
}
...
struct CompletionSuggestion_Previews: PreviewProvider, TestablePreview {
  static let multipleItems: [SuggestionItem] = (0...10).map { index in
    .init(
      suggestionType: .user(.init(id: "\(index)", displayName: "\(index)", avatarURL: nil)),

...
class RoomRolesAndPermissionsFlowCoordinator: FlowCoordinatorProtocol {
  ...
  private func configureStateMachine() {
    ...
    stateMachine.addRoutes(
      event: .changeRoles, transitions: [.rolesAndPermissionsScreen => .changingRoles]
    ...
    stateMachine.addRoutes(
      event: .finishedChangingRoles, transitions: [.changingRoles => .rolesAndPermissionsScreen])

    stateMachine.addRoutes(
      event: .changePermissions, transitions: [.rolesAndPermissionsScreen => .changingPermissions]
    ) { [weak self] context in
      guard
    ...
    stateMachine.addRoutes(
      event: .finishedChangingPermissions,
      transitions: [.changingPermissions => .rolesAndPermissionsScreen])

    stateMachine.addRoutes(
      event: .demotedOwnUser, transitions: [.rolesAndPermissionsScreen => .complete]
    ) { [weak self] _ in
  ...
  private func presentRolesAndPermissionsScreen() {
    let parameters = RoomRolesAndPermissionsScreenCoordinatorParameters(
      roomProxy: roomProxy,
  ...
  private func presentChangePermissionsScreen(
    ownPowerLevel: RoomPowerLevel, permissions: RoomPermissions
  ) {
    let parameters = RoomChangePermissionsScreenCoordinatorParameters(

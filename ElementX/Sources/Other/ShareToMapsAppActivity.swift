...
final class ShareToMapsAppActivity: UIActivity {
  ...
  override func prepare(withActivityItems activityItems: [Any]) {
    UIApplication.shared.open(type.activityURL(for: location, senderName: senderName), options: [:])
    { [weak self] result in
...
extension UIActivity.ActivityType {
  fileprivate static let shareToMapsApp = UIActivity.ActivityType("ElementX.ShareToMapsApp")
}

...
struct MediaUploadPreviewScreen: View {
    ...
    mainContent
      ...
      .safeAreaInset(edge: .top) {
        if context.viewState.mediaURLs.count > 1 {
          Text(
            L10n.screenMediaUploadPreviewItemCount(
              currentIndex + 1, context.viewState.mediaURLs.count)
          )
  ...
  private var mainContent: some View {
    if ProcessInfo.processInfo.isiOSAppOnMac {
      ...
      PreviewView(
        mediaURLs: context.viewState.mediaURLs,
    ...
    HStack(spacing: 12) {
      HStack(spacing: 6) {
        MessageComposerTextField(
          placeholder: L10n.richTextEditorComposerCaptionPlaceholder,
...
struct MediaUploadPreviewScreen_Previews: PreviewProvider, TestablePreview {
  ...
  static let viewModel = MediaUploadPreviewScreenViewModel(
    mediaURLs: [snapshotURL],
    title: "App Icon.png",

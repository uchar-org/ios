...
struct RoomMembersListScreenViewModelTests {
  ...
  mutating func sortingMembers() async throws {
    ...
    let sortedMembers: [RoomMemberListScreenEntry] = [
      .init(
        member: .init(withProxy: RoomMemberProxyMock.mockAdmin),
        verificationState: .notVerified),
      .init(
        member: .init(withProxy: RoomMemberProxyMock.mockModerator),
        verificationState: .notVerified),
      .init(
        member: .init(withProxy: RoomMemberProxyMock.mockAlice),
        verificationState: .notVerified),
      .init(
        member: .init(withProxy: RoomMemberProxyMock.mockDan),
        verificationState: .notVerified),
    ]
  ...
  mutating func selectUserAsUser() async throws {
    ...
    guard
      let user = viewModel.state.visibleJoinedMembers.first(where: {
        $0.member.role == .user && $0.member.id != RoomMemberProxyMock.mockMe.userID
      })?.member
    else {
  ...
  mutating func selectUserAsAdmin() async throws {
    ...
    var deferred = deferFulfillment(context.$viewState) {
      !$0.visibleInvitedMembers.isEmpty && $0.canKickUsers && $0.canBanUsers
    }
    try await deferred.fulfill()
  ...
  mutating func selectModeratorAsAdmin() async throws {
    ...
    deferred = deferFulfillment(context.$viewState) { $0.bindings.manageMemeberViewModel != nil }
    guard
      let moderator = viewModel.state.visibleJoinedMembers.first(where: {
        $0.member.role == .moderator
      })?.member
    else {
  ...
  mutating func selectAdminAsAdmin() async throws {
    ...
    deferred = deferFulfillment(context.$viewState) { $0.bindings.manageMemeberViewModel != nil }
    guard
      let admin = viewModel.state.visibleJoinedMembers.first(where: {
        $0.member.role.isAdminOrHigher && $0.member.id != RoomMemberProxyMock.mockMe.userID
      })?.member
    else {
  ...
  mutating func selectOwnMemberAsAdmin() async throws {
    ...
    guard
      let ownMember = viewModel.state.visibleJoinedMembers.first(where: {
        $0.member.id == RoomMemberProxyMock.mockMe.userID
      })?.member
    else {
  ...
  mutating func switchesToMembersModeWhenThereAreNoBannedMembers() async throws {
    ...
    let subject = CurrentValueSubject<[RoomMemberProxyProtocol], Never>(
      [RoomMemberProxyMock].allMembersAsAdmin + RoomMemberProxyMock.mockBanned)
    roomProxy.membersPublisher = subject.asCurrentValuePublisher()

    viewModel = RoomMembersListScreenViewModel(
    ...
    var deferred = deferFulfillment(context.$viewState) {
      $0.visibleBannedMembers.count == 4 && $0.bindings.mode == .banned
    }
    context.mode = .banned
    try await deferred.fulfill()

    deferred = deferFulfillment(context.$viewState) {
      $0.visibleBannedMembers.count == 0 && $0.bindings.mode == .members
    }

...
final class JoinRoomScreenViewModelTests {
  ...
  func interaction() async throws {
    #expect(
      appSettings.seenInvites.isEmpty,
      "There shouldn't be any seen invites before running the tests.")
    ...
    #expect(
      appSettings.seenInvites.isEmpty,
      "Only an invited room should register the room ID as a seen invite.")
  ...
  func acceptInviteInteraction() async throws {
    ...
    try await deferFulfillment(viewModel.context.$viewState) { $0.mode == .invited(isDM: false) }
      .fulfill()

    #expect(
      appSettings.seenInvites == ["1"],
      "The invited room's ID should be registered as a seen invite.")
    ...
    #expect(
      appSettings.seenInvites.isEmpty,
      "The after accepting an invite the invite should be forgotten in case the user leaves.")
  }
  ...
  func declineInviteInteraction() async throws {
    ...
    #expect(
      appSettings.seenInvites.isEmpty,
      "The after declining an invite the invite should be forgotten in case another invite is received."
    )
  }
  ...
  func declineAndBlockInviteLegacyInteraction() async throws {
    ...
    try await deferFulfillment(viewModel.context.$viewState) { $0.bindings.alertInfo != nil }
      .fulfill()
  ...
  func declineAndBlockInviteInteraction() async throws {
    setupViewModel(mode: .invited)
    try await deferFulfillment(viewModel.context.$viewState) { $0.roomDetails != nil }.fulfill()
    let deferredAction = deferFulfillment(viewModel.actionsPublisher) {
      $0 == .presentDeclineAndBlock(userID: "@test:matrix.org")
    }
    context.send(viewAction: .declineInviteAndBlock(userID: "@test:matrix.org"))
  ...
  private func setupViewModel(throwing: Bool = false, mode: TestMode = .joined) {
    ...
    clientProxy.joinRoomViaReturnValue =
      throwing ? .failure(.sdkError(ClientProxyMockError.generic)) : .success(())
    ...
    viewModel = JoinRoomScreenViewModel(
      source: .generic(roomID: "1", via: []),
      appSettings: appSettings,
...
extension JoinRoomScreenViewModelAction: @retroactive Equatable {
  ...
  public static func == (lhs: JoinRoomScreenViewModelAction, rhs: JoinRoomScreenViewModelAction)
    -> Bool
  {
    switch (lhs, rhs) {

...
struct PreviewTests: AsyncParsableCommand {
  ...
  private static let simulatorType =
  ...
  func run() async throws {
    ...
    await CI.collectCoverage(
      resultBundle: "\(Self.scheme).xcresult", outputName: "preview-cobertura.xml")
    await CI.collectTestResults(
      resultBundle: "\(Self.scheme).xcresult", outputName: "preview-junit.xml")
  ...
  private func setRecordFailures(enabled: Bool) throws {
    ...
    guard var plan = try JSONSerialization.jsonObject(with: data) as? [String: Any],
      ...
      var envVars = defaultOptions["environmentVariableEntries"] as? [[String: Any]]
    else {

...
class NotificationServiceExtension: UNNotificationServiceExtension {
  static let receivedWhileOfflineNotificationID =
  ...
  override init() {
    ...
    if !BootDetectionManager.isDeviceLockedAfterReboot(
      containerURL: URL.appGroupContainerDirectory),
      Self.targetConfiguration == nil
    {
      Self.targetConfiguration = Target.nse.configure(
  ...
  private func handle(
    ...
    do {
      let userSession = try await NSEUserSession(
  ...
  private func shouldDeliverReceivedWhileOfflineNotification() -> Bool {
    ...
    if Date.now.timeIntervalSince(Date(timeIntervalSince1970: currentBootTime))
      > Self.firstNotificationThreshold
    {
      return false
    } else {
      return true
  ...
  private func deliverReceivedWhileOfflineNotification(for originalRequest: UNNotificationRequest) {
    ...
    let request = UNNotificationRequest(
      identifier: Self.receivedWhileOfflineNotificationID, content: content, trigger: nil)

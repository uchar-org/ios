...
typealias RoomMembersListScreenViewModelType = StateStoreViewModel<
  RoomMembersListScreenViewState, RoomMembersListScreenViewAction
>
  ...
  RoomMembersListScreenViewModelProtocol
{
    ...
    await Task.detached { [userSession, roomProxy] in
      ...
      for member in members {
        ...
        if roomProxy.infoPublisher.value.isEncrypted,  // We don't care about identity statuses on non-encrypted rooms
          case .success(let userIdentity) = await userSession.clientProxy.userIdentity(
        ...
        case .invite:
          invitedMembers.append(
            .init(member: .init(withProxy: member), verificationState: verificationState))
        case .join:
          joinedMembers.append(
            .init(member: .init(withProxy: member), verificationState: verificationState))
        case .ban:
          bannedMembers.append(
            .init(member: .init(withProxy: member), verificationState: verificationState))
        default:
          continue
        }
  ...
  private func selectMember(_ member: RoomMemberDetails) {
    ...
    let manageMemberViewModel = ManageRoomMemberSheetViewModel(

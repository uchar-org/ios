...
class ElementCallService: NSObject, ElementCallServiceProtocol, PKPushRegistryDelegate,
  CXProviderDelegate
{
  ...
  func setAudioEnabled(_ enabled: Bool, roomID: String) {
    ...
    guard ongoingCallID.roomID == roomID else {
      MXLog.error(
        "Failed toggling call microphone, rooms don't match: \(ongoingCallID.roomID) != \(roomID)")
      return
    }

    let transaction = CXTransaction(
      action: CXSetMutedCallAction(call: ongoingCallID.callKitID, muted: !enabled))
  ...
  func pushRegistry(
    ...
    guard
      let roomID = payload.dictionaryPayload[ElementCallServiceNotificationKey.roomID.rawValue]
        as? String
    else {
      MXLog.error(
        "Something went wrong, missing room identifier for incoming voip call: \(payload)")
      completion()
      return
    }

    guard
      let rtcNotificationID = payload.dictionaryPayload[
        ElementCallServiceNotificationKey.rtcNotifyEventID.rawValue] as? String
    else {
      MXLog.error(
        "Something went wrong, missing rtc notification event identifier for incoming voip call: \(payload)"
      )
      completion()
      return
    }
    ...
    let isVoiceCall =
      payload.dictionaryPayload[ElementCallServiceNotificationKey.isVoiceCall.rawValue] as? Bool
      ?? false

    let callID = CallID(
      callKitID: UUID(), roomID: roomID, rtcNotificationID: rtcNotificationID,
      isVoiceCall: isVoiceCall)
    incomingCallID = callID

    guard
      let expirationDate =
        (payload.dictionaryPayload[ElementCallServiceNotificationKey.expirationDate.rawValue]
          as? Date)
    else {
      MXLog.error(
        "Something went wrong, missing expiration timestamp for incoming voip call: \(payload)")
      completion()
      return
    }
    ...
    let ringDuration: Duration = .seconds(
      min(expirationDate.timeIntervalSince1970 - nowDate.timeIntervalSince1970, 90))

    let roomDisplayName =
      payload.dictionaryPayload[ElementCallServiceNotificationKey.roomDisplayName.rawValue]
      as? String
    ...
    callProvider.reportNewIncomingCall(with: callID.callKitID, update: update) {
      [weak self] error in
      if let error {
  ...
  func provider(_ provider: CXProvider, perform action: CXAnswerCallAction) {
    ...
    DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
      ...
      self.actionsSubject.send(
        .startCall(roomID: incomingCallID.roomID, isVoiceCall: incomingCallID.isVoiceCall))
  ...
  private func sendDeclineCallEvent(_ incomingCallID: CallID) async {
    ...
    guard case .joined(let roomProxy) = await clientProxy.roomForIdentifier(incomingCallID.roomID)
    else {
  ...
  private func observeIncomingCall() async {
    ...
    guard
      case .success(let handle) = roomProxy.subscribeToCallDeclineEvents(
        rtcNotificationEventID: rtcNotificationID, listener: listener)
    else {

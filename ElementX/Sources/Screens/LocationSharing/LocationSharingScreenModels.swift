...
struct LocationSharingScreenViewState: BindableState {
  init(
    ...
    let initialProfile: UserProfileProxy =
    ...
    if case .viewLive(_, let initialLiveLocationShare) = interactionMode,
      let initialLiveLocationShare
    {
      liveLocationShares = [initialLiveLocationShare]
    }

    bindings.showsUserLocationMode =
      switch interactionMode {
  ...
  var annotations: [LocationAnnotation] {
    ...
    case .viewStatic(let location):
      ...
      let kind: LocationMarkerKind =
        if location.kind == .sender, let profile {
          .staticUser(profile)
        } else {
          .pin
        }
      let coordinate = CLLocationCoordinate2D(
        latitude: location.geoURI.latitude, longitude: location.geoURI.longitude)
    ...
    case .viewLive:
      return liveLocationShares.compactMap { share in
        ...
        let coordinate = CLLocationCoordinate2D(
          latitude: geoURI.latitude, longitude: geoURI.longitude)
  ...
  var initialMapCenter: CLLocationCoordinate2D {
    ...
    case .viewLive(_, let initialLiveLocationShare):
      if let initialLiveLocationShare {
        .init(
          latitude: initialLiveLocationShare.geoURI?.latitude ?? 0,
...
struct LocationSharingScreenBindings {
  ...
  var mapError: MapLibreError? {
    get {
      if case .mapError(let error) = alertInfo?.id {
        return error
      }
      return nil
    }
...
extension AlertInfo where T == LocationSharingViewAlert {
  init(
    alertID: LocationSharingViewAlert,
    ...
    case .missingAlwaysAuthorization:
      self.init(
        id: alertID,
        title: L10n.dialogAllowAccess,
        message: L10n.dialogPermissionLiveLocationDescriptionIos(
          InfoPlistReader.main.bundleDisplayName),
        primaryButton: primaryButton,
        secondaryButton: secondaryButton)
    case .liveLocationDisclaimer:

    ...
    HStack(spacing: 12) {
      ...
      HStack(spacing: 16) {
        VStack(alignment: .leading, spacing: 0) {
          ...
          HStack(spacing: 4) {
            if case .static(let isUserLocation, let timestamp) = kind {
              CompoundIcon(
                isUserLocation ? \.locationNavigatorCentred : \.locationNavigator,
                size: .xSmall,
                relativeTo: .compound.bodyMD
              )
              .foregroundStyle(.compound.iconSecondary)
              .accessibilityLabel(
                isUserLocation ? L10n.a11ySenderLocation : L10n.a11yPinnedLocation)
              Text(
                L10n.screenStaticLocationSheetTimestampDescription(

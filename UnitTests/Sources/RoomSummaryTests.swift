...
struct RoomSummaryTests {
  ...
  let roomDetails: (id: String, name: String, avatarURL: URL) = (
    "room_id", "Room Name", "mxc://hs.tld/room/avatar"
  )
  let heroes = [
    UserProfileProxy(userID: "hero_1", displayName: "Hero 1", avatarURL: "mxc://hs.tld/user/avatar")
  ]

  @Test
  func roomAvatar() {
    let details = makeSummary(
      isDirect: false, isSpace: false, hasRoomAvatar: true, isTombstoned: false)
  ...
  func dmAvatarSet() {
    let details = makeSummary(
      isDirect: true, isSpace: false, hasRoomAvatar: true, isTombstoned: false)
  ...
  func dmAvatarNotSet() {
    let details = makeSummary(
      isDirect: true, isSpace: false, hasRoomAvatar: false, isTombstoned: false)

    switch details.avatar {
    case .room:
      Issue.record(
        "A DM without an avatar should defer to the hero for the correct placeholder tint colour.")
  ...
  func spaceAvatar() {
    let details = makeSummary(
      isDirect: false, isSpace: true, hasRoomAvatar: true, isTombstoned: false)
  ...
  func tombstonedAvatar() {
    let details = makeSummary(
      isDirect: false, isSpace: false, hasRoomAvatar: true, isTombstoned: true)
  ...
  func makeSummary(isDirect: Bool, isSpace: Bool, hasRoomAvatar: Bool, isTombstoned: Bool)
    -> RoomSummary
  {
    RoomSummary(

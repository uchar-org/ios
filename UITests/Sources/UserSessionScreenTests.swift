...
class UserSessionScreenTests: XCTestCase {
  ...
  func testSpaceExploration() async throws {
    ...
    XCTAssertTrue(
      app.buttons[A11yIdentifiers.createRoomScreen.cancel].waitForExistence(timeout: 5.0))
    try await Task.sleep(for: .seconds(1))
    ...
    XCTAssert(
      app.buttons[A11yIdentifiers.spaceAddRoomsScreen.cancel].waitForExistence(timeout: 5.0))
    try await Task.sleep(for: .seconds(1))
    ...
    XCTAssert(
      app.buttons[A11yIdentifiers.roomMembersListScreen.invite].waitForExistence(timeout: 5.0))
    try await Task.sleep(for: .seconds(1))
    ...
    XCTAssert(
      app.buttons[A11yIdentifiers.spaceSettingsScreen.editBaseInfo].waitForExistence(timeout: 5.0))
    try await Task.sleep(for: .seconds(1))
  ...
  private func tapOnBackButton(_ label: String = "Back", _ app: XCUIApplication) {
    let button = app.buttons.matching(
      NSPredicate(format: "label == %@ && identifier == 'BackButton'", label)
    ).firstMatch

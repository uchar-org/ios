...
struct VoiceMessageRecorderTests {
  ...
  init() async throws {
    ...
    voiceMessageCache.urlForRecording = FileManager.default.temporaryDirectory
      .appendingPathComponent("test-voice-message").appendingPathExtension("m4a")
  ...
  func startPlayback() async throws {
    ...
    #expect(
      audioPlayer.loadSourceURLPlaybackURLAutoplayReceivedArguments?.sourceURL == recordingURL)
    #expect(
      audioPlayer.loadSourceURLPlaybackURLAutoplayReceivedArguments?.playbackURL == recordingURL)
  ...
  func buildRecordedWaveform() async throws {
    ...
    guard case .failure(.missingRecordingFile) = await voiceMessageRecorder.buildRecordingWaveform()
    else {
      Issue.record("An error is expected")
      return
    }

    let audioFileURL = try #require(
      Bundle(for: UnitTestsAppCoordinator.self).url(
        forResource: "test_audio", withExtension: "mp3"), "Test audio file is missing")
  ...
  func sendVoiceMessage_NoRecordingFile() async {
    ...
    audioRecorder.audioFileURL = nil
    guard
      case .failure(.missingRecordingFile) = await voiceMessageRecorder.sendVoiceMessage(
  ...
  func sendVoiceMessage_ConversionError() async {
    ...
    audioConverter.convertToOpusOggSourceURLDestinationURLThrowableError =
      AudioConverterError.conversionFailed(nil)

    let timelineController = MockTimelineController()
    guard
      case .failure(.failedSendingVoiceMessage) = await voiceMessageRecorder.sendVoiceMessage(
        timelineController: timelineController,
        audioConverter: audioConverter)
    else {
      Issue.record("An error is expected")
  ...
  func sendVoiceMessage_InvalidFile() async throws {
    let audioFileURL = try #require(
      Bundle(for: UnitTestsAppCoordinator.self).url(
        forResource: "test_voice_message", withExtension: "m4a"), "Test audio file is missing")
    audioRecorder.audioFileURL = audioFileURL
    ...
    timelineProxy.sendVoiceMessageUrlAudioInfoWaveformRequestHandleReturnValue = .failure(
  ...
  func sendVoiceMessage_WaveformAnlyseFailed() async throws {
    let imageFileURL = try #require(
      Bundle(for: UnitTestsAppCoordinator.self).url(
        forResource: "test_image", withExtension: "png"), "Test image file is missing")
  ...
  func sendVoiceMessage() async throws {
    ...
    timelineProxy.sendVoiceMessageUrlAudioInfoWaveformRequestHandleClosure = {
      url, audioInfo, waveform, _ in
    ...
    guard
      case .success = await voiceMessageRecorder.sendVoiceMessage(

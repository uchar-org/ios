...
final class RoomSummaryProviderTests {
  private let baseFilters: [RoomListEntriesDynamicFilterKind] = [
    .any(filters: [
  ...
  func defaultRustFilters() async {
    ...
    #expect(
      dynamicEntriesController.setFilterKindReceivedInvocations.last == .all(filters: baseFilters))
    ...
    #expect(
      dynamicEntriesController.setFilterKindReceivedInvocations.last
  ...
  func lowPriorityRustFilters() async {
    ...
    #expect(
      dynamicEntriesController.setFilterKindReceivedInvocations.last
        == .all(
          filters: [.all(filters: [.category(expect: .group), .joined])] + baseFilters + [
            .nonLowPriority
          ]))
  }
  ...
  func roomIdentifierFilters() async {
    ...
    #expect(
      dynamicEntriesController.setFilterKindReceivedInvocations.last
        == .all(
          filters: [.all(filters: [.favourite, .joined])] + baseFilters + [
            .identifiers(identifiers: ["SomeRoom"])
          ]))
  }
  ...
  private func setup(isLowPriorityFilterEnabled: Bool = false) {
    ...
    roomSummaryProvider = RoomSummaryProvider(
      roomListService: RoomListServiceSDKMock(),
    ...
    roomList.loadingStateListenerReturnValue = .some(
      .init(state: .notLoaded, stateStream: .init(noHandle: .init())))

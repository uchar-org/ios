...
final class AppLockScreenViewModelTests {
  ...
  func forgotPIN() async throws {
    ...
    #expect(
      context.alertInfo?.id == .confirmResetPIN, "An alert should be shown before logging out.")
  ...
  func unlockFailure() async throws {
    ...
    #expect(
      context.viewState.numberOfPINAttempts == 1, "A failed attempt should have been recorded.")
    ...
    #expect(
      context.viewState.numberOfPINAttempts == 3, "All the attempts should have been recorded.")
  ...
  func forceQuitRequiresLogout() async throws {
    ...
    try await deferred.fulfill()
    #expect(
      appSettings.appLockNumberOfPINAttempts == 3,
      "The app should have 3 failed attempts before the force quit.")
    #expect(
      context.alertInfo?.id == .forcedLogout,
      "The app should be showing the alert before the force quit.")
    ...
    #expect(
      freshViewModel.context.alertInfo?.id == .forcedLogout,
      "The new view model from the fresh launch should also show the alert")
  }
}

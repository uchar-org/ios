...
struct RoomDetailsScreen: View {
  ...
  var body: some View {
    ...
    .alert(
      item: $context.ignoreUserRoomAlertItem,
  ...
  private var roomHeaderSection: some View {
    AvatarHeaderView(
      room: context.viewState.details,
  ...
  private var topicSection: some View {
    if context.viewState.hasTopicSection {
      Section {
        if let topic = context.viewState.topic, !topic.characters.isEmpty,
          let topicSummary = context.viewState.topicSummary
        {
          ListRow(
            kind: .custom {
          ...
          ListRow(
            label: .plain(title: L10n.screenRoomDetailsAddTopicTitle),
  ...
  private var aboutSection: some View {
    Section {
      ListRow(
        label: .default(title: L10n.screenMediaBrowserTitle, icon: \.image),
        kind: .navigationLink {
          context.send(viewAction: .processTapMediaEvents)
        })

      ListRow(
        label: .default(title: L10n.screenRoomDetailsPinnedEventsRowTitle, icon: \.pin),
        details: context.viewState.pinnedEventsActionState.isLoading
          ? .isWaiting(true) : .title(context.viewState.pinnedEventsActionState.count),
        kind: context.viewState.pinnedEventsActionState.isLoading
          ? .label
          : .navigationLink {
      ...
      ListRow(
        label: .default(title: L10n.screenPollsHistoryTitle, icon: \.polls),
        kind: .navigationLink {
  ...
  private var configurationSection: some View {
    Section {
      ListRow(
        label: .default(title: L10n.screenRoomDetailsNotificationTitle, icon: \.notifications),
        details: context.viewState.notificationSettingsState.isLoading
          ? .isWaiting(true)
          : context.viewState.notificationSettingsState.isError
            ? .systemIcon(.exclamationmarkCircle)
      ...
      ListRow(
        label: .default(title: L10n.commonFavourite, icon: \.favourite),
      ...
      if context.viewState.canSeeSecurityAndPrivacy {
        ListRow(
          label: .default(title: L10n.screenRoomDetailsSecurityAndPrivacyTitle, icon: \.lock),
          kind: .navigationLink {
      ...
      if context.viewState.dmRecipientInfo != nil {
        ...
        case .verified:
          ListRow(
            label: .default(title: L10n.screenRoomDetailsProfileRowTitle, icon: \.userProfile),
        ...
        case .verificationViolation:
          ListRow(
            label: .default(title: L10n.screenRoomDetailsProfileRowTitle, icon: \.userProfile),
            details: .icon(
              CompoundIcon(\.infoSolid).foregroundStyle(.compound.iconCriticalPrimary)),
            kind: .navigationLink {
              context.send(viewAction: .processTapRecipientProfile)
            })
        default:
  ...
  private var peopleSection: some View {
    Section {
      if context.viewState.hasMemberIdentityVerificationStateViolations {
        ListRow(
          label: .default(title: L10n.commonPeople, icon: \.user),
      ...
      if context.viewState.canSeeKnockingRequests {
        ListRow(
          label: .default(title: L10n.screenRoomDetailsRequestsToJoinTitle, icon: \.askToJoin),
          details: context.viewState.knockRequestsCount > 0
            ? .counter(context.viewState.knockRequestsCount) : nil,
          kind: .navigationLink {
      ...
      if context.viewState.canEditRolesOrPermissions, context.viewState.dmRecipientInfo == nil {
        ListRow(
          label: .default(title: L10n.screenRoomDetailsRolesAndPermissions, icon: \.admin),
          kind: .navigationLink {
  ...
  private var securitySection: some View {
    if context.viewState.isEncrypted {
      Section {
        ListRow(
          label: .default(
            title: L10n.screenRoomDetailsEncryptionEnabledTitle,
  ...
  private var leaveRoomSection: some View {
    Section {
      ...
      ListRow(
        label: .action(
          title: L10n.screenRoomDetailsLeaveRoomTitle,
          icon: \.leave,
          role: .destructive),
  ...
  private func ignoreUserSection(user: RoomMemberDetails) -> some View {
    Section {
      ListRow(
        label: .default(
          title: user.isIgnored ? L10n.screenDmDetailsUnblockUser : L10n.screenDmDetailsBlockUser,
          icon: \.block,
  ...
  @ViewBuilder
  private func blockUserAlertActions(_ item: RoomDetailsScreenViewStateBindings.IgnoreUserAlertItem)
    -> some View
  {
    Button(item.cancelTitle, role: .cancel) {}
    Button(
  ...
  private func blockUserAlertMessage(_ item: RoomDetailsScreenViewStateBindings.IgnoreUserAlertItem)
    -> some View
  {
...
struct RoomDetailsScreen_Previews: PreviewProvider, TestablePreview {
  static let genericWorldReadableRoomViewModel = makeGenericRoomViewModel(
  ...
  static let dmRoomVerificationViolationViewModel = makeDMViewModel(
    verificationState: .verificationViolation)

  static var previews: some View {
    RoomDetailsScreen(context: genericJoinedRoomViewModel.context)
      .snapshotPreferences(
        expect: genericJoinedRoomViewModel.context.observe(\.viewState.permalink).map { $0 != nil }
      )
    ...
    RoomDetailsScreen(context: genericWorldReadableRoomViewModel.context)
      .snapshotPreferences(
        expect: genericWorldReadableRoomViewModel.context.observe(\.viewState.permalink).map {
          $0 != nil
        }
      )
    ...
    RoomDetailsScreen(context: simpleRoomViewModel.context)
      .snapshotPreferences(
        expect: simpleRoomViewModel.context.observe(\.viewState.permalink).map { $0 != nil }
      )
      .previewDisplayName("Simple Room")

    RoomDetailsScreen(context: dmRoomViewModel.context)
      .snapshotPreferences(
        expect: dmRoomViewModel.context.observe(\.viewState.accountOwner).map { $0 != nil }
      )
      .previewDisplayName("DM Room")

    RoomDetailsScreen(context: dmRoomVerifiedViewModel.context)
      .snapshotPreferences(
        expect: dmRoomVerifiedViewModel.context.observe(
          \.viewState.dmRecipientInfo?.verificationState
        ).map { $0 == .verified }
      )
    ...
    RoomDetailsScreen(context: dmRoomVerificationViolationViewModel.context)
      .snapshotPreferences(
        expect: dmRoomVerificationViolationViewModel.context.observe(\.viewState.accountOwner).map {
          $0 != nil
        }
      )
  ...
  private static func makeGenericRoomViewModel(historyVisibility: RoomHistoryVisibility)
  ...
  private static func makeDMViewModel(verificationState: UserIdentityVerificationState)
    -> RoomDetailsScreenViewModel
  {
    let members: [RoomMemberProxyMock] = [
      .mockMe,

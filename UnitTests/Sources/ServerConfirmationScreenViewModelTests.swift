...
final class ServerConfirmationScreenViewModelTests {
    ...
    func confirmLoginWithoutConfiguration() async throws {
        ...
        #expect(client.urlForOauthOauthConfigurationPromptLoginHintDeviceIdAdditionalScopesReceivedArguments?
            .prompt == .consent)
    ...
    func confirmRegisterAfterConfiguration() async throws {
        ...
        setupViewModel(authenticationFlow: .register)
        guard
            case .success = await service.configure(for: viewModel.state.homeserverAddress, flow: .register)
        else {
            Issue.record("The configuration should succeed.")
            return
        }
    ...
    func loginNotSupportedAlert() async throws {
        ...
        setupViewModel(authenticationFlow: .login, supportsOIDC: false, supportsOIDCCreatePrompt: false,

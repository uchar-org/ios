...
struct RoomMembersListScreen: View {
  ...
  @ViewBuilder
  private func membersSection(entries: [RoomMemberListScreenEntry], section: MembersSection)
    -> some View
  {
    if !entries.isEmpty {
      Section {
        LazyVStack(alignment: .leading, spacing: 0) {
          ForEach(entries, id: \.member.id) { entry in
            ListRow(
              kind: .custom {
                RoomMembersListScreenMemberCell(
                  listEntry: entry, isLast: entries.last == entry, context: context)
              })
          }
        }
...
struct RoomMembersListScreen_Previews: PreviewProvider, TestablePreview {
  ...
  static let emptyBannedViewModel = makeViewModel(
    withBanned: false, isAdmin: false, initialMode: .members)
  ...
  static var previews: some View {
    ...
    .snapshotPreferences(
      expect: invitesViewModel.context.$viewState.map { state in
        !state.visibleJoinedMembers.isEmpty
      }
    )
    ...
    .snapshotPreferences(
      expect: adminViewModel.context.$viewState.map { state in
    ...
    .snapshotPreferences(
      expect: bannedViewModel.context.$viewState.map { state in
        state.canBanUsers == true
      }
    )
    ...
    .snapshotPreferences(
      expect: emptyBannedViewModel.context.$viewState.map(\.shouldShowEmptyState)
    )
  ...
  static func makeViewModel(
    ...
    return RoomMembersListScreenViewModel(

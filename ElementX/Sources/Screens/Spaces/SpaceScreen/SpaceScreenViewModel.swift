...
class SpaceScreenViewModel: SpaceScreenViewModelType, SpaceScreenViewModelProtocol {
    ...
    init(spaceRoomListProxy: SpaceRoomListProxyProtocol,
        ...
        Task {
            if case .joined(let roomProxy) = await userSession.clientProxy.roomForIdentifier(spaceRoomListProxy.id) {
    ...
    private func showLeaveSpaceConfirmation() async {
        guard
            case .success(let leaveHandle) = await spaceServiceProxy.leaveSpace(spaceID: spaceRoomListProxy.id)
        else {
            showFailureIndicator()
            return
        }

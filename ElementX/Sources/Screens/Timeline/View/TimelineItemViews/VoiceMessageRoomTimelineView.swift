...
struct VoiceMessageRoomTimelineView: View {
  ...
  var body: some View {
    TimelineStyler(timelineItem: timelineItem) {
      VoiceMessageRoomTimelineContent(
...
struct VoiceMessageRoomTimelineContent: View {
  ...
  private func onPlaybackSpeedChange() {
    context?.send(
      viewAction: .handleAudioPlayerAction(.changePlaybackSpeed(itemID: timelineItem.id)))
  }

  private func onPlaybackSeek(_ progress: Double) {
    context?.send(
      viewAction: .handleAudioPlayerAction(.seek(itemID: timelineItem.id, progress: progress)))
  }
...
struct VoiceMessageRoomTimelineView_Previews: PreviewProvider, TestablePreview {
  ...
  static let voiceRoomTimelineItem = VoiceMessageRoomTimelineItem(
    id: timelineItemIdentifier,
    timestamp: .mock,
    isOutgoing: false,
    isEditable: false,
    canBeRepliedTo: true,
  ...
  static let playerState = AudioPlayerState(
    id: .timelineItemIdentifier(timelineItemIdentifier),

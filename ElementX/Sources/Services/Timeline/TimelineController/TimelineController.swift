...
class TimelineController: TimelineControllerProtocol {
    ...
    func sendLocation(body: String,
        ...
        await activeTimeline.sendLocation(body: body, geoURI: geoURI, description: description, zoomLevel: zoomLevel,
    ...
    private func donateSendMessageIntent() async {
        ...
        if let avatarURL, let mediaSource = try? MediaSourceProxy(url: avatarURL, mimeType: nil) {
            if case .success(let avatarData) = await mediaProvider.loadThumbnailForSource(source: mediaSource, size: .init(width: 100, height: 100)) {

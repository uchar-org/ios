...
struct RestorationTokenTests {
  ...
  func decodeTokenWithSlidingSyncProxy() throws {
    ...
    let originalToken = RestorationTokenV4(
      session: SessionV1(
        accessToken: "1234",
        refreshToken: "5678",
  ...
  func decodeFromTokenV4() throws {
    ...
    #expect(
      decodedToken.passphrase == originalToken.passphrase, "The passphrase should not be changed.")
    #expect(
      decodedToken.pusherNotificationClientIdentifier
        == originalToken.pusherNotificationClientIdentifier,
      "The push notification client identifier should not be changed.")
    #expect(
      decodedToken.sessionDirectories.dataDirectory == originalToken.sessionDirectory,
      "The session directory should not be changed.")
    #expect(
      decodedToken.sessionDirectories.cacheDirectory
        == .sessionCachesBaseDirectory.appending(component: sessionDirectoryName),
  ...
  func decodeFromTokenV5() throws {
    ...
    let sessionDirectoryName = UUID().uuidString
    let originalToken = RestorationTokenV5(
      session: SessionV1(
        accessToken: "1234",
        refreshToken: "5678",
        userId: "@user:example.com",
    ...
    #expect(
      ...
      "The session directory should not be changed.")
    #expect(
      decodedToken.sessionDirectories.cacheDirectory == originalToken.cacheDirectory,
  ...
  func decodeFromCurrentToken() throws {
    ...
    let originalToken = RestorationToken(
      session: Session(
        accessToken: "1234",
        refreshToken: "5678",
        userId: "@user:example.com",
  ...
  func assertEqual(session: Session, originalSession: SessionV1) {
    #expect(
      session.accessToken == originalSession.accessToken, "The access token should not be changed.")
    #expect(
      session.refreshToken == originalSession.refreshToken,
      "The refresh token should not be changed.")
    ...
    #expect(
      session.homeserverUrl == originalSession.homeserverUrl,
      "The homeserver URL should not be changed.")
...
enum SlidingSyncVersionV1: Equatable {
  ...
  var proxyURL: String? {
    guard case .proxy(let url) = self else { return nil }
    return url
  }
}

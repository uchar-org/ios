...
final class AppSettings {
    ...
    @UserPreference(key: UserDefaultsKeys.hasSeenNewSoundBanner, defaultValue: true,
    ...
    private(set) var accountProviders = ["uchar.uz"]
    ...
    let backgroundAppRefreshTaskIdentifier = "uz.uzinfocom.uchar.background.refresh"

    /// A URL where users can go read more about the app.
    private(set) var websiteURL: URL = "https://uchar.uz"
    /// A URL that contains the app's logo that may be used when showing content in a web view.
    private(set) var logoURL: URL = "https://uchar.uz/mobile-icon.png"
    /// A URL that contains that app's copyright notice.
    private(set) var copyrightURL: URL = "https://uchar.uz/copyright"
    /// A URL that contains the app's Terms of use.
    private(set) var acceptableUseURL: URL = "https://uchar.uz/acceptable-use-policy-terms"
    /// A URL that contains the app's Privacy Policy.
    private(set) var privacyURL: URL = "https://uchar.uz/privacy"
    /// A URL where users can go read more about encryption in general.
    private(set) var encryptionURL: URL = "https://uchar.uz/help#encryption"
    /// A URL where users can go read more about device verification..
    private(set) var deviceVerificationURL: URL =
        "https://uchar.uz/help#encryption-device-verification"
    /// A URL where users can go read more about the chat backup.
    private(set) var chatBackupDetailsURL: URL = "https://uchar.uz/help#encryption5"
    /// A URL where users can go read more about identity pinning violations
    private(set) var identityPinningViolationDetailsURL: URL = "https://uchar.uz/help#encryption18"
    /// A URL describing how history sharing works
    private(set) var historySharingDetailsURL: URL = "https://uchar.uz/en/help#e2ee-history-sharing"

    /// Any domains that Element web may be hosted on - used for handling links.
    private(set) var elementWebHosts = ["chat.uchar.uz"]
    /// The domain that account provisioning links will be hosted on - used for handling the links.
    private(set) var accountProvisioningHost = "mobile.uchar.uz"
    ...
    private(set) var oidcRedirectURL: URL = "https://uchar.uz/oidc/login"
    ...
    private(set) var analyticsTermsURL: URL? = "https://uchar.uz/cookie-policy"
    ...
    @UserPreference(key: UserDefaultsKeys.hasRequestedLocationAlwaysLocationAuthorization, defaultValue: false,
                    storageType: .userDefaults(store))
    ...
    @UserPreference(key: UserDefaultsKeys.liveLocationSharingTimeoutDatesByRoomID,
                    defaultValue: [String: LiveLocationSession](), storageType: .userDefaults(store))
    var liveLocationSharingSessionsByRoomID

    @UserPreference(key: UserDefaultsKeys.liveLocationMinimumDistanceUpdate, defaultValue: 10,
                    storageType: .userDefaults(store))
    var liveLocationMinimumDistanceUpdate

    @UserPreference(key: UserDefaultsKeys.liveLocationDisclaimerDisplayed, defaultValue: false,
                    storageType: .userDefaults(store))
    var liveLocationDisclaimerDisplayed

    // MARK: - Home Screen

    @UserPreference(key: UserDefaultsKeys.roomListActivityVisibility, defaultValue: .current,
                    storageType: .userDefaults(store))
    var roomListActivityVisibility: RoomListActivityVisibility

    // MARK: - Room Screen

    @UserPreference(key: UserDefaultsKeys.viewSourceEnabled, defaultValue: appBuildType == .debug,
                    storageType: .userDefaults(store))
    ...
    @UserPreference(key: UserDefaultsKeys.voiceMessagePlaybackSpeed, defaultValue: AudioPlaybackSpeed.default,
                    storageType: .userDefaults(store))
    ...
    @UserPreference(key: UserDefaultsKeys.lowPriorityFilterEnabled, defaultValue: false,
                    storageType: .userDefaults(store))
    ...
    @UserPreference(key: UserDefaultsKeys.roomThreadListEnabled, defaultValue: false,
                    storageType: .userDefaults(store))
    var roomThreadListEnabled

    @UserPreference(key: UserDefaultsKeys.focusEventOnNotificationTap, defaultValue: false,
                    storageType: .userDefaults(store))
    var focusEventOnNotificationTap

    @UserPreference(key: UserDefaultsKeys.linkPreviewsEnabled, defaultValue: false,
                    storageType: .userDefaults(store))
    var linkPreviewsEnabled

    @UserPreference(key: UserDefaultsKeys.linkNewDeviceEnabled, defaultValue: false,
                    storageType: .userDefaults(store))
    var linkNewDeviceEnabled

    @UserPreference(key: UserDefaultsKeys.automaticBackPaginationEnabled, defaultValue: false,
                    storageType: .userDefaults(store))
    var automaticBackPaginationEnabled

    @UserPreference(key: UserDefaultsKeys.developerOptionsEnabled, defaultValue: appBuildType != .release,
                    storageType: .userDefaults(store))

...
struct CameraPicker: UIViewControllerRepresentable {
  ...
  final class Coordinator: NSObject, UIImagePickerControllerDelegate, UINavigationControllerDelegate
  {
    ...
    func imagePickerController(
      ...
      cameraPicker.userIndicatorController.submitIndicator(
        UserIndicator(id: Self.loadingIndicatorIdentifier, type: .modal, title: L10n.commonLoading))
      defer {
      ...
      if let videoURL = info[.mediaURL] as? URL {
        ...
        let fileName =
          "\(Date.now.formatted(.iso8601.dateSeparator(.omitted).timeSeparator(.omitted))).jpg"

        do {
          let url = try FileManager.default.writeDataToTemporaryDirectory(
            data: jpegData, fileName: fileName)

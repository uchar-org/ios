...
typealias DeveloperOptionsScreenViewModelType = StateStoreViewModelV2<
  DeveloperOptionsScreenViewState, DeveloperOptionsScreenViewAction
>
  ...
  DeveloperOptionsScreenViewModelProtocol
{
  ...
  init(
    developerOptions: DeveloperOptionsProtocol, elementCallBaseURL: URL, appHooks: AppHooks,
    ...
    Task {
      if case .success(let sizes) = await clientProxy?.storeSizes() {
        ...
        if let eventCacheStore = sizes.eventCacheStore {
          components.append(
            .init(name: "EventCacheStore", size: formatter.format(Int64(eventCacheStore))))
        }

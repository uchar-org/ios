...
class NotificationHandler {
  ...
  init(
    ...
    let eventStringBuilder = RoomMessageEventStringBuilder(
    ...
    notificationContentBuilder = NotificationContentBuilder(
      messageEventStringBuilder: eventStringBuilder,
  ...
  func processEvent(_ eventID: String, roomID: String) async {
    ...
    guard
      let notificationItemProxy = await userSession.notificationItemProxy(
    ...
    case .shouldDisplay:
      ...
      await notificationContentBuilder.process(
  ...
  private func preprocessNotification(_ itemProxy: NotificationItemProxyProtocol) async
    -> NotificationProcessingResult
  {
    guard case .timeline(let event) = itemProxy.event else {
    ...
    case .messageLike(let messageContent):
      ...
      case .roomRedaction(let redactedEventID, _):
        ...
        let deliveredNotifications = await UNUserNotificationCenter.current()
          .deliveredNotifications()

        if let targetNotification = deliveredNotifications.first(where: {
          $0.request.content.eventID == redactedEventID
        }) {
          UNUserNotificationCenter.current().removeDeliveredNotifications(withIdentifiers: [
            targetNotification.request.identifier
          ])
        }

        return .processedShouldDiscard
      case .rtcNotification(let notificationType, let expirationTimestamp, let callIntent):
        return await handleCallNotification(
  ...
  private func handleCallNotification(
    ...
    roomID: String,
    roomDisplayName: String, callIntent: RtcCallIntent?
      ...
      if !room.hasActiveRoomCall() {  // If I don't have an active call wait a bit and make sure
        let expiringTask = ExpiringTaskRunner {
          await withCheckedContinuation { [weak self] continuation in
            self?.roomInfoObservationToken = room.subscribeToRoomInfoUpdates(
              listener: SDKListener { info in
                if info.hasRoomCall {
                  MXLog.info("Received room info update and the room has an active call now.")
                  continuation.resume()
                } else {
                  MXLog.info(
                    "Received a room info update but the room still doesn't have an ongoing call.")
                }
              })
          }
        }
    ...
    let payload =
      [
        ElementCallServiceNotificationKey.roomID.rawValue: roomID,
        ...
        ElementCallServiceNotificationKey.isVoiceCall.rawValue: callIntent == RtcCallIntent.audio,
      ] as [String: Any]

...
final class VoiceMessageCacheTests {
    ...
    func fileURL() throws {
        ...
        guard
            case .success(let cachedURL) = voiceMessageCache.cache(mediaSource: mediaSource, using: temporaryFileURL, move: true)
        else {
    ...
    func cacheInvalidFileExtension() throws {
        ...
        guard
            case .failure(let error) = voiceMessageCache.cache(mediaSource: mediaSource, using: mpegFileURL, move: true)
        else {
    ...
    func cacheCopy() throws {
        ...
        guard
            case .success(let cacheURL) = voiceMessageCache.cache(mediaSource: mediaSource, using: fileURL, move: false)
        else {
            Issue.record("A success is expected")
            return
        }
    ...
    func cacheMove() throws {
        let fileURL = try createTemporaryFile(named: testFilename, withExtension: mpeg4aacFileExtension)
        guard
            case .success(let cacheURL) = voiceMessageCache.cache(mediaSource: mediaSource, using: fileURL, move: true)
        else {
            Issue.record("A success is expected")
            return
        }

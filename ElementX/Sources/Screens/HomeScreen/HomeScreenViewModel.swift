...
class HomeScreenViewModel: HomeScreenViewModelType, HomeScreenViewModelProtocol {
  ...
  init(
    ...
    userSession.sessionSecurityStatePublisher
      ...
      .sink { [weak self] state in
        ...
        self.analyticsService.updateUserProperties(
          AnalyticsEvent.newVerificationStateUserProperty(
            verificationState: state.verificationState, recoveryState: state.recoveryState))
    ...
    isSearchFieldFocused
      ...
      .sink { [weak self] isSearchFieldFocused, _, _, _ in
        ...
        let delay =
          isSearchFieldFocused == self.context.viewState.bindings.isSearchFieldFocused ? 0.0 : 0.05
  ...
  override func process(viewAction: HomeScreenViewAction) {
    ...
    case .spaceFilters:
      if spaceFilterSubject.value != nil {
        ...
        state.bindings.spaceFiltersViewModel = ChatsSpaceFiltersScreenViewModel(
          spaceService: userSession.clientProxy.spaceService,
          mediaProvider: userSession.mediaProvider)
    ...
    case .markRoomAsUnread(let roomIdentifier):
      Task {
        guard
          case .joined(let roomProxy) = await userSession.clientProxy.roomForIdentifier(
            roomIdentifier)
        else {
    ...
    case .markRoomAsRead(let roomIdentifier):
      Task {
        ...
        case .success:
          ...
          if case .failure(let error) = await roomProxy.markAsRead(
            receiptType: appSettings.sharePresence ? .read : .readPrivate)
          {
  ...
  func presentCrashedLastRunAlert() {
    ...
    DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
      self.state.bindings.alertInfo = AlertInfo(
        id: UUID(),
      ...
      if state.bindings.isSearchFieldFocused {
        ...
        if let spaceFilter = spaceFilterSubject.value {
          roomSummaryProvider?.setFilter(
            .rooms(
              roomsIDs: spaceFilter.descendants,
              filters: state.bindings.filtersState.activeFilters.set))
        } else {
          roomSummaryProvider?.setFilter(
            .all(filters: state.bindings.filtersState.activeFilters.set))
        }
      }
    }
  }
  ...
  private func updateRoomListMode(with roomSummaryProviderState: RoomSummaryProviderState) {
    ...
    let hasNoRooms =
      roomSummaryProviderState.isLoaded && roomSummaryProviderState.totalNumberOfRooms == 0
    ...
    MXLog.info(
      "Received room summary provider update, setting view room list mode to \"\(state.roomListMode)\""
    )
  ...
  private func startLeaveRoomProcess(roomID: String) {
    Task {
      ...
      userIndicatorController.submitIndicator(
        UserIndicator(
          id: Self.leaveRoomLoadingID, type: .modal, title: L10n.commonLoading, persistent: true))
      ...
      if !roomProxy.isDirectOneToOneRoom {
        if case .success(let ownMember) = await roomProxy.getMember(userID: roomProxy.ownUserID),
          ...
          if isLastOwner {
            state.bindings.alertInfo = .init(
              ...
              secondaryButton: .init(
                title: L10n.leaveRoomAlertSelectNewOwnerAction, role: .destructive
              ) { [weak self] in
      ...
      state.bindings.leaveRoomAlertItem = LeaveRoomAlertItem(
        roomID: roomID, isDM: roomProxy.isDirectOneToOneRoom,
  ...
  private func leaveRoom(roomID: String) async {
    ...
    userIndicatorController.submitIndicator(
      UserIndicator(
        id: Self.leaveRoomLoadingID, type: .modal, title: L10n.commonLeavingRoom, persistent: true))
  ...
  private func acceptInvite(roomID: String) async {
    ...
    userIndicatorController.submitIndicator(
      UserIndicator(id: roomID, type: .modal, title: L10n.commonLoading, persistent: true))

    guard case .invited(let roomProxy) = await userSession.clientProxy.roomForIdentifier(roomID)
    else {
      displayError()
      return
    }
  ...
  private func finishAcceptInvite(roomProxy: InvitedRoomProxyProtocol) async {
    ...
    analyticsService.trackJoinedRoom(
      isDM: roomProxy.info.isDirect,
  ...
  private func showDeclineInviteConfirmationAlert(roomID: String) async {
    ...
    let title =
      room.isDirect ? L10n.screenInvitesDeclineDirectChatTitle : L10n.screenInvitesDeclineChatTitle
    let message =
      room.isDirect
      ? L10n.screenInvitesDeclineDirectChatMessage(roomPlaceholder)
      : L10n.screenInvitesDeclineChatMessage(roomPlaceholder)
    ...
    if await userSession.clientProxy.isReportRoomSupported, let userID = room.inviter?.id {
      state.bindings.alertInfo = .init(
        ...
        message: message,
        primaryButton: .init(title: L10n.actionCancel, role: .cancel, action: nil),
        secondaryButton: .init(title: L10n.actionDeclineAndBlock, role: .destructive) {
          [weak self] in self?.declineAndBlockInvite(userID: userID, roomID: roomID)
        },
        verticalButtons: [
          .init(title: L10n.actionDecline) { [weak self] in
            Task { await self?.declineInvite(roomID: room.id) }
          }
        ])
    } else {
      state.bindings.alertInfo = .init(
        ...
        message: message,
        primaryButton: .init(title: L10n.actionCancel, role: .cancel, action: nil),
        secondaryButton: .init(title: L10n.actionDecline, role: .destructive) { [weak self] in
          Task { await self?.declineInvite(roomID: room.id) }
        })
    }
  }

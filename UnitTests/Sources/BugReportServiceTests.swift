...
final class BugReportServiceTests {
  ...
  init() throws {
    ...
    bugReportServiceMock.submitBugReportProgressListenerReturnValue = .success(
  ...
  func submitBugReportWithMockService() async throws {
    ...
    let response = try await bugReportService.submitBugReport(
      bugReport, progressListener: progressSubject
    ).get()
  ...
  func initialStateWithRealService() {
    let urlPublisher: CurrentValueSubject<RageshakeConfiguration, Never> = .init(
      .url("https://example.com/submit"))
    let service = BugReportService(
      rageshakeURLPublisher: urlPublisher.asCurrentValuePublisher(),
  ...
  func submitBugReportWithRealService() async throws {
    ...
    let bugReport = BugReport(
      ...
      files: [])
    let progressSubject = CurrentValueSubject<Double, Never>(0.0)
    let response = try await service.submitBugReport(bugReport, progressListener: progressSubject)
      .get()
  ...
  func configurations() async throws {
    guard case .url(let initialURL) = appSettings.bugReportRageshakeURL.publisher.value else {
    ...
    let bugReport = BugReport(
      ...
      files: [])
    let progressSubject = CurrentValueSubject<Double, Never>(0.0)
    let customConfigurationResponse = try await service.submitBugReport(
      bugReport, progressListener: progressSubject
    ).get()
    ...
    let defaultConfigurationResponse = try await service.submitBugReport(
      bugReport, progressListener: progressSubject
    ).get()

    #expect(
      defaultConfigurationResponse.reportURL
        == initialURL.absoluteString.replacingOccurrences(of: "submit", with: "123"))
  }
}
...
extension URLSession {
  fileprivate static var mock: URLSession {

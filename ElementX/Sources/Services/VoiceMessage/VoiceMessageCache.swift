...
class VoiceMessageCache: VoiceMessageCacheProtocol {
  ...
  var urlForRecording: URL {
    ...
    return temporaryFilesFolderURL.appendingPathComponent("voice-message-recording")
  ...
  func cache(mediaSource: MediaSourceProxy, using fileURL: URL, move: Bool = false) -> Result<
  ...
  private func setupTemporaryFilesFolder() {
    do {
      try FileManager.default.createDirectoryIfNeeded(
        at: temporaryFilesFolderURL, withIntermediateDirectories: true)
    } catch {
  ...
  private func cacheURL(for mediaSource: MediaSourceProxy) -> URL {
    temporaryFilesFolderURL.appendingPathComponent(mediaSource.url.lastPathComponent)
      .appendingPathExtension(preferredFileExtension)
  }
}

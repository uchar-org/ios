...
class EncryptionSettingsFlowCoordinator: FlowCoordinatorProtocol {
  ...
  private let actionsSubject: PassthroughSubject<EncryptionSettingsFlowCoordinatorAction, Never> =
    .init()
  ...
  private func configureStateMachine() {
    stateMachine.addRoutes(event: .start, transitions: [.initial => .secureBackupScreen]) {
      [weak self] _ in
      self?.presentSecureBackupScreen()
    }

    stateMachine.addRoutes(
      event: .manageRecoveryKey, transitions: [.secureBackupScreen => .recoveryKeyScreen]
    ) { [weak self] _ in
      self?.presentRecoveryKeyScreen()
    }
    stateMachine.addRoutes(
      event: .finishedManagingRecoveryKey, transitions: [.recoveryKeyScreen => .secureBackupScreen])

    stateMachine.addRoutes(
      event: .disableKeyBackup, transitions: [.secureBackupScreen => .keyBackupScreen]
    ) { [weak self] _ in
      self?.presentKeyBackupScreen()
    }
    stateMachine.addRoutes(
      event: .finishedDisablingKeyBackup, transitions: [.keyBackupScreen => .secureBackupScreen])
  ...
  private func presentSecureBackupScreen(animated: Bool = true) {
    let coordinator = SecureBackupScreenCoordinator(
      parameters: .init(
  ...
  private func presentRecoveryKeyScreen() {
    ...
    let coordinator = SecureBackupRecoveryKeyScreenCoordinator(
      parameters: .init(
    ...
    navigationStackCoordinator.setSheetCoordinator(sheetNavigationStackCoordinator) {
      [stateMachine] in
  ...
  private func presentKeyBackupScreen() {
    ...
    let coordinator = SecureBackupKeyBackupScreenCoordinator(
      parameters: .init(
        secureBackupController: userSession.clientProxy.secureBackupController,
        userIndicatorController: userIndicatorController))

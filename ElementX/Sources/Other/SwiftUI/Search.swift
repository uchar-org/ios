...
extension View {
  ...
  func searchController(
    ...
    accessibilityFocusOnStart: Bool = false
  ) -> some View {
    modifier(
...
private struct SearchControllerModifier: ViewModifier {
  ...
  func body(content: Content) -> some View {
    ...
    if #available(iOS 26, *) {
      content
        .searchable(
          text: $searchQuery, placement: .navigationBarDrawer(displayMode: .always), prompt: text
        )
        .interactiveDismissDisabled(!searchQuery.isEmpty && disablesInteractiveDismiss)
        .introspect(.navigationStack, on: .supportedVersions, scope: .ancestor) {
          ...
          guard let searchController = navigationController.navigationBar.topItem?.searchController
          else { return }
...
private struct SearchController: UIViewControllerRepresentable {
  ...
  func makeUIViewController(context: Context) -> SearchInjectionViewController {
    let controller = SearchInjectionViewController(
    ...
    if accessibilityFocusOnStart {
      DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
        UIAccessibility.post(
          notification: .screenChanged, argument: controller.searchController.searchBar)
      }
    }
    return controller
  }

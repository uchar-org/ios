...
class ClientProxy: ClientProxyProtocol {
  ...
  private let verificationStateSubject = CurrentValueSubject<SessionVerificationState, Never>(
    .unknown)
  ...
  private let homeserverReachabilitySubject = CurrentValueSubject<
    NetworkMonitorReachability, Never
  >(.reachable)
  ...
  private let timelineMediaVisibilitySubject = CurrentValueSubject<TimelineMediaVisibility, Never>(
    .always)
  ...
  private let liveLocationOwnInfoUpdatesSubject = PassthroughSubject<
    LiveLocationOwnInfoUpdate, Never
  >()
  ...
  init(
    ...
    notificationSettings = await NotificationSettingsProxy(
      notificationSettings: client.getNotificationSettings())

    secureBackupController = SecureBackupController(encryption: client.encryption())

    spaceService = await SpaceServiceProxy(spaceService: client.spaceService())

    capabilities = HomeserverCapabilitiesProxy(
      underlyingCapabilities: client.homeserverCapabilities())

    let configuredAppService = try await ClientProxyServices(
      client: client,
    ...
    roomListStateLoadingStateUpdateTaskHandle = createRoomListLoadingStateUpdateObserver(
      roomListService)

    delegateHandle = try client.setDelegate(
      delegate: ClientDelegateWrapper { [weak self] isSoftLogout in
        ...
        case .panic(let message, let backtrace):
          MXLog.error(
            "Received background task panic: \(message ?? "Missing message")\nBacktrace:\n\(backtrace ?? "Missing backtrace")"
          )
          fatalError(message ?? "")
    ...
    try await client.setUtdDelegate(
      utdDelegate: ClientDecryptionErrorDelegate(actionsSubject: actionsSubject))
    ...
    Task {
      do {
        try await client.setMediaRetentionPolicy(
          policy: .init(
            maxCacheSize: nil,
            maxFileSize: nil,
            // 30 days in seconds
  ...
  func uploadMedia(_ media: MediaInfo) async -> Result<String, ClientProxyError> {
    ...
    do {
      ...
      let matrixUrl = try await client.uploadMedia(
        mimeType: mimeType, data: data, progressWatcher: nil)
  ...
  > {
    do {
      let roomPreview = try await client.getRoomPreviewFromRoomId(
        roomId: identifier, viaServers: via)
  ...
  > {
    do {
      return try await .success(
        .init(sdkResults: client.searchUsers(searchTerm: searchTerm, limit: UInt64(limit))))
    } catch {
  ...
  func roomDirectorySearchProxy() -> RoomDirectorySearchProxyProtocol {
    RoomDirectorySearchProxy(
      roomDirectorySearch: client.roomDirectorySearch(), appSettings: appSettings)
  }
  ...
  func resolveRoomAlias(_ alias: String) async -> Result<ResolvedRoomAlias, ClientProxyError> {
    do {
      ...
      let limitedAlias = ResolvedRoomAlias(
        roomId: resolvedAlias.roomId, servers: Array(resolvedAlias.servers.prefix(50)))
  ...
  func recentConversationCounterparts() async -> [UserProfileProxy] {
    ...
    for roomID in roomIdentifiers {
      ...
      for member in members where member.isActive && member.userID != userID {
        users.append(
          .init(userID: member.userID, displayName: member.displayName, avatarURL: member.avatarURL)
        )
  ...
  private func setupSubscriptions() async {
    ...
    ignoredUsersListenerTaskHandle = client.subscribeToIgnoredUsers(
      listener: SDKListener { [weak self] ignoredUsers in
        self?.ignoredUsersSubject.send(ignoredUsers)
      })

    await updateVerificationState(client.encryption().verificationState())
    verificationStateListenerTaskHandle = client.encryption().verificationStateListener(
      listener: SDKListener { [weak self] verificationState in
        Task { await self?.updateVerificationState(verificationState) }
      })

    sendQueueStatusListenerTaskHandle = client.subscribeToSendQueueStatus(
      listener: SDKListener { [weak self] roomID, error in
        MXLog.error("Send queue failed in room: \(roomID) with error: \(error)")
        self?.sendQueueStatusSubject.send(false)
      })

    sendQueueUpdatesListenerTaskHandle = try? await client.subscribeToSendQueueUpdates(
      listener: SDKListener { [analyticsService] _, update in
        switch update {
    ...
    sendQueueStatusSubject
      ...
      .sink { [client] enabled, reachability in
        MXLog.info(
          "Send queue status changed to enabled: \(enabled), homeserver reachability: \(reachability)"
        )
  ...
  private func buildSessionVerificationControllerProxyIfPossible(
    ...
    do {
      ...
      self.sessionVerificationController = SessionVerificationControllerProxy(
        sessionVerificationController: sessionVerificationController)
    } catch {
  ...
  private func createMediaPreviewConfigObserver() async -> TaskHandle? {
    do {
      return try await client.subscribeToMediaPreviewConfig(
        listener: SDKListener { [weak self] config in
          guard let self else { return }
  ...
  private func createLiveLocationOwnInfoUpdatesObserver() -> TaskHandle? {
    do {
      return try client.subscribeToOwnBeaconInfoUpdates(
        listener: SDKListener { [weak self] update in
          guard let self else { return }
          let appUpdate = LiveLocationOwnInfoUpdate(
    ...
    -> TaskHandle
  {
    roomListService.syncIndicator(
      delayBeforeShowingInMs: 1000, delayBeforeHidingInMs: 0,
      listener: SDKListener { [weak self] state in
        guard let self else { return }
  ...
  private func buildRoomForIdentifier(_ roomID: String) async -> RoomProxyType? {
    do {
      ...
      case .joined:
        let roomProxy = try await JoinedRoomProxy(
  ...
  func pinUserIdentity(_ userID: String) async -> Result<Void, ClientProxyError> {
    ...
    do {
      guard
        let userIdentity = try await client.encryption().userIdentity(
          userId: userID, fallbackToServer: true)
      else {
  ...
  > {
    do {
      return try await .success(
        client.encryption().userIdentity(userId: userID, fallbackToServer: fallBackToServer).map(
          UserIdentityProxy.init))
    } catch {
...
private final class ClientDelegateWrapper: ClientDelegate {
  ...
  init(
    authErrorCallback: @escaping @Sendable (Bool) -> Void,
  ...
  func onBackgroundTaskErrorReport(
...
private struct ClientProxyServices {
  ...
  init(
    ...
    let roomMessageEventStringBuilder = RoomMessageEventStringBuilder(
      attributedStringBuilder: AttributedStringBuilder(
        cacheKey: "roomList",
        mentionBuilder: PlainMentionBuilder()),
      style: .senderPrefixed)

    eventStringBuilder = try RoomEventStringBuilder(
      stateEventStringBuilder: RoomStateEventStringBuilder(userID: client.userId()),
    ...
    alternateRoomSummaryProvider = RoomSummaryProvider(
      roomListService: roomListService,
      eventStringBuilder: eventStringBuilder,
    ...
    staticRoomSummaryProvider = RoomSummaryProvider(
      roomListService: roomListService,
      eventStringBuilder: eventStringBuilder,
      name: "StaticAllRooms",
...
extension MediaPreviewConfig {
  fileprivate var mediaPreviewVisibility: TimelineMediaVisibility {
  ...
  fileprivate var hideInviteAvatars: Bool {
...
extension TimelineMediaVisibility {
  fileprivate var rustValue: MediaPreviews {
    switch self {
    case .always:
      .on
    case .never:
...
extension CreateRoomAccessType {
  fileprivate var isEncrypted: Bool {
    switch self {
    case .public:
      false
    default:
  ...
  fileprivate var visibility: RoomVisibility {
    isVisibilityPrivate ? .private : .public
  }

  fileprivate var preset: RoomPreset {
    isVisibilityPrivate ? .privateChat : .publicChat
  }

  fileprivate var historyVisibilityOverride: RoomHistoryVisibility? {
    isVisibilityPrivate ? .invited : nil
  }

  fileprivate var joinRuleOverride: JoinRule? {
    switch self {
    case .askToJoin:
      .knock
  ...
  fileprivate var isAskToJoin: Bool {
    switch self {

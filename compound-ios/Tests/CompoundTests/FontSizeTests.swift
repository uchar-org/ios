...
struct FontSizeTests {
  ...
  func customFontWithTextStyle() {
    let customTitle21FontSize = FontSize.reflecting(
      .custom("Baskerville", size: 21, relativeTo: .title))
    #expect(customTitle21FontSize?.value == 21)
    #expect(customTitle21FontSize?.style == .title)

    let styledCustomFootnote15 = Font.custom("Baskerville", size: 15, relativeTo: .footnote).weight(
      .thin
    ).monospaced().italic()

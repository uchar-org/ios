...
struct AttributedStringBuilderTests {
  ...
  func renderHTMLStringWithHeaders() throws {
    let attributedString = try #require(
      attributedStringBuilder.fromHTML(HTMLFixtures.headers.rawValue),
  ...
  func renderHTMLStringWithPreCode() throws {
    let attributedString = try #require(
      attributedStringBuilder.fromHTML(HTMLFixtures.code.rawValue),
      "Could not build the attributed string")

    #expect(
      attributedString.runs.first?.uiKit.font?.fontName == ".AppleSystemUIFontMonospaced-Regular")

    let string = String(attributedString.characters)

    let regex = try #require(
      try? NSRegularExpression(pattern: "\\R", options: []),
      "Could not build the regex for the test.")

    #expect(
      regex.numberOfMatches(
        in: string, options: [], range: .init(location: 0, length: string.count)) == 23)
  }
  ...
  func renderPlainStringWithLink() throws {
    ...
    let attributedString = try #require(
      attributedStringBuilder.fromPlain(plainString), "Could not build the attributed string")
  ...
  func punctuationAtTheEndOfPlainStringLinks() throws {
    let plainString =
      "Most punctuation marks are removed https://www.matrix.org:;., but closing brackets are kept https://example.com/(test)"
  ...
  func renderHTMLStringWithLinkInHeader() throws {
    ...
    let h1AttributedString = try #require(
      attributedStringBuilder.fromHTML(h1HTMLString), "Could not build the attributed string")
    let h2AttributedString = try #require(
      attributedStringBuilder.fromHTML(h2HTMLString), "Could not build the attributed string")
    let h3AttributedString = try #require(
      attributedStringBuilder.fromHTML(h3HTMLString), "Could not build the attributed string")

    let h1Font = try #require(
      h1AttributedString.runs.first?.uiKit.font, "Could not extract a font from the strings.")
    let h2Font = try #require(
      h2AttributedString.runs.first?.uiKit.font, "Could not extract a font from the strings.")
    let h3Font = try #require(
      h3AttributedString.runs.first?.uiKit.font, "Could not extract a font from the strings.")
  ...
  func renderHTMLStringWithIFrame() throws {
    ...
    let attributedString = try #require(
      attributedStringBuilder.fromHTML(htmlString), "Could not build the attributed string")

    #expect(
      attributedString.uiKit.attachment == nil,
  ...
  func linkWithFragment() throws {
    ...
    try checkLinkIn(
      attributedString: attributedStringBuilder.fromHTML(string),
      expectedLink: "https://example.com", expectedRuns: 1)
    try checkLinkIn(
      attributedString: attributedStringBuilder.fromPlain(string),
      expectedLink: "https://example.com", expectedRuns: 1)

    string = "https://example.com/#/some_fragment/"
    try checkLinkIn(
      attributedString: attributedStringBuilder.fromHTML(string),
      expectedLink: "https://example.com/#/some_fragment", expectedRuns: 1)
    try checkLinkIn(
      attributedString: attributedStringBuilder.fromPlain(string),
      expectedLink: "https://example.com/#/some_fragment", expectedRuns: 1)
  }
  ...
  func permalink() throws {
    ...
    try checkLinkIn(
      attributedString: attributedStringBuilder.fromHTML(string), expectedLink: string,
      expectedRuns: 1)
    try checkLinkIn(
      attributedString: attributedStringBuilder.fromPlain(string), expectedLink: string,
      expectedRuns: 1)
  }
  ...
  func userIDLink() throws {
    ...
    try checkLinkIn(
      attributedString: attributedStringBuilder.fromHTML(string), expectedLink: expectedLink,
      expectedRuns: 3)
    try checkLinkIn(
      attributedString: attributedStringBuilder.fromPlain(string), expectedLink: expectedLink,
      expectedRuns: 3)
  }
  ...
  func roomAliasLink() throws {
    ...
    let expectedLink = try #require(
      URL(string: "https://matrix.to/#/\(roomAlias)"), "The expected link should be valid.")
    try checkLinkIn(
      attributedString: attributedStringBuilder.fromHTML(string),
      expectedLink: expectedLink.absoluteString, expectedRuns: 3)
    try checkLinkIn(
      attributedString: attributedStringBuilder.fromPlain(string),
      expectedLink: expectedLink.absoluteString, expectedRuns: 3)
  }
  ...
  func defaultForegroundColor() throws {
    let htmlString =
      "<b>Test</b> <i>string</i> <a href=\"https://www.matrix.org/\">link</a> <code><a href=\"https://www.matrix.org/\">link</a></code>"
  ...
  func blockquoteWithLink() throws {
    ...
    #expect(
      coalescedComponents.first?.attributedString.runs.count == 3,
      "Link not present in the component")
  ...
  func replyBlockquote() throws {
    let htmlString =
      "<blockquote><a href=\"https://matrix.to/#/someroom/someevent\">In reply to</a> <a href=\"https://matrix.to/#/@user:matrix.org\">@user:matrix.org</a><br>The future is <code>swift run tools</code> 😎</blockquote>"
  ...
  func multipleGroupedBlockquotes() throws {
    let attributedString = try #require(
      attributedStringBuilder.fromHTML(HTMLFixtures.groupedBlockQuotes.rawValue),
      "Could not build the attributed string")
  ...
  func multipleSeparatedBlockquotes() throws {
    let attributedString = try #require(
      attributedStringBuilder.fromHTML(HTMLFixtures.separatedBlockQuotes.rawValue),
      "Could not build the attributed string")
  ...
  func userIDMentionAtachment() {
    ...
    #expect(attributedStringFromHTML?.userID == "@test:matrix.org")
    #expect(
      attributedStringFromHTML?.link?.absoluteString == "https://matrix.to/#/@test:matrix.org")
    let attributedStringFromPlain = attributedStringBuilder.fromPlain(string)
    #expect(attributedStringFromPlain?.attachment != nil)
    #expect(attributedStringFromPlain?.userID == "@test:matrix.org")
    #expect(
      attributedStringFromPlain?.link?.absoluteString == "https://matrix.to/#/@test:matrix.org")
  }
  ...
  func roomAliasPermalinkMentionAttachment() {
    ...
    #expect(
      attributedStringFromHTML?.link?.absoluteString == "https://matrix.to/#/%23test:matrix.org")
    let attributedStringFromPlain = attributedStringBuilder.fromPlain(string)
    #expect(attributedStringFromPlain?.attachment != nil)
    #expect(attributedStringFromHTML?.roomAlias == "#test:matrix.org")
    #expect(
      attributedStringFromPlain?.link?.absoluteString == "https://matrix.to/#/%23test:matrix.org")
  }
  ...
  func eventRoomIDPermalinkMentionAttachment() {
    ...
    #expect(attributedStringFromHTML?.attachment != nil)
    #expect(
      attributedStringFromHTML?.eventOnRoomID
        == .some(.init(roomID: "!test:matrix.org", eventID: "$test")))
    #expect(attributedStringFromHTML?.link?.absoluteString == string)
    let attributedStringFromPlain = attributedStringBuilder.fromPlain(string)
    #expect(attributedStringFromPlain?.attachment != nil)
    #expect(
      attributedStringFromPlain?.eventOnRoomID
        == .some(.init(roomID: "!test:matrix.org", eventID: "$test")))
    #expect(attributedStringFromPlain?.link?.absoluteString == string)
  }
  ...
  func eventRoomAliasPermalinkMentionAttachment() {
    ...
    #expect(attributedStringFromHTML?.attachment != nil)
    #expect(
      attributedStringFromHTML?.eventOnRoomAlias
        == .some(.init(alias: "#test:matrix.org", eventID: "$test")))
    #expect(
      attributedStringFromHTML?.link?.absoluteString
        == "https://matrix.to/#/%23test:matrix.org/$test")
    let attributedStringFromPlain = attributedStringBuilder.fromPlain(string)
    #expect(attributedStringFromPlain?.attachment != nil)
    #expect(
      attributedStringFromPlain?.eventOnRoomAlias
        == .some(.init(alias: "#test:matrix.org", eventID: "$test")))
    #expect(
      attributedStringFromPlain?.link?.absoluteString
        == "https://matrix.to/#/%23test:matrix.org/$test")
  }
  ...
  func userMentionAtachmentInBlockQuotes() throws {
    ...
    let attributedStringFromHTML = try #require(
      attributedStringBuilder.fromHTML(string), "Attributed string is nil")
  ...
  func linksHavePriorityOverAllUserMention() throws {
    ...
    let attributedStringFromHTML = attributedStringBuilder.fromHTML(string)
    try checkLinkIn(
      attributedString: attributedStringFromHTML, expectedLink: string, expectedRuns: 1)
    let attributedStringFromPlain = attributedStringBuilder.fromPlain(string)
    try checkLinkIn(
      attributedString: attributedStringFromPlain, expectedLink: string, expectedRuns: 1)

    let string2 = "https://matrix.to/#/@roomusername:matrix.org"
    let attributedStringFromHTML2 = attributedStringBuilder.fromHTML(string2)
    try checkLinkIn(
      attributedString: attributedStringFromHTML2, expectedLink: string2, expectedRuns: 1)
    try checkAttachment(attributedString: attributedStringFromHTML2, expectedRuns: 1)
    let attributedStringFromPlain2 = attributedStringBuilder.fromPlain(string2)
    try checkLinkIn(
      attributedString: attributedStringFromPlain2, expectedLink: string2, expectedRuns: 1)
  ...
  func multipleMentions() throws {
    ...
    let attributedStringFromPlain = try #require(
      attributedStringBuilder.fromPlain(string), "Attributed string is nil")
  ...
  func imageTags() throws {
    let htmlString =
      "Hey <img src=\"smiley.gif\" alt=\"Smiley face\">! How's work<img src=\"workplace.jpg\">?"
  ...
  func nestedUnorderedList() throws {
    let htmlString =
      "<ul><li>A<ul><li>A1</li><li>A2</li><li>A3</li></ul></li><li>B</li><li>C</li></ul>"
  ...
  func nestedOrderedList() throws {
    let htmlString =
      "<ol><li>A<ol><li>A1</li><li>A2</li><li>A3</li></ol></li><li>B</li><li>C</li></ol>"
  ...
  func nestedHeterogeneousLists() throws {
    let htmlString =
      "<ol><li>A<ul><li>A1</li><li>A2</li><li>A3</li></ul></li><li>B</li><li>C</li></ol>"
  ...
  func normalisedWhitespaces() throws {
    let html = """
      ...
      """
    let attributedString = try #require(
      attributedStringBuilder.fromHTML(html), "Could not build the attributed string")

    #expect(
      String(attributedString.characters)
        == "Stefan pushed 2 commits to main:\n   •  Some update \n   •  Some other update")
  }
  ...
  func phishingLink() throws {
    let htmlString =
      "Hey check the following link <a href=\"https://matrix.org\">https://element.io</a>"

    let attributedString = try #require(
      attributedStringBuilder.fromHTML(htmlString), "Could not build the attributed string")

    #expect(
      String(attributedString.characters) == "Hey check the following link https://element.io")

    #expect(attributedString.runs.count == 2)

    let link = try #require(
      attributedString.runs.first { $0.link != nil }?.link, "Couldn't find the link")
  ...
  func phishingUserID() throws {
    let htmlString =
      "Hey check the following user <a href=\"https://matrix.org\">@alice:matrix.org</a>"
  ...
  func validUserIDLink() throws {
    let htmlString =
      "Hey check the following user <a href=\"https://matrix.to/#/@alice:matrix.org\">@alice:matrix.org</a>"
  ...
  func phishingUserIDWithAnotherUserIDPermalink() throws {
    let htmlString =
      "Hey check the following user <a href=\"https://matrix.to/#/@bob:matrix.org\">@alice:matrix.org</a>"
    ...
    #expect(link.requiresConfirmation)
    #expect(
      link.confirmationParameters?.internalURL.absoluteString
  ...
  func phishingUserIDWithDistractingCharacters() throws {
    let htmlString =
      "Hey check the following user <a href=\"https://matrix.org\">👉️ @alice:matrix.org</a>"

    let attributedString = try #require(
      attributedStringBuilder.fromHTML(htmlString), "Could not build the attributed string")

    #expect(
      String(attributedString.characters) == "Hey check the following user 👉️ @alice:matrix.org")
  ...
  func phishingLinkWithDistractingCharacters() throws {
    let htmlString =
      "Hey check the following link <a href=\"https://matrix.org\">👉️ https://element.io</a>"

    let attributedString = try #require(
      attributedStringBuilder.fromHTML(htmlString), "Could not build the attributed string")

    #expect(
      String(attributedString.characters) == "Hey check the following link 👉️ https://element.io")
  ...
  func validLinkWithDistractingCharacters() throws {
    let htmlString =
      "Hey check the following link <a href=\"https://element.io\">👉️ https://element.io</a>"
  ...
  func phishingMatrixPermalinks() throws {
    let htmlString =
      "Hey check the following room <a href=\"https://matrix.to/#/#offensive-room:matrix.org\">https://matrix.to/#/#beautiful-room:matrix.org</a>"
    ...
    #expect(
      link.confirmationParameters?.internalURL.absoluteString
        == "https://matrix.to/#/%23offensive-room:matrix.org")
    #expect(
      link.confirmationParameters?.displayString == "https://matrix.to/#/#beautiful-room:matrix.org"
    )
  }
  ...
  func validMatrixPermalinks() throws {
    let htmlString =
      "Hey check the following room <a href=\"https://matrix.to/#/#beautiful-room:matrix.org\">https://matrix.to/#/#beautiful-room:matrix.org</a>"
  ...
  func phishingRoomAlias() throws {
    let htmlString =
      "Hey check the following room <a href=\"https://matrix.org\">#room:matrix.org</a>"
  ...
  func validRoomAliasLink() throws {
    let htmlString =
      "Hey check the following user <a href=\"https://matrix.to/#/#room:matrix.org\">#room:matrix.org</a>"
  ...
  func phishingRoomAliasWithAnotherRoomAliasPermalink() throws {
    let htmlString =
      "Hey check the following room <a href=\"https://matrix.to/#/#another-room:matrix.org\">#room:matrix.org</a>"
    ...
    #expect(
      link.confirmationParameters?.internalURL.absoluteString
        == "https://matrix.to/#/%23another-room:matrix.org")
    #expect(link.confirmationParameters?.displayString == "#room:matrix.org")
  }
  ...
  func roomAliasWithDistractingCharacters() throws {
    let htmlString =
      "Hey check the following user <a href=\"https://matrix.org\">👉️ #room:matrix.org</a>"

    let attributedString = try #require(
      attributedStringBuilder.fromHTML(htmlString), "Could not build the attributed string")

    #expect(
      String(attributedString.characters) == "Hey check the following user 👉️ #room:matrix.org")
  ...
  func mxExternalPaymentDetailsRemoved() throws {
    var htmlString =
      "This is visible.<span data-msc4286-external-payment-details> But this is hidden <a href=\"https://matrix.org\">and this link too</a></span>"
    ...
    htmlString =
      "This is visible.<span> And this text <a href=\"https://matrix.org\">and link</a> are visible too.</span>"

    let attributedString2 = try #require(
      attributedStringBuilder.fromHTML(htmlString), "Could not build the attributed string")

    #expect(
      String(attributedString2.characters)
        == "This is visible. And this text and link are visible too.")
  ...
  private func checkLinkIn(
    attributedString: AttributedString?, expectedLink: String, expectedRuns: Int
  ) throws {

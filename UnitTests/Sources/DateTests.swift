...
struct DateTests {
  ...
  func minimalDateFormatting() throws {
    ...
    #expect(
      yesterday.formattedMinimal()
        == yesterday.formatted(
          Date.RelativeFormatStyle(
            presentation: .named, capitalizationContext: .beginningOfSentence)))
    ...
    let sometimeInTheLastYear = try #require(
      calendar.date(byAdding: .month, value: -10, to: startOfToday))
    #expect(
      sometimeInTheLastYear.formattedMinimal()
        == sometimeInTheLastYear.formatted(.dateTime.day().month()))

    let theMillennium = try #require(
      calendar.date(from: DateComponents(year: 2000, month: 1, day: 1)))
    #expect(
      theMillennium.formattedMinimal() == theMillennium.formatted(.dateTime.year().day().month()))
  }
  ...
  func expirationDateFormatting() throws {
    ...
    #expect(
      in23Hours.formattedExpiration() == in23Hours.formatted(date: .omitted, time: .shortened))
  ...
  func dateSeparatorFormatting() throws {
    ...
    let nearYesterday = try #require(calendar.date(byAdding: DateComponents(hour: -10), to: today))
    #expect(
      nearYesterday.formattedDateSeparator()
        == yesterday.formatted(
          Date.RelativeFormatStyle(
            presentation: .named, capitalizationContext: .beginningOfSentence)))
    ...
    let threeDaysAgo = try #require(calendar.date(byAdding: .day, value: -3, to: startOfToday))
    #expect(
      threeDaysAgo.formattedDateSeparator() == threeDaysAgo.formatted(.dateTime.weekday(.wide)))
    ...
    #expect(
      startOfTheYear.formattedDateSeparator()
        == startOfTheYear.formatted(.dateTime.weekday(.wide).day().month(.wide)))

    let theMillennium = try #require(
      calendar.date(from: DateComponents(year: 2000, month: 1, day: 1)))
    #expect(
      theMillennium.formattedDateSeparator()
        == theMillennium.formatted(.dateTime.weekday(.wide).day().month(.wide).year()))
  }
}

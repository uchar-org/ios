...
@Observable class NavigationSplitCoordinator: CoordinatorProtocol, CustomStringConvertible {
    ...
    fileprivate var compactLayoutRootModule: NavigationModule? {
        if let sidebarNavigationStackCoordinator = sidebarModule?.coordinator
            as? NavigationStackCoordinator {
    ...
    private func getCompactStackModules() -> [NavigationModule] {
        ...
        if let detailNavigationStackCoordinator = detailModule?.coordinator
            as? NavigationStackCoordinator {
    ...
    func setSidebarCoordinator(_ coordinator: (any CoordinatorProtocol)?, animated: Bool = true,
    ...
    func setDetailCoordinator(_ coordinator: (any CoordinatorProtocol)?, animated: Bool = true,
                              dismissalCallback: (() -> Void)? = nil) {
        guard let coordinator else {
            detailModule = nil
            return
...
@Observable class NavigationStackCoordinator: CoordinatorProtocol, CustomStringConvertible {
    ...
    func setRootCoordinator(_ coordinator: (any CoordinatorProtocol)?, animated: Bool = true,
                            dismissalCallback: (() -> Void)? = nil) {
        guard let coordinator else {
            rootModule = nil
            return
    ...
    func push(_ coordinator: any CoordinatorProtocol, animated: Bool = true,
              dismissalCallback: (() -> Void)? = nil) {
        var transaction = Transaction()
        transaction.disablesAnimations = !animated

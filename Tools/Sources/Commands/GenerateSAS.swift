...
struct GenerateSAS: ParsableCommand {
  static let configuration = CommandConfiguration(
    abstract: "A tool to download and generate SAS localization strings")
  ...
  func run() throws {
    ...
    guard let data = try? Data(contentsOf: baseURL),
      let json = try? JSONDecoder().decode([SASEmoji].self, from: data)
    else {
  ...
  private func writeToFile(file: String, dict: [String: String], subdirectory: String) {
    let fileDirectory = URL.projectDirectory.appendingPathComponent(
      "ElementX/Resources/Localizations"

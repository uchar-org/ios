...
struct SpaceRoomCell: View {
  ...
  enum Action {
    case select(SpaceServiceRoom)
    case join(SpaceServiceRoom)
  }
  ...
  var body: some View {
    Button {
      ...
      HStack(spacing: 0) {
        if isEditModeActive,
          !spaceServiceRoom.isSpace
        {  // We only support selection of rooms (so don't show this while removing the cell).
          ZStack {
...
struct SpaceRoomCellButtonStyle: ButtonStyle {
  ...
  func makeBody(configuration: Configuration) -> some View {
    configuration.label
      ...
      .animation(
        isHighlighted ? .none : .easeOut(duration: 0.1).disabledDuringTests(), value: isHighlighted)
  }
}
  ...
  static var previews: some View {
    ScrollView {
      VStack(spacing: 0) {
        ...
        SpaceRoomCell(
          spaceServiceRoom: SpaceServiceRoom.mock(id: "Space being joined", isSpace: true),
          isSelected: false,
          isJoining: true,
          mediaProvider: mediaProvider
        ) { _ in }
        SpaceRoomCell(
          spaceServiceRoom: SpaceServiceRoom.mock(id: "Room being joined", isSpace: false),
          isSelected: false,
          isJoining: true,
          mediaProvider: mediaProvider
        ) { _ in }
        ...
        SpaceRoomCell(
          spaceServiceRoom: SpaceServiceRoom.mock(id: "Selected", isSpace: false, state: .joined),
          isSelected: true,
          isJoining: false,
          mediaProvider: mediaProvider
        ) { _ in }
        ...
        SpaceRoomCell(
          spaceServiceRoom: SpaceServiceRoom.mock(id: "Unselected", isSpace: false, state: .joined),
          isSelected: false,
          isJoining: false,
          mediaProvider: mediaProvider
        ) { _ in }

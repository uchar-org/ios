...
typealias TimelineMediaPreviewViewModelType = StateStoreViewModel<
  TimelineMediaPreviewViewState, TimelineMediaPreviewViewAction
>
...
class TimelineMediaPreviewViewModel: TimelineMediaPreviewViewModelType {
  ...
  private let actionsSubject: PassthroughSubject<TimelineMediaPreviewViewModelAction, Never> =
    .init()
  ...
  init(
    ...
    super.init(
      initialViewState: TimelineMediaPreviewViewState(
        dataSource: .init(
          itemViewStates: timelineState.itemViewStates,
    ...
    let canRedactSelfPublisher = timelineViewModel.context.$viewState.map(
      \.canCurrentUserRedactSelf)
    let canRedactOthersPublisher = timelineViewModel.context.$viewState.map(
  ...
  private func forwardItem(itemID: TimelineItemIdentifier) async {
    guard let forwardingItem = await timelineViewModel.makeForwardingItem(for: itemID) else {
      return
    }
    state.previewControllerDriver.send(.dismissDetailsSheet)
  ...
  private func updateCurrentItem(_ previewItem: TimelineMediaPreviewItem) async {
    if case .media(let item) = previewItem {
    ...
    state.currentItemActions =
      ...
      case .media(let mediaItem):
        TimelineItemMenuActionProvider(
          timelineItem: mediaItem.timelineItem,
  ...
  private func saveCurrentItem() async {
    guard case .media(let mediaItem) = state.currentItem, let fileURL = mediaItem.fileHandle?.url
    else {
  ...
  private func redactItem(_ item: TimelineMediaPreviewItem.Media) {
    timelineViewModel.context.send(
      viewAction: .handleTimelineItemMenuAction(itemID: item.timelineItem.id, action: .redact))

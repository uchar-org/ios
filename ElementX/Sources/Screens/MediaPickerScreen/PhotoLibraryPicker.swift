...
struct PhotoLibraryPicker: UIViewControllerRepresentable {
  ...
  init(
    selectionType: MediaPickerScreenSelectionType,
  ...
  final class Coordinator: NSObject, PHPickerViewControllerDelegate {
    ...
    func picker(_ picker: PHPickerViewController, didFinishPicking results: [PHPickerResult]) {
      ...
      photoLibraryPicker.userIndicatorController.submitIndicator(
        UserIndicator(
          id: Self.loadingIndicatorIdentifier,
          type: .modal,
          title: L10n.commonLoading))
      defer {
        photoLibraryPicker.userIndicatorController.retractIndicatorWithId(
    ...
    func processResult(_ result: PHPickerResult) async -> URL? {
      ...
      return await withCheckedContinuation { continuation in
        provider.loadFileRepresentation(forTypeIdentifier: contentType.type.identifier) {

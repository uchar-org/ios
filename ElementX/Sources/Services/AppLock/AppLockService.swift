...
class AppLockService: AppLockServiceProtocol {
  ...
  init(
    keychainController: KeychainControllerProtocol, appSettings: AppSettings,
  ...
  func enableBiometricUnlock() -> Result<Void, AppLockServiceError> {
    ...
    guard let state = context.evaluatedPolicyDomainState else {
      return .failure(.biometricUnlockNotSupported)
    }
    ...
    do {
      ...
      guard
        try await context.evaluatePolicy(
          unlockPolicy, localizedReason: L10n.screenAppLockBiometricUnlockReasonIos)
      else {

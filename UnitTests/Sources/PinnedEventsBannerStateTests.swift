...
struct PinnedEventsBannerStateTests {
  ...
  func loading() {
    ...
    #expect(
      state.bannerIndicatorDescription.string
        == L10n.screenRoomPinnedBannerIndicatorDescription(
          L10n.screenRoomPinnedBannerIndicator(5, 5)))
  }
  ...
  func loadingToLoaded() {
    ...
    #expect(
      state
        == .loaded(
          state: .init(
            pinnedEventContents: ["1": "test1", "2": "test2"], selectedPinnedEventID: "2")))
    #expect(!state.isLoading)
  }

  @Test
  func loaded() {
    let state = PinnedEventsBannerState.loaded(
      state: .init(pinnedEventContents: ["1": "test1", "2": "test2"], selectedPinnedEventID: "2"))
    #expect(!state.isLoading)
    #expect(!state.isEmpty)
    ...
    #expect(
      state.bannerIndicatorDescription.string
        == L10n.screenRoomPinnedBannerIndicatorDescription(
          L10n.screenRoomPinnedBannerIndicator(2, 2)))
  }

  @Test
  func previousPin() {
    var state = PinnedEventsBannerState.loaded(
      state: .init(
        pinnedEventContents: ["1": "test1", "2": "test2", "3": "test3"], selectedPinnedEventID: "1")
    )
  ...
  func setContent() {
    var state = PinnedEventsBannerState.loaded(
      state: .init(
        pinnedEventContents: ["1": "test1", "2": "test2", "3": "test3", "4": "test4"],
        selectedPinnedEventID: "2"))
    #expect(state.selectedPinnedEventID == "2")
    #expect(state.selectedPinnedIndex == 1)
    #expect(state.displayedMessage.string == "test2")
    ...
    state.setPinnedEventContents([
      "0": "test0", "1": "test1", "3": "test3", "4": "test4", "5": "test5",
    ])
    // selected item doesn't change
    #expect(state.selectedPinnedEventID == "4")

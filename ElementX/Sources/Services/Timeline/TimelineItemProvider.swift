...
class TimelineItemProvider: TimelineItemProviderProtocol {
  ...
  init(
    timeline: Timeline, kind: TimelineKind,
    paginationStatePublisher: AnyPublisher<TimelinePaginationState, Never>
  ) {
    serialDispatchQueue = DispatchQueue(
      label: "io.element.elementx.timeline_item_provider", qos: .utility)
  ...
  private func buildDiff(from diff: TimelineDiff, on itemProxies: [TimelineItemProxy])
    -> CollectionDifference<TimelineItemProxy>?
  {
    ...
    case .append(let items):
      for (index, item) in items.enumerated() {
        ...
        changes.append(
          .insert(offset: Int(itemProxies.count) + index, element: itemProxy, associatedWith: nil))
      }
    case .clear:
    ...
    case .popBack:
      ...
      changes.append(
        .remove(offset: itemProxies.count - 1, element: itemProxy, associatedWith: nil))
    case .popFront:
    ...
    case .pushBack(let item):
      ...
      changes.append(
        .insert(offset: Int(itemProxies.count), element: itemProxy, associatedWith: nil))
    ...
    case .reset(let items):
      ...
      for (index, timelineItem) in items.enumerated() {
        changes.append(
          .insert(
            offset: index, element: TimelineItemProxy(item: timelineItem), associatedWith: nil))
      }
...
extension TimelineItemProxy {
  fileprivate var isMembershipChange: Bool {
    switch self {

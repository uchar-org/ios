...
typealias RoomPollsHistoryScreenViewModelType = StateStoreViewModel<
  RoomPollsHistoryScreenViewState, RoomPollsHistoryScreenViewAction
>

class RoomPollsHistoryScreenViewModel: RoomPollsHistoryScreenViewModelType,
  RoomPollsHistoryScreenViewModelProtocol
{
  ...
  private var actionsSubject: PassthroughSubject<RoomPollsHistoryScreenViewModelAction, Never> =
    .init()
  ...
  init(
    ...
    super.init(
      initialViewState: RoomPollsHistoryScreenViewState(
        title: L10n.screenPollsHistoryTitle,
  ...
  private func sendPollResponse(pollStartID: String, optionID: String) {
    Task {
      do {
        try await pollInteractionHandler.sendPollResponse(
  ...
  private func paginateBackwards() {
    ...
    paginateBackwardsTask = Task { [weak self] in
      ...
      switch await timelineController.paginateBackwards(
        requestSize: Constants.backPaginationEventLimit)
      {

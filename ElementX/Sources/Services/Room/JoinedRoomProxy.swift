...
class JoinedRoomProxy: JoinedRoomProxyProtocol {
  ...
  private var innerPinnedEventsTimelineTask:
    Task<Result<TimelineProxyProtocol, RoomProxyError>, Never>?
  ...
  init(
    roomListService: RoomListServiceProtocol,
    room: RoomProtocol,
    ...
    timeline = try await TimelineProxy(
      timeline: room.timelineWithConfiguration(
        configuration: .init(
          focus: .live(hideThreadedEvents: appSettings.threadsEnabled),
  ...
  func subscribeToRoomInfoUpdates() {
    ...
    roomInfoObservationToken = room.subscribeToRoomInfoUpdates(
      listener: SDKListener { [weak self] roomInfo in
  ...
  > {
    do {
      let openRoomSpan = analyticsService.signpost.addSpan(
        .timelineLoad, toTransaction: .notificationToMessage)
      let sdkTimeline = try await room.timelineWithConfiguration(
        configuration: .init(
          focus: .event(
            eventId: eventID,
  ...
  func threadTimeline(eventID: String) async -> Result<TimelineProxyProtocol, RoomProxyError> {
    do {
      let sdkTimeline = try await room.timelineWithConfiguration(
        configuration: .init(
          focus: .thread(rootEventId: eventID),
          filter: .all,
          internalIdPrefix: UUID().uuidString,
          dateDividerMode: .daily,
          trackReadReceipts: .messageLikeEvents,
  ...
  func threadListService() -> RoomThreadListServiceProxyProtocol {
    RoomThreadListProxy(
      threadListService: room.threadListService(), eventStringBuilder: eventStringBuilder)
  }
  ...
  ) async -> Result<any TimelineProxyProtocol, RoomProxyError> {
    do {
      let rustFocus: MatrixRustSDK.TimelineFocus =
        switch focus {
        case .live: .live(hideThreadedEvents: false)
        case .eventID(let eventID):
          .event(
            eventId: eventID, numContextEvents: 100,
  ...
  {
    do {
      try await room.ignoreDeviceTrustAndResend(
        devices: devices, sendHandle: sendHandle.underlyingHandle)
      return .success(())
    } catch {
      MXLog.error(
        "Failed trusting devices \(devices) and resending \(sendHandle.itemID) with error: \(error)"
      )
      return .failure(.sdkError(error))
    }
  }
  ...
  {
    do {
      try await room.withdrawVerificationAndResend(
        userIds: userIDs, sendHandle: sendHandle.underlyingHandle)
      return .success(())
    } catch {
      MXLog.error(
        "Failed withdrawing verification of \(userIDs) and resending \(sendHandle.itemID) with error: \(error)"
      )
      return .failure(.sdkError(error))
    }
  }
  ...
  > {
    do {
      let updates = updates.map {
        UserPowerLevelUpdate(userId: $0.userID, powerLevel: $0.powerLevel)
      }
  ...
  {
    do {
      let handle = try room.subscribeToCallDeclineEvents(
  ...
  func matrixToPermalink() async -> Result<URL, RoomProxyError> {
    do {
      ...
      guard let url = URL(string: urlString) else {
        MXLog.error(
          "Failed creating permalink for roomID: \(id), invalid permalink URL string: \(urlString)")
  ...
  func matrixToEventPermalink(_ eventID: String) async -> Result<URL, RoomProxyError> {
    do {
      ...
      guard let url = URL(string: urlString) else {
        MXLog.error(
          "Failed creating permalink for eventID: \(eventID), invalid permalink URL string: \(urlString)"
        )
        return .failure(.invalidURL)
      }
  ...
  func startLiveLocationShare(duration: Duration) async -> Result<String, RoomProxyError> {
    do {
      let eventID = try await room.startLiveLocationShare(
        durationMillis: UInt64(duration.seconds * 1000))
  ...
  private func subscribeToTypingNotifications() {
    typingNotificationObservationToken = room.subscribeToTypingNotifications(
      listener: SDKListener { [weak self] typingUserIDs in
  ...
  private func subscribeToIdentityStatusChanges() async {
    do {
      identityStatusChangesObservationToken = try await room.subscribeToIdentityStatusChanges(
        listener: SDKListener { [weak self] changes in
          guard let self else { return }
  ...
  private func subscribeToKnockRequests() async {
    do {
      knockRequestsChangesObservationToken = try await room.subscribeToKnockRequests(
        listener: SDKListener { [weak self] requests in
          guard let self else { return }
  ...
  private static let excludedEventsFilter: TimelineEventFilter = {
    var stateEventFilters: [StateEventType] = [
      .roomCanonicalAlias,
      .roomGuestAccess,
    ...
    return .excludeEventTypes(
      eventTypes: stateEventFilters.map { FilterTimelineEventType.state(eventType: $0) })
  }()
}

...
struct ConfigureNightly: AsyncParsableCommand {
  static let configuration = CommandConfiguration(
    abstract: "Configures the project for a Nightly build.",
    discussion:
      "Adds the Nightly variant to project.yml, updates secrets, runs xcodegen, and generates the app icon banner."
  )
  ...
  private func addNightlyVariant() throws {
    ...
    if projectConfig["include"]?.sequence?.contains(where: {
      $0.mapping?["path"] == "Variants/Nightly/nightly.yml"
    }) == false {

...
final class MediaUploadingPreprocessorTests {
  ...
  func audioFileProcessing() async throws {
    let url = try #require(
      Bundle(for: Self.self).url(forResource: "test_audio.mp3", withExtension: nil),
      "Failed retrieving test asset")

    guard
      case .success(let result) = await mediaUploadingPreprocessor.processMedia(
        at: url, maxUploadSize: maxUploadSize),
      case .audio(let audioURL, let audioInfo) = result
    else {
  ...
  func landscapeMovVideoProcessing() async throws {
    let url = try #require(
      Bundle(for: Self.self).url(forResource: "landscape_test_video.mov", withExtension: nil),
      "Failed retrieving test asset")

    guard
      case .success(let result) = await mediaUploadingPreprocessor.processMedia(
        at: url, maxUploadSize: maxUploadSize),
      case .video(let videoURL, let thumbnailURL, let videoInfo) = result
    else {
      Issue.record("Failed processing asset")
      return
    }
    ...
    #expect(
      videoURL.pathExtension == "mp4", "The file extension should match the container we use.")
    ...
    #expect(
      thumbnail.size.height <= MediaUploadingPreprocessor.Constants.maximumThumbnailSize.height)
    ...
    guard
      case .success(let optimizedResult) = await mediaUploadingPreprocessor.processMedia(
        at: url, maxUploadSize: maxUploadSize),
      case .video(let optimizedVideoURL, _, let optimizedVideoInfo) = optimizedResult
    else {
      Issue.record("Failed processing asset")
      return
    }
    ...
    #expect(
      optimizedVideoURL.pathExtension == "mp4",
      "The file extension should match the container we use.")
  ...
  func portraitMp4VideoProcessing() async throws {
    let url = try #require(
      Bundle(for: Self.self).url(forResource: "portrait_test_video.mp4", withExtension: nil),
      "Failed retrieving test asset")
  ...
  func landscapeImageProcessing() async throws {
    let url = try #require(
      Bundle(for: Self.self).url(forResource: "landscape_test_image.jpg", withExtension: nil),
      "Failed retrieving test asset")

    guard
      case .success(let result) = await mediaUploadingPreprocessor.processMedia(
        at: url, maxUploadSize: maxUploadSize),
      case .image(let convertedImageURL, let thumbnailURL, let imageInfo) = result
    else {
      Issue.record("Failed processing asset")
      return
    }
    ...
    try compare(
      originalImageAt: url, toConvertedImageAt: convertedImageURL, withThumbnailAt: thumbnailURL)
    ...
    guard
      case .success(let optimizedResult) = await mediaUploadingPreprocessor.processMedia(
        at: url, maxUploadSize: maxUploadSize),
      case .image(let optimizedImageURL, let thumbnailURL, let optimizedImageInfo) = optimizedResult
    else {
      Issue.record("Failed processing asset")
      return
    }
    ...
    try compare(
      originalImageAt: url, toConvertedImageAt: optimizedImageURL, withThumbnailAt: thumbnailURL)
  ...
  func portraitImageProcessing() async throws {
    let url = try #require(
      Bundle(for: Self.self).url(forResource: "portrait_test_image.jpg", withExtension: nil),
      "Failed retrieving test asset")
  ...
  func pngImageProcessing() async throws {
    let url = try #require(
      Bundle(for: Self.self).url(forResource: "test_image.png", withExtension: nil),
      "Failed retrieving test asset")

    guard
      case .success(let result) = await mediaUploadingPreprocessor.processMedia(
        at: url, maxUploadSize: maxUploadSize),
      case .image(let convertedImageURL, _, let imageInfo) = result
    else {
      Issue.record("Failed processing asset")
      return
    }
    ...
    #expect(
      mimeType(from: convertedImageURL) == "image/png",
      "PNGs should always be sent as PNG to preserve the alpha channel.")
    #expect(
      convertedImageURL.pathExtension == "png", "The file extension should match the MIME type.")
    ...
    guard
      case .success(let optimizedResult) = await mediaUploadingPreprocessor.processMedia(
        at: url, maxUploadSize: maxUploadSize),
      case .image(let optimizedImageURL, _, let optimizedImageInfo) = optimizedResult
    else {
      Issue.record("Failed processing asset")
      return
    }
    ...
    // Make sure the output file matches the image info.
    #expect(
      mimeType(from: optimizedImageURL) == "image/png",
      "PNGs should always be sent as PNG to preserve the alpha channel.")
    #expect(
      optimizedImageURL.pathExtension == "png", "The file extension should match the MIME type.")
  ...
  func heicImageProcessing() async throws {
    let url = try #require(
      Bundle(for: Self.self).url(forResource: "test_apple_image.heic", withExtension: nil),
      "Failed retrieving test asset")
    ...
    // Make sure the output file matches the image info.
    #expect(
      mimeType(from: convertedImageURL) == "image/heic",
      "Unoptimised HEICs should always be sent as is.")
    #expect(
      convertedImageURL.pathExtension == "heic", "The file extension should match the MIME type.")
    ...
    // Make sure the output file matches the image info.
    #expect(
      mimeType(from: optimizedImageURL) == "image/jpeg",
      "Optimised HEICs should always be converted to JPEG for compatibility.")
    #expect(
      optimizedImageURL.pathExtension == "jpeg", "The file extension should match the MIME type.")
  ...
  func gifImageProcessing() async throws {
    let url = try #require(
      Bundle(for: Self.self).url(forResource: "test_animated_image.gif", withExtension: nil),
      "Failed retrieving test asset")
    let originalSizeValue = try UInt64(FileManager.default.sizeForItem(at: url))
    let originalSize = try #require(
      originalSizeValue > 0 ? originalSizeValue : nil, "File size must be greater than zero")
    ...
    // Make sure the output file matches the image info.
    #expect(
      mimeType(from: convertedImageURL) == "image/gif",
      "GIFs should always be sent as GIF to preserve the animation.")
    #expect(
      convertedImageURL.pathExtension == "gif", "The file extension should match the MIME type.")
    ...
    // Make sure the output file matches the image info.
    #expect(
      mimeType(from: optimizedImageURL) == "image/gif",
      "GIFs should always be sent as GIF to preserve the animation.")
    #expect(
      optimizedImageURL.pathExtension == "gif", "The file extension should match the MIME type.")
  ...
  func rotatedImageProcessing() async throws {
    let url = try #require(
      Bundle(for: Self.self).url(forResource: "test_rotated_image.jpg", withExtension: nil),
      "Failed retrieving test asset")
  ...
  private func isEqual<N: SignedNumeric & Comparable>(_ lhs: N, _ rhs: N, within tolerance: N)
    -> Bool
  {
  ...
  private func compare(
    originalImageAt originalImageURL: URL, toConvertedImageAt convertedImageURL: URL,
    ...
    if appSettings.optimizeMediaUploads {
      ...
      #expect(
        convertedImage.size.width <= MediaUploadingPreprocessor.Constants.optimizedMaxPixelSize)
      #expect(
        convertedImage.size.height <= MediaUploadingPreprocessor.Constants.optimizedMaxPixelSize)
    } else {
      // Check that the file name is preserved
    ...
    if thumbnail.size.width > thumbnail.size.height {
      #expect(
        thumbnail.size.width <= MediaUploadingPreprocessor.Constants.maximumThumbnailSize.width)
      #expect(
        thumbnail.size.height <= MediaUploadingPreprocessor.Constants.maximumThumbnailSize.height)
    } else {
      #expect(
        thumbnail.size.width <= MediaUploadingPreprocessor.Constants.maximumThumbnailSize.height)
      #expect(
        thumbnail.size.height <= MediaUploadingPreprocessor.Constants.maximumThumbnailSize.width)
    }
  ...
  private func metadata(from imageData: Data) throws -> NSDictionary {
    let imageSource = try #require(
      CGImageSourceCreateWithData(imageData as NSData, nil), "Invalid asset")
    return try #require(
      CGImageSourceCopyPropertiesAtIndex(imageSource, 0, nil) as NSDictionary?,
      "Test asset is expected to contain metadata")
  }

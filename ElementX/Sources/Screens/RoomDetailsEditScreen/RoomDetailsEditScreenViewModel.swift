...
typealias RoomDetailsEditScreenViewModelType = StateStoreViewModel<
  RoomDetailsEditScreenViewState, RoomDetailsEditScreenViewAction
>

class RoomDetailsEditScreenViewModel: RoomDetailsEditScreenViewModelType,
  RoomDetailsEditScreenViewModelProtocol
{
  private let actionsSubject: PassthroughSubject<RoomDetailsEditScreenViewModelAction, Never> =
    .init()
  ...
  init(
    ...
    super.init(
      initialViewState: RoomDetailsEditScreenViewState(
        roomID: roomProxy.id,
        isSpace: isSpace,
  ...
  func didSelectMediaUrl(url: URL) {
    Task {
      ...
      let mediaResult = await mediaUploadingPreprocessor.processMedia(

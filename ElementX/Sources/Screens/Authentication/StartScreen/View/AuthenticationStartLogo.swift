  ...
  var body: some View {
    if let sizeMetrics {
      appLogoImage
        ...
        .modifier(
          AuthenticationBrandLogoModifier(
            scale: sizeMetrics.scale,
    ...
    content
      .background {
        Circle()
          .inset(by: 1 * scale)
          .shadow(
            color: .black.opacity(!isLight && isOnGradient ? 0.3 : 0.4),
        ...
        ZStack {
          if !isLight, isOnGradient {
            ...
            outerShape
              ...
              .padding(extra)
              .shadow(
                color: outerShapeShadowColor.opacity(isLight ? 0.23 : 0.08),
                radius: 16 * scale,
                y: 8 * scale)

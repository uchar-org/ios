...
extension URL {
  ...
  static var sessionsBaseDirectory: URL {
    let applicationSupportSessionsURL = applicationSupportBaseDirectory.appending(
      component: "Sessions", directoryHint: .isDirectory)
  ...
  var globalProxy: String? {
    ...
    let proxies =
      CFNetworkCopyProxiesForURL(self as CFURL, proxySettings).takeRetainedValue()
      as? [[CFString: Any]]
...
extension ConfirmURLParameters {
  init?(queryItems: [URLQueryItem]) {
    guard
      let internalURLString = queryItems.first(where: { $0.name == Self.internalURLKey })?.value,
      let internalURL = URL(string: internalURLString),
      let externalURLString = queryItems.first(where: { $0.name == Self.displayStringKey })?.value
    else {
      return nil
    }

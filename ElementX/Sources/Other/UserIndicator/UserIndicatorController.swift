...
class UserIndicatorController: ObservableObject, UserIndicatorControllerProtocol {
  ...
  var window: UIWindow? {
    didSet {
      let hostingController = UIHostingController(
        rootView: UserIndicatorPresenter(userIndicatorController: self).statusBarHidden(
  ...
  func retractIndicatorWithId(_ id: String) {
    ...
    guard let displayTime = displayTimes[id],
      abs(displayTime.timeIntervalSinceNow) <= minimumDisplayDuration
    else {

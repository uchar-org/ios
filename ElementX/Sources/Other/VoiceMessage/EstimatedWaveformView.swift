  ...
  func normalisedData(maxSamplesCount: Int) -> [Float] {
    ...
    if data.count > maxSamplesCount {
      result = (0..<maxSamplesCount)
        .map { index in
          let targetIndex = Int(
            (Double(index) * (Double(data.count) / Double(maxSamplesCount))).rounded())
...
extension EstimatedWaveform {
  static let mockWaveform = EstimatedWaveform(data: [
    0, 0, 0, 3, 3, 127, 400, 266, 126, 122, 373, 251, 45, 112,
...
struct EstimatedWaveformView: View {
  ...
  var body: some View {
    GeometryReader { geometry in
      ...
      .stroke(
        Color.compound.iconSecondary, style: StrokeStyle(lineWidth: lineWidth, lineCap: .round)
      )

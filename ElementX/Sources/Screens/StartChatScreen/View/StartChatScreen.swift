...
struct StartChatScreen: View {
  ...
  private var joinRoomByAddressSection: some View {
    Section {
      ListRow(
        label: .default(
          title: L10n.screenStartChatJoinRoomByAddressAction,
          icon: \.room),
        kind: .button {
  ...
  private var roomDirectorySearch: some View {
    Section {
      ListRow(
        label: .default(
          title: L10n.screenRoomDirectorySearchTitle,
          icon: \.listBulleted),
        kind: .navigationLink {
  ...
  private var createRoomSection: some View {
    Section {
      ListRow(
        label: .default(
          title: L10n.screenCreateRoomActionCreateRoom,
          icon: \.plus),
        ...
        kind: .custom {
          MatrixUserShareLink(userID: context.viewState.userID) {
            ListRowLabel.default(
              title: L10n.actionInvitePeopleToApp(InfoPlistReader.main.bundleDisplayName),
              icon: \.shareIos)
          }
        }
      )
...
struct StartChatScreen_Previews: PreviewProvider, TestablePreview {
  static let viewModel = {
    ...
    let userSession = UserSessionMock(
      .init(clientProxy: ClientProxyMock(.init(userID: "@userid:example.com"))))

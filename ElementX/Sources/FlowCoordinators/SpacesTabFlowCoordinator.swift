...
class SpacesTabFlowCoordinator: FlowCoordinatorProtocol {
  ...
  init(navigationSplitCoordinator: NavigationSplitCoordinator, flowParameters: CommonFlowParameters)
  {
    ...
    detailNavigationStackCoordinator = NavigationStackCoordinator(
      navigationSplitCoordinator: navigationSplitCoordinator)
  ...
  private func configureStateMachine() {
    stateMachine.addRoutes(
      event: .start, transitions: [.initial => .spacesScreen(selectedSpaceID: nil)]
    ) { [weak self] _ in
    ...
    stateMachine.addRouteMapping { event, fromState, userInfo in
      ...
      guard let spaceRoomListProxy = userInfo as? SpaceRoomListProxyProtocol else {
        fatalError("A space proxy must be provided.")
      }
      return .spacesScreen(selectedSpaceID: spaceRoomListProxy.id)
    } handler: { [weak self] context in
      guard let self, let spaceRoomListProxy = context.userInfo as? SpaceRoomListProxyProtocol
      else { return }
    ...
    stateMachine.addRouteMapping { event, fromState, userInfo in
      guard event == .dismissedCreateSpaceFlow, case .createSpaceFlow = fromState else {
        return nil
      }
  ...
  private func presentSpacesScreen() {
    let parameters = SpacesScreenCoordinatorParameters(

...
typealias BugReportScreenViewModelType = StateStoreViewModelV2<
  BugReportScreenViewState, BugReportScreenViewAction
>
...
class BugReportScreenViewModel: BugReportScreenViewModelType, BugReportScreenViewModelProtocol {
  ...
  init(
    ...
    let bindings = BugReportScreenViewStateBindings(
      reportText: "", sendingLogsEnabled: canSendLogFiles, canContact: false)
    super.init(
      initialViewState: BugReportScreenViewState(
        canSendLogFiles: canSendLogFiles,
  ...
  override func process(viewAction: BugReportScreenViewAction) {
    ...
    case .attachScreenshot(let image):
  ...
  private static func validate(_ logFiles: [URL]) -> Bool {
    for fileURL in logFiles {
      if let size = try? FileManager.default.sizeForItem(at: fileURL),
        size > 1024 * 1024 * 1024
      {  // Consider anything over 1GB to be excessive.
        return false
      }
    }
    return true
  ...
  private func submitBugReport() async {
    ...
    actionsSubject.send(
      .submitStarted(progressPublisher: progressSubject.asCurrentValuePublisher()))
    ...
    switch await bugReportService.submitBugReport(
      bugReport,
      progressListener: progressSubject)
    {
    case .success(let response):
      MXLog.info(
        "Submission uploaded: \(response.reportURL ?? "The server didn't generate a report URL")")

...
extension AVMetadataMachineReadableCodeObject {
  var qrBinaryValue: Data? {
    get throws(AVMetadataBinaryValueError) {
      ...
      return try Self.removeQRProtocolData(
        qrCodeDescriptor.errorCorrectedPayload, symbolVersion: qrCodeDescriptor.symbolVersion)
    }
  }

  static func removeQRProtocolData(_ input: Data, symbolVersion: Int)
    throws(AVMetadataBinaryValueError) -> Data?
  {
    var bits = input.bits()
    var segment: [UInt8]
    var output: [UInt8] = []
  ...
  private static func takeSegment(_ input: inout [Bit], version: Int)
    throws(AVMetadataBinaryValueError) -> [UInt8]
  {
    do {
...
extension [Bit] {
  fileprivate mutating func takeBits(_ count: Int) throws(BitError) -> UInt8 {
  ...
  fileprivate mutating func takeBits16(_ count: Int) throws(BitError) -> UInt16 {
  ...
  fileprivate mutating func takeUInt8() throws(BitError) -> UInt8 {
    try takeBits(8)
  }

  fileprivate mutating func takeUInt16() throws(BitError) -> UInt16 {
    try takeBits16(16)
  }

  fileprivate mutating func takeBinarySegment(_ version: Int) throws(BitError) -> [UInt8] {
  ...
  fileprivate mutating func takeAlphanumericSegment(_ version: Int) throws(BitError) -> [UInt8] {

...
struct VoiceMessageRecorderTests {
    ...
    func sendVoiceMessage_NoRecordingFile() async {
        ...
        audioRecorder.audioFileURL = nil
        guard
            case .failure(.missingRecordingFile) = await voiceMessageRecorder.sendVoiceMessage(timelineController: timelineController,
    ...
    func sendVoiceMessage_ConversionError() async {
        ...
        let timelineController = MockTimelineController()
        guard
            case .failure(.failedSendingVoiceMessage) = await voiceMessageRecorder.sendVoiceMessage(timelineController: timelineController,
                                                                                                    audioConverter: audioConverter)
        else {
            Issue.record("An error is expected")
            return
    ...
    func sendVoiceMessage() async throws {
        ...
        guard
            case .success = await voiceMessageRecorder.sendVoiceMessage(timelineController: timelineController, audioConverter: audioConverter)
        else {

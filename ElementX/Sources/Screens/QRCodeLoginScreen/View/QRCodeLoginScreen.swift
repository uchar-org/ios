...
struct QRCodeLoginScreen: View {
  ...
  var mainContent: some View {
    ...
    case .error(let errorState):
      QRCodeErrorView(
        errorState: errorState, canSignInManually: context.viewState.canSignInManually
      ) { action in
  ...
  private var loginInstructionsContent: some View {
    FullscreenDialog(topPadding: 24, horizontalPadding: 24) {
      VStack(alignment: .leading, spacing: 40) {
        TitleAndIcon(
          title: L10n.screenQrCodeLoginInitialStateTitle(InfoPlistReader.main.productionAppName),
  ...
  private var linkDesktopInstructionsContent: some View {
    FullscreenDialog(topPadding: 24, horizontalPadding: 24) {
      VStack(alignment: .leading, spacing: 40) {
        TitleAndIcon(
          title: L10n.screenLinkNewDeviceDesktopTitle(InfoPlistReader.main.productionAppName),
          icon: \.computer,
          iconStyle: .default)
  ...
  private var displayCodeContent: some View {
    if case .displayCode(let displayCodeState) = context.viewState.state {
      FullscreenDialog(topPadding: 24) {
        VStack(spacing: 32) {
          VStack(spacing: 24) {
            ...
            PINTextField(
              pinCode: .constant(displayCodeState.code),
  ...
  private func displayCodeHeader(state: QRCodeLoginState.DisplayCodeState) -> some View {
    ...
    case .deviceCode:
      TitleAndIcon(
        title: L10n.screenQrCodeLoginDeviceCodeTitle,
        subtitle: L10n.screenQrCodeLoginDeviceCodeSubtitle,
        icon: \.computer,
        iconStyle: .default)
    case .verificationCode:
      TitleAndIcon(
        title: L10n.screenQrCodeLoginVerifyCodeTitle,
  ...
  private var qrScannerContent: some View {
    FullscreenDialog(topPadding: 24) {
      VStack(spacing: 40) {
        TitleAndIcon(
          title: L10n.screenQrCodeLoginScanningStateTitle,
          icon: \.takePhotoSolid,
          iconStyle: .default)
  ...
  private var qrScannerFooter: some View {
    if case .scan(let scanState) = context.viewState.state {
      switch scanState {
      case .connecting:
        VStack(spacing: 4) {
          ProgressView()
  ...
  private var displayQRContent: some View {
    if case .displayQR(let image) = context.viewState.state {
      FullscreenDialog(topPadding: 24, horizontalPadding: 24) {
        VStack(spacing: 32) {
          TitleAndIcon(
            title: L10n.screenLinkNewDeviceMobileTitle(InfoPlistReader.main.productionAppName),
            icon: \.takePhotoSolid,
            iconStyle: .default)
  ...
  private var confirmCodeContent: some View {
    if case .confirmCode(let confirmCode) = context.viewState.state {
      FullscreenDialog(topPadding: 24, horizontalPadding: 24) {
        VStack(spacing: 24) {
          TitleAndIcon(
            title: L10n.screenLinkNewDeviceEnterNumberTitle,
          ...
          VStack(spacing: 10) {
            ...
            if case .confirmCode(.invalidCode) = context.viewState.state {
              Label(
                L10n.screenLinkNewDeviceEnterNumberErrorNumbersDoNotMatch,
                icon: \.errorSolid,
                iconSize: .medium,
                relativeTo: .compound.bodyMDSemibold
              )
  ...
  var body: some View {
    Rectangle()
      .stroke(
        ...
        style: StrokeStyle(
          lineWidth: 6.0, lineCap: .square, dash: [dashLength, emptyLength], dashPhase: dashPhase))
  }
}
...
struct QRCodeLoginScreen_Previews: PreviewProvider, TestablePreview {
  // Instructions
  static let loginInstructionsStateViewModel = QRCodeLoginScreenViewModel.mock(
    state: .loginInstructions)
  static let linkInstructionsStateViewModel = QRCodeLoginScreenViewModel.mock(
    state: .linkDesktopInstructions)
  ...
  static let invalidStateViewModel = QRCodeLoginScreenViewModel.mock(
    state: .scan(.scanFailed(.invalid)))

  static let notAllowedStateViewModel = QRCodeLoginScreenViewModel.mock(
    state: .scan(
      .scanFailed(
        .notAllowed(
          scannedProvider: "evil.com",
          allowedProviders: [
            "example.com",
            "server.net",
          ]))))

  static let deviceNotSignedInStateViewModel = QRCodeLoginScreenViewModel.mock(
    state: .scan(.scanFailed(.deviceNotSignedIn)))

  /// Showing
  static let showingStateViewModel = QRCodeLoginScreenViewModel.mock(
    state: .displayQR(LinkNewDeviceServiceMock.mockQRCodeImage))

  // Displaying codes
  static let deviceCodeStateViewModel = QRCodeLoginScreenViewModel.mock(
    state: .displayCode(.deviceCode("12")))
  static let verificationCodeStateViewModel = QRCodeLoginScreenViewModel.mock(
    state: .displayCode(.verificationCode("123456")))

  static let confirmCodeStateViewModel = QRCodeLoginScreenViewModel.mock(
    state: .confirmCode(
      .inputCode(CheckCodeSenderProxy(underlyingSender: CheckCodeSenderSDKMock()))))
  static let confirmCodeEnteredStateViewModel = QRCodeLoginScreenViewModel.mock(
    state: .confirmCode(
      .inputCode(CheckCodeSenderProxy(underlyingSender: CheckCodeSenderSDKMock()))),
    checkCodeInput: "12")
  static let confirmCodeInvalidStateViewModel = QRCodeLoginScreenViewModel.mock(
    state: .confirmCode(.invalidCode))

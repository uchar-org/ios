...
typealias RoomDetailsScreenViewModelType = StateStoreViewModelV2<
  RoomDetailsScreenViewState, RoomDetailsScreenViewAction
>
...
class RoomDetailsScreenViewModel: RoomDetailsScreenViewModelType, RoomDetailsScreenViewModelProtocol
{
  ...
  private var pinnedEventsTimelineItemProvider: TimelineItemProviderProtocol? {
    didSet {
      ...
      state.pinnedEventsActionState = .loaded(
        numberOfItems: pinnedEventsTimelineItemProvider.itemProxies.filter(\.isEvent).count)
      ...
      pinnedEventsTimelineItemProvider.updatePublisher
        ...
        .sink { [weak self] updatedItems, _ in
          self?.state.pinnedEventsActionState = .loaded(
            numberOfItems: updatedItems.filter(\.isEvent).count)
        }
  ...
  init(
    ...
    Task {
      if case .success(let permalinkURL) = await roomProxy.matrixToPermalink() {
  ...
  private func processLeaveSpace() async {
    ...
    case .success(let leaveHandle):
      let leaveSpaceViewModel = LeaveSpaceViewModel(
        spaceName: state.details.name ?? state.details.id,
  ...
  private func setupRoomSubscription() {
    ...
    roomProxy.knockRequestsStatePublisher
      .map { requestsState in
        guard case .loaded(let requests) = requestsState else {
          return 0
        }
  ...
  private func updateMemberIdentityVerificationStates() async {
    ...
    if roomProxy.isDirectOneToOneRoom {
      if var dmRecipientInfo = state.dmRecipientInfo {
        if case .success(let userIdentity) = await userSession.clientProxy.userIdentity(
          for: dmRecipientInfo.member.id, fallBackToServer: true)
        {
      ...
      for member in roomProxy.membersPublisher.value {
        if case .success(let userIdentity) = await userSession.clientProxy.userIdentity(
          for: member.userID, fallBackToServer: false)
        {
  ...
  private func fetchRoomNotificationSettings() async {
    do {
      let notificationMode = try await notificationSettingsProxy.getNotificationSettings(
        roomId: roomProxy.id,
        isEncrypted: roomProxy.infoPublisher.value.isEncrypted,
  ...
  private func toggleMuteNotifications() async {
    ...
    case .mute:
      do {
        try await notificationSettingsProxy.unmuteRoom(
          roomId: roomProxy.id,
          isEncrypted: roomProxy.infoPublisher.value.isEncrypted,
          isOneToOne: roomProxy.infoPublisher.value.activeMembersCount == 2)
      } catch {
  ...
  private func toggleFavourite(_ isFavourite: Bool) async {
    if case .failure(let error) = await roomProxy.flagAsFavourite(isFavourite) {
  ...
  private func displayFullScreenAvatar(_ url: URL) {
    ...
    Task {
      ...
      if let mediaSource = try? MediaSourceProxy(url: url, mimeType: "image/jpeg"),
        case .success(let file) = await userSession.mediaProvider.loadFileFromSource(mediaSource)
      {
        state.bindings.mediaPreviewItem = MediaPreviewItem(
          file: file, title: roomProxy.infoPublisher.value.displayName)
      }
    }
  }
  ...
  private func setupPinnedEventsTimelineItemProviderIfNeeded() {
    ...
    Task {
      guard case .success(let pinnedEventsTimeline) = await roomProxy.pinnedEventsTimeline() else {
        return
      }
...
extension AttributedString {
  ...
  fileprivate func unattributedStringByReplacingNewlinesWithSpaces() -> AttributedString {

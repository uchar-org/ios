...
extension PreviewTests {
  ...
  func authenticationClassicAppBackupInstructionsView() async throws {
    AppSettings.resetAllSettings()  // Ensure this test's previews start with fresh settings.
    for (index, preview) in AuthenticationClassicAppBackupInstructionsView_Previews._allPreviews
      .enumerated()
    {
      try await assertSnapshots(matching: preview, step: index)
    }
  ...
  func homeScreenRecoveryKeyConfirmationBanner() async throws {
    AppSettings.resetAllSettings()  // Ensure this test's previews start with fresh settings.
    for (index, preview) in HomeScreenRecoveryKeyConfirmationBanner_Previews._allPreviews
      .enumerated()
    {
      try await assertSnapshots(matching: preview, step: index)
    }
  ...
  func notificationSettingsEditScreenRoomCell() async throws {
    AppSettings.resetAllSettings()  // Ensure this test's previews start with fresh settings.
    for (index, preview) in NotificationSettingsEditScreenRoomCell_Previews._allPreviews
      .enumerated()
    {
      try await assertSnapshots(matching: preview, step: index)
    }
  ...
  func roomNotificationSettingsCustomSectionView() async throws {
    AppSettings.resetAllSettings()  // Ensure this test's previews start with fresh settings.
    for (index, preview) in RoomNotificationSettingsCustomSectionView_Previews._allPreviews
      .enumerated()
    {
      try await assertSnapshots(matching: preview, step: index)
    }
  ...
  func roomNotificationSettingsUserDefinedScreen() async throws {
    AppSettings.resetAllSettings()  // Ensure this test's previews start with fresh settings.
    for (index, preview) in RoomNotificationSettingsUserDefinedScreen_Previews._allPreviews
      .enumerated()
    {
      try await assertSnapshots(matching: preview, step: index)
    }
  ...
  func timelineMediaPreviewRedactConfirmationView() async throws {
    AppSettings.resetAllSettings()  // Ensure this test's previews start with fresh settings.
    for (index, preview) in TimelineMediaPreviewRedactConfirmationView_Previews._allPreviews
      .enumerated()
    {
      try await assertSnapshots(matching: preview, step: index)
    }

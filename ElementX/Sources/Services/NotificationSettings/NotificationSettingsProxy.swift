...
final class NotificationSettingsProxy: NotificationSettingsProxyProtocol {
  ...
  {
    let roomMotificationSettings = try await notificationSettings.getRoomNotificationSettings(
  ...
  func setNotificationMode(roomId: String, mode: RoomNotificationModeProxy) async throws {
    try await notificationSettings.setRoomNotificationMode(
      roomId: roomId, mode: mode.roomNotificationMode)
    await updatedSettings()
  }

  func getUserDefinedRoomNotificationMode(roomId: String) async throws -> RoomNotificationModeProxy?
  {
    let roomNotificationMode = try await notificationSettings.getUserDefinedRoomNotificationMode(
      roomId: roomId)
  ...
  {
    let roomNotificationMode = await notificationSettings.getDefaultRoomNotificationMode(
  ...
  func setDefaultRoomNotificationMode(
    ...
    do {
      try await notificationSettings.setDefaultRoomNotificationMode(
        isEncrypted: isEncrypted, isOneToOne: isOneToOne, mode: mode.roomNotificationMode)
  ...
  func unmuteRoom(roomId: String, isEncrypted: Bool, isOneToOne: Bool) async throws {
    try await notificationSettings.unmuteRoom(
      roomId: roomId, isEncrypted: isEncrypted, isOneToOne: isOneToOne)
    await updatedSettings()
  }

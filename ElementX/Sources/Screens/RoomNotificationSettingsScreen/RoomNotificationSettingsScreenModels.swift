...
struct RoomNotificationSettingsScreenViewState: BindableState {
  ...
  var availableCustomRoomNotificationModes: [RoomNotificationModeProxy] = [
    .allMessages, .mentionsAndKeywordsOnly, .mute,
  ]
  ...
  func description(mode: RoomNotificationModeProxy) -> String? {
    guard mode == .mentionsAndKeywordsOnly,
      shouldDisplayMentionsOnlyDisclaimer
    else {
      return nil
    }
  ...
  func isSelected(mode: RoomNotificationModeProxy) -> Bool {
    if case .loaded(let settings) = notificationSettingsState, settings.mode == mode,
...
struct RoomNotificationSettingsScreenStrings {
  ...
  init() {
    ...
    var customSettingFootnote = AttributedString(
      L10n.screenRoomNotificationSettingsDefaultSettingFootnote(linkPlaceholder))
    var linkString = AttributedString(
      L10n.screenRoomNotificationSettingsDefaultSettingFootnoteContentLink)

...
class ClassicAppAccountManager {
  ...
  func loadAccounts() {
    ...
    if FileManager.default.fileExists(atPath: accountFile.path(percentEncoded: false)) {
      ...
      do {
        ...
        guard let mxAccounts = decoder.decodeObject(forKey: "mxAccounts") as? [ClassicAppMXAccount]
        else {
        ...
        MXLog.info(
          "\(mxAccounts.count) accounts loaded in \(Date().timeIntervalSince(startDate) * 1000)ms.")
  ...
  private func loadUser(for mxAccount: ClassicAppMXAccount) -> ClassicAppMXUser? {
    ...
    do {
      ...
      let groupUsers =
        decoder.decodeObject(forKey: NSKeyedArchiveRootObjectKey) as? [String: ClassicAppMXUser]
  ...
  func accountFile() -> URL {
    cacheFolder.appending(component: Self.matrixKitFolder).appending(
      component: Self.kMXKAccountsKey)
  }

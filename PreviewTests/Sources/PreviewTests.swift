...
struct PreviewTests {
  ...
  private let snapshotDevices: [SnapshotDevice] = [
    .init(name: "iPhone", device: "iPhone 17"),
  ...
  private func checkEnvironments() {
    if let simulatorDevice {
      ...
      guard deviceModel?.contains(simulatorDevice) ?? false else {
        fatalError(
          "\(deviceModel ?? "Unknown") is the wrong one. Switch to using \(simulatorDevice) for these tests."
        )
      }
    }

    let osVersion = ProcessInfo().operatingSystemVersion
    guard osVersion.majorVersion == requiredOSVersion.major,
      osVersion.minorVersion == requiredOSVersion.minor
    else {
  ...
  func assertSnapshots(
    ...
    let preferenceReadingView = preview.content
      ...
      .onPreferenceChange(SnapshotPerceptualPrecisionPreferenceKey.self) {
        preferences.perceptualPrecision = $0
      }
      .onPreferenceChange(SnapshotFulfillmentPreferenceKey.self) {
        preferences.fulfillmentSource = $0?.source
      }
...
extension PreviewDevice {
  fileprivate func snapshotDevice() -> ViewImageConfig? {
    switch rawValue {
    case "iPhone 17", "iPhone 16", "iPhone 15", "iPhone 14", "iPhone 13", "iPhone 12", "iPhone 11",
      "iPhone 10":
      return .iPhoneX
...
extension Snapshotting where Value: SwiftUI.View, Format == UIImage {
  fileprivate static func prefireImage(
    ...
    return SimplySnapshotting<UIImage>(
      pathExtension: "png",
      diffing: .prefireImage(preferences: preferences, scale: traits.displayScale)
    )
...
extension Diffing where Value == UIImage {
  fileprivate static func prefireImage(preferences: SnapshotPreferences, scale: CGFloat?) -> Diffing
  {
    lazy var originalDiffing = Diffing.image(
      precision: preferences.precision, perceptualPrecision: preferences.perceptualPrecision,
      scale: scale)
    return Diffing(
...
extension UIEdgeInsets {
  ...
  fileprivate static var one: UIEdgeInsets {

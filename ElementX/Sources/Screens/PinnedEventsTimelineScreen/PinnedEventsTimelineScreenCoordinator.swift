...
final class PinnedEventsTimelineScreenCoordinator: CoordinatorProtocol {
  ...
  private let actionsSubject:
    PassthroughSubject<PinnedEventsTimelineScreenCoordinatorAction, Never> = .init()
  ...
  func start() {
    viewModel.actionsPublisher.sink { [weak self] action in
      ...
      case .viewInRoomTimeline(let eventID, let threadRootEventID):
        actionsSubject.send(
          .displayRoomScreenWithFocussedPin(eventID: eventID, threadRootEventID: threadRootEventID))
      case .dismiss:
    ...
    timelineViewModel.actions.sink { [weak self] action in
      ...
      case .displayEmojiPicker, .displayReportContent, .displayCameraPicker, .displayMediaPicker,
        .displayDocumentPicker, .displayLocationPicker, .displayPollForm,
  ...
  func toPresentable() -> AnyView {
    AnyView(
      PinnedEventsTimelineScreen(
        context: viewModel.context, timelineContext: timelineViewModel.context))
  }
}

...
struct AppLockScreen: View {
    ...
    var pinInputField: some View {
        HStack(spacing: 24) {
            ...
            Circle()
                .fill(context.viewState.numberOfDigitsEntered > 0
                    ? .compound.iconPrimary : .compound.bgSubtlePrimary)
                .scaledFrame(size: pinDotSize)
            Circle()
                .fill(context.viewState.numberOfDigitsEntered > 1
                    ? .compound.iconPrimary : .compound.bgSubtlePrimary)
                .scaledFrame(size: pinDotSize)
            Circle()
                .fill(context.viewState.numberOfDigitsEntered > 2
                    ? .compound.iconPrimary : .compound.bgSubtlePrimary)
                .scaledFrame(size: pinDotSize)
            Circle()
                .fill(context.viewState.numberOfDigitsEntered > 3
                    ? .compound.iconPrimary : .compound.bgSubtlePrimary)
                .scaledFrame(size: pinDotSize)
        }
    }

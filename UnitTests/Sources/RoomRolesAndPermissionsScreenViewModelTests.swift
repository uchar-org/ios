...
struct RoomRolesAndPermissionsScreenViewModelTests {
    ...
    mutating func demoteToModerator() async throws {
        ...
        context.alertInfo?.verticalButtons?.first { $0.title.localizedStandardContains("moderator") }?
            .action?()

        try await Task.sleep(for: .milliseconds(100))

        #expect(roomProxy.updatePowerLevelsForUsersCalled)
        #expect(roomProxy.updatePowerLevelsForUsersReceivedUpdates?.first?.powerLevel
    ...
    mutating func demoteToMember() async throws {
        ...
        context.alertInfo?.verticalButtons?.first { $0.title.localizedStandardContains("member") }?
            .action?()

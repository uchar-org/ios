...
struct RoomMembersFlowCoordinatorTests {
  ...
  mutating func clearRoute() async throws {
    ...
    var membersFlowStateExpectation = deferFulfillment(
      stateMachineFactory.membersFlowStatePublisher
    ) { $0 == .roomMemberDetails(userID: "test", previousState: .roomMembersList) }
  ...
  private mutating func setup(entryPoint: RoomMembersFlowCoordinatorEntryPoint) async throws {
    ...
    navigationStackCoordinator.setRootCoordinator(
      PlaceholderScreenCoordinator(hideBrandChrome: false))
    ...
    membersFlowCoordinator = RoomMembersFlowCoordinator(

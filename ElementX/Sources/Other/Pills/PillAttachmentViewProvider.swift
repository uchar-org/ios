...
final class PillAttachmentViewProvider: NSTextAttachmentViewProvider, NSSecureCoding {
  ...
  override init(
    textAttachment: NSTextAttachment, parentView: UIView?, textLayoutManager: NSTextLayoutManager?,
    location: NSTextLocation
  ) {
    super.init(
      textAttachment: textAttachment, parentView: parentView, textLayoutManager: textLayoutManager,
      location: location)
  ...
  override func loadView() {
    ...
    if ProcessInfo.isXcodePreview || ProcessInfo.isRunningTests {
      ...
      context = PillContext.mock(
        viewState: .mention(isOwnMention: false, displayText: "Alice"), delay: .seconds(2))

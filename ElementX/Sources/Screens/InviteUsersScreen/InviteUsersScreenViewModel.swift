...
typealias InviteUsersScreenViewModelType = StateStoreViewModel<
  InviteUsersScreenViewState, InviteUsersScreenViewAction
>
...
class InviteUsersScreenViewModel: InviteUsersScreenViewModelType, InviteUsersScreenViewModelProtocol
{
  ...
  init(
    ...
    super.init(
      initialViewState: InviteUsersScreenViewState(
        selectedUsers: [],
  ...
  private func toggleUser(_ user: UserProfileProxy) {
    if state.selectedUsers.contains(user) {
      ...
      Task {
        let identityUnknown =
          if case .success(let identity) = await self.clientProxy.userIdentity(
  ...
  private func inviteUsers(_ users: [String], roomProxy: JoinedRoomProxyProtocol) {
    showLoadingIndicator(
      title: L10n.screenRoomDetailsInvitePeoplePreparing,
    ...
    Task {
      ...
      let result: Result<Void, RoomProxyError> = await withTaskGroup(
        of: Result<Void, RoomProxyError>.self
      ) { group in
        for user in users {
          group.addTask {

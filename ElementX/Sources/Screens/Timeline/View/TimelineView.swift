...
struct TimelineView: View {
  ...
  var body: some View {
    TimelineViewRepresentable()
      ...
      .environment(
        \.focussedEventID, timelineContext.viewState.timelineState.focussedEvent?.eventID
      )
      ...
      .sheet(item: $timelineContext.actionMenuInfo) { info in
        let actions = TimelineItemMenuActionProvider(
          timelineItem: info.item,
      ...
      .translationPresentation(
        isPresented: $timelineContext.showTranslation,
...
struct TimelineView_Previews: PreviewProvider {  // Not testable as this preview is built the same way as RoomScreen.
  static let roomProxyMock = JoinedRoomProxyMock(
    .init(
      id: "stable_id",
      name: "Preview room"))

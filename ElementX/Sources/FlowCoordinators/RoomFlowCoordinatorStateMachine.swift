...
extension RoomFlowCoordinator {
  ...
  indirect enum State: StateType {
    ...
    case emojiPicker(
      itemID: TimelineItemIdentifier, selectedEmojis: Set<String>, previousState: State)
  ...
  func addRouteMapping(stateMachine: StateMachine<State, Event>) {
    stateMachine.addRouteMapping { event, fromState, _ in
      ...
      case (
        .manageAuthorizedSpacesScreen(let previousState), .dismissedManageAuthorizedSpacesScreen
      ):
        return previousState
      ...
      case (
        .mediaUploadPicker(_, let previousMediaUploadPickerState),
        .presentMediaUploadPreview(let mediaURLs)
      ):
        return .mediaUploadPreview(
          mediaURLs: mediaURLs, previousState: previousMediaUploadPickerState)

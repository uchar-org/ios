...
final class AuthenticationStartScreenViewModelTests {
  ...
  func initialState() async throws {
    ...
    #expect(
      client.urlForOauthOauthConfigurationPromptLoginHintDeviceIdAdditionalScopesCallsCount == 0)
  ...
  func provisionedOIDCState() async throws {
    ...
    await setupViewModel(
      provisioningParameters: .init(accountProvider: "company.com", loginHint: "user@company.com"))
    #expect(authenticationService.homeserver.value.loginMode == .unknown)
    #expect(
      client.urlForOauthOauthConfigurationPromptLoginHintDeviceIdAdditionalScopesCallsCount == 0)
    ...
    #expect(
      clientFactory
        ...
        == 1)
    #expect(
      client.urlForOauthOauthConfigurationPromptLoginHintDeviceIdAdditionalScopesCallsCount == 1)
    #expect(
      client.urlForOauthOauthConfigurationPromptLoginHintDeviceIdAdditionalScopesReceivedArguments?
        .prompt == .consent)
    #expect(
      client.urlForOauthOauthConfigurationPromptLoginHintDeviceIdAdditionalScopesReceivedArguments?
  ...
  func classicAppAccountWithoutWellKnown() async throws {
    ...
    #expect(
      clientFactory
        .makeClientHomeserverAddressSessionDirectoriesPassphraseClientSessionDelegateAppSettingsAppHooksReceivedArguments?
        .homeserverAddress == "https://matrix.company.com")
    #expect(authenticationService.homeserver.value.loginMode == .oidc(supportsCreatePrompt: false))
    #expect(
      client.urlForOauthOauthConfigurationPromptLoginHintDeviceIdAdditionalScopesReceivedArguments?
        .loginHint == "mxid:\(classicAppAccount.userID)")
  ...
  func classicAppAccountOnUnsupportedServer() async {
    ...
    let classicAppAccount = makeClassicAppAccount()
    await setupViewModel(
      classicAppAccount: classicAppAccount, supportsOIDC: false, supportsPasswordLogin: false)
    guard case .welcomeBack(let account) = context.viewState.classicAppMode else {
      Issue.record("Expected classicAppMode to be .welcomeBack")
      return
    }
  ...
  func classicAppAccountWithProvisioningLink() async {
    ...
    let classicAppAccount = makeClassicAppAccount()
    await setupViewModel(
      classicAppAccount: classicAppAccount,
  ...
  func classicAppAccountRequiresBackup() async throws {
    ...
    var deferred = deferFulfillment(
      context.observe(\.viewState.bindings.showClassicAppBackupInstructions)
    ) { $0 }
    context.send(viewAction: .continueWithClassic(classicAppAccount))
  ...
  private func setupViewModel(
    classicAppAccount: ClassicAppAccount? = nil,
    ...
    client = ClientSDKMock(
      configuration: .init(
        oidcLoginURL: supportsOIDC ? "https://account.company.com/authorize" : nil,
        supportsOIDCCreatePrompt: false,
        supportsPasswordLogin: supportsPasswordLogin))
    // Map both the server name and the homeserver URL so fallback lookups work.
    let homeserverClients: [String: ClientSDKMock] = [
      "company.com": client,
      "https://matrix.company.com": client,
    ]
    let configuration = AuthenticationClientFactoryMock.Configuration(
      homeserverClients: homeserverClients)

    if let classicAppAccount {
      classicAppManager = ClassicAppManagerMock(
        .init(accounts: [classicAppAccount], availableSecrets: availableSecrets))
    } else {
  ...
  private func makeClassicAppAccount(
    serverName: String = "company.com",

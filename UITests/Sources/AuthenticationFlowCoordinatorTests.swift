...
class AuthenticationFlowCoordinatorUITests: XCTestCase {
  func testLoginWithPassword() async throws {
    ...
    app.textFields[A11yIdentifiers.changeServerScreen.server].clearAndTypeText(
      "example.com\n", app: app)

    // Await for the button to be hittable, since a loader may appear
    let serverConfirmationContinueButton = app.buttons[
      A11yIdentifiers.serverConfirmationScreen.continue]
    XCTAssertTrue(
      serverConfirmationContinueButton.wait(for: \.isHittable, toEqual: true, timeout: 2.0))
    ...
    app.secureTextFields[A11yIdentifiers.loginScreen.password].clearAndTypeText(
      "12345678", app: app)
  ...
  func testLoginWithIncorrectPassword() {
    ...
    XCTAssertTrue(
      app.alerts.element.waitForExistence(timeout: 2.0),
      "An error alert should be shown when attempting login with invalid credentials.")
  }
  ...
  func testLoginWithUnsupportedUserID() async throws {
    ...
    app.textFields[A11yIdentifiers.loginScreen.emailUsername].clearAndTypeText(
      "@test:server.net\n", app: app)
  ...
  func testMultipleProvidersLoginWithPassword() async throws {
    ...
    app.switches.matching(identifier: A11yIdentifiers.serverConfirmationScreen.serverPicker)
  ...
  func verifyReportBugButton(_ app: XCUIApplication) async throws {
    ...
    app.staticTexts[A11yIdentifiers.authenticationStartScreen.appVersion].tap(
      withNumberOfTaps: 7, numberOfTouches: 1)

...
struct PollFormScreenViewStateBindings: Equatable {
    ...
    static func == (lhs: PollFormScreenViewStateBindings, rhs: PollFormScreenViewStateBindings)
        -> Bool {
        lhs.question == rhs.question && lhs.options.map(\.text) == rhs.options.map(\.text)

...
extension JoinedRoomProxyMock {
  ...
  convenience init(_ configuration: JoinedRoomProxyMockConfiguration) {
    ...
    timeline = TimelineProxyMock(
      .init(
        isAutoUpdating: configuration.shouldUseAutoUpdatingTimeline,
    ...
    knockRequestsStatePublisher = CurrentValueSubject(configuration.knockRequestsState)
    ...
    let powerLevelsProxyMock = RoomPowerLevelsProxyMock(
      configuration: configuration.powerLevelsConfiguration)
    ...
    powerLevelsProxyMock.canOwnUserSendStateEventClosure = { [weak self] _ in
      self?.membersPublisher.value.first { $0.userID == configuration.ownUserID }?.role ?? .user
        != .user
    }
    ...
    powerLevelsProxyMock.canOwnUserEditRolesAndPermissionsClosure = { [weak self] in
      self?.membersPublisher.value.first { $0.userID == configuration.ownUserID }?.role
    ...
    guard let url = URL(string: "https://call.element.io/\(UUID().uuidString)#?appPrompt=false")
    else {
      fatalError()
    }
...
extension RoomInfoProxyMock {
  @MainActor convenience init(_ configuration: JoinedRoomProxyMockConfiguration) {
    ...
    activeMembersCount =
      configuration.members.filter { $0.membership == .join || $0.membership == .invite }.count
...
extension RoomHero {
  fileprivate init(from memberProxy: RoomMemberProxyMock) {
    self.init(
      userId: memberProxy.userID,
...
extension Array where Element == JoinedRoomProxyProtocol {
  static var mockRooms: [JoinedRoomProxyProtocol] {
    [
      JoinedRoomProxyMock(
        .init(id: "1", name: "Room Name", canonicalAlias: "#room-name:example.com")),
      JoinedRoomProxyMock(
        .init(id: "2", name: "Room Name", canonicalAlias: "#room-name:example.com")),
      JoinedRoomProxyMock(
        .init(id: "3", name: "Room Name", canonicalAlias: "#room-name:example.com")),
    ]
  }
}

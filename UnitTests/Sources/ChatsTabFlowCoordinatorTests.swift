...
struct ChatsTabFlowCoordinatorTests {
  ...
  init() async throws {
    ...
    splitCoordinator = NavigationSplitCoordinator(
    ...
    let deferred = deferFulfillment(stateMachineFactory.chatsTabFlowStatePublisher) {
      $0 == .roomList(detailState: nil)
    }
  ...
  mutating func roomPresentation() async throws {
    try await process(
      route: .room(roomID: "1", via: []), expectedState: .roomList(detailState: .room(roomID: "1")))
    ...
    try await process(
      route: .room(roomID: "2", via: []), expectedState: .roomList(detailState: .room(roomID: "2")))
    #expect(detailNavigationStack?.rootCoordinator is RoomScreenCoordinator)
    #expect(detailCoordinator != nil)
  ...
  mutating func roomAliasPresentation() async throws {
    ...
    try await process(
      route: .roomAlias("#alias:matrix.org"),
      expectedState: .roomList(detailState: .room(roomID: "1")))
    #expect(detailNavigationStack?.rootCoordinator is RoomScreenCoordinator)
    #expect(detailCoordinator != nil)
  ...
  mutating func roomDetailsPresentation() async throws {
    try await process(
      route: .roomDetails(roomID: "1"), expectedState: .roomList(detailState: .room(roomID: "1")))
  ...
  mutating func noOp() async throws {
    ...
    let unexpectedFulfillment = deferFailure(
      stateMachineFactory.chatsTabFlowStatePublisher, timeout: .seconds(1)
    ) { _ in true }
  ...
  mutating func switchToDifferentDetails() async throws {
    ...
    try await process(
      route: .roomDetails(roomID: "2"), expectedState: .roomList(detailState: .room(roomID: "2")))
    #expect(detailNavigationStack?.rootCoordinator is RoomDetailsScreenCoordinator)
    #expect(detailCoordinator != nil)
  }
  ...
  mutating func userProfileClearsStack() async throws {
    ...
    #expect(detailNavigationStack?.rootCoordinator == nil)
    let sheetStackCoordinator = try #require(
      splitCoordinator.sheetCoordinator as? NavigationStackCoordinator,
      "There should be a navigation stack presented as a sheet.")
  ...
  mutating func roomClearsStack() async throws {
    ...
    try await process(
      route: .room(roomID: "3", via: []), expectedState: .roomList(detailState: .room(roomID: "3")))
    #expect(detailNavigationStack?.rootCoordinator is RoomScreenCoordinator)
    #expect(detailNavigationStack?.stackCoordinators.count == 0)
    #expect(detailCoordinator != nil)
  }
  ...
  mutating func eventRoutes() async throws {
    ...
    try await process(
      route: .event(eventID: "1", roomID: "1", via: []),
      expectedState: .roomList(detailState: .room(roomID: "1")))
    #expect(detailNavigationStack?.rootCoordinator is RoomScreenCoordinator)
    #expect(detailNavigationStack?.stackCoordinators.count == 0)
    #expect(detailCoordinator != nil)
    ...
    chatsTabFlowCoordinator.handleAppRoute(
      .childEvent(eventID: "2", roomID: "2", via: []), animated: true)
    try await Task.sleep(for: .milliseconds(100))
    #expect(detailNavigationStack?.rootCoordinator is RoomScreenCoordinator)
    #expect(detailNavigationStack?.stackCoordinators.count == 1)
    #expect(detailNavigationStack?.stackCoordinators.first is RoomScreenCoordinator)
    ...
    try await process(
      route: .event(eventID: "3", roomID: "3", via: []),
      expectedState: .roomList(detailState: .room(roomID: "3")))
    #expect(detailNavigationStack?.rootCoordinator is RoomScreenCoordinator)
    #expect(detailNavigationStack?.stackCoordinators.count == 0)
    #expect(detailCoordinator != nil)
    #expect(
    ...
    try await process(
      route: .event(eventID: "4", roomID: "3", via: []),
      expectedState: .roomList(detailState: .room(roomID: "3")))
    #expect(detailNavigationStack?.rootCoordinator is RoomScreenCoordinator)
    #expect(detailNavigationStack?.stackCoordinators.count == 0)
    #expect(detailCoordinator != nil)
  ...
  mutating func shareMediaRouteWithRoom() async throws {
    ...
    let sharePayload: ShareExtensionPayload = .mediaFiles(
      roomID: "2", mediaFiles: [.init(url: .picturesDirectory, suggestedName: nil)])
    try await process(
      route: .share(sharePayload),
      expectedState: .roomList(detailState: .room(roomID: "2")))

    #expect(detailNavigationStack?.rootCoordinator is RoomScreenCoordinator)
    #expect(
      (splitCoordinator.sheetCoordinator as? NavigationStackCoordinator)?.rootCoordinator
        is MediaUploadPreviewScreenCoordinator)
  }
  ...
  mutating func shareTextRouteWithRoom() async throws {
    ...
    #expect(detailNavigationStack?.rootCoordinator is RoomScreenCoordinator)
    #expect(
      splitCoordinator.sheetCoordinator == nil,
  ...
  private mutating func process(
    route: AppRoute, expectedState: ChatsTabFlowCoordinatorStateMachine.State
  ) async throws {
    // Sometimes the state machine's state changes before the coordinators have updated the stack.
    let delayedPublisher = stateMachineFactory.chatsTabFlowStatePublisher.delay(

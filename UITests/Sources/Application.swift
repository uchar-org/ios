...
enum Application {
  static func launch(
    _ identifier: UITestsScreenIdentifier, disableTimelineAccessibility: Bool = true
  ) -> XCUIApplication {
  ...
  private static func checkEnvironments() {
    ...
    guard osVersion.majorVersion == requiredOSVersion.major,
      ...
      fatalError(
        "Switch to iOS \(requiredOSVersion.major).\(requiredOSVersion.minor) for these tests.")
    }
    ...
    guard deviceModel == requirediPhoneSimulator || deviceModel == requirediPadSimulator else {
      fatalError(
        "Running on \(deviceModel) but we only support \(requirediPhoneSimulator) and \(requirediPadSimulator)."
      )
    }
  }
}
...
extension XCUIApplication {
  ...
  func assertScreenshot(
    fileName: String = #file, testName: String = #function, step: Int? = nil,
...
extension UIImage {
  ...
  fileprivate func inset(by insets: UIEdgeInsets) -> UIImage {

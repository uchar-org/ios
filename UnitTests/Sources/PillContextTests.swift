...
struct PillContextTests {
  ...
  func user() async {
    ...
    let context = PillContext(
      timelineContext: mock.context,
      data: PillTextAttachmentData(
        type: .user(userID: id), font: .preferredFont(forTextStyle: .body)))
    ...
    subject.send([
      RoomMemberProxyMock(
        with: .init(userID: id, displayName: name, avatarURL: avatarURL, membership: .join))
    ])
    await Task.yield()
  ...
  func roomAliasMention() {
    ...
    mockController.roomProxy = proxyMock
    let clientMock = ClientProxyMock(.init())
    clientMock.roomSummaryForAliasReturnValue = .mock(
      id: "2",
    ...
    let context = PillContext(
      timelineContext: mock.context,
      data: PillTextAttachmentData(
        type: .roomAlias("#foundation-and-empire:matrix.org"),
  ...
  func eventOnRoomIDMention() {
    ...
    let context = PillContext(
      timelineContext: mock.context,
      data: PillTextAttachmentData(
        type: .event(room: .roomID("1")), font: .preferredFont(forTextStyle: .body)))
  ...
  func eventOnRoomAliasMention() {
    ...
    let context = PillContext(
      timelineContext: mock.context,
      data: PillTextAttachmentData(
        type: .event(room: .roomAlias("#foundation-and-empire:matrix.org")),
        font: .preferredFont(forTextStyle: .body)))

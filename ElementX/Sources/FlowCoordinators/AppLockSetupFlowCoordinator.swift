...
class AppLockSetupFlowCoordinator: FlowCoordinatorProtocol {
  ...
  init(
    presentingFlow: PresentationFlow, appLockService: AppLockServiceProtocol,
  ...
  private func configureStateMachine() {
    stateMachine.addRouteMapping { [weak self] event, fromState, _ in
      ...
      case (.createPIN(let replacingExitingPIN), .pinEntered):
        if presentingFlow == .onboarding {
          ...
          return appLockService.biometricUnlockEnabled || appLockService.biometryType == .none
            ? .settings : .biometricsPrompt
        } else {
          return .settings
        }
    ...
    stateMachine.addErrorHandler { context in
      fatalError(
        "Unexpected transition from `\(context.fromState)` to `\(context.toState)` with event `\(String(describing: context.event))`."
      )
    }
  }
  ...
  private func showCreatePIN() {
    ...
    let coordinator = AppLockSetupPINScreenCoordinator(
      parameters: .init(
        initialMode: .create,
  ...
  private func showBiometricsPrompt() {
    let coordinator = AppLockSetupBiometricsScreenCoordinator(
      parameters: .init(appLockService: appLockService))
    coordinator.actions.sink { [weak self] action in
      guard let self else { return }
      switch action {
      case .continue:
  ...
  private func showSettings() {
    let coordinator = AppLockSetupSettingsScreenCoordinator(
      parameters: .init(appLockService: appLockService))
    coordinator.actions.sink { [weak self] action in
      guard let self else { return }
      switch action {

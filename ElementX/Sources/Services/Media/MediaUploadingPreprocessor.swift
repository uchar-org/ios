...
struct MediaUploadingPreprocessor {
  ...
  func processMedia(at urls: [URL], maxUploadSize: UInt) async -> Result<
    [MediaInfo], MediaUploadingPreprocessorError
  > {
  ...
  func processMedia(at url: URL, maxUploadSize: UInt) async -> Result<
    MediaInfo, MediaUploadingPreprocessorError
  > {
    ...
    let uniqueFolder = FileManager.default.temporaryDirectory.appendingPathComponent(
      UUID().uuidString)
    ...
    guard let type = UTType(filenameExtension: newURL.pathExtension),
      ...
      return processFile(
        at: newURL, mimeType: "application/octet-stream", maxUploadSize: maxUploadSize)
    }
  ...
  private func processImage(at url: inout URL, type: UTType, mimeType: String, maxUploadSize: UInt)
    -> Result<MediaInfo, MediaUploadingPreprocessorError>
  {
    do {
      ...
      if appSettings.optimizeMediaUploads, !type.conforms(to: .gif) {
        ...
        try resizeImage(
          at: url, maxPixelSize: Constants.optimizedMaxPixelSize, destination: url, type: outputType
        )

        if let preferredFilenameExtension = outputType.preferredFilenameExtension,
          url.pathExtension != preferredFilenameExtension
        {
          let convertedURL = url.deletingPathExtension().appendingPathExtension(
      ...
      guard fileSize < maxUploadSize, thumbnailFileSize < maxUploadSize else {
        return .failure(.maxUploadSizeExceeded(limit: maxUploadSize))
      }

      let thumbnailInfo = ThumbnailInfo(
        height: UInt64(thumbnailResult.height),
        width: UInt64(thumbnailResult.width),
        mimetype: thumbnailResult.mimeType,
        size: UInt64(thumbnailFileSize))

      let imageInfo = ImageInfo(
        height: UInt64(imageSize.height),
      ...
      let mediaInfo = MediaInfo.image(
        imageURL: url, thumbnailURL: thumbnailResult.url, imageInfo: imageInfo)
  ...
  private func processVideo(at url: URL, maxUploadSize: UInt) async -> Result<
    MediaInfo, MediaUploadingPreprocessorError
  > {
    do {
      ...
      guard videoSize < maxUploadSize, thumbnailSize < maxUploadSize else {
        return .failure(.maxUploadSizeExceeded(limit: maxUploadSize))
      }
      ...
      let mediaInfo = MediaInfo.video(
        videoURL: result.url, thumbnailURL: thumbnailResult.url, videoInfo: videoInfo)
  ...
  private func processAudio(at url: URL, mimeType: String?, maxUploadSize: UInt) async -> Result<
    MediaInfo, MediaUploadingPreprocessorError
  > {
    let fileSize = (try? FileManager.default.sizeForItem(at: url)) ?? 0

    guard fileSize < maxUploadSize else {
      return .failure(.maxUploadSizeExceeded(limit: maxUploadSize))
    }
    ...
    let audioInfo = AudioInfo(
      duration: durationInSeconds, size: UInt64(fileSize), mimetype: mimeType)
  ...
  private func processFile(at url: URL, mimeType: String?, maxUploadSize: UInt) -> Result<
    MediaInfo, MediaUploadingPreprocessorError
  > {
    let fileSize = (try? FileManager.default.sizeForItem(at: url)) ?? 0
    ...
    let fileInfo = FileInfo(
      mimetype: mimeType, size: UInt64(fileSize), thumbnailInfo: nil, thumbnailSource: nil)
  ...
  private func stripLocationFromImage(at url: URL, type: UTType)
    throws(MediaUploadingPreprocessorError)
  {
    guard let originalData = NSData(contentsOf: url),
      let imageSource = CGImageSourceCreateWithData(originalData, nil)
    else {
      throw .failedStrippingLocationData
    }

    guard let originalMetadata = CGImageSourceCopyPropertiesAtIndex(imageSource, 0, nil),
      (originalMetadata as NSDictionary).value(forKeyPath: "\(kCGImagePropertyGPSDictionary)")
        != nil
    else {
    ...
    let destination =
      CGImageDestinationCreateWithData(
        data as CFMutableData, type.identifier as CFString, count, nil)
      ?? CGImageDestinationCreateWithData(
        data as CFMutableData, UTType.jpeg.identifier as CFString, count, nil)
    guard let destination else {
      throw .failedStrippingLocationData
    }

    CGImageDestinationAddImageFromSource(
      destination, imageSource, 0, metadataKeysToRemove as NSDictionary)
  ...
  private func generateThumbnailForImage(at url: URL) throws(MediaUploadingPreprocessorError)
    -> ImageProcessingInfo
  {
    let thumbnailFileName =
      "thumbnail-\((url.lastPathComponent as NSString).deletingPathExtension).jpeg"
    let thumbnailURL = url.deletingLastPathComponent().appendingPathComponent(thumbnailFileName)
    let thumbnailMaxPixelSize = max(
      Constants.maximumThumbnailSize.height, Constants.maximumThumbnailSize.width)

    do {
      try resizeImage(
        at: url, maxPixelSize: thumbnailMaxPixelSize, destination: thumbnailURL, type: .jpeg)
    } catch {
    ...
    return .init(
      url: thumbnailURL, height: thumbnail.size.height, width: thumbnail.size.width,
      mimeType: "image/jpeg", blurhash: blurhash)
  }

  private func resizeImage(at url: URL, maxPixelSize: CGFloat, destination: URL, type: UTType)
    throws(MediaUploadingPreprocessorError)
  {
    ...
    try resizeImage(
      withSource: imageSource, maxPixelSize: maxPixelSize, destination: destination, type: type)
  }
  ...
  private func resizeImage(
    withSource imageSource: CGImageSource, maxPixelSize: CGFloat, destination destinationURL: URL,
    type: UTType
  ) throws(MediaUploadingPreprocessorError) {
    ...
    guard
      let scaledImage = CGImageSourceCreateThumbnailAtIndex(
        imageSource, 0, options as NSDictionary),
      let destination = CGImageDestinationCreateWithURL(
        destinationURL as CFURL, type.identifier as CFString, 1, nil)
    else {
      throw .failedResizingImage
    }
  ...
  /// - Returns: the URL for the resulting thumbnail and its sizing info as an `ImageProcessingResult`
  private func generateThumbnailForVideoAt(_ url: URL) async throws(MediaUploadingPreprocessorError)
    -> ImageProcessingInfo
  {
  ...
  private func convertVideoToMP4(_ url: URL, targetFileSize: UInt)
    async throws(MediaUploadingPreprocessorError) -> VideoProcessingInfo
  {
    let asset = AVURLAsset(url: url)
    let presetName =
      appSettings.optimizeMediaUploads ? AVAssetExportPreset1280x720 : AVAssetExportPreset1920x1080
    ...
    let outputURL = url.deletingLastPathComponent().appendingPathComponent(
      "\(uuid)-\(originalFilenameWithoutExtension).mp4")
    ...
    let newOutputURL = url.deletingLastPathComponent().appendingPathComponent(
      "\(originalFilenameWithoutExtension).mp4")
    ...
    guard let track = try? await newAsset.loadTracks(withMediaType: .video).first,
      ...
      let adjustedNaturalSize = try? await track.size
    else {
      throw .failedConvertingVideo
    }

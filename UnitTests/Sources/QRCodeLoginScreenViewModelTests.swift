...
struct QRCodeLoginScreenViewModelTests {
  ...
  var linkMobileProgressSubject:
    CurrentValueSubject<LinkNewDeviceService.LinkMobileProgress, QRCodeLoginError>!
  var linkDesktopProgressSubject:
    CurrentValueSubject<LinkNewDeviceService.LinkDesktopProgress, QRCodeLoginError>!
  ...
  mutating func linkDesktopComputer() async throws {
    ...
    let deferredFailure = deferFailure(context.$viewState, timeout: .seconds(1)) {
  ...
  mutating func linkMobileDevice() async throws {
    ...
    var deferredState = deferFulfillment(context.$viewState) {
      $0.state == .confirmCode(.inputCode(checkCodeSenderProxy))
    }
  ...
  private mutating func setup(mode: Mode) {
    ...
    qrCodeLoginService.loginWithQRCodeDataReturnValue =
      qrLoginProgressSubject.asCurrentValuePublisher()

    linkMobileProgressSubject = .init(.qrReady(LinkNewDeviceServiceMock.mockQRCodeImage))
    linkDesktopProgressSubject = .init(.starting)
    linkNewDeviceService = LinkNewDeviceServiceMock(
      .init(
        linkMobileProgressPublisher: linkMobileProgressSubject.asCurrentValuePublisher(),

...
struct LiveLocationSheet: View {
  ...
  var body: some View {
    mainContent
      ...
      .presentationDetents(
        context.viewState.liveLocationShares.isEmpty
          ? .init([Self.supportedDetents[0]]) : .init(Self.supportedDetents),
  ...
  private var title: some View {
    Text(
      context.viewState.liveLocationShares.isEmpty
        ? L10n.screenLiveLocationSheetNobodySharing : L10n.screenLiveLocationSheetTitle
    )
...
struct LiveLocationSheet_Previews: PreviewProvider, TestablePreview {
  static let viewModel = LocationSharingScreenViewModel.mock(
    type: .viewLive, senderID: RoomMemberProxyMock.mockMe.userID)
  static let emptyViewModel = LocationSharingScreenViewModel.mock(
    type: .viewLiveEmpty, senderID: RoomMemberProxyMock.mockMe.userID)

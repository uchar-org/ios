...
struct RoomRolesAndPermissionsScreen: View {
  ...
  private var rolesSection: some View {
    Section {
      if context.viewState.ownPowerLevel.role == .creator {
        ListRow(
          label: .default(
            title: L10n.screenRoomRolesAndPermissionsAdminsAndOwners,
            icon: \.admin),
        ...
        ListRow(
          label: .default(
            title: L10n.screenRoomRolesAndPermissionsAdmins,
            icon: \.admin),
      ...
      ListRow(
        label: .default(
          title: L10n.screenRoomRolesAndPermissionsModerators,
          icon: \.chatProblem),
  ...
  private var permissionsSection: some View {
    Section {
      ListRow(
        label: .default(
          title: L10n.screenRoomRolesAndPermissionsPermissionsHeader,
          icon: \.settings),
  ...
  private var resetSection: some View {
    Section {
      ListRow(
        label: .default(
          title: L10n.screenRoomRolesAndPermissionsReset,
          icon: \.delete,
          role: .destructive),
        kind: .button {
...
struct RoomRolesAndPermissionsScreen_Previews: PreviewProvider, TestablePreview {
  static let viewModel = RoomRolesAndPermissionsScreenViewModel(
    initialPermissions: RoomPermissions(powerLevels: .mock),
    roomProxy: JoinedRoomProxyMock(.init(members: .allMembersAsAdmin)),
    userIndicatorController: UserIndicatorControllerMock(),
    analytics: ServiceLocator.shared.analytics)

  static let creatorViewModel = RoomRolesAndPermissionsScreenViewModel(
    initialPermissions: RoomPermissions(powerLevels: .mock),

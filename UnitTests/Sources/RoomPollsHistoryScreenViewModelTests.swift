...
struct RoomPollsHistoryScreenViewModelTests {
  ...
  init() throws {
    ...
    viewModel = RoomPollsHistoryScreenViewModel(
  ...
  func backPaginate() async throws {
    ...
    let deferredViewState = deferFulfillment(
      viewModel.context.$viewState, keyPath: \.isBackPaginating,
  ...
  func endPoll() async throws {
    let deferred = deferFulfillment(
      interactionHandler.publisher.delay(for: 0.1, scheduler: DispatchQueue.main)
    ) { _ in true }
  ...
  func sendPollResponse() async throws {
    ...
    viewModel.context.send(
      viewAction: .sendPollResponse(pollStartID: "somePollID", optionID: "someOptionID"))

    try await deferred.fulfill()

    #expect(interactionHandler.sendPollResponsePollStartIDOptionIDCalled)
    #expect(
      interactionHandler.sendPollResponsePollStartIDOptionIDReceivedInvocations[0].pollStartID
        == "somePollID")
    #expect(
      interactionHandler.sendPollResponsePollStartIDOptionIDReceivedInvocations[0].optionID
        == "someOptionID")
  }

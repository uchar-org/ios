...
struct MentionSuggestionItemView: View {
    ...
    private var avatar: some View {
        ...
        case .user(let user):
            LoadableAvatarImage(url: user.avatarURL, name: user.displayName, contentID: user.id,
        ...
        case .room(let room):
            RoomAvatarImage(avatar: room.avatar, avatarSize: .room(on: .completionSuggestions),
...
struct MentionSuggestionItemView_Previews: PreviewProvider, TestablePreview {
    ...
    static var previews: some View {
        MentionSuggestionItemView(mediaProvider: mockMediaProvider,
                                  item: .init(suggestionType: .user(.init(id: "test", displayName: "Test", avatarURL: .mockMXCUserAvatar)), range: .init(),
                                              rawSuggestionText: ""))
            .previewDisplayName("User")
        MentionSuggestionItemView(mediaProvider: mockMediaProvider,
                                  item: .init(suggestionType: .user(.init(id: "test2", displayName: nil, avatarURL: nil)), range: .init(),
                                              rawSuggestionText: ""))

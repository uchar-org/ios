...
class LinkNewDeviceFlowCoordinator: FlowCoordinatorProtocol {
  ...
  private let actionsSubject: PassthroughSubject<LinkNewDeviceFlowCoordinatorAction, Never> =
    .init()
  ...
  private func presentLinkNewDeviceScreen() {
    let coordinator = LinkNewDeviceScreenCoordinator(
      parameters: .init(
    ...
    coordinator.actionsPublisher
      .sink { [weak self] action in
        ...
        case .linkDesktopComputer:
          presentQRCodeScreen(
            mode: .linkDesktop(flowParameters.userSession.clientProxy.linkNewDeviceService()))
        case .dismiss:

...
class StartChatFlowCoordinator: FlowCoordinatorProtocol {
  ...
  init(
    entryPoint: StartChatFlowCoordinatorEntryPoint,
  ...
  private func configureStateMachine() {
    ...
    stateMachine.addRoutes(
      event: .createRoom(isSpace: true), transitions: [.initial => .createRoom]
    ) { [weak self] _ in
      self?.presentCreateRoomScreen(isSpace: true, spaceSelectionMode: .none, isRoot: true)
    }
    stateMachine.addRoutes(
      event: .createRoom(isSpace: false), transitions: [.initial => .createRoom]
      ...
      self?.presentCreateRoomScreen(
        isSpace: false, spaceSelectionMode: .editableSpacesList(preSelectedSpace: space),
        isRoot: true)
    }

    stateMachine.addRoutes(
      event: .createRoom(isSpace: false), transitions: [.startChat => .createRoom]
    ) { [weak self] context in
      guard let self, context.fromState == .startChat else { return }  // Required check because the event is used in another route.
      presentCreateRoomScreen(
        isSpace: false,
        spaceSelectionMode: .editableSpacesList(preSelectedSpace: nil),
        isRoot: false)
    }
    stateMachine.addRoutes(event: .dismissedCreateRoom, transitions: [.createRoom => .startChat]) {
      [weak self] _ in
      self?.createRoomScreenCoordinator = nil
    }

    stateMachine.addRoutes(
      event: .presentRoomAvatarPicker, transitions: [.createRoom => .roomAvatarPicker]
    ) { [weak self] context in
    ...
    stateMachine.addRoutes(
      event: .dismissedRoomAvatarPicker, transitions: [.roomAvatarPicker => .createRoom])

    stateMachine.addRoutes(event: .createdRoom, transitions: [.createRoom => .inviteUsers]) {
      [weak self] context in
      ...
      self?.presentInviteUsersScreen(
        roomProxy: result.roomProxy, spaceRoomListProxy: result.spaceRoomListProxy)
    }
  ...
  private func presentCreateRoomScreen(
    ...
    let createParameters = CreateRoomScreenCoordinatorParameters(
      isSpace: isSpace,
    ...
    coordinator.actions.sink { [weak self] action in
      ...
      case .createdRoom(let roomProxy, let spaceRoomListProxy):
        stateMachine.tryEvent(
          .createdRoom,
          userInfo: CreatedRoomResult(roomProxy: roomProxy, spaceRoomListProxy: spaceRoomListProxy))
  ...
  private func presentInviteUsersScreen(
    roomProxy: JoinedRoomProxyProtocol, spaceRoomListProxy: SpaceRoomListProxyProtocol?
  ) {

...
struct CollapsibleReactionLayout: Layout {
    ...
    func placeSubviews(in bounds: CGRect, proposal: ProposedViewSize, subviews: some FlowLayoutSubviews,
                       cache: inout ()) {
        guard let subviewsByType = getSubviewsByItemType(subviews: Array(subviews)),
              subviewsByType.reactions.count > 0
        else {
        ...
        // Calculate the layout of the rows with the reactions button and add more button
        let reactionsAndAddMore = calculateRows(proposal: ProposedViewSize(bounds.size),
    ...
    private func sizeThatFits(rows: [[FlowLayoutSubview]]) -> CGSize {
        ...
        return sizes.enumerated().reduce(CGSize.zero) { partialResult, rowItem in
            ...
            return CGSize(width: max(partialResult.width, rowSize.width),
    ...
    private func replaceTrailingItemsWithButtons(rowWidth: CGFloat, rows: [[FlowLayoutSubview]], collapseButton: FlowLayoutSubview,
                                                 addMoreButton: FlowLayoutSubview) -> ([[FlowLayoutSubview]], [FlowLayoutSubview]) {
        var rows = rows

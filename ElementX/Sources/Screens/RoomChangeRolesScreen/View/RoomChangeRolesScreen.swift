  ...
  private var mainContent: some View {
    GeometryReader { proxy in
      Form {
        ...
        RoomChangeRolesScreenSection(
          members: context.viewState.visibleOwners,
          role: .owner,
          context: context)

        RoomChangeRolesScreenSection(
          members: context.viewState.visibleAdministrators,
          role: .administrator,
          context: context)
        RoomChangeRolesScreenSection(
          members: context.viewState.visibleModerators,
          role: .moderator,
          context: context)
        RoomChangeRolesScreenSection(
          members: context.viewState.visibleUsers,
          role: .user,
          context: context)
      }
    }
      ...
      ScrollViewReader { scrollView in
        HStack(spacing: 16) {
          ForEach(context.viewState.membersWithRole, id: \.id) { member in
            let dismissAction =
              context.viewState.isMemberDisabled(member)
              ? nil
              : {
                context.send(viewAction: .demoteMember(member))
              }
            RoomChangeRolesScreenSelectedItem(
              member: member, mediaProvider: context.mediaProvider,
...
struct RoomChangeRolesScreen_Previews: PreviewProvider, TestablePreview {
  ...
  static func makeViewModel(mode: RoomRole, ownRole: RoomRole) -> RoomChangeRolesScreenViewModel {
    ...
    return RoomChangeRolesScreenViewModel(
      mode: mode,

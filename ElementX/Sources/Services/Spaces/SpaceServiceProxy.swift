...
class SpaceServiceProxy: SpaceServiceProxyProtocol {
  ...
  private func setupSubscriptions() async {
    topLevelSpacesHandle = await spaceService.subscribeToTopLevelJoinedSpaces(
      listener: SDKListener { [weak self] updates in
        self?.handleSpaceListUpdates(updates)
      })

    spaceFilterHandle = await spaceService.subscribeToSpaceFilters(
      listener: SDKListener { [weak self] updates in
  ...
  > {
    do {
      return try await .success(
        spaceService.getSpaceRoom(roomId: spaceID).map(SpaceServiceRoom.init))
    } catch {
  ...
  func leaveSpace(spaceID: String) async -> Result<LeaveSpaceHandleProxy, SpaceServiceProxyError> {
    do {
      return try await .success(
        .init(spaceID: spaceID, leaveHandle: spaceService.leaveSpace(spaceId: spaceID)))
    } catch {
  ...
  func joinedParents(childID: String) async -> Result<[SpaceServiceRoom], SpaceServiceProxyError> {
    do {
      return try await .success(
        spaceService.joinedParentsOfChild(childId: childID).map(SpaceServiceRoom.init))
    } catch {
  ...
  > {
    do {
      return try await .success(
        spaceService.removeChildFromSpace(childId: childID, spaceId: spaceID))
    } catch {

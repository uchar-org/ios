...
class EffectsScene: SCNScene {
  ...
  static func confetti() -> EffectsScene? {
    ...
    if let particles = scene.rootNode.childNode(
      withName: Constants.particlesNodeName, recursively: false)?.particleSystems?.first
    {
...
extension Color {
  ...
  fileprivate var floatComponents: [Float]? {

            ...
            LazyVStack(spacing: 0, pinnedViews: .sectionHeaders) {
                Section {
                    ...
                    if context.viewState.leaveHandle.mode == .manyRooms {
                        Button(context.viewState.leaveHandle.selectedCount > 0
                            ? L10n.actionDeselectAll : L10n.actionSelectAll) {
                                context.send(viewAction: context.viewState.leaveHandle.selectedCount > 0
...
struct LeaveSpaceView_Previews: PreviewProvider, TestablePreview {
    ...
    static func makeViewModel(mode: LeaveSpaceHandleProxy.Mode) -> LeaveSpaceViewModel {
        let rooms: [LeaveSpaceRoom] =
            ...
            case .noRooms:
                .mockSingleSpace(spaceServiceRoom: spaceServiceRoom,
                                 isLastOwner: false,
                                 areCreatorsPrivileged: false)
            case .spaceNeedsNewOwner(let useTransferOwnershipFlow):
                .mockRoomsWithSpace(spaceServiceRoom: spaceServiceRoom,
                                    isLastOwner: true,

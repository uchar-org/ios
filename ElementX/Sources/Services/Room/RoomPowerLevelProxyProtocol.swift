...
extension RoomPowerLevelsProxyProtocol {
  ...
  func canOwnUserEditBaseInfo() -> Bool {
    canOwnUser(sendStateEvent: .roomAvatar) || canOwnUser(sendStateEvent: .roomName)
      || canOwnUser(sendStateEvent: .roomTopic)
  }
  ...
  func canOwnUserEditSecurityAndPrivacy(isSpace: Bool, joinRule: JoinRule?) -> Bool {
    ...
    return if isSpace {
      ...
      canOwnUser(sendStateEvent: .roomEncryption) || canOwnUser(sendStateEvent: .roomJoinRules)
        || canOwnUser(sendStateEvent: .roomHistoryVisibility) || canOwnUserChangeAddress
    }
  }
}

...
protocol RemoteSettingsHookProtocol {
  #if IS_MAIN_APP
    @MainActor func initializeCache(
      using client: ClientProtocol, applyingTo appSettings: CommonSettingsProtocol
    ) async -> Result<Void, RemoteSettingsError>
    func updateCache(using client: ClientProtocol) async
    @MainActor func reset(_ appSettings: CommonSettingsProtocol)
  #endif
  @MainActor func loadCache(
    forHomeserver homeserver: String, applyingTo appSettings: CommonSettingsProtocol)
}
...
struct DefaultRemoteSettingsHook: RemoteSettingsHookProtocol {
    ...
    func initializeCache(
      ...
      guard case .success(let wellKnownData) = await client.elementWellKnown() else {
      ...
      do {
        ...
        if wellKnown.enforceElementPro == true {
          ...
          let displayableServerName = LoginHomeserver(address: serverName, loginMode: .unknown)
            .address

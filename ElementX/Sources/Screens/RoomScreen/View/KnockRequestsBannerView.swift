...
struct KnockRequestsBannerView: View {
  ...
  private var mainContent: some View {
    if requests.count == 1 {
      SingleKnockRequestBannerContent(
        request: requests[0],
        onDismiss: onDismiss,
        onAccept: onAccept,
        onViewAll: onViewAll,
      ...
      MultipleKnockRequestsBannerContent(
        requests: requests,
        onDismiss: onDismiss,
        onViewAll: onViewAll,
        mediaProvider: mediaProvider)
...
private struct MultipleKnockRequestsBannerContent: View {
  ...
  private var multipleKnockRequestsTitle: String {
    ...
    return L10n.tr(
      "Localizable", "screen_room_multiple_knock_requests_title", string, avatars.count - 1)
  }
  ...
  var body: some View {
    VStack(spacing: 14) {
      HStack(spacing: 10) {
        StackedAvatarsView(
          overlap: 16, lineWidth: 2, avatars: avatars,
          avatarSize: .user(on: .knockingUsersBannerStack), mediaProvider: mediaProvider)
        HStack(alignment: .top, spacing: 0) {
...
struct KnockRequestsBannerView_Previews: PreviewProvider, TestablePreview {
  static let singleRequest: [KnockRequestInfo] = [
    .init(
      displayName: "Alice", avatarURL: nil, userID: "@alice:matrix.org", reason: nil, eventID: "1")
  ]

  static let singleRequestWithReason: [KnockRequestInfo] = [
    .init(
      displayName: "Alice",
      avatarURL: nil,
      userID: "@alice:matrix.org",
      reason:
        "Hey, I’d like to join this room because of xyz topic and I’d like to participate in the room.",
      eventID: "1")
  ]

  static let singleRequestNoDisplayName: [KnockRequestInfo] = [
    .init(displayName: nil, avatarURL: nil, userID: "@alice:matrix.org", reason: nil, eventID: "1")
  ]
  ...
  static let multipleRequests: [KnockRequestInfo] = [
    ...
    .init(
      displayName: "Charlie", avatarURL: nil, userID: "@charlie:matrix.org", reason: nil,
      eventID: "3"),
    .init(displayName: "Dan", avatarURL: nil, userID: "@dan:matrix.org", reason: nil, eventID: "4"),
    .init(
      displayName: "Test", avatarURL: nil, userID: "@dan:matrix.org", reason: nil, eventID: "5"),
  ]
  ...
  static var allPreviews: some View {
    KnockRequestsBannerView(requests: singleRequest) {
    } onAccept: { _ in
    } onViewAll: {
    }
    ...
    KnockRequestsBannerView(requests: singleRequestWithReason) {
    } onAccept: { _ in
    } onViewAll: {
    }
    .previewDisplayName("Single Request with reason")
    KnockRequestsBannerView(requests: singleRequestNoDisplayName) {
    } onAccept: { _ in
    } onViewAll: {
    }
    .previewDisplayName("Single Request, No Display Name")
    KnockRequestsBannerView(requests: multipleRequests) {
    } onAccept: { _ in
    } onViewAll: {
    }

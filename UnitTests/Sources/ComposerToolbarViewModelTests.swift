...
final class ComposerToolbarViewModelTests {
    ...
    func suggestions() {
        let suggestions: [SuggestionItem] = [
            .init(suggestionType: .user(.init(id: "@user_mention_1:matrix.org", displayName: "User 1", avatarURL: nil)),
                  range: .init(), rawSuggestionText: ""),
            .init(suggestionType: .user(.init(id: "@user_mention_2:matrix.org", displayName: "User 2", avatarURL: nil)),
                  range: .init(), rawSuggestionText: "")
        ]
    ...
    func suggestionTrigger() async throws {
        ...
        #expect(completionSuggestionServiceMock.setSuggestionTriggerReceivedInvocations == [
            nil,
    ...
    func selectedUserSuggestion() {
        let suggestion = SuggestionItem(suggestionType: .user(.init(id: "@test:matrix.org", displayName: "Test", avatarURL: nil)),
                                        range: .init(), rawSuggestionText: "")
        ...
        #expect(wysiwygViewModel.content.html
    ...
    func allUsersSuggestion() throws {
        let suggestion = SuggestionItem(suggestionType: .allUsers(.room(id: "", name: nil, avatarURL: nil)), range: .init(),
                                        rawSuggestionText: "")
        viewModel.context.send(viewAction: .selectedSuggestion(suggestion))
    ...
    func userMentionPillInRTE() async {
        ...
        let suggestion = SuggestionItem(suggestionType: .user(.init(id: userID, displayName: "Test", avatarURL: nil)), range: .init(),
                                        rawSuggestionText: "")
        viewModel.context.send(viewAction: .selectedSuggestion(suggestion))
    ...
    func roomMentionPillInRTE() async {
        ...
        let suggestion = SuggestionItem(suggestionType: .room(.init(id: "room-id", canonicalAlias: roomAlias, name: "Room",
    ...
    func restoreEdit() async {
        ...
        #expect(!viewModel.context.composerFormattingEnabled)
        #expect(viewModel.state.composerMode
        ...
        let deferred = deferFulfillment(viewModel.actions) { action in
            ...
            case .sendMessage(let plainText, _, _, let intentionalMentions):
                ...
                return plainText
                    ...
                    && intentionalMentions
                    == IntentionalMentions(userIDs: ["@user1:matrix.org", "@user2:matrix.org"], atRoom: true)
            default:
                return false
            }
        }

    ...
    HStack(spacing: 8) {
      ...
      HStack(spacing: 4) {
        ...
        if let dmRecipientVerificationState {
          VerificationBadge(
            verificationState: dmRecipientVerificationState, size: .xSmall,
...
extension View {
  fileprivate func roomHeaderAction(_ action: @escaping () -> Void) -> some View {
...
struct RoomHeaderView_Previews: PreviewProvider, TestablePreview {
  static var previews: some View {
    VStack(alignment: .leading, spacing: 8) {
      ...
      makeHeader(
        avatarURL: .mockMXCAvatar, verificationState: .notVerified, historySharingState: .shared)
      makeHeader(
        avatarURL: .mockMXCAvatar, verificationState: .notVerified,
        historySharingState: .worldReadable)
      makeHeader(
        avatarURL: .mockMXCAvatar, verificationState: .verified, historySharingState: .shared)
      makeHeader(
        avatarURL: .mockMXCAvatar, verificationState: .verificationViolation,
        historySharingState: .worldReadable)
    }
  ...
  static func makeHeader(
    ...
    historySharingState: RoomHistorySharingState? = nil
  ) -> some View {
    RoomHeaderView(

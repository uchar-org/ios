...
enum TimelineViewModelAction {
  ...
  case displayResolveSendFailure(
...
struct TimelineViewState: BindableState {
  ...
  var audioPlayerStateProvider:
    (@MainActor (_ itemId: TimelineItemIdentifier) -> AudioPlayerState?)?
...
extension TimelineViewState {
  ...
  func buildMessagePreview(formattedBody: AttributedString?, plainBody: String) -> String {
    guard let formattedBody,
      let attributedString = try? NSMutableAttributedString(formattedBody, including: \.elementX)
    else {
      return plainBody
    }
    ...
    attributedString.enumerateAttributes(in: range) { attributes, range, _ in
      ...
      if let roomAlias = attributes[.MatrixRoomAlias] as? String {
        ...
        attributedString.replaceCharacters(
          in: range,
          with: PillUtilities.roomPillDisplayText(roomName: roomName, rawRoomText: roomAlias))
      }
      ...
      if let roomID = attributes[.MatrixRoomID] as? String {
        ...
        attributedString.replaceCharacters(
          in: range,
          with: PillUtilities.roomPillDisplayText(roomName: roomName, rawRoomText: roomID))
      }
      ...
      if let eventOnRoomID = attributes[.MatrixEventOnRoomID] as? EventOnRoomIDAttribute.Value {
        ...
        attributedString.replaceCharacters(
          in: range,
          with: PillUtilities.eventPillDisplayText(roomName: roomName, rawRoomText: roomID))
      }

      if let eventOnRoomAlias = attributes[.MatrixEventOnRoomAlias]
        as? EventOnRoomAliasAttribute.Value
      {
        let roomAlias = eventOnRoomAlias.alias
        let roomName = roomNameForAliasResolver?(roomAlias)
        attributedString.replaceCharacters(
          in: range,
          with: PillUtilities.eventPillDisplayText(
            roomName: roomName, rawRoomText: eventOnRoomAlias.alias))
      }
    }

    ...
    HStack(alignment: .top, spacing: 16) {
      ...
      if let timestamp = room.timestamp {
        Text(timestamp)
          ...
          .foregroundColor(
            room.isHighlighted ? .compound.textActionAccent : .compound.textSecondary)
      }
    }
  }
...
struct HomeScreenRoomCellButtonStyle: ButtonStyle {
  ...
  func makeBody(configuration: Configuration) -> some View {
    configuration.label
      ...
      .animation(
        isSelected ? .none : .easeOut(duration: 0.1).disabledDuringTests(), value: isSelected)
  }
}

extension View {
  fileprivate func lastMessageFormatting(hasFailed: Bool) -> some View {
...
struct HomeScreenRoomCell_Previews: PreviewProvider, TestablePreview {
  ...
  static let summaryProviderForNotificationsState = RoomSummaryProviderMock(
    .init(state: .loaded(.mockRoomsWithNotificationsState)))
  static let notificationsStateRooms = summaryProviderForNotificationsState.roomListPublisher.value
    .compactMap(mockRoom)

  static let lastMessageStateRooms = [
    makeRoom(lastMessageState: .sending), makeRoom(lastMessageState: .failed),
  ]
  ...
  static var previews: some View {
    VStack(spacing: 0) {
      ForEach(genericRooms) { room in
        HomeScreenRoomCell(
          room: room, isSelected: false, mediaProvider: MediaProviderMock(configuration: .init())
        ) { _ in }
      }

      HomeScreenRoomCell(
        room: .placeholder(), isSelected: false,
    ...
    let userSession = UserSessionMock(
      .init(
        clientProxy: ClientProxyMock(
          .init(userID: "John Doe", roomSummaryProvider: roomSummaryProvider))))
  ...
  static func makeRoom(lastMessageState: RoomSummary.LastMessageState) -> HomeScreenRoom {
    let summary = RoomSummary(
      ...
      lastMessage: AttributedString(
        "How do you see the Emperor then? You think he keeps office hours?"),

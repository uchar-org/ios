  ...
  private var waveformView: some View {
    if let url = playerState.fileURL {
      WaveformView(
        ...
        configuration: .init(
          style: .striped(
            .init(color: .black, width: waveformLineWidth, spacing: waveformLinePadding)),
...
extension DateFormatter {
  ...
  fileprivate static let longElapsedTimeFormatter: DateFormatter = {
    let dateFormatter = DateFormatter()
...
struct VoiceMessageRoomPlaybackView_Previews: PreviewProvider, TestablePreview {
  static let waveform = EstimatedWaveform(data: [
    3, 127, 400, 266, 126, 122, 373, 251, 45, 112,

...
final class AppLockSetupPINScreenCoordinator: CoordinatorProtocol {
  ...
  private let actionsSubject: PassthroughSubject<AppLockSetupPINScreenCoordinatorAction, Never> =
    .init()
  ...
  init(parameters: AppLockSetupPINScreenCoordinatorParameters) {
    guard parameters.initialMode != .confirm else {
      fatalError(".confirm is an invalid initial mode")
    }

    viewModel = AppLockSetupPINScreenViewModel(
      initialMode: parameters.initialMode,

...
class UITestsAppCoordinator: AppCoordinatorProtocol, SecureWindowManagerDelegate {
  ...
  func start() {
    guard let screenID = ProcessInfo.testScreenID else {
      fatalError("Unable to launch with unknown screen.")
    }

    let mockScreen = MockScreen(
      id: screenID, windowManager: windowManager,
      navigationRootCoordinator: navigationRootCoordinator)
  ...
  func windowManagerDidConfigureWindows(_ windowManager: SecureWindowManagerProtocol) {
    ...
    guard let screenID = ProcessInfo.testScreenID,
      screenID == .appLockFlow || screenID == .appLockFlowDisabled
    else { return }
    let screen = MockScreen(
      id: screenID == .appLockFlow
        ? .appLockFlowAlternateWindow : .appLockFlowDisabledAlternateWindow,
    ...
    windowManager.alternateWindow.rootViewController = UIHostingController(
      rootView: coordinator.toPresentable().statusBarHidden())
...
class MockScreen: Identifiable {
  ...
  lazy var coordinator: CoordinatorProtocol? = {
    ...
    case .serverSelection:
      ...
      let coordinator = ServerSelectionScreenCoordinator(
        parameters: .init(
    ...
    case .authenticationFlow, .provisionedAuthenticationFlow, .singleProviderAuthenticationFlow,
      .multipleProvidersAuthenticationFlow:
      ...
      if id == .singleProviderAuthenticationFlow || id == .multipleProvidersAuthenticationFlow {
        let accountProviders =
          id == .singleProviderAuthenticationFlow
          ? ["example.com"] : ["guest.example.com", "example.com"]
      ...
      let flowCoordinator = AuthenticationFlowCoordinator(
        authenticationService: AuthenticationService.mock,
      ...
      if id == .provisionedAuthenticationFlow {
        flowCoordinator.handleAppRoute(
          .accountProvisioningLink(.init(accountProvider: "example.com", loginHint: nil)),
          animated: false)
      }
    ...
    case .appLockFlowAlternateWindow, .appLockFlowDisabledAlternateWindow:
      ...
      let keychainController = KeychainController(
        service: .tests, accessGroup: InfoPlistReader.main.keychainAccessGroupIdentifier)
      ...
      if id == .appLockFlowAlternateWindow {
        ...
        guard case .success = appLockService.setupPINCode(pinCode),
          appLockService.unlock(with: pinCode)
        else {
      ...
      let flowCoordinator = AppLockFlowCoordinator(
    ...
    case .appLockSetupFlow, .appLockSetupFlowUnlock, .appLockSetupFlowMandatory:
      ...
      let flow: AppLockSetupFlowCoordinator.PresentationFlow =
        id == .appLockSetupFlowMandatory ? .onboarding : .settings
      let flowCoordinator = AppLockSetupFlowCoordinator(
        presentingFlow: flow,
        appLockService: appLockService,
    ...
    case .bugReport:
      ...
      let coordinator = BugReportScreenCoordinator(
        parameters: .init(
          bugReportService: BugReportServiceMock(.init()),
    ...
    case .roomSmallTimelineLargePagination:
      ...
      let parameters = RoomScreenCoordinatorParameters(
        ...
        roomProxy: JoinedRoomProxyMock(
          .init(name: "Small timeline, paginating", avatarURL: .mockMXCAvatar)),
        timelineController: timelineController,
        mediaPlayerProvider: MediaPlayerProviderMock(),
        emojiProvider: EmojiProvider(appSettings: ServiceLocator.shared.settings),
        linkMetadataProvider: LinkMetadataProvider(),
    ...
    case .roomLayoutHighlight:
      ...
      let parameters = RoomScreenCoordinatorParameters(
        userSession: UserSessionMock(.init()),
        roomProxy: JoinedRoomProxyMock(
          .init(name: "Timeline highlight", avatarURL: .mockMXCAvatar)),
        timelineController: timelineController,
        mediaPlayerProvider: MediaPlayerProviderMock(),
        emojiProvider: EmojiProvider(appSettings: ServiceLocator.shared.settings),
        linkMetadataProvider: LinkMetadataProvider(),
    ...
    case .sessionVerification:
      var sessionVerificationControllerProxy = SessionVerificationControllerProxyMock.configureMock(
        otherDeviceStartsSasVerification: true,
        requestDelay: .seconds(5))
      let parameters = SessionVerificationScreenCoordinatorParameters(
        sessionVerificationControllerProxy: sessionVerificationControllerProxy,
    ...
    case .userSessionScreen, .userSessionScreenReply, .userSessionSpacesFlow:
      ...
      let clientProxy = ClientProxyMock(
        .init(
          ...
          spaceServiceConfiguration: .init(
            topLevelSpaces: .mockSpaceList.filter(\.isSpace) + .mockSingleRoom),
      ...
      let spaceServiceProxy = SpaceServiceProxyMock(
        id == .userSessionSpacesFlow ? .populated : .init())
      ...
      let flowCoordinator = UserSessionFlowCoordinator(
        isNewLogin: false,
        navigationRootCoordinator: navigationRootCoordinator,
        appLockService: AppLockService(
    ...
    case .roomMembersListScreenPendingInvites:
      ...
      let coordinator = RoomMembersListScreenCoordinator(
        parameters: .init(
          userSession: UserSessionMock(.init()),
    ...
    case .roomRolesAndPermissionsFlow:
      let navigationStackCoordinator = NavigationStackCoordinator()
      navigationStackCoordinator.setRootCoordinator(BlankFormCoordinator())
      let coordinator = RoomRolesAndPermissionsFlowCoordinator(
        parameters: .init(
    ...
    case .startChatFlow:
      ...
      clientProxy.roomForIdentifierClosure = { roomID in
        .joined(JoinedRoomProxyMock(.init(id: roomID, members: [])))
      }
      ...
      let navigationStackCoordinator = NavigationStackCoordinator()
      let flowCoordinator = StartChatFlowCoordinator(
    ...
    case .encryptionSettings, .encryptionSettingsOutOfSync:
      let recoveryState: SecureBackupRecoveryState =
        id == .encryptionSettings ? .enabled : .incomplete
      let clientProxy = ClientProxyMock(
        .init(userID: "@mock:client.com", recoveryState: recoveryState))
      let userSession = UserSessionMock(.init(clientProxy: clientProxy))

      let navigationStackCoordinator = NavigationStackCoordinator()
      navigationStackCoordinator.setRootCoordinator(BlankFormCoordinator())

      let coordinator = EncryptionSettingsFlowCoordinator(
        parameters: .init(
          userSession: userSession,
          appSettings: ServiceLocator.shared.settings,
          userIndicatorController: UserIndicatorControllerMock(),
    ...
    case .encryptionReset:
      ...
      let coordinator = EncryptionResetFlowCoordinator(
        parameters: .init(
          userSession: userSession,
          appSettings: ServiceLocator.shared.settings,
      ...
      return navigationStackCoordinator
    case .linkNewDevice:
      let linkMobileProgressSubject:
        CurrentValueSubject<LinkNewDeviceService.LinkMobileProgress, QRCodeLoginError> = .init(
          .qrReady(LinkNewDeviceServiceMock.mockQRCodeImage))
      let linkNewDeviceService = LinkNewDeviceServiceMock(
        .init(linkMobileProgressPublisher: linkMobileProgressSubject.asCurrentValuePublisher()))

      let clientProxy = ClientProxyMock(.init())
      clientProxy.linkNewDeviceServiceReturnValue = linkNewDeviceService

      let navigationStackCoordinator = NavigationStackCoordinator()
      let flowCoordinator = LinkNewDeviceFlowCoordinator(
        navigationStackCoordinator: navigationStackCoordinator,
        flowParameters: CommonFlowParameters(
          userSession: UserSessionMock(.init(clientProxy: clientProxy)),
          bugReportService: BugReportServiceMock(.init()),
    ...
    case .autoUpdatingTimeline:
      ...
      let navigationSplitCoordinator = NavigationSplitCoordinator(
        placeholderCoordinator: PlaceholderScreenCoordinator(hideBrandChrome: false))
      ...
      let roomProxy = JoinedRoomProxyMock(
        .init(id: "whatever", name: "okay", shouldUseAutoUpdatingTimeline: true))
      ...
      let flowCoordinator = ChatsTabFlowCoordinator(
        isNewLogin: false,
        ...
        flowParameters: CommonFlowParameters(
          ...
          timelineControllerFactory: TimelineControllerFactoryMock(
            .init(timelineController: timelineController)),
          emojiProvider: EmojiProvider(appSettings: appSettings),
          linkMetadataProvider: LinkMetadataProvider(),
          appMediator: AppMediatorMock.default,
          appSettings: appSettings,

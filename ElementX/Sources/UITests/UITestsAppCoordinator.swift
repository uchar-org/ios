...
class UITestsAppCoordinator: AppCoordinatorProtocol, SecureWindowManagerDelegate {
    ...
    func windowManagerDidConfigureWindows(_ windowManager: SecureWindowManagerProtocol) {
        ...
        let screen = MockScreen(id: screenID == .appLockFlow
    ...
    lazy var coordinator: CoordinatorProtocol? = {
        ...
        case .authenticationFlow, .provisionedAuthenticationFlow, .singleProviderAuthenticationFlow,
            ...
            if id == .provisionedAuthenticationFlow {
                flowCoordinator.handleAppRoute(.accountProvisioningLink(.init(accountProvider: "example.com", loginHint: nil)),
                                               animated: false)
            }
        ...
        case .linkNewDevice:
            let linkMobileProgressSubject:
                CurrentValueSubject<LinkNewDeviceService.LinkMobileProgress, QRCodeLoginError> = .init(.qrReady(LinkNewDeviceServiceMock.mockQRCodeImage))

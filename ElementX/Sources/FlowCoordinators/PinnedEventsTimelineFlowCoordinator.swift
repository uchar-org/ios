...
class PinnedEventsTimelineFlowCoordinator: FlowCoordinatorProtocol {
  ...
  private let actionsSubject: PassthroughSubject<PinnedEventsTimelineFlowCoordinatorAction, Never> =
    .init()
  ...
  private func presentPinnedEventsTimeline() async {
    ...
    guard
      case .success(let timelineController) = await flowParameters.timelineControllerFactory
        .buildPinnedEventsTimelineController(
          roomProxy: roomProxy,
          timelineItemFactory: timelineItemFactory,
          mediaProvider: userSession.mediaProvider)
    else {
      fatalError(
        "This can never fail because we allow this view to be presented only when the timeline is fully loaded and not nil"
      )
    }

    let coordinator = PinnedEventsTimelineScreenCoordinator(
      parameters: .init(
        roomProxy: roomProxy,
    ...
    coordinator.actions
      .sink { [weak self] action in
        ...
        case .presentLocationViewer(let location):
          presentMapNavigator(
            interactionMode: .viewStatic(location),
            timelineController: timelineController)
        ...
        case .displayRoomScreenWithFocussedPin(let eventID, let threadRootEventID):
          actionsSubject.send(
            .displayRoomScreenWithFocussedPin(
              eventID: eventID, threadRootEventID: threadRootEventID))
        }
      }

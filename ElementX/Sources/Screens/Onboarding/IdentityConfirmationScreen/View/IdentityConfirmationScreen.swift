  ...
  private var actionButtons: some View {
    VStack(spacing: 16) {
      if let availableActions = context.viewState.availableActions {
        ...
        Button { /* Placeholder button, there is no action */
        } label: {
          Label {
...
struct IdentityConfirmationScreen_Previews: PreviewProvider, TestablePreview {
  ...
  static var previews: some View {
    ...
    .snapshotPreferences(
      expect: viewModel.context.observe(\.viewState.availableActions).map { actions in
    ...
    -> IdentityConfirmationScreenViewModel
  {
    ...
    userSession.sessionSecurityStatePublisher = CurrentValuePublisher<SessionSecurityState, Never>(
      .init(verificationState: .unverified, recoveryState: recoveryState))

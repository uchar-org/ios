...
struct LoginScreen_Previews: PreviewProvider, TestablePreview {
  ...
  static let unconfiguredViewModel = makeViewModel(
    homeserverAddress: "somethingtofailconfiguration")
  ...
  static var previews: some View {
    ...
    .snapshotPreferences(
      expect: viewModel.context.observe(\.viewState.homeserver.loginMode).map { $0 == .password }
    )
    ...
    .snapshotPreferences(
      expect: credentialsViewModel.context.observe(\.viewState.homeserver.loginMode).map {
        $0 == .password
      }
    )
  ...
  static func makeViewModel(
    homeserverAddress: String = "example.com", withCredentials: Bool = false

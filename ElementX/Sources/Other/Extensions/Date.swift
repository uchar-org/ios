...
extension Date {
  ...
  func formattedMinimal() -> String {
    ...
    if calendar.isDateInToday(self) {
      ...
      return yesterday.formatted(
        Date.RelativeFormatStyle(presentation: .named, capitalizationContext: .beginningOfSentence))
    } else if let sixDaysAgo = calendar.date(
  ...
  static var mock: Date {
    DateComponents(calendar: .current, year: 2007, month: 1, day: 9, hour: 9, minute: 41).date
      ?? .now
  }
}
...
extension DateFormatter {
  ...
  fileprivate static let relative: DateFormatter = {

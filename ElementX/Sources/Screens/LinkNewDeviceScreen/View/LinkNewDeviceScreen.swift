...
struct LinkNewDeviceScreen: View {
  ...
  var mainContent: some View {
    ...
    case .loading, .readyToLink:
      TitleAndIcon(
        title: L10n.screenLinkNewDeviceRootTitle,
        icon: \.computer,
        iconStyle: .default)
    case .error:
      ...
      VStack(spacing: 16) {
        Button {
          ...
          Label {
            Text(
              isGeneratingCode
                ? L10n.screenLinkNewDeviceRootLoadingQrCode
                : L10n.screenLinkNewDeviceRootMobileDevice)
          } icon: {
            if isGeneratingCode {
              ProgressView()
                .tint(.compound.iconOnSolidPrimary)
...
struct LinkNewDeviceScreen_Previews: PreviewProvider, TestablePreview {
  ...
  static var previews: some View {
    ...
    .snapshotPreferences(
      expect: viewModel.context.observe(\.viewState.mode).map {
        $0 == .readyToLink(isGeneratingCode: false)
      })
    ...
    .snapshotPreferences(
      expect: generatingViewModel.context.observe(\.viewState.mode).map {
        $0 == .readyToLink(isGeneratingCode: true)
      })
    ...
    .snapshotPreferences(
      expect: unsupportedViewModel.context.observe(\.viewState.mode).map {
        $0 == .error(.notSupported)
      })
    ...
    .snapshotPreferences(
      expect: unknownErrorViewModel.context.observe(\.viewState.mode).map { $0 == .error(.unknown) }
    )
  }

  static func makeViewModel(mode: LinkNewDeviceScreenViewState.Mode) -> LinkNewDeviceScreenViewModel
  {
    LinkNewDeviceScreenViewModel(

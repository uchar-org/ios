...
protocol MentionBuilderProtocol {
  func handleUserMention(
    for attributedString: NSMutableAttributedString, in range: NSRange, url: URL, userID: String,
    userDisplayName: String?)
  func handleRoomIDMention(
    for attributedString: NSMutableAttributedString, in range: NSRange, url: URL, roomID: String)
  func handleRoomAliasMention(
    for attributedString: NSMutableAttributedString, in range: NSRange, url: URL, roomAlias: String,
    roomDisplayName: String?)
  func handleEventOnRoomAliasMention(
    for attributedString: NSMutableAttributedString, in range: NSRange, url: URL, eventID: String,
    roomAlias: String)
  func handleEventOnRoomIDMention(
    for attributedString: NSMutableAttributedString, in range: NSRange, url: URL, eventID: String,
    roomID: String)
...
extension NSAttributedString.Key {
  ...
  static let MatrixUserDisplayName: NSAttributedString.Key = .init(
    rawValue: UserDisplayNameAttribute.name)
  static let MatrixRoomDisplayName: NSAttributedString.Key = .init(
    rawValue: RoomDisplayNameAttribute.name)
  ...
  static let MatrixEventOnRoomID: NSAttributedString.Key = .init(
    rawValue: EventOnRoomIDAttribute.name)
  static let MatrixEventOnRoomAlias: NSAttributedString.Key = .init(
    rawValue: EventOnRoomAliasAttribute.name)
  static let MatrixAllUsersMention: NSAttributedString.Key = .init(
    rawValue: AllUsersMentionAttribute.name)
...
struct AttributedStringBuilder: AttributedStringBuilderProtocol {
  ...
  private static let cacheDispatchQueue = DispatchQueue(
    label: "io.element.elementx.attributed_string_builder_v2_cache")
  ...
  func fromHTML(_ htmlString: String?) -> AttributedString? {
    ...
    var listIndex = 1
    let mutableAttributedString = attributedString(
      element: body, documentBody: body, preserveFormatting: false, listTag: nil,
      listIndex: &listIndex, indentLevel: 0)
    ...
    for node in element.getChildNodes() {
      ...
      case "h1", "h2", "h3", "h4", "h5", "h6":
        ...
        content = attributedString(
          element: childElement, documentBody: documentBody, preserveFormatting: preserveFormatting,
          listTag: listTag, listIndex: &childIndex, indentLevel: indentLevel)
      ...
      case "sup":
        content = attributedString(
          ...
          listTag: listTag, listIndex: &childIndex, indentLevel: indentLevel)
        content.addAttribute(
          .baselineOffset, value: 6, range: NSRange(location: 0, length: content.length))
      ...
      case "sub":
        content = attributedString(
          ...
          listTag: listTag, listIndex: &childIndex, indentLevel: indentLevel)
        content.addAttribute(
          .baselineOffset, value: -4, range: NSRange(location: 0, length: content.length))
        content.setFontPreservingSymbolicTraits(UIFont.systemFont(ofSize: fontPointSize * 0.7))
      ...
      case "blockquote":
        content = attributedString(
          ...
          listTag: listTag, listIndex: &childIndex, indentLevel: indentLevel)
        content.addAttribute(
          .MatrixBlockquote, value: true, range: NSRange(location: 0, length: content.length))
      ...
      case "code", "pre":
        ...
        content.setFontPreservingSymbolicTraits(
          UIFont.monospacedSystemFont(ofSize: fontPointSize, weight: .regular))

        if isCodeBlock {
          content.addAttribute(
            .CodeBlock, value: true, range: NSRange(location: 0, length: content.length))
          // The scroll view provides the background colour for code blocks.
        } else {
          content.addAttribute(
            .InlineCode, value: true, range: NSRange(location: 0, length: content.length))
          // But inline code is (obviously) inline so it's much easier to set the background colour here.
          content.addAttribute(
            .backgroundColor, value: UIColor.compound._bgCodeBlock as Any,
            range: NSRange(location: 0, length: content.length))
        }
        ...
        content.removeAttribute(
          .MatrixRoomAlias, range: NSRange(location: 0, length: content.length))
        content.removeAttribute(.MatrixUserID, range: NSRange(location: 0, length: content.length))
        content.removeAttribute(
          .MatrixEventOnRoomID, range: NSRange(location: 0, length: content.length))
        content.removeAttribute(
          .MatrixEventOnRoomAlias, range: NSRange(location: 0, length: content.length))
        content.removeAttribute(
          .MatrixAllUsersMention, range: NSRange(location: 0, length: content.length))
      ...
      case "a":
        ...
        if let href = try? childElement.attr("href"), let url = URL(string: href) {
          content.addAttribute(
            .link, value: url, range: NSRange(location: 0, length: content.length))
        }

      case "span":
        if childElement.dataset()[Self.attributeMSC4286] == nil {
          content = attributedString(
            element: childElement, documentBody: documentBody,
            preserveFormatting: preserveFormatting, listTag: listTag, listIndex: &childIndex,
            indentLevel: indentLevel)
        }

      case "ul", "ol":
        var listIndex = 1
        if let startAttribute = try? childElement.attr("start"),
          let startIndex = Int(startAttribute)
        {
        ...
        content = attributedString(
          element: childElement, documentBody: documentBody, preserveFormatting: preserveFormatting,
          listTag: tag, listIndex: &listIndex, indentLevel: indentLevel + 1)
      ...
      case "li":
        ...
        content = attributedString(
          element: childElement, documentBody: documentBody, preserveFormatting: preserveFormatting,
          listTag: listTag, listIndex: &childIndex, indentLevel: indentLevel + 1)
  ...
  private func addLinksAndMentions(_ attributedString: NSMutableAttributedString) {
    ...
    var matches: [TextParsingMatch] = MatrixEntityRegex.userIdentifierRegex.matches(in: string)
      .compactMap { match in
    ...
    matches.append(
      contentsOf: MatrixEntityRegex.roomAliasRegex.matches(in: string).compactMap { match in
        guard let matchRange = Range(match.range, in: string) else {
          return nil
        }
    ...
    matches.append(
      contentsOf: MatrixEntityRegex.uriRegex.matches(in: string).compactMap { match in
        guard let matchRange = Range(match.range, in: string) else {
          return nil
        }
    ...
    matches.append(
      contentsOf: MatrixEntityRegex.linkRegex.matches(in: string).compactMap { match in
        ...
        if url.scheme?.contains("http") ?? false {
          ...
          return TextParsingMatch(
            type: .link(urlString: String(string[matchRange]).asSanitizedLink), range: match.range)
    ...
    matches.append(
      contentsOf: MatrixEntityRegex.allUsersRegex.matches(in: attributedString.string).map {
        match in
    ...
    matches.sorted { $0.range.length > $1.range.length }.forEach { [attributedString] match in
      ...
      let isCode =
        attributedString.attribute(.CodeBlock, at: match.range.location, effectiveRange: nil) != nil
        || attributedString.attribute(.InlineCode, at: match.range.location, effectiveRange: nil)
          != nil
      if isCode {
        return
      }
  ...
  func addMatrixEntityPermalinkAttributesTo(_ attributedString: NSMutableAttributedString) {
    attributedString.enumerateAttribute(
      .link, in: .init(location: 0, length: attributedString.length), options: []
    ) { value, range, _ in
      if value != nil {
          ...
          let matrixEntity = parseMatrixEntityFrom(uri: url.absoluteString)
        {
          ...
          case .user(let userID):
            mentionBuilder.handleUserMention(
              for: attributedString, in: range, url: url, userID: userID, userDisplayName: nil)
          case .room(let roomID):
            mentionBuilder.handleRoomIDMention(
              for: attributedString, in: range, url: url, roomID: roomID)
          case .roomAlias(let alias):
            mentionBuilder.handleRoomAliasMention(
              for: attributedString, in: range, url: url, roomAlias: alias, roomDisplayName: nil)
          case .eventOnRoomId(let roomID, let eventID):
            mentionBuilder.handleEventOnRoomIDMention(
              for: attributedString, in: range, url: url, eventID: eventID, roomID: roomID)
          case .eventOnRoomAlias(let alias, let eventID):
            mentionBuilder.handleEventOnRoomAliasMention(
              for: attributedString, in: range, url: url, eventID: eventID, roomAlias: alias)
          }
        }
      }
    }
    ...
    attributedString.enumerateAttribute(
      .MatrixAllUsersMention, in: .init(location: 0, length: attributedString.length), options: []
    ) { value, range, _ in
  ...
  private func detectPhishingAttempts(_ attributedString: NSMutableAttributedString) {
    attributedString.enumerateAttribute(
      ...
      guard
        PhishingDetector.isPhishingAttempt(displayString: displayString, internalURL: internalURL)
      else {
        return
      }
      handlePhishingAttempt(
        for: attributedString, in: range, internalURL: internalURL, displayString: displayString)
    }
  }

  private func handlePhishingAttempt(
...
extension NSMutableAttributedString {
  fileprivate func setFontPreservingSymbolicTraits(_ newFont: UIFont) {
...
extension NSString {
  fileprivate func hasSuffixCharacter(from characterSet: CharacterSet) -> Bool {
    if length == 0 {
      return false
    }

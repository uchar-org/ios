...
struct SecureBackupRecoveryKeyScreen: View {
  ...
  private var recoveryCreatedActionButtons: some View {
    VStack(spacing: 16) {
      ...
      .buttonStyle(.compound(.primary))
      .disabled(
        context.viewState.recoveryKey == nil || context.viewState.doneButtonEnabled == false
      )
        ...
        if context.viewState.recoveryKey == nil {
          if !context.viewState.isGeneratingKey {
            ...
            .accessibilityIdentifier(
              A11yIdentifiers.secureBackupRecoveryKeyScreen.generateRecoveryKey)
          } else {
            HStack(spacing: 8) {
              ProgressView()
  ...
  private var generateButtonTitle: String {
    context.viewState.mode == .setupRecovery
      ? L10n.screenRecoveryKeySetupGenerateKey : L10n.screenRecoveryKeyChangeGenerateKey
  }
  ...
  private var confirmRecoveryKeySection: some View {
    VStack(alignment: .leading, spacing: 8) {
      ...
      SecureField(
        L10n.screenRecoveryKeyConfirmKeyPlaceholder, text: $context.confirmationRecoveryKey
      )
...
struct SecureBackupRecoveryKeyScreen_Previews: PreviewProvider, TestablePreview {
  ...
  static var previews: some View {
    ...
    .snapshotPreferences(
      expect: setupViewModel.context.observe(\.viewState.recoveryKey).map { $0 != nil }
    )
  ...
  static func viewModel(
    recoveryState: SecureBackupRecoveryState, generateKey: Bool = false, key: String? = nil
  ) -> SecureBackupRecoveryKeyScreenViewModelType {
    ...
    let viewModel = SecureBackupRecoveryKeyScreenViewModel(

...
typealias ComposerToolbarViewModelType = StateStoreViewModel<
  ComposerToolbarViewState, ComposerToolbarViewAction
>
...
final class ComposerToolbarViewModel: ComposerToolbarViewModelType, ComposerToolbarViewModelProtocol
{
  ...
  init(
    ...
    attributedStringBuilder = AttributedStringBuilder(
      cacheKey: "Composer", mentionBuilder: mentionBuilder)

    super.init(
      initialViewState: ComposerToolbarViewState(
    ...
    let identityStatusChangesPublisher = roomProxy.identityStatusChangesPublisher.receive(
  ...
  override func process(viewAction: ComposerToolbarViewAction) {
    ...
    case .sendMessage:
      ...
      default:
        if context.composerFormattingEnabled {
          actionsSubject.send(
            .sendMessage(
              plain: wysiwygViewModel.content.markdown,
    ...
    case .plainComposerTextChanged:
      completionSuggestionService.processTextMessage(
        state.bindings.plainComposerText.string,
        selectedRange: context.viewState.bindings.selectedRange)
    ...
    case .didToggleFormattingOptions:
      if context.composerFormattingEnabled {
        ...
        context.plainComposerText = NSAttributedString(
          string: wysiwygViewModel.attributedContent.text.string)
      }
    }
  }
  ...
  func loadDraft() async {
    if let initialText {
      ...
      guard case .success(let draft) = await draftService.loadDraft(),
        let draft
      else {
        return
      }
  ...
  private func handleLoadDraft(_ draft: ComposerDraftProxy) {
    ...
    case .reply(let eventID):
      ...
      replyLoadingTask = Task {
        let reply =
          switch await draftService.getReply(eventID: eventID) {
          case .success(let reply):
            reply
          case .failure:
            TimelineItemReply(
              details: .error(eventID: eventID, message: L10n.commonSomethingWentWrong),
              isThreaded: false)
          }
  ...
  private func handleSaveDraft(isVolatile: Bool) {
    ...
    if isVolatile {
      draftService.saveVolatileDraft(
        .init(plainText: plainText, htmlText: htmlText, draftType: type))
    } else {
      Task {
  ...
  private var plainComposerContent: PlainComposerContent {
    ...
    repeat {  // Don't enumerate and mutate at the same time, big no no
      ...
      attributedString.enumerateAttribute(
        ...
        if let userID = attributedString.attribute(
          .MatrixUserID, at: range.location, effectiveRange: nil) as? String
        {
          let displayName = attributedString.attribute(
            .MatrixUserDisplayName, at: range.location, effectiveRange: nil)
          attributedString.replaceCharacters(
            in: range, with: "[\(displayName ?? userID)](\(value))")
          userIDs.insert(userID)
          stop.pointee = true
        } else if let roomAlias = attributedString.attribute(
          .MatrixRoomAlias, at: range.location, effectiveRange: nil) as? String
        {
          let displayName = attributedString.attribute(
            .MatrixRoomDisplayName, at: range.location, effectiveRange: nil)
          attributedString.replaceCharacters(
            in: range, with: "[\(displayName ?? roomAlias)](\(value))")
          stop.pointee = true
        } else {
          return
        }
    ...
    return .init(
      text: attributedString.string, mentionedUserIDs: userIDs,
  ...
  private func sendPlainComposerText() {
    ...
    actionsSubject.send(
      .sendMessage(
        plain: plainComposerContent.text, html: nil,
        mode: state.composerMode,
        intentionalMentions: .init(
          userIDs: plainComposerContent.mentionedUserIDs,
    ...
    wysiwygViewModel.mentionReplacer = ComposerMentionReplacer { [weak self] urlString, string in
      ...
      if urlString == PillUtilities.composerAtRoomURLString {
        attributedString = NSMutableAttributedString(
          string: string, attributes: [.MatrixAllUsersMention: true])
      } else {
        attributedString = NSMutableAttributedString(
          string: string, attributes: [.link: URL(string: urlString) as Any])
      }
      ...
      attributedString.removeAttribute(
        .link, range: NSRange(location: 0, length: attributedString.length))
  ...
  private func handleSuggestion(_ suggestion: SuggestionItem) {
    ...
    case .user(let user):
      ...
      if context.composerFormattingEnabled {
        ...
        let attributedString = NSMutableAttributedString(
          attributedString: state.bindings.plainComposerText)
        mentionBuilder.handleUserMention(
          for: attributedString, in: suggestion.range, url: url, userID: user.id,
          userDisplayName: user.displayName)
        state.bindings.plainComposerText = attributedString

        let newSelectedRange = NSRange(
          location: state.bindings.selectedRange.location - suggestion.rawSuggestionText.count,
          length: 0)
    ...
    case .room(let room):
      guard let url = try? URL(string: matrixToRoomAliasPermalink(roomAlias: room.canonicalAlias))
      else {
      ...
      if context.composerFormattingEnabled {
        ...
        mentionBuilder.handleRoomAliasMention(
          for: attributedString, in: suggestion.range, url: url, roomAlias: room.canonicalAlias,
  ...
  private func processIdentityStatusChanges(_ changes: [IdentityStatusChange]) async {
    for change in changes {
      ...
      case .verificationViolation:
        guard case .success(let member) = await roomProxy.getMember(userID: change.userId) else {
  ...
  private func createLinkAlert() {
    ...
    currentLinkData = WysiwygLinkData(
      action: linkAction,
    ...
    switch linkAction {
    case .createWithText:
      state.bindings.alertInfo = makeCreateWithTextAlertInfo(
        urlBinding: urlBinding, textBinding: textBinding)
    case .create:
  ...
  private func makeCreateWithTextAlertInfo(
    urlBinding: Binding<String>, textBinding: Binding<String>
  ) -> AlertInfo<UUID> {
    AlertInfo(
      id: UUID(),
      ...
      textFields: [
        AlertInfo<UUID>.AlertTextField(
          placeholder: L10n.commonText,
          text: textBinding,
          autoCapitalization: .never,
          autoCorrectionDisabled: false),
        AlertInfo<UUID>.AlertTextField(
          placeholder: L10n.richTextEditorUrlPlaceholder,
          text: urlBinding,
          autoCapitalization: .never,
  ...
  private func makeEditChoiceAlertInfo(urlBinding: Binding<String>) -> AlertInfo<UUID> {
    AlertInfo(
      ...
      verticalButtons: [
        AlertInfo<UUID>.AlertButton(title: L10n.actionEdit) {
          self.state.bindings.alertInfo = nil
          DispatchQueue.main.async {
            self.state.bindings.alertInfo = self.makeSetUrlAlertInfo(
              urlBinding: urlBinding, isEdit: true)
          }
        }
      ])
  }
  ...
  private func createLinkWithText() {
    guard let currentLinkData else { return }
    wysiwygViewModel.applyLinkOperation(
      .createLink(
        urlString: currentLinkData.url,
  ...
  private func focusComposerIfHardwareKeyboardConnected() {
    // The simulator always detects the hardware keyboard as connected
      ...
      NotificationCenter.default.addObserver(
        self, selector: #selector(hardwareKeyboardDidConnect), name: .GCKeyboardDidConnect,
        object: nil)
    #endif
  }
...
extension ComposerToolbarViewModel {
  enum MockMode {
    case editing, recordVoiceMessage
    case previewVoiceMessage(isUploading: Bool)
    case reply(isLoading: Bool)
  }
  ...
  static func mock(
    ...
    let suggestions: [SuggestionItem] =
      if hasSuggestions {
        [
          .init(
            suggestionType: .user(
              .init(id: "@user_mention_1:matrix.org", displayName: "User 1", avatarURL: nil)),
            range: .init(), rawSuggestionText: ""),
          .init(
            suggestionType: .user(
              .init(
                id: "@user_mention_2:matrix.org", displayName: "User 2",
                avatarURL: .mockMXCUserAvatar)), range: .init(), rawSuggestionText: ""),
        ]
      } else {
        []
      }
    ...
    if !canSend {
      roomProxy.identityStatusChangesPublisher = .init([
        .init(userId: RoomMemberProxyMock.mockAlice.userID, changedTo: .verificationViolation)
      ])
    }
    ...
    let viewModel = ComposerToolbarViewModel(
      ...
      completionSuggestionService: CompletionSuggestionServiceMock(
        configuration: .init(suggestions: suggestions)),
    ...
    switch mockMode {
    case .editing:
      viewModel.state.composerMode = .edit(
        originalEventOrTransactionID: .eventID(""), type: .default)
    ...
    case .previewVoiceMessage(let isUploading):
      viewModel.state.composerMode = .previewVoiceMessage(
    ...
    case .reply(let isLoading):
      let replyDetails: TimelineItemReplyDetails =
        if isLoading {
          ...
          .loaded(
            sender: .init(id: "", displayName: "Test"),
            eventID: "",
            eventContent: .message(.text(.init(body: "Hello World!"))))
        }
      viewModel.state.composerMode = .reply(
        eventID: UUID().uuidString, replyDetails: replyDetails, isThread: false)
    case nil:
      break
    }

...
struct LocationSharingScreenViewModelTests {
  ...
  mutating func errorMapping() {
    ...
    #expect(
      locationError.title == L10n.errorFailedLocatingUser(InfoPlistReader.main.bundleDisplayName))
    let AuthorizationError = AlertInfo(alertID: .missingAuthorization)
    #expect(
      AuthorizationError.message
        == L10n.dialogPermissionLocationDescriptionIos(InfoPlistReader.main.bundleDisplayName))
  }
  ...
  mutating func sendUserLocation() async throws {
    ...
    try await confirmation { confirmation in
      timelineProxy.sendLocationBodyGeoURIDescriptionZoomLevelAssetTypeClosure = {
        _, geoURI, _, _, assetType in
  ...
  mutating func isLocationNotLoadingInNonPickerModeWithAuthorizationNotDetermined() {
    ...
    setupViewModelForViewLive(
      sender: sender, initialShare: aliceShare, liveLocationsSubject: liveLocationsSubject)
    context.isLocationAuthorized = nil
    context.hasLoadedUserLocation = false
    #expect(!context.viewState.isLocationLoading)
  }
  ...
  mutating func startLiveLocationWithWhenInUseAuthorizationAlreadyRequested() {
    setupViewModel(
      liveLocationManagerConfiguration: .init(
  ...
  mutating func startLiveLocationDisclaimerDeclineSkipsStart() {
    let liveLocationManagerMock = LiveLocationManagerMock(
      .init(authorizationStatus: .authorizedAlways))
    setupViewModel(liveLocationManagerMock: liveLocationManagerMock)
    context.send(viewAction: .startLiveLocation)
  ...
  mutating func startLiveLocationDisclaimerAcceptShowsDurationPicker() async throws {
    ...
    context.send(viewAction: .startLiveLocation)
    #expect(context.alertInfo?.id == .liveLocationDisclaimer)
    let deferred = deferFulfillment(context.observe(\.alertInfo)) {
      $0?.id == .liveLocationDurationSelection
    }
  ...
  mutating func startLiveLocationSuccess() async throws {
    ...
    setupViewModel(liveLocationManagerMock: liveLocationManagerMock)
    context.send(viewAction: .startLiveLocation)
    let durationPicker = deferFulfillment(context.observe(\.alertInfo)) {
      $0?.id == .liveLocationDurationSelection
    }
    context.alertInfo?.secondaryButton?.action?()
    ...
    let arguments = try #require(
      liveLocationManagerMock.startLiveLocationRoomIDDurationReceivedArguments)
  ...
  mutating func viewLiveInitialSenderShownCorrectly() {
    let aliceShare = makeLiveLocationShare(
      userID: "@alice:matrix.org", latitude: 51.5, longitude: -0.1)
    let sender = TimelineItemSender(id: "@alice:matrix.org", displayName: "Alice")
    let liveLocationsSubject = CurrentValueSubject<[LiveLocationShare], Never>([aliceShare])
  ...
  mutating func viewLiveReceivesAdditionalLocationUpdates() async throws {
    ...
    let charlieShare = makeLiveLocationShare(
      userID: "@charlie:matrix.org", latitude: 40.7, longitude: -74.0)
  ...
  mutating func viewLiveProfilesResolvedFromRoomMembers() async throws {
    ...
    let annotations = context.viewState.annotations
    #expect(
      annotations.first { $0.id == "@alice:matrix.org" }?.kind
        == .liveUser(.init(userID: "@alice:matrix.org", displayName: "Alice")))
    #expect(
      annotations.first { $0.id == "@bob:matrix.org" }?.kind
        == .liveUser(.init(userID: "@bob:matrix.org", displayName: "Bob")))
    #expect(
      annotations.first { $0.id == "@charlie:matrix.org" }?.kind
        == .liveUser(.init(userID: "@charlie:matrix.org", displayName: "Charlie")))
  }
  ...
  mutating func viewLiveFromBannerAwaitsFirstShareThenCentersOnIt() async throws {
    ...
    viewModel = LocationSharingScreenViewModel(
      interactionMode: .viewLive(sender: nil, initialLiveLocationShare: nil),
    ...
    let ownUserShare = makeLiveLocationShare(
      userID: RoomMemberProxyMock.mockMe.userID, latitude: 51.5, longitude: -0.1)
  ...
  private mutating func setupViewModel(
    liveLocationManagerConfiguration: LiveLocationManagerMock.Configuration = .init()
  ) {
  ...
  private mutating func setupViewModelForViewLive(
    ...
    viewModel = LocationSharingScreenViewModel(
      interactionMode: .viewLive(sender: sender, initialLiveLocationShare: initialShare),
      mapURLBuilder: ServiceLocator.shared.settings.mapTilerConfiguration,
      roomProxy: roomProxyMock,
      timelineController: MockTimelineController(timelineProxy: TimelineProxyMock(.init())),
      liveLocationManager: LiveLocationManagerMock(.init()),
  ...
  private func makeLiveLocationShare(
    userID: String, latitude: Double = 0.0, longitude: Double = 0.0
  ) -> LiveLocationShare {
    LiveLocationShare(
      userID: userID,

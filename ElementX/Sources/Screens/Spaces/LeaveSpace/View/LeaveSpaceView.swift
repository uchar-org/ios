      ...
      LazyVStack(spacing: 0, pinnedViews: .sectionHeaders) {
        Section {
          ...
          if context.viewState.leaveHandle.mode == .manyRooms {
            Button(
              context.viewState.leaveHandle.selectedCount > 0
                ? L10n.actionDeselectAll : L10n.actionSelectAll
            ) {
              context.send(
                viewAction: context.viewState.leaveHandle.selectedCount > 0
...
struct LeaveSpaceView_Previews: PreviewProvider, TestablePreview {
  ...
  static let lastAdminViewModel = makeViewModel(
    mode: .spaceNeedsNewOwner(useTransferOwnershipFlow: false))
  static let lastOwnerViewModel = makeViewModel(
    mode: .spaceNeedsNewOwner(useTransferOwnershipFlow: true))
  ...
  static let spaceServiceRoom = SpaceServiceRoom.mock(
  ...
  static func makeViewModel(mode: LeaveSpaceHandleProxy.Mode) -> LeaveSpaceViewModel {
    ...
    let leaveHandle = LeaveSpaceHandleProxy(
      spaceID: spaceServiceRoom.id,
      leaveHandle: LeaveSpaceHandleSDKMock(.init(rooms: rooms)))

    return LeaveSpaceViewModel(
      spaceName: spaceServiceRoom.name,

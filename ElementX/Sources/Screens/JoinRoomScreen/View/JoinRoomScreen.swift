...
struct JoinRoomScreen: View {
  ...
  private var defaultView: some View {
    VStack(spacing: 16) {
      RoomAvatarImage(
        avatar: context.viewState.avatar ?? .room(id: "", name: nil, avatarURL: nil),
      ...
      VStack(spacing: 8) {
        ...
        {
          JoinedMembersBadgeView(
            heroes: context.viewState.roomDetails?.heroes ?? [],
  ...
  private var knockMessage: some View {
    VStack(alignment: .leading, spacing: 12) {
      HStack(spacing: 0) {
        TextField(
          L10n.screenJoinRoomKnockMessageDescription, text: $context.knockMessage, axis: .vertical
        )
        ...
        .accessibilityHint(
          L10n.screenJoinRoomKnockMessageCharactersCount(
            context.knockMessage.count, maxKnockMessageLength))
      }
    ...
    case .banned(let sender, let reason):
      VStack(spacing: 24) {
        if let sender, let reason {
          bottomErrorMessage(
            title: L10n.screenJoinRoomBanByMessage(sender),
  ...
  private var forbiddenView: some View {
    VStack(spacing: 24) {
      bottomErrorMessage(
        title: L10n.screenJoinRoomFailMessage, subtitle: L10n.screenJoinRoomFailReason)
    ...
    if context.viewState.mode == .knocked {
      ToolbarItem(placement: .principal) {
        if let avatar = context.viewState.avatar {
          RoomHeaderView(
            roomName: context.viewState.title,
            roomAvatar: avatar,
            mediaProvider: context.mediaProvider
          ) {
...
private struct InviterView: View {
  ...
  var body: some View {
    VStack(spacing: 8) {
      ...
      LoadableAvatarImage(
        url: shouldHideAvatar ? nil : inviter.avatarURL,
...
struct JoinRoomScreen_Previews: PreviewProvider, TestablePreview {
  static let previewWrappers: [JoinRoomScreenPreviewWrapper] = [
    ...
    .init(
      mode: .invited(isDM: false), hideInviteAvatars: true,
...
struct JoinRoomScreenSpace_Previews: PreviewProvider, TestablePreview {
  static let previewWrappers: [JoinRoomScreenPreviewWrapper] = [
    ...
    .init(
      isSpace: true, mode: .invited(isDM: false), hideInviteAvatars: true,
      customPreviewName: "InvitedWithHiddenAvatars"),
...
struct JoinRoomScreenPreviewWrapper: Identifiable {
  ...
  init(
    ...
    case .restricted:
      ...
      clientProxy.roomForIdentifierReturnValue = nil
    case .inviteRequired:
      clientProxy.roomPreviewForIdentifierViaReturnValue = .success(
        RoomPreviewProxyMock.inviteRequired)
      clientProxy.roomForIdentifierReturnValue = nil
    case .invited(let isDM):
      clientProxy.roomPreviewForIdentifierViaReturnValue = .success(
        isDM ? RoomPreviewProxyMock.inviteDM() : .invited())
  ...
  var preview: some View {
    ...
    if mode == .forbidden {
      ...
      .snapshotPreferences(
        expect: viewModel.context.$viewState.map { state in

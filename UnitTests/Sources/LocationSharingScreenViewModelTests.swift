...
struct LocationSharingScreenViewModelTests {
    ...
    mutating func errorMapping() {
        ...
        #expect(AuthorizationError.message
    ...
    mutating func viewLiveProfilesResolvedFromRoomMembers() async throws {
        ...
        let annotations = context.viewState.annotations
        #expect(annotations.first { $0.id == "@alice:matrix.org" }?.kind
            == .liveUser(.init(userID: "@alice:matrix.org", displayName: "Alice")))
        #expect(annotations.first { $0.id == "@bob:matrix.org" }?.kind
            == .liveUser(.init(userID: "@bob:matrix.org", displayName: "Bob")))
        #expect(annotations.first { $0.id == "@charlie:matrix.org" }?.kind

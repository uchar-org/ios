...
typealias RoomDirectorySearchScreenViewModelType = StateStoreViewModel<
  RoomDirectorySearchScreenViewState, RoomDirectorySearchScreenViewAction
>

class RoomDirectorySearchScreenViewModel: RoomDirectorySearchScreenViewModelType,
  RoomDirectorySearchScreenViewModelProtocol
{
  private let roomDirectorySearchProxy: RoomDirectorySearchProxyProtocol
  private let userIndicatorController: UserIndicatorControllerProtocol

  private let actionsSubject: PassthroughSubject<RoomDirectorySearchScreenViewModelAction, Never> =
    .init()
  ...
  init(
    ...
    super.init(
      initialViewState: RoomDirectorySearchScreenViewState(),
    ...
    context.$viewState.map(\.bindings.searchString)
      ...
      .combineLatest(
        context.$viewState.map(\.isLoading)
          .filter { $0 == false }
      )
      .map(\.0)

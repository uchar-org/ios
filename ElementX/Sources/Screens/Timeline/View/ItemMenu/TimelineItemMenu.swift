...
struct TimelineItemMenu: View {
  ...
  private var messagePreview: some View {
    VStack(alignment: .leading, spacing: 20) {
      ...
      if case .sendingFailed(.verifiedUser(let failure)) = item.properties.deliveryStatus {
        Divider()
          .padding(.horizontal, -16)
        ...
        Label(
          authenticity.message, icon: authenticity.icon, iconSize: .small,
  ...
  private func reactionButton(for emoji: String) -> some View {
    ...
    .accessibilityLabel(
      hasReacted(to: emoji) ? L10n.a11yRemoveReaction(emoji) : L10n.a11yAddReaction(emoji))
  }
...
private struct VerifiedUserSendFailureView: View {
  ...
  init(
    failure: TimelineItemSendFailure.VerifiedUser,
  ...
  var title: String {
    ...
    case .hasUnsignedDevice:
      isYou
        ? L10n.screenTimelineItemMenuSendFailureYouUnsignedDevice
        : L10n.screenTimelineItemMenuSendFailureUnsignedDevice(memberDisplayName)
...
extension EncryptionAuthenticity {
  fileprivate var foregroundStyle: SwiftUI.Color {
    switch color {
...
struct TimelineItemMenu_Previews: PreviewProvider, TestablePreview {
  ...
  static let (identityChangedItem, _) = makeActions(
    deliveryStatus: .sendingFailed(
      .verifiedUser(
        .changedIdentity(users: [
          "@alice:matrix.org"
        ]))))
  static let (unsignedDevicesItem, _) = makeActions(
    deliveryStatus: .sendingFailed(
      .verifiedUser(
        .hasUnsignedDevice(devices: [
          "@alice:matrix.org": ["DEVICE1", "DEVICE2"]
        ]))))
  static let (ownUnsignedDevicesItem, _) = makeActions(
    deliveryStatus: .sendingFailed(
      .verifiedUser(
        .hasUnsignedDevice(devices: [
  ...
  static let (mediaItemWithCaption, mediaItemWithCaptionActions) = makeActions(
    itemType: .outgoingMediaWithCaption)
  ...
  static func makeActions(
    itemType: ItemType = .incomingText,
    authenticity: EncryptionAuthenticity? = nil,
    deliveryStatus: TimelineItemDeliveryStatus? = nil
  ) -> (EventBasedTimelineItemProtocol, TimelineItemMenuActions)! {

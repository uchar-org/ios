...
typealias IdentityConfirmationScreenViewModelType = StateStoreViewModelV2<
  IdentityConfirmationScreenViewState, IdentityConfirmationScreenViewAction
>

class IdentityConfirmationScreenViewModel: IdentityConfirmationScreenViewModelType,
  IdentityConfirmationScreenViewModelProtocol
{
  private let userSession: UserSessionProtocol
  private let userIndicatorController: UserIndicatorControllerProtocol

  private let actionsSubject: PassthroughSubject<IdentityConfirmationScreenViewModelAction, Never> =
    .init()
  ...
  init(
    userSession: UserSessionProtocol, appSettings: AppSettings,
    ...
    super.init(
      initialViewState: IdentityConfirmationScreenViewState(
        learnMoreURL: appSettings.deviceVerificationURL))
  ...
  private func updateWithSessionSecurityState(_ sessionSecurityState: SessionSecurityState) async {
    ...
    if case .success(let hasDevicesToVerifyAgainst) = await userSession.clientProxy
    ...
    if sessionSecurityState.recoveryState == .enabled
      || sessionSecurityState.recoveryState == .incomplete
    {
  ...
  private static let loadingIndicatorIdentifier =
    "\(IdentityConfirmationScreenViewModel.self)-Loading"

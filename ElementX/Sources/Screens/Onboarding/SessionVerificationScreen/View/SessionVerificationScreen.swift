...
struct SessionVerificationScreen: View {
  ...
  private var screenHeader: some View {
    VStack(spacing: 0) {
      BigIcon(
        icon: context.viewState.headerIcon.keyPath,
  ...
  private var mainContent: some View {
    ...
    case .initial, .acceptingVerificationRequest, .requestingVerification,
      ...
      case .deviceResponder(let details):
        SessionVerificationRequestDetailsView(
          details: details,
    ...
    case .showingChallenge(let emojis), .acceptingChallenge(let emojis),
      ...
      VStack(spacing: 16) {
        ...
        if context.viewState.showIgnoreButton {
          ...
          .accessibilityIdentifier(
            A11yIdentifiers.sessionVerificationScreen.ignoreVerificationRequest
          )
...
struct SessionVerification_Previews: PreviewProvider, TestablePreview {
  static var previews: some View {
    ...
    let details = SessionVerificationRequestDetails(
      senderProfile: UserProfileProxy(
    ...
    sessionVerificationScreen(
      state: .acceptingVerificationRequest,
      flow: .deviceResponder(requestDetails: details)
    )
    ...
    sessionVerificationScreen(
      state: .showingChallenge(emojis: SessionVerificationControllerProxyMock.emojis)
    )
    .previewDisplayName("Showing Challenge")
    sessionVerificationScreen(
      state: .acceptingChallenge(emojis: SessionVerificationControllerProxyMock.emojis)
    )
    .previewDisplayName("Accepting Challenge")
    sessionVerificationScreen(
      state: .decliningChallenge(emojis: SessionVerificationControllerProxyMock.emojis)
    )
  ...
  static func sessionVerificationScreen(
    state: SessionVerificationScreenStateMachine.State,
    flow: SessionVerificationScreenFlow = .deviceInitiator
  ) -> some View {
    let viewModel = SessionVerificationScreenViewModel(
      sessionVerificationControllerProxy: SessionVerificationControllerProxyMock.configureMock(),
      flow: flow,

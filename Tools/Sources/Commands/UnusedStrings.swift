...
struct UnusedStrings: ParsableCommand {
  static let configuration = CommandConfiguration(
    abstract: "Generates a report showing which strings aren't used in the project.")
  ...
  func peripheryScan() throws {
    ...
    let command =
      "periphery scan --quiet --relative-results --report-include ElementX/Sources/Generated/Strings.swift"
    ...
    if saveToFile {
      try output.write(
        to: .projectDirectory.appending(component: "Unused Strings.txt"), atomically: true,
        encoding: .utf8)

...
struct EditRoomAddressScreenViewModelTests {
  ...
  mutating func canonicalAliasChosen() async throws {
    let roomProxy = JoinedRoomProxyMock(
      .init(
        name: "Room Name", canonicalAlias: "#room-name:matrix.org",
        alternativeAliases: [
          "#beta:homeserver.io",
          "#alternative-room-name:matrix.org",
        ]))

    viewModel = EditRoomAddressScreenViewModel(
      roomProxy: roomProxy,
  ...
  mutating func alternativeAliasChosen() async throws {
    let roomProxy = JoinedRoomProxyMock(
      .init(
        name: "Room Name", canonicalAlias: "#alpha:homeserver.io",
        alternativeAliases: [
          "#beta:homeserver.io",
  ...
  mutating func correctMethodsCalledOnSaveWhenAliasOnSameHomeserverExists() async {
    ...
    clientProxy.isAliasAvailableReturnValue = .success(true)
    let roomProxy = JoinedRoomProxyMock(
      .init(name: "Room Name", canonicalAlias: "#old-room-name:matrix.org"))
  ...
  mutating func correctMethodsCalledOnSaveWhenAliasOnOtherHomeserverExists() async {
    ...
    clientProxy.isAliasAvailableReturnValue = .success(true)
    let roomProxy = JoinedRoomProxyMock(
      .init(name: "Room Name", canonicalAlias: "#old-room-name:element.io"))

...
class BugReportService: NSObject, BugReportServiceProtocol {
  ...
  init(
    rageshakeURLPublisher: CurrentValuePublisher<RageshakeConfiguration, Never>,
  ...
  func submitBugReport(
    _ bugReport: BugReport,
    progressListener: CurrentValueSubject<Double, Never>
  ) async -> Result<SubmitBugReportResponse, BugReportServiceError> {
    guard case .url(let rageshakeURL) = rageshakeURL else {
    ...
    if let crashEventID = lastCrashEventID {
      params.append(
        MultipartFormData(
          ...
          type: .text(
            value:
              "<https://sentry.tools.element.io/organizations/element/issues/?project=44&query=\(crashEventID)>"
          )))
  ...
  private var defaultParams: [MultipartFormData] {
    ...
    let version =
      "\(InfoPlistReader.main.bundleShortVersionString) (\(InfoPlistReader.main.bundleVersion))"
    return [
      ...
      MultipartFormData(
        ...
        type: .text(value: Bundle.app.preferredLocalizations.joined(separator: ", "))),
      MultipartFormData(
        key: "user_languages", type: .text(value: Locale.preferredLanguages.joined(separator: ", "))
      ),
      MultipartFormData(
        key: "fallback_language", type: .text(value: Bundle.app.developmentLocalization ?? "null")),
      MultipartFormData(key: "local_time", type: .text(value: localTime)),
      MultipartFormData(key: "utc_time", type: .text(value: utcTime)),
      MultipartFormData(
        key: "base_bundle_identifier", type: .text(value: InfoPlistReader.main.baseBundleIdentifier)
      ),
    ]
  }
  ...
  private func zipFiles(_ logFiles: [URL]) async -> Logs {
    ...
    MXLog.info(
      "zipFiles: originalSize: \(compressedLogs.originalSize), zippedSize: \(compressedLogs.zippedSize)"
    )
  ...
  private func attachFile(at url: URL, to zippedFiles: inout Logs) throws {
    ...
    while let data = try fileHandle.readToEnd() {
      if let zippedData = (data as NSData).gzipped() {
        ...
        zippedFiles.appendFile(
          at: zippedURL, zippedSize: zippedData.count, originalSize: data.count)
      }
    }
  }
...
extension Data {
  fileprivate mutating func appendString(string: String, encoding: String.Encoding = .utf8) {
    if let data = string.data(using: encoding) {
      append(data)
    }
  }

  fileprivate mutating func appendParam(_ param: MultipartFormData, boundary: String) throws {

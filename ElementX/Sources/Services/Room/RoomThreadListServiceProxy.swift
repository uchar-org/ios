...
class RoomThreadListProxy: RoomThreadListServiceProxyProtocol {
  ...
  init(
    threadListService: ThreadListServiceProtocol, eventStringBuilder: RoomEventStringBuilder,
    paginationToken: String? = nil
  ) {
    self.threadListService = threadListService
    self.eventStringBuilder = eventStringBuilder

    let paginationStateSubject = CurrentValueSubject<RoomThreadListPaginationState, Never>(
      .init(sdkState: threadListService.paginationState()))
    paginationStatePublisher = paginationStateSubject.asCurrentValuePublisher()

    paginationStateHandle = threadListService.subscribeToPaginationStateUpdates(
      listener: SDKListener { state in
        paginationStateSubject.send(.init(sdkState: state))
      })

    updateItems()

    itemUpdatesHandle = self.threadListService.subscribeToItemsUpdates(
      listener: SDKListener { [weak self] _ in
        self?.updateItems()
      })
  }
  ...
  private func buildMessageDetails(_ threadListItemEvent: ThreadListItemEvent?)
    -> RoomThreadListItem.MessageDetails?
  {
    ...
    let sender = TimelineItemSender(
      senderID: threadListItemEvent.sender, senderProfile: threadListItemEvent.senderProfile)
    let timestamp = Date(timeIntervalSince1970: TimeInterval(threadListItemEvent.timestamp / 1000))

    let message: AttributedString? =
      if let content = threadListItemEvent.content {
        eventStringBuilder.buildAttributedString(
          for: content,
          sender: sender,

...
typealias MediaUploadPreviewScreenViewModelType = StateStoreViewModelV2<
  MediaUploadPreviewScreenViewState, MediaUploadPreviewScreenViewAction
>

class MediaUploadPreviewScreenViewModel: MediaUploadPreviewScreenViewModelType,
  MediaUploadPreviewScreenViewModelProtocol
{
  ...
  private var actionsSubject: PassthroughSubject<MediaUploadPreviewScreenViewModelAction, Never> =
    .init()
  ...
  init(
    ...
    processingTask = Self.processMedia(
      at: mediaURLs, preprocessor: mediaUploadingPreprocessor, clientProxy: clientProxy)

    super.init(
      initialViewState: MediaUploadPreviewScreenViewState(
        mediaURLs: mediaURLs,
        title: title,
  ...
  private static func processMedia(
    at urls: [URL],
    preprocessor: MediaUploadingPreprocessor,
    clientProxy: ClientProxyProtocol
  ) -> Task<Result<[MediaInfo], MediaUploadingPreprocessorError>, Never> {
    Task {
      guard case .success(let maxUploadSize) = await clientProxy.maxMediaUploadSize else {
        return .failure(.maxUploadSizeUnknown)
      }
      return await preprocessor.processMedia(at: urls, maxUploadSize: maxUploadSize)
    }
  }

  private func sendAttachment(mediaInfo: MediaInfo, caption: String?) async -> Result<
  ...
  > {
    ...
    switch mediaInfo {
    case .image(let imageURL, let thumbnailURL, let imageInfo):
      return await timelineController.sendImage(
        url: imageURL,
        thumbnailURL: thumbnailURL,
        imageInfo: imageInfo,
        caption: caption,
        requestHandle: requestHandle)
    case .video(let videoURL, let thumbnailURL, let videoInfo):
      return await timelineController.sendVideo(
        url: videoURL,
        thumbnailURL: thumbnailURL,
        videoInfo: videoInfo,
        caption: caption,
        requestHandle: requestHandle)
    case .audio(let audioURL, let audioInfo):
      return await timelineController.sendAudio(
        url: audioURL,
        audioInfo: audioInfo,
        caption: caption,
        requestHandle: requestHandle)
    case .file(let fileURL, let fileInfo):
      return await timelineController.sendFile(
        url: fileURL,
        fileInfo: fileInfo,
        caption: caption,
        requestHandle: requestHandle)
  ...
  private static let loadingIndicatorIdentifier =
    "\(MediaUploadPreviewScreenViewModel.self)-Loading"
  ...
  private func showAlert(_ alertType: MediaUploadPreviewAlertType) {
    ...
    case .maxUploadSizeExceeded(let limit):
      state.bindings.alertInfo = .init(
        ...
        message: L10n.screenMediaUploadPreviewErrorTooLargeMessage(
          limit.formatted(.byteCount(style: .file))),

...
enum UserIndicatorType: Equatable {
  ...
  case modal(
    progress: UserIndicator.Progress?, interactiveDismissDisabled: Bool, allowsInteraction: Bool)
...
struct UserIndicator: Equatable, Identifiable {
  enum Progress: Equatable {
    static func == (lhs: UserIndicator.Progress, rhs: UserIndicator.Progress) -> Bool {
      ...
      case (.published(let lhsPublisher), .published(let rhsPublisher)):
        return lhsPublisher.value == rhsPublisher.value

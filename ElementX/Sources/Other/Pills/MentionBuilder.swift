...
struct MentionBuilder: MentionBuilderProtocol {
  ...
  func handleUserMention(
    ...
    let attachmentData = PillTextAttachmentData(
      type: .user(userID: userID), font: attributesToRestore.font)
  ...
  func handleRoomIDMention(
    ...
    let attachmentData = PillTextAttachmentData(
      type: .roomID(roomID), font: attributesToRestore.font)
    guard let attachment = PillTextAttachment(attachmentData: attachmentData) else {
  ...
  func handleRoomAliasMention(
    ...
    let attachmentData = PillTextAttachmentData(
      type: .roomAlias(roomAlias), font: attributesToRestore.font)
    guard let attachment = PillTextAttachment(attachmentData: attachmentData) else {
  ...
  func handleEventOnRoomAliasMention(
    ...
    let attachmentData = PillTextAttachmentData(
      type: .event(room: .roomAlias(roomAlias)), font: attributesToRestore.font)
    guard let attachment = PillTextAttachment(attachmentData: attachmentData) else {
      attributedString.addAttribute(
        .MatrixEventOnRoomAlias,
        value: EventOnRoomAliasAttribute.Value(alias: roomAlias, eventID: eventID), range: range)
      return
    }
  ...
  func handleEventOnRoomIDMention(
    ...
    let attachmentData = PillTextAttachmentData(
      type: .event(room: .roomID(roomID)), font: attributesToRestore.font)
    guard let attachment = PillTextAttachment(attachmentData: attachmentData) else {
      attributedString.addAttribute(
        .MatrixEventOnRoomID, value: EventOnRoomIDAttribute.Value(roomID: roomID, eventID: eventID),
        range: range)
      return
    }
  ...
  private func getAttributesToRestore(
    for attributedString: NSMutableAttributedString, in range: NSRange
  ) -> AttributesToRestore {
  ...
  private func setPillAttachment(
    attachment: PillTextAttachment,
    ...
    attachmentString.addAttributes(
      attributes, range: NSRange(location: 0, length: attachmentString.length))
    attributedString.replaceCharacters(in: range, with: attachmentString)
  }
}

extension Dictionary where Key == NSAttributedString.Key, Value == Any {
  fileprivate mutating func addBlockquoteIfNeeded(_ value: Bool?) {
    if let value {
      self[.MatrixBlockquote] = value
    }
  }

  fileprivate mutating func addMatrixUsernameIfNeeded(_ value: String?) {
    if let value {
      self[.MatrixUserDisplayName] = value
    }
  }

  fileprivate mutating func addMatrixRoomNameIfNeeded(_ value: String?) {
    if let value {

...
final class LeaveSpaceHandleProxy {
  ...
  init(spaceID: String, leaveHandle: LeaveSpaceHandleProtocol) {
    ...
    self.rooms =
      ...
      .compactMap { room in
        guard room.spaceRoom.state == .joined,  // The SDK is going to do this but not yet.
          ...
          room.spaceRoom.roomId != spaceID
        else {
          return nil
        }
        return .init(
          spaceServiceRoom: SpaceServiceRoom(spaceRoom: room.spaceRoom),
    ...
    mode =
      if let space, space.isLastOwner, space.spaceRoom.numJoinedMembers > 1 {
...
@Observable class LeaveSpaceRoomDetails {
  ...
  init(
    spaceServiceRoom: SpaceServiceRoom, isLastOwner: Bool, areCreatorsPrivileged: Bool,
    isSelected: Bool
  ) {

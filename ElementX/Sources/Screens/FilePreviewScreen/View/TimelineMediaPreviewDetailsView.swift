          ...
          Group {
            if let fileSize = item.fileSize {
              Text(contentType) + Text(" • ")
                + Text(UInt(fileSize).formatted(.byteCount(style: .file)))
            } else {
              Text(contentType)
            }
          }
...
struct TimelineMediaPreviewDetailsView_Previews: PreviewProvider, TestablePreview {
  ...
  static let loadingViewModel = makeViewModel(
    contentType: .jpeg, isOutgoing: true, isDownloaded: false)
  ...
  static var previews: some View {
    if case .media(let mediaItem) = viewModel.state.currentItem {
      TimelineMediaPreviewDetailsView(
        item: mediaItem, context: viewModel.context, sheetHeight: $sheetHeight
      )
      .previewDisplayName("Image")
      .snapshotPreferences(expect: mediaItem.observe(\.fileHandle).map { $0 != nil })
    }

    if case .media(let mediaItem) = loadingViewModel.state.currentItem {
      TimelineMediaPreviewDetailsView(
        item: mediaItem, context: loadingViewModel.context, sheetHeight: $sheetHeight
      )
      .previewDisplayName("Loading")
    }

    if case .media(let mediaItem) = unknownTypeViewModel.state.currentItem {
      TimelineMediaPreviewDetailsView(
        item: mediaItem, context: unknownTypeViewModel.context, sheetHeight: $sheetHeight
      )
      .previewDisplayName("Unknown type")
      .snapshotPreferences(expect: mediaItem.observe(\.fileHandle).map { $0 != nil })
    }

    if case .media(let mediaItem) = presentedOnRoomViewModel.state.currentItem {
      TimelineMediaPreviewDetailsView(
        item: mediaItem, context: presentedOnRoomViewModel.context, sheetHeight: $sheetHeight
      )
  ...
  static func makeViewModel(
    ...
    isPresentedOnRoomScreen: Bool = false
  ) -> TimelineMediaPreviewViewModel {
    let item = ImageRoomTimelineItem(
      id: .randomEvent,
      timestamp: .mock,
    ...
    let timelineKind = TimelineKind.media(
      isPresentedOnRoomScreen ? .roomScreenLive : .mediaFilesScreen)

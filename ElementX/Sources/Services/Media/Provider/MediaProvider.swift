...
struct MediaProvider: MediaProviderProtocol {
  ...
  > {
    ...
    do {
      let imageData: Data
      if let size {
        imageData = try await mediaLoader.loadMediaThumbnailForSource(
          source, width: UInt(size.width), height: UInt(size.height))
      } else {
  ...
  > {
    guard let homeserverReachabilityPublisher else {
      fatalError(
        "This method shouldn't be invoked without a homeserver reachability publisher set.")
    }

    return Task {
      if case .success(let image) = await loadImageFromSource(source, size: size) {
        return image
      }
  ...
  > {
    do {
      let thumbnailData = try await mediaLoader.loadMediaThumbnailForSource(
        source, width: UInt(size.width), height: UInt(size.height))

...
struct AdvancedSettingsScreen: View {
  ...
  private var moderationAndSafetySection: some View {
    ...
    Section {
      ListRow(
        label: .plain(title: L10n.screenAdvancedSettingsHideInviteAvatarsToggleTitle),
  ...
  private var timelineMediaSection: some View {
    ...
    Section {
      ListRow(
        label: .plain(title: L10n.screenAdvancedSettingsShowMediaTimelineTitle),
      ...
      ListRow(
        kind: .custom {
          VStack(alignment: .leading, spacing: 0) {
            Text(
              L10n.screenAdvancedSettingsLiveLocationUpdateDistance(
                context.liveLocationMinimumDistanceUpdate)
            )
            ...
            Slider(value: binding, in: 1...100) {
              ...
              Text(
                Self.measurementFormatter.string(
                  from: .init(
                    value: 1,
...
struct AdvancedSettingsScreen_Previews: PreviewProvider, TestablePreview {
  static let viewModel = {
    ...
    return AdvancedSettingsScreenViewModel(
      advancedSettings: AppSettings(),
...
extension TimelineMediaVisibility {
  fileprivate static var items: [(title: String, tag: TimelineMediaVisibility)] {
    [

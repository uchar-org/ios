        ...
        HStack(spacing: ListRowTrailingSectionSpacing.horizontal) {
            ...
            if let counter {
                Text("\(counter)")
                    ...
                    .padding(.vertical, 2)
                    .background {
                        Capsule().fill(isEnabled ? .compound.iconSuccessPrimary : .compound.iconDisabled)
                    }
            }
...
struct ListRowTrailingSection_Previews: PreviewProvider, TestablePreview {
    ...
    static var details: some View {
        VStack(spacing: 20) {
            ...
            ListRowTrailingSection(someCondition
                ? .isWaiting(true) : otherCondition ? .systemIcon(.checkmark) : .title("Hello"))

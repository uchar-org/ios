...
struct SecureBackupScreen: View {
  ...
  private var keyBackupSection: some View {
    Section {
      ...
      ListRow(
        label: .plain(
          title: L10n.screenChatBackupKeyStorageToggleTitle,
  ...
  private var recoveryKeySection: some View {
    Section {
      ...
      case .enabled:
        ListRow(
          label: .default(
            title: L10n.screenChatBackupRecoveryActionChange,
      ...
      case .disabled:
        ListRow(
          label: .default(
            title: L10n.screenChatBackupRecoveryActionSetup,
            description: L10n.screenChatBackupRecoveryActionChangeDescription,
            icon: \.key),
      ...
      case .incomplete:
        ListRow(
          label: .plain(title: L10n.screenChatBackupRecoveryActionConfirm),
          details: .icon(BadgeView(size: 10)),
          kind: .navigationLink { context.send(viewAction: .recoveryKey) }
        )
        .accessibilityIdentifier(A11yIdentifiers.secureBackupScreen.recoveryKey)
...
struct SecureBackupScreen_Previews: PreviewProvider, TestablePreview {
  ...
  static let onlyKeyBackupSetUpViewModel = viewModel(
    keyBackupState: .enabled, recoveryState: .disabled)
  static let keyBackupDisabledViewModel = viewModel(
    keyBackupState: .unknown, recoveryState: .disabled)
  static let recoveryIncompleteViewModel = viewModel(
    keyBackupState: .enabled, recoveryState: .incomplete)
  ...
  static var previews: some View {
    ...
    .snapshotPreferences(
      expect: bothSetupViewModel.context.observe(\.viewState.keyBackupState).map { $0 == .enabled }
    )
    ...
    .snapshotPreferences(
      expect: onlyKeyBackupSetUpViewModel.context.observe(\.viewState.keyBackupState).map {
        $0 == .enabled
      }
    )
    ...
    .snapshotPreferences(
      expect: keyBackupDisabledViewModel.context.observe(\.viewState.keyBackupState).map {
        $0 == .unknown
      }
    )
    ...
    .snapshotPreferences(
      expect: recoveryIncompleteViewModel.context.observe(\.viewState.recoveryState).map {
        $0 == .incomplete
      }
    )
  ...
  static func viewModel(
    keyBackupState: SecureBackupKeyBackupState,
    recoveryState: SecureBackupRecoveryState
  ) -> SecureBackupScreenViewModelType {
    let backupController = SecureBackupControllerMock()
    backupController.underlyingKeyBackupState = CurrentValueSubject<
      SecureBackupKeyBackupState, Never
    >(keyBackupState).asCurrentValuePublisher()
    backupController.underlyingRecoveryState = CurrentValueSubject<
      SecureBackupRecoveryState, Never
    >(recoveryState).asCurrentValuePublisher()

    return SecureBackupScreenViewModel(
      secureBackupController: backupController,

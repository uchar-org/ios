...
final class AccessibilityTests: XCTestCase {
    ...
    XCTContext.runActivity(named: name) { _ in
      do {
        ...
        try app.performAccessibilityAudit(for: [
          ...
          if issue.compactDescription == Self.notHumanReadableMessage,
            Self.isMatrixIdentifier(element.label)
          {
            return true
          }

          // Additional filters for specific elements that lead to false positives or neglectable issues.
          if Self.ignoredA11yIdentifiers[element.identifier]?.isAccessibilityIssueFiltered(issue)
            == true
          {
            return true
          }
  ...
  private static func isMatrixIdentifier(_ string: String) -> Bool {
    MatrixEntityRegex.isMatrixRoomAlias(string) || MatrixEntityRegex.isMatrixUserIdentifier(string)
      || string == PillUtilities.atRoom || MatrixEntityRegex.isLegacyMatrixRoomID(string)
  }

  private static let partiallyUnsupportedDynamicTypeMessage =
  ...
  private static let ignoredA11yIdentifiers: [String: [FilterType]] = [
    A11yIdentifiers.serverConfirmationScreen.serverPicker: [
      .compactDescription(notHumanReadableMessage)
    ]
  ]
...
extension Array where Element == FilterType {
  fileprivate func isAccessibilityIssueFiltered(_ issue: XCUIAccessibilityAuditIssue) -> Bool {
    for filter in self {
      switch filter {

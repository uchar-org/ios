...
struct NavigationTabCoordinatorTests {
  ...
  mutating func tabs() {
    ...
    navigationTabCoordinator.setTabs([
      ...
      .init(
        coordinator: spacesCoordinator,
        details: .init(tag: .spaces, title: "Spaces", icon: \.space, selectedIcon: \.spaceSolid)),
    ])
    assertCoordinatorsEqual(
      navigationTabCoordinator.tabCoordinators, [chatsCoordinator, spacesCoordinator])
  }
  ...
  mutating func tabDismissalCallbacks() async {
    ...
    await confirmation("Wait for callback", expectedCount: 2) { confirm in
      ...
      navigationTabCoordinator.setTabs([
        .init(
          coordinator: SomeTestCoordinator(),
          details: .init(tag: .tab, title: "Tab", icon: \.help, selectedIcon: \.helpSolid))
      ])
    }
  }
  ...
  mutating func overlayDismissalCallbackWhenChangingMode() async {
    ...
    await waitForConfirmation(
      "Callback should not be called when just changing mode",
      expectedCount: 0,
      timeout: .seconds(1)
    ) { confirmation in
      navigationTabCoordinator.setOverlayCoordinator(overlayCoordinator) {

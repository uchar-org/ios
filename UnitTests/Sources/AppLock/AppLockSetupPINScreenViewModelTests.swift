...
final class AppLockSetupPINScreenViewModelTests {
  ...
  func createWeakPIN() async throws {
    ...
    #expect(
      context.viewState.mode == .create, "The mode shouldn't transition after an invalid PIN code.")
  }
  ...
  func createPINMismatch() async throws {
    ...
    #expect(context.viewState.mode == .confirm, "The mode should transition to confirmation.")
    #expect(
      context.viewState.numberOfConfirmAttempts == 0, "The mode should start with zero attempts.")
  ...
  func forgotPIN() async throws {
    ...
    #expect(
      !context.viewState.isLoggingOut, "The view should not be disabled until the user confirms.")
  ...
  func unlockFailed() async throws {
    ...
    keychainController.containsPINCodeBiometricStateReturnValue = false
    #expect(
      context.viewState.numberOfUnlockAttempts == 0, "The screen should start with zero attempts.")
    ...
    var deferred = deferFulfillment(
      context.$viewState, keyPath: \.bindings.pinCode, transitionValues: ["", "2024", ""])
    context.pinCode = "2024"
    try await deferred.fulfill()
    ...
    #expect(
      context.viewState.numberOfUnlockAttempts == 3, "All invalid attempts should be counted.")
  ...
  private func setup(mode: AppLockSetupPINScreenMode) {
    ...
    viewModel = AppLockSetupPINScreenViewModel(
      initialMode: mode, isMandatory: false, appLockService: appLockService)
  }
}

...
final class MediaUploadingPreprocessorTests {
    ...
    func audioFileProcessing() async throws {
        let url = try #require(Bundle(for: Self.self).url(forResource: "test_audio.mp3", withExtension: nil),
                               "Failed retrieving test asset")

        guard
            case .success(let result) = await mediaUploadingPreprocessor.processMedia(at: url, maxUploadSize: maxUploadSize),
    ...
    func landscapeMovVideoProcessing() async throws {
        let url = try #require(Bundle(for: Self.self).url(forResource: "landscape_test_video.mov", withExtension: nil),
                               "Failed retrieving test asset")
        ...
        guard
            case .success(let optimizedResult) = await mediaUploadingPreprocessor.processMedia(at: url, maxUploadSize: maxUploadSize),
        ...
        #expect(optimizedVideoURL.pathExtension == "mp4",
    ...
    func portraitMp4VideoProcessing() async throws {
        let url = try #require(Bundle(for: Self.self).url(forResource: "portrait_test_video.mp4", withExtension: nil),
                               "Failed retrieving test asset")
    ...
    func landscapeImageProcessing() async throws {
        let url = try #require(Bundle(for: Self.self).url(forResource: "landscape_test_image.jpg", withExtension: nil),
                               "Failed retrieving test asset")
    ...
    func portraitImageProcessing() async throws {
        let url = try #require(Bundle(for: Self.self).url(forResource: "portrait_test_image.jpg", withExtension: nil),
                               "Failed retrieving test asset")
    ...
    func pngImageProcessing() async throws {
        let url = try #require(Bundle(for: Self.self).url(forResource: "test_image.png", withExtension: nil),
                               "Failed retrieving test asset")
        ...
        #expect(mimeType(from: convertedImageURL) == "image/png",
        ...
        // Make sure the output file matches the image info.
        #expect(mimeType(from: optimizedImageURL) == "image/png",
                "PNGs should always be sent as PNG to preserve the alpha channel.")
    ...
    func heicImageProcessing() async throws {
        let url = try #require(Bundle(for: Self.self).url(forResource: "test_apple_image.heic", withExtension: nil),
                               "Failed retrieving test asset")
        ...
        // Make sure the output file matches the image info.
        #expect(mimeType(from: convertedImageURL) == "image/heic",
        ...
        // Make sure the output file matches the image info.
        #expect(mimeType(from: optimizedImageURL) == "image/jpeg",
    ...
    func gifImageProcessing() async throws {
        let url = try #require(Bundle(for: Self.self).url(forResource: "test_animated_image.gif", withExtension: nil),
                               "Failed retrieving test asset")
        ...
        // Make sure the output file matches the image info.
        #expect(mimeType(from: convertedImageURL) == "image/gif",
        ...
        // Make sure the output file matches the image info.
        #expect(mimeType(from: optimizedImageURL) == "image/gif",
                "GIFs should always be sent as GIF to preserve the animation.")
    ...
    func rotatedImageProcessing() async throws {
        let url = try #require(Bundle(for: Self.self).url(forResource: "test_rotated_image.jpg", withExtension: nil),
                               "Failed retrieving test asset")
    ...
    private func compare(originalImageAt originalImageURL: URL, toConvertedImageAt convertedImageURL: URL,
                         withThumbnailAt thumbnailURL: URL) throws {
    ...
    private func metadata(from imageData: Data) throws -> NSDictionary {
        ...
        return try #require(CGImageSourceCopyPropertiesAtIndex(imageSource, 0, nil) as NSDictionary?,

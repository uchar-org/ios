...
struct MapTilerURLBuilderTests {
  ...
  func staticMapBuilder() {
    let url = builder.staticMapTileImageURL(
      for: .light,
    ...
    let expectedURL: URL =
      "http://www.foo.com/9bc819c8-e627-474a-a348-ec144fe3d810/static/2.000000,1.000000,5.000000/300x200@2x.png?key=some_key&attribution=false"
  ...
  func staticMapBuilderWithAttribution() {
    ...
    let expectedURL: URL =
      "http://www.foo.com/dea61faf-292b-4774-9660-58fcef89a7f3/static/2.000000,1.000000,5.000000/300x200@2x.png?key=some_key&attribution=topleft"
    #expect(url == expectedURL)
  }
  ...
  func dynamicMapBuilder() {
    ...
    let expectedURL: URL =
      "http://www.foo.com/dea61faf-292b-4774-9660-58fcef89a7f3/style.json?key=some_key"
    #expect(url == expectedURL)
  }

  @Test
  mutating func nilAPIKey() {
    let configuration = MapTilerConfiguration(
      baseURL: Self.baseURL,
      apiKey: nil,
      lightStyleID: Self.lightStyleID,
      darkStyleID: Self.darkStyleID)
    ...
    let staticMapURL = builder.staticMapTileImageURL(
      for: .dark,
      coordinates: .init(latitude: 1, longitude: 2),
      zoomLevel: 5,
      size: .init(width: 300, height: 200),

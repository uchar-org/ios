...
typealias BlockedUsersScreenViewModelType = StateStoreViewModelV2<
  BlockedUsersScreenViewState, BlockedUsersScreenViewAction
>

class BlockedUsersScreenViewModel: BlockedUsersScreenViewModelType,
  BlockedUsersScreenViewModelProtocol
{
  ...
  init(
    ...
    super.init(
      initialViewState: BlockedUsersScreenViewState(blockedUsers: ignoredUsers ?? []),
  ...
  override func process(viewAction: BlockedUsersScreenViewAction) {
    ...
    case .unblockUser(let user):
      state.bindings.alertInfo = .init(
        ...
        primaryButton: .init(title: L10n.screenBlockedUsersUnblockAlertAction, role: .destructive) {
          [weak self] in

...
typealias SecurityAndPrivacyScreenViewModelType = StateStoreViewModel<
  SecurityAndPrivacyScreenViewState, SecurityAndPrivacyScreenViewAction
>

class SecurityAndPrivacyScreenViewModel: SecurityAndPrivacyScreenViewModelType,
  SecurityAndPrivacyScreenViewModelProtocol
{
  ...
  private let actionsSubject: PassthroughSubject<SecurityAndPrivacyScreenViewModelAction, Never> =
    .init()
  ...
  init(
    ...
    super.init(
      initialViewState: SecurityAndPrivacyScreenViewState(
        serverName: clientProxy.userIDServerName ?? "",
        ...
        historyVisibility: roomProxy.infoPublisher.value.historyVisibility
          .toSecurityAndPrivacyHistoryVisibility,
  ...
  override func process(viewAction: SecurityAndPrivacyScreenViewAction) {
    ...
    case .tryUpdatingEncryption(let updatedValue):
      if updatedValue {
        state.bindings.alertInfo = .init(
          ...
          primaryButton: .init(
            title: L10n.screenSecurityAndPrivacyEnableEncryptionAlertConfirmButtonTitle
          ) { [weak self] in self?.state.bindings.desiredSettings.isEncryptionEnabled = true },
    ...
    case .manageSpaces:
      displayManageAuthorizedSpacesScreen(
        isAskToJoin: state.bindings.desiredSettings.accessType.isAskToJoinWithSpaceMembers)
  ...
  private func setupSubscriptions() {
    ...
    context.$viewState
      ...
      .sink { [weak self] accessType in
        guard let self else { return }
        if state.bindings.desiredSettings.isVisibileInRoomDirectory == true,
          !accessType.isAddressRequired
        {
  ...
  private func showUnsavedChangesAlert() {
    state.bindings.alertInfo = .init(
      ...
      primaryButton: .init(title: L10n.actionSave) {
        Task { await self.saveDesiredSettings(shouldDismiss: true) }
      },
      secondaryButton: .init(title: L10n.actionDiscard, role: .cancel) {
  ...
  private func saveDesiredSettings(shouldDismiss: Bool = false) async {
    ...
    if state.currentSettings.isEncryptionEnabled
      != state.bindings.desiredSettings.isEncryptionEnabled
    {
      switch await roomProxy.enableEncryption() {
      case .success:
        state.currentSettings.isEncryptionEnabled =
          state.bindings.desiredSettings.isEncryptionEnabled
      case .failure:
        userIndicatorController.submitIndicator(.init(title: L10n.errorUnknown))
        hasFailures = true
    ...
    if state.currentSettings.historyVisibility != state.bindings.desiredSettings.historyVisibility {
      switch await roomProxy.updateHistoryVisibility(
        state.bindings.desiredSettings.historyVisibility.toRoomHistoryVisibility)
      {
      case .success:
    ...
    if state.currentSettings.isVisibileInRoomDirectory
      != state.bindings.desiredSettings.isVisibileInRoomDirectory
    {
      let visibility: RoomVisibility =
        state.bindings.desiredSettings.isVisibileInRoomDirectory == true ? .public : .private
  ...
  private func handleSelectedAskToJoinWithSpaceMembersAccess() {
    ...
    switch state.spaceSelection {
    case .singleJoined(let joinedSpace):
      state.bindings.desiredSettings.accessType = .askToJoinWithSpaceMembers(spaceIDs: [
        joinedSpace.id
      ])
    case .singleUnknown(let id):
  ...
  private func displayManageAuthorizedSpacesScreen(isAskToJoin: Bool) {
    ...
    authorizedSpacesSelection.selectedIDs
      .sink { [weak self] desiredSelectedIDs in
        ...
        self?.state.bindings.desiredSettings.accessType =
          isAskToJoin
          ? .askToJoinWithSpaceMembers(spaceIDs: sortedIDs) : .spaceMembers(spaceIDs: sortedIDs)
      }
      .store(in: &cancellables)
  ...
  private func setupSelectableJoinedSpaces() async {
    ...
    for spaceID in state.currentSettings.accessType.spaceIDs
    where !joinedParentSpaces.contains(where: { $0.id == spaceID }) {
      if case .success(.some(let space)) = await clientProxy.spaceService.spaceForIdentifier(
        spaceID: spaceID)
      {
...
extension SecurityAndPrivacyRoomAccessType {
  fileprivate var toJoinRule: JoinRule {
    switch self {
    case .inviteOnly:
      .invite
    case .askToJoin:
...
extension RoomHistoryVisibility {
  fileprivate var toSecurityAndPrivacyHistoryVisibility: SecurityAndPrivacyHistoryVisibility {
    switch self {
    case .joined, .invited:
      return .invited
    case .shared, .custom:
...
extension SecurityAndPrivacyHistoryVisibility {
  fileprivate var toRoomHistoryVisibility: RoomHistoryVisibility {
    switch self {
    case .shared:
      return .shared
    case .invited:
...
extension Optional where Wrapped == JoinRule {
  fileprivate var toSecurityAndPrivacyRoomAccessType: SecurityAndPrivacyRoomAccessType {
    switch self {
    case .none, .public:
      return .anyone
    case .invite:
  ...
  private static func spaceIDs(from rules: [AllowRule]) -> [String] {
    rules.compactMap { rule in
      if case .roomMembership(let id) = rule {
        id
      } else {
        nil
      }

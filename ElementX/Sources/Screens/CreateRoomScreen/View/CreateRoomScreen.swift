...
struct CreateRoomScreen: View {
  ...
  var body: some View {
    ...
    .navigationTitle(
      context.viewState.isSpace
        ? L10n.screenCreateRoomNewSpaceTitle : L10n.screenCreateRoomNewRoomTitle
    )
      ...
      HStack(alignment: .center, spacing: 16) {
        ...
        VStack(alignment: .leading, spacing: 8) {
          ...
          TextField(
            ...
            prompt: Text(L10n.screenCreateRoomNamePlaceholder).foregroundColor(
  ...
  private var topicSection: some View {
    Section {
      ListRow(
        label: .plain(title: L10n.screenCreateRoomTopicPlaceholder),
  ...
  private var roomAliasSection: some View {
    Section {
      EditRoomAddressListRow(
        aliasLocalPart: aliasBinding,
      ...
      VStack(alignment: .leading, spacing: 12) {
        if let errorDescription = context.viewState.aliasErrors.errorDescription {
          Label(
            errorDescription, icon: \.errorSolid, iconSize: .xSmall, relativeTo: .compound.bodySM
          )
  ...
  private var selectSpaceSection: some View {
    Section {
      if let selectedSpace = context.selectedSpace {
        ...
        ListRow(
          label: .plain(title: L10n.screenCreateRoomSpaceSelectionNoSpaceTitle),
          kind: .navigationLink {
            context.showSpaceSelectionSheet = true
          })
      }
...
struct CreateRoom_Previews: PreviewProvider, TestablePreview {
  ...
  static let avatarViewModel = {
    ...
    viewModel.updateAvatar(
      fileURL: Bundle.main.url(forResource: "preview_avatar_room", withExtension: "jpg")
        ?? .picturesDirectory)
    return viewModel
  }()
  ...
  static let selectedSpaceViewModel = makeViewModel(
    selectionMode: .editableSpacesList(
      preSelectedSpace: SpaceServiceRoom.mock(
        id: "awesome-space-id",
        name: "Awesome Space",
        isSpace: true,
        joinRule: .invite)))
  ...
  static var previews: some View {
    ...
    .snapshotPreferences(
      expect: spaceWithAvatarViewModel.context.$viewState.map { $0.avatarImage != nil })
    ...
    .snapshotPreferences(
      expect: publicRoomInvalidAliasViewModel.context.$viewState.map { !$0.aliasErrors.isEmpty }
    )
    ...
    .snapshotPreferences(
      expect: publicRoomExistingAliasViewModel.context.$viewState.map { !$0.aliasErrors.isEmpty }
    )
  ...
  private static func makeViewModel(
    ...
    return CreateRoomScreenViewModel(
      isSpace: isSpace,

...
class ChatsTabFlowCoordinator: FlowCoordinatorProtocol {
  ...
  init(
    ...
    sidebarNavigationStackCoordinator = NavigationStackCoordinator(
      navigationSplitCoordinator: navigationSplitCoordinator)
  ...
  func asyncHandleAppRoute(_ appRoute: AppRoute, animated: Bool) async {
    ...
    case .room(let roomID, let via):
      stateMachine.processEvent(
        .selectRoom(roomID: roomID, via: via, entryPoint: .room),
        userInfo: .init(animated: animated))
    case .roomAlias(let alias):
      switch await userSession.clientProxy.resolveRoomAlias(alias) {
      case .success(let resolved):
        await asyncHandleAppRoute(
          .room(roomID: resolved.roomId, via: resolved.servers), animated: animated)
    ...
    case .childRoomAlias(let alias):
      ...
      case .success(let resolved):
        await asyncHandleAppRoute(
          .childRoom(roomID: resolved.roomId, via: resolved.servers), animated: animated)
      case .failure: showFailureIndicator()
      }
    ...
    case .roomDetails(let roomID):
      if case .room(roomID) = stateMachine.state.detailState {
        ...
        stateMachine.processEvent(
          .selectRoom(roomID: roomID, via: [], entryPoint: .roomDetails),
          userInfo: .init(animated: animated))
      }
    case .roomList:
      roomFlowCoordinator?.clearRoute(animated: animated)
    ...
    case .event(let eventID, let roomID, let via):
      stateMachine.processEvent(
        .selectRoom(roomID: roomID, via: via, entryPoint: .eventID(eventID)),
        userInfo: .init(animated: animated))
    case .eventOnRoomAlias(let eventID, let alias):
      switch await userSession.clientProxy.resolveRoomAlias(alias) {
      case .success(let resolved):
        await asyncHandleAppRoute(
          .event(eventID: eventID, roomID: resolved.roomId, via: resolved.servers),
          animated: animated)
      case .failure: showFailureIndicator()
      }
    ...
    case .childEventOnRoomAlias(let eventID, let alias):
      ...
      case .success(let resolved):
        await asyncHandleAppRoute(
          .childEvent(eventID: eventID, roomID: resolved.roomId, via: resolved.servers),
          animated: animated)
      case .failure: showFailureIndicator()
      }
    ...
    case .userProfile(let userID):
      stateMachine.processEvent(
        .showUserProfileScreen(userID: userID), userInfo: .init(animated: animated))
    case .share(let payload):
      if let roomID = payload.roomID {
        ...
        stateMachine.processEvent(
          .showShareExtensionRoomList(sharePayload: payload), userInfo: .init(animated: animated))
      }
  ...
  private func setupStateMachine() {
    stateMachine.addTransitionHandler { [weak self] context in
      ...
      case (.roomList(let detailState), .selectRoom(let roomID, let via, let entryPoint), .roomList):
        handleSelectRoomTransition(
          roomID: roomID, via: via, entryPoint: entryPoint, detailState: detailState,
          animated: animated)
      case (.roomList, .deselectRoom, .roomList):
        dismissRoomFlow(animated: animated)

      case (.roomList, .startSpaceFlow, .roomList):
        guard let spaceRoomListProxy = userInfo?.spaceRoomListProxy else {
          fatalError("A space room list proxy is required.")
        }
      ...
      case (.roomList, .feedbackScreen, .feedbackScreen):
        bugReportFlowCoordinator = BugReportFlowCoordinator(
          parameters: .init(
            presentationMode: .sheet(sidebarNavigationStackCoordinator),
      ...
      case (
        .roomList, .presentDeclineAndBlockScreen(let userID, let roomID), .declineAndBlockUserScreen
      ):
      ...
      case (
        .roomList(let roomListSelectedRoomID), .showShareExtensionRoomList,
        .shareExtensionRoomList(let sharePayload)
      ):
        Task {
  ...
  private func handleSelectRoomTransition(
    roomID: String, via: [String], entryPoint: RoomFlowCoordinatorEntryPoint,
    detailState: ChatsTabFlowCoordinatorStateMachine.DetailState?, animated: Bool
  ) {
      ...
      let roomFlowCoordinator
    {
      let route: AppRoute =
        ...
        case .thread(let rootEventID, let focusEventID):
          .thread(roomID: roomID, threadRootEventID: rootEventID, focusEventID: focusEventID)
        }
    ...
    userSession.clientProxy.actionsPublisher
      ...
      .sink { [weak self] action in
        ...
        case .receivedSyncUpdate:
          Task {
            let roomSummaries = self.userSession.clientProxy.staticRoomSummaryProvider
              .roomListPublisher.value
            await self.flowParameters.notificationManager
              .removeDeliveredNotificationsForFullyReadRooms(roomSummaries)
          }
        default:
          break
        }
  ...
  private func processDecryptionError(_ info: UnableToDecryptInfo) {
    ...
    let errorName: AnalyticsEvent.Error.Name =
      switch info.cause {
    ...
    flowParameters.analytics.trackError(
      context: nil,
      domain: .E2EE,
      name: errorName,
    ...
    coordinator.actions
      .sink { [weak self] action in
        ...
        case .presentSpace(let spaceRoomListProxy):
          stateMachine.processEvent(
            .startSpaceFlow, userInfo: .init(animated: true, spaceRoomListProxy: spaceRoomListProxy)
          )
  ...
  private func startRoomFlow(
    ...
    let navigationStackCoordinator = NavigationStackCoordinator(
      navigationSplitCoordinator: detached ? nil : navigationSplitCoordinator)
    ...
    coordinator.actions.sink { [weak self] action in
      ...
      case .continueWithSpaceFlow(let spaceRoomListProxy):
        stateMachine.processEvent(
          .startSpaceFlow, userInfo: .init(animated: false, spaceRoomListProxy: spaceRoomListProxy))
      case .finished:
    ...
    if detached {
      flowParameters.windowManager.registerCoordinator(
        navigationStackCoordinator,
        flowCoordinator: coordinator,
        forWindowType: .room(roomID: roomID))
    } else {
      roomFlowCoordinator = coordinator
      navigationSplitCoordinator.setDetailCoordinator(
        navigationStackCoordinator, animated: animated)
    }
    ...
    case .eventID(let eventID):
      coordinator.handleAppRoute(
        .event(eventID: eventID, roomID: roomID, via: via), animated: animated)
    case .roomDetails:
  ...
  private func startStartChatFlow(animated: Bool) {
    let navigationStackCoordinator = NavigationStackCoordinator()
    let coordinator = StartChatFlowCoordinator(
      entryPoint: .startChat,
      ...
      .sink { [weak self] action in
        ...
        case .finished(let result):
          ...
          case .space(let spaceRoomListProxy):
            ...
            stateMachine.processEvent(
              .selectRoom(roomID: spaceRoomListProxy.id, via: [], entryPoint: .room))
          case .cancelled:
            break
          }
    ...
    navigationSplitCoordinator.setSheetCoordinator(navigationStackCoordinator, animated: animated) {
      [weak self] in
  ...
  private func presentRecoveryKeyScreen(animated: Bool) {
    ...
    let parameters = SecureBackupRecoveryKeyScreenCoordinatorParameters(
    ...
    navigationSplitCoordinator.setSheetCoordinator(
      sheetNavigationStackCoordinator, animated: animated
    ) { [weak self] in
  ...
  private func startEncryptionResetFlow(animated: Bool) {
    let sheetNavigationStackCoordinator = NavigationStackCoordinator()
    let parameters = EncryptionResetFlowCoordinatorParameters(
      userSession: userSession,
      appSettings: flowParameters.appSettings,
      userIndicatorController: flowParameters.userIndicatorController,
  ...
  private func presentGlobalSearch() {
    ...
    let coordinator = GlobalSearchScreenCoordinator(
      parameters: .init(
  ...
  private func presentRoomDirectorySearch() {
    let coordinator = RoomDirectorySearchScreenCoordinator(
      parameters: .init(
        userSession: userSession,
        userIndicatorController: flowParameters.userIndicatorController))
  ...
  private func presentUserProfileScreen(userID: String, animated: Bool) {
    ...
    let navigationStackCoordinator = NavigationStackCoordinator()
    let parameters = UserProfileScreenCoordinatorParameters(
      userID: userID,
  ...
  private func presentRoomSelectionScreen(sharePayload: ShareExtensionPayload, animated: Bool) {
    ...
    let coordinator = RoomSelectionScreenCoordinator(
      parameters: .init(
        userSession: userSession,
        roomSummaryProvider: roomSummaryProvider))
    ...
    navigationSplitCoordinator.setSheetCoordinator(stackCoordinator, animated: animated) {
      [weak self] in
  ...
  private func showFailureIndicator() {
    flowParameters.userIndicatorController.submitIndicator(
      UserIndicator(
        id: Self.failureIndicatorIdentifier,
        type: .toast,

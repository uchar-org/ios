...
typealias DeclineAndBlockScreenViewModelType = StateStoreViewModelV2<
  DeclineAndBlockScreenViewState, DeclineAndBlockScreenViewAction
>

class DeclineAndBlockScreenViewModel: DeclineAndBlockScreenViewModelType,
  DeclineAndBlockScreenViewModelProtocol
{
  let userID: String
  let roomID: String
  ...
  private let actionsSubject: PassthroughSubject<DeclineAndBlockScreenViewModelAction, Never> =
    .init()
  ...
  private func decline() async {
    ...
    guard case .invited(let roomProxy) = await clientProxy.roomForIdentifier(roomID) else {
      MXLog.error(
        "DeclineAndBlockScreenViewModel: Unable to find an invited room for identifier \(roomID)")
    ...
    case .success:
      ...
      if state.bindings.shouldReport {
        shouldShowFailure = await clientProxy.reportRoomForIdentifier(
          roomID, reason: state.bindings.reportReason
        ).isFailure
      }

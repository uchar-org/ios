...
typealias RoomChangeRolesScreenViewModelType = StateStoreViewModelV2<
  RoomChangeRolesScreenViewState, RoomChangeRolesScreenViewAction
>

class RoomChangeRolesScreenViewModel: RoomChangeRolesScreenViewModelType,
  RoomChangeRolesScreenViewModelProtocol
{
  ...
  private let actionsSubject: PassthroughSubject<RoomChangeRolesScreenViewModelAction, Never> =
    .init()
  ...
  init(
    ...
    guard mode != .user || mode != .creator else {
      fatalError("Invalid screen configuration: \(mode)")
    }
    ...
    super.init(
      initialViewState: RoomChangeRolesScreenViewState(
        mode: mode,
        ownRole: roomProxy.membersPublisher.value.first { $0.userID == roomProxy.id }?.role
          ?? .administrator),
  ...
  private func save() async {
    ...
    case .success:
      ...
      trackChanges(
        promotionCount: promotingUpdates.count,

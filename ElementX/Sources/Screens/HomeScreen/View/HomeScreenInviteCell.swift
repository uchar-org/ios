...
struct HomeScreenInviteCell: View {
  ...
  private var avatar: RoomAvatar {
    ...
    if room.isDirect, let inviter = room.inviter {
      .heroes([
        .init(
          userID: inviter.id, displayName: inviter.displayName,
          avatarURL: hideInviteAvatars ? nil : inviter.avatarURL)
      ])
    } else {
...
struct HomeScreenInviteCell_Previews: PreviewProvider, TestablePreview {
  static var previews: some View {
    VStack(spacing: 0) {
      ...
      HomeScreenInviteCell(
        room: .invite(alias: "#footest:somewhere.org"),
        context: makeViewModel().context,
        hideInviteAvatars: false
      )
...
extension HomeScreenRoom {
  ...
  fileprivate static func invite(
    name: String = "Awesome Room",
    isSpace: Bool = false,
    alias: String? = nil,
    avatarURL: URL? = nil
  ) -> HomeScreenRoom {

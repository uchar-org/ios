...
class AnalyticsService {
  ...
  func startIfEnabled() {
    guard isEnabled, !client.isRunning, let configuration = appSettings.analyticsConfiguration
    else { return }
...
extension AnalyticsService {
  ...
  func trackError(
    context: String?, domain: AnalyticsEvent.Error.Domain,
  ...
  func trackViewRoom(isDM: Bool, isSpace: Bool) {
    capture(
      event: AnalyticsEvent.ViewRoom(
        activeSpace: nil, isDM: isDM, isSpace: isSpace, trigger: nil, viaKeyboard: nil))
  }
  ...
  func trackJoinedRoom(isDM: Bool, isSpace: Bool, activeMemberCount: UInt) {
    ...
    capture(
      event: AnalyticsEvent.JoinedRoom(
        isDM: isDM, isSpace: isSpace, roomSize: roomSize, trigger: nil))
  }
  ...
  func trackPollCreated(isUndisclosed: Bool, numberOfAnswers: Int) {
    capture(
      event: AnalyticsEvent.PollCreation(
        action: .Create,
  ...
  func trackSessionSecurityState(_ state: SessionSecurityState) {
    ...
    let event = AnalyticsEvent.CryptoSessionStateChange(
      recoveryState: analyticsRecoveryState, verificationState: analyticsVerificationState)
    client.capture(event)
  }

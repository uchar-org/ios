  ...
  ImageDataProvider
{
  ...
  init(
    ...
    assert(
      mediaProvider != nil,
      "Missing image provider, make sure one has been supplied to the view model.")
    ...
    self.transformer = transformer
    self.placeholder = placeholder
    _contentLoader = StateObject(
      wrappedValue: ContentLoader(
        mediaSource: mediaSource, size: size, mediaProvider: mediaProvider))
  }
  ...
  func data(handler: @escaping (Result<Data, Error>) -> Void) {
    guard case .gifData(let data) = contentLoader.content else {
...
private class ContentLoader: ObservableObject {
  ...
  func load() async {
    if isGIF {
      if case .success(let data) = await mediaProvider?.loadImageDataFromSource(mediaSource) {
        cachedContent = .gifData(data)
      }
    } else {
      guard let task = mediaProvider?.loadImageRetryingOnReconnection(mediaSource, size: size)
      else {
...
struct LoadableImage_Previews: PreviewProvider, TestablePreview {
  ...
  static func makeMediaProvider(isLoading: Bool = false) -> MediaProviderProtocol {
    ...
    if isLoading {
      ...
      mediaProvider.loadThumbnailForSourceSourceSizeClosure = { _, _ in
        .failure(.failedRetrievingThumbnail)
      }
...
extension View {
  fileprivate func layout(title: String, hideTimelineMedia: Bool = false) -> some View {

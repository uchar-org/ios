...
typealias MediaEventsTimelineScreenViewModelType = StateStoreViewModelV2<
  MediaEventsTimelineScreenViewState, MediaEventsTimelineScreenViewAction
>
  ...
  MediaEventsTimelineScreenViewModelProtocol
{
  ...
  private let actionsSubject: PassthroughSubject<MediaEventsTimelineScreenViewModelAction, Never> =
    .init()
  ...
  init(
    mediaTimelineViewModel: TimelineViewModelProtocol,
    ...
    let activeTimelineContext =
      switch initialScreenMode {
    ...
    super.init(
      initialViewState: .init(
        activeTimelineContext: activeTimelineContext, bindings: .init(screenMode: initialScreenMode)
    ...
    mediaTimelineViewModel.actions.sink { [weak self] action in
      ...
      case .displayMediaDetails(let item):
        displayMediaPreviewSheet(for: item)
      case .displayEmojiPicker, .displayReportContent, .displayCameraPicker, .displayMediaPicker,
        .displayDocumentPicker, .displayLocationPicker, .displayLiveLocation, .displayPollForm,
        .displayMediaUploadPreviewScreen,
        .displaySenderDetails, .displayMessageForwarding, .displayLocation,
  ...
  override func process(viewAction: MediaEventsTimelineScreenViewAction) {
    ...
    case .oldestItemDidAppear:
      ...
      backPaginateIfNecessary(
        backPaginationState: activeTimelineViewModel.context.viewState.timelineState.paginationState
          .backward)
    ...
    case .longPressedItem(let item):
      activeTimelineViewModel.context.send(
        viewAction: .displayTimelineItemMenu(itemID: item.identifier))
    }
  }
  ...
  private func displayMediaPreviewSheet(for item: EventBasedMessageTimelineItemProtocol) {
    let sheetModel = TimelineMediaPreviewViewModel(
      initialItem: item,
  ...
  private func updateWithTimelineViewState(_ timelineViewState: TimelineViewState) {
    ...
    state.shouldShowEmptyState =
      newGroups.isEmpty && timelineViewState.timelineState.paginationState.backward == .endReached
    backPaginateIfNecessary(
      backPaginationState: timelineViewState.timelineState.paginationState.backward)
  }
  ...
  private func displayMessageForwarding(forwardingItem: MessageForwardingItem) {
    ...
    DispatchQueue.main.asyncAfter(
      deadline: .now() + TimelineMediaPreviewViewModel.displayMessageForwardingDelay
    ) {

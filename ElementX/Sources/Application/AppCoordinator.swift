  ...
  NotificationManagerDelegate, SecureWindowManagerDelegate
{
  ...
  init(appDelegate: AppDelegate) {
    ...
    posthogAnalyticsClient.updateSuperProperties(
      AnalyticsEvent.SuperProperties(
        appPlatform: .EXI, cryptoSDK: .Rust, cryptoSDKVersion: sdkGitSha()))
    let analyticsService = AnalyticsService(
      client: posthogAnalyticsClient, appSettings: appSettings)
    ...
    appLockFlowCoordinator = AppLockFlowCoordinator(
    ...
    bugReportService = BugReportService(
      rageshakeURLPublisher: appSettings.bugReportRageshakeURL.publisher,
  ...
  func handlePotentialPhishingAttempt(url: URL, openURLAction: @escaping (URL) -> Void) -> Bool {
    ...
    navigationRootCoordinator.alertInfo = .init(
      ...
      message: L10n.dialogConfirmLinkMessage(
        confirmationParameters.displayString,
  ...
  func handleAppRoute(_ appRoute: AppRoute, windowType: SecondaryWindowType?) {
    ...
    case .accountProvisioningLink:
      if let authenticationFlowCoordinator {
        authenticationFlowCoordinator.handleAppRoute(
          appRoute, animated: appMediator.appState == .active)
        handled = true
      }
    default:
      if let userSessionFlowCoordinator {
        userSessionFlowCoordinator.handleAppRoute(
          appRoute, animated: appMediator.appState == .active)
        handled = true
      }
    }
    ...
    if let route = appRouteURLParser.route(from: url) {
      ...
      case .userProfile(let userID):
        if isExternalURL {
          ...
          handleAppRoute(
            .roomMemberDetails(userID: userID),
            windowType: windowType)
        }
      case .room(let roomID, let via):
        if isExternalURL {
          ...
          handleAppRoute(
            .childRoom(roomID: roomID, via: via),
            windowType: windowType)
        }
      ...
      case .event(let eventID, let roomID, let via):
        if isExternalURL {
          ...
          handleAppRoute(
            .childEvent(eventID: eventID, roomID: roomID, via: via),
            windowType: windowType)
        }
      case .eventOnRoomAlias(let eventID, let alias):
        if isExternalURL {
          ...
          handleAppRoute(
            .childEventOnRoomAlias(eventID: eventID, alias: alias),
            windowType: windowType)
        }
      case .share(let payload):
        ...
        do {
          try handleAppRoute(
            .share(payload.withDefaultTemporaryDirectory()),
            windowType: windowType)
        } catch {
  ...
  func windowManagerDidConfigureWindows(_ windowManager: SecureWindowManagerProtocol) {
    windowManager.alternateWindow.rootViewController = UIHostingController(
      rootView: appLockFlowCoordinator.toPresentable())
  ...
  func notificationTapped(content: UNNotificationContent) async {
    ...
    if content.categoryIdentifier == NotificationConstants.Category.invite {
      ...
      handleAppRoute(
        .thread(roomID: roomID, threadRootEventID: threadRootEventID, focusEventID: eventID),
        windowType: nil)
    } else if let eventID {
  ...
  func handleInlineReply(
    _ service: NotificationManagerProtocol, content: UNNotificationContent, replyText: String
  ) async {
  ...
  private func performUserSessionMigrations(_ userSession: UserSessionProtocol) async {
    ...
    if oldVersion < Version(25, 9, 2) {
      MXLog.info(
        "Migrating to version 25.09.2, triggering sync to ensure m.space state is up to date.")
    ...
    Task {
      ...
      case .success(let config):
        ...
        if let timelineMediaVisibility,
          case .success = await userSession.clientProxy.setTimelineMediaVisibility(
            timelineMediaVisibility)
        {
          userDefaults.removeObject(forKey: "timelineMediaVisibility")
        } else if let hideTimelineMedia,
          case .success = await userSession.clientProxy.setTimelineMediaVisibility(
            hideTimelineMedia ? .never : .always)
        {
          userDefaults.removeObject(forKey: "hideTimelineMedia")
        }
  ...
  private func clearTemporaryDirectories() {
    ...
    do {
      let fileURLs = try FileManager.default.contentsOfDirectory(
        at: URL.temporaryDirectory, includingPropertiesForKeys: nil, options: [])
    ...
    do {
      let fileURLs = try FileManager.default.contentsOfDirectory(
        at: URL.appGroupTemporaryDirectory, includingPropertiesForKeys: nil, options: [])
  ...
  private func runPostSessionSetupTasks() async {
    ...
    if let storedInlineReply {
      await processInlineReply(
        roomID: storedInlineReply.roomID, replyText: storedInlineReply.message)
    }
  }
  ...
  private func startAuthenticationSoftLogout() {
    ...
    Task {
      ...
      let authenticationService = AuthenticationService(
        ...
        appSettings: appSettings,
        appHooks: appHooks)
      _ = await authenticationService.configure(
        for: userSession.clientProxy.homeserver, flow: .login)

      let parameters = SoftLogoutScreenCoordinatorParameters(
        authenticationService: authenticationService,
  ...
  private func setupUserSession(isNewLogin: Bool) {
    ...
    let flowParameters = CommonFlowParameters(
  ...
  private func clearCache() {
    ...
    navigationRootCoordinator.setRootCoordinator(
      PlaceholderScreenCoordinator(hideBrandChrome: appSettings.hideBrandChrome))
  ...
  private static func setupSentry(
    bugReportService: BugReportServiceProtocol, appSettings: AppSettings
  ) {
  ...
  private func processInlineReply(roomID: String, replyText: String) async {
    ...
    switch await roomProxy.timeline.sendMessage(
      replyText,
      html: nil,
  ...
  private func hideLoadingIndicator() {
    ServiceLocator.shared.userIndicatorController.retractIndicatorWithId(
      Self.loadingIndicatorIdentifier)
  }

  private func showLoginErrorToast() {
    ServiceLocator.shared.userIndicatorController.submitIndicator(
      UserIndicator(title: "Failed logging in"))
  }
      ...
      .sink { [weak self] state in
        ...
        case .loading:
            ...
            self?.appMediator.networkMonitor.reachabilityPublisher.value == .reachable
          {
            ServiceLocator.shared.userIndicatorController.submitIndicator(
              .init(
                id: toastIdentifier, type: .toast(progress: .indeterminate),
  ...
  private func registerBackgroundAppRefresh() {
    let result = BGTaskScheduler.shared.register(
      forTaskWithIdentifier: appSettings.backgroundAppRefreshTaskIdentifier, using: .main
    ) { [weak self] task in
  ...
  private func scheduleBackgroundAppRefresh() {
    let request = BGAppRefreshTaskRequest(
      identifier: appSettings.backgroundAppRefreshTaskIdentifier)

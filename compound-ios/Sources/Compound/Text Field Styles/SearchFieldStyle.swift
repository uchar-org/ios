...
extension View {
  ...
  public func compoundSearchField() -> some View {
    introspect(.navigationStack, on: .supportedVersions, scope: .ancestor) { navigationController in
      ...
      if #unavailable(iOS 26.0) {
        ...
        let effectBackgroundBottom =
          searchTextField.value(forKey: "_effectBackgroundBottom") as? UIView
  ...
  static var previews: some View {
    ...
    NavigationStack {
      Form {
        ...
        Section {
          ListRow(
            label: .plain(title: "Some setting"),

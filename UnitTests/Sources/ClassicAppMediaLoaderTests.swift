...
final class ClassicAppMediaLoaderTests {
  ...
  func loadMediaContentWithInvalidToken() async throws {
    let accountWithoutToken = ClassicAppAccount(
    ...
    let loaderWithoutToken = ClassicAppMediaLoader(
      classicAppAccount: accountWithoutToken, urlSession: urlSession)
...
private class MockURLProtocol: URLProtocol {
  ...
  override func startLoading() {
    ...
    let isAuthenticated =
      request.value(forHTTPHeaderField: "Authorization") == "Bearer \(MockURLProtocol.accessToken)"
    let path = url.path()
    let (statusCode, data) =
      isAuthenticated ? MockURLProtocol.responses[path] ?? (404, Data()) : (401, Data())

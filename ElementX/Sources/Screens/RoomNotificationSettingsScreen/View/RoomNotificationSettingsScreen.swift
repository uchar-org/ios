...
struct RoomNotificationSettingsScreen: View {
  ...
  private var allowCustomSettingSection: some View {
    Section {
      ListRow(
        label: .plain(title: L10n.screenRoomNotificationSettingsAllowCustom),
  ...
  private var defaultSettingSection: some View {
    Section {
      ListRow(
        label: .plain(
          title: context.viewState.isRestoringDefaultSetting
            ? L10n.commonLoading
            : context.viewState.strings.string(for: context.viewState.notificationSettingsState)),
        kind: .label
      )
      ...
      Text(context.viewState.strings.customSettingFootnote)
        .environment(
          ...
          OpenURLAction { url in
            guard url == context.viewState.strings.customSettingFootnoteLink else {
              return .discarded
            }
...
struct RoomNotificationSettingsScreen_Previews: PreviewProvider, TestablePreview {
  ...
  static let viewModelCustom = {
    let notificationSettingsProxy = NotificationSettingsProxyMock(
      with: .init(defaultRoomMode: .allMessages, roomMode: .mentionsAndKeywordsOnly))

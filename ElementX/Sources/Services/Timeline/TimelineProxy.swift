...
final class TimelineProxy: TimelineProxyProtocol {
  ...
  private let forwardPaginationStateSubject = CurrentValueSubject<PaginationState, Never>(
    .endReached)
  ...
  func subscribeForUpdates() async {
    ...
    let paginationStatePublisher =
      backPaginationStateSubject
    ...
    let provider = await TimelineItemProvider(
      timeline: timeline, kind: kind, paginationStatePublisher: paginationStatePublisher)
  ...
  {
    guard
      let content = await timelineItemProvider.itemProxies.firstEventTimelineItemUsingStableID(
        timelineItemID)?.content,
      case .msgLike(let messageLikeContent) = content,
      case .message(let messageContent) = messageLikeContent.kind
    else {
      return nil
    }
  ...
  private func paginateBackwardsOnLive(requestSize: UInt16) async -> Result<
  ...
  private func focussedPaginate(_ direction: PaginationDirection, requestSize: UInt16) async
  ...
  func edit(
    ...
    do {
      try await timeline.edit(
        eventOrTransactionId: eventOrTransactionID.rustValue, newContent: newContent)
    ...
    async -> Result<Void, TimelineProxyError>
  {
    ...
    do {
      try await timeline.redactEvent(
        eventOrTransactionId: eventOrTransactionID.rustValue, reason: reason)
  ...
  func createPoll(
    ...
    do {
      try await timeline.createPoll(
        question: question, answers: answers, maxSelections: 1, pollKind: .init(pollKind: pollKind))
  ...
  > {
    ...
    do {
      ...
      MXLog.error(
        "Failed sending response for poll with eventID: \(pollStartID) with error: \(error)")
      return .failure(.sdkError(error))
    }
  }

  func buildMessageContentFor(
    _ message: String,
    html: String?,
  ...
  private func buildEmoteMessageContentFor(_ message: String, html: String?)
    -> RoomMessageEventContentWithoutRelation
  {
    if let html {
  ...
  private func subscribeToPagination() async {
    ...
    case .live:
      ...
      do {
        backPaginationStateObservationToken = try await timeline.subscribeToBackPaginationStatus(
          listener: backPaginationListener)
      } catch {
...
extension MatrixRustSDK.PollKind {
  fileprivate init(pollKind: Poll.Kind) {
    switch pollKind {
    case .disclosed:
      self = .disclosed
    case .undisclosed:
...
extension Array where Element == TimelineItemProxy {
  func firstEventTimelineItemUsingStableID(_ id: TimelineItemIdentifier) -> EventTimelineItem? {
    for item in self {
      if case .event(let eventTimelineItem) = item {
  ...
  func firstEventTimelineItemUsingEventOrTransactionID(
    _ eventOrTransactionID: TimelineItemIdentifier.EventOrTransactionID
  ) -> EventTimelineItem? {
    for item in self {
      if case .event(let eventTimelineItem) = item,
        case .event(_, let identifier) = eventTimelineItem.id,

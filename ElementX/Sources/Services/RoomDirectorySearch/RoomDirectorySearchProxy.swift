...
final class RoomDirectorySearchProxy: RoomDirectorySearchProxyProtocol {
  ...
  private let serialDispatchQueue = DispatchQueue(
    label: "io.element.elementx.room_directory_search_proxy", qos: .default)
  ...
  init(
    roomDirectorySearch: RoomDirectorySearchProtocol,
  ...
  private func processDiff(
    _ diff: RoomDirectorySearchEntryUpdate, on currentItems: [RoomDirectorySearchResult]
  ...
  private func buildDiff(
    from diff: RoomDirectorySearchEntryUpdate, on currentItems: [RoomDirectorySearchResult]
  ) -> CollectionDifference<RoomDirectorySearchResult>? {
    ...
    case .reset(let values):
      ...
      for (index, value) in values.enumerated() {
        changes.append(
          .insert(offset: index, element: buildResultForRoomDescription(value), associatedWith: nil)
        )
      }
  ...
  private func buildResultForRoomDescription(_ value: RoomDescription) -> RoomDirectorySearchResult
  {
    RoomDirectorySearchResult(
      ...
      topic: value.topic,
      avatar: .room(
        id: value.roomId, name: value.name, avatarURL: value.avatarUrl.flatMap(URL.init(string:))),

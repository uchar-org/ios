...
final class MediaUploadPreviewScreenViewModelTests {
  ...
  func processingFailure() async throws {
    ...
    let deferredFailure = deferFailure(
      viewModel.actions, timeout: .seconds(1), message: "The screen should remain visible."
    ) { $0 == .dismiss }
    context.send(viewAction: .send)
    #expect(
      context.viewState.shouldDisableInteraction,
      "The interaction should be disabled while sending.")
    ...
    #expect(!context.viewState.shouldDisableInteraction)
    #expect(
      userIndicatorController.submitIndicatorDelayCallsCount == 2,
      "An error indicator should be shown.")
  }
  ...
  func uploadWithUnknownMaxUploadSize() async throws {
    ...
    setUpViewModel(
      urls: [imageURL], expectedCaption: nil,
      maxUploadSizeResult: .failure(.sdkError(ClientProxyMockError.generic)))
    #expect(!context.viewState.shouldDisableInteraction)
    #expect(context.alertInfo == nil)

    // When attempting to send the media.
    let deferredAlert = deferFulfillment(context.observe(\.viewState.bindings.alertInfo)) {
      $0 != nil
    }
    let deferredFailure = deferFailure(
      viewModel.actions, timeout: .seconds(1), message: "The screen should remain visible."
    ...
    #expect(
      context.viewState.shouldDisableInteraction,
      "The interaction should be disabled while retrying.")
  ...
  func multipleFiles() async throws {
    ...
    #expect(
      userIndicatorController.submitIndicatorDelayCallsCount == 1,
      "Only a loading indicator should be shown.")
  }
  ...
  func multipleFilesWithSendFailure() async throws {
    ...
    setUpViewModel(
      urls: [fileURL, imageURL, imageURL, fileURL], expectedCaption: nil,
      simulateImageSendFailures: true)
    #expect(!context.viewState.shouldDisableInteraction)
    #expect(userIndicatorController.submitIndicatorDelayCallsCount == 0)
    ...
    #expect(timelineProxy.sendFileUrlFileInfoCaptionRequestHandleCallsCount == 2)
    #expect(
      userIndicatorController.submitIndicatorDelayCallsCount == 3,
      "Error indicators for each failure should be shown.")
  }
  ...
  private func setUpViewModel(
    ...
    timelineProxy.sendAudioUrlAudioInfoCaptionRequestHandleClosure = {
      [weak self] _, _, caption, _ in
      self?.verifyCaption(caption, expectedCaption: expectedCaption)
        ?? .failure(.sdkError(TestError.unknown))
    }
    timelineProxy.sendFileUrlFileInfoCaptionRequestHandleClosure = { [weak self] _, _, caption, _ in
      self?.verifyCaption(caption, expectedCaption: expectedCaption)
        ?? .failure(.sdkError(TestError.unknown))
    }
    timelineProxy.sendImageUrlThumbnailURLImageInfoCaptionRequestHandleClosure = {
      [weak self] _, _, _, caption, _ in
    ...
    timelineProxy.sendVideoUrlThumbnailURLVideoInfoCaptionRequestHandleClosure = {
      [weak self] _, _, _, caption, _ in
      self?.verifyCaption(caption, expectedCaption: expectedCaption)
        ?? .failure(.sdkError(TestError.unknown))
    }
  ...
  private func verifyCaption(_ caption: String?, expectedCaption: String?) -> Result<
    Void, TimelineProxyError
  > {
    guard caption == expectedCaption else {
      Issue.record(
        "The sent caption '\(caption ?? "nil")' does not match the expected value '\(expectedCaption ?? "nil")'"
      )
  ...
  private func send() async throws {
    #expect(
      !context.viewState.shouldDisableInteraction,
      "Attempting to send when interaction is disabled.")

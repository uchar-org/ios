...
typealias ReportRoomScreenViewModelType = StateStoreViewModelV2<
  ReportRoomScreenViewState, ReportRoomScreenViewAction
>
...
class ReportRoomScreenViewModel: ReportRoomScreenViewModelType, ReportRoomScreenViewModelProtocol {
    ...
    case .success:
      if state.bindings.shouldLeaveRoom {
        ...
        userIndicatorController.submitIndicator(
          .init(title: L10n.dialogRoomReported, iconName: "checkmark"))
        actionsSubject.send(.dismiss(shouldLeaveRoom: false))
      }
    case .failure:
      hideLoadingIndicator()
      state.bindings.alert = .init(
        ...
        secondaryButton: .init(title: L10n.actionTryAgain) { [weak self] in
  ...
  private func leaveRoom(showLoading: Bool) async {
    ...
    case .success:
      userIndicatorController.submitIndicator(
        .init(title: L10n.dialogRoomReportedAndLeft, iconName: "checkmark"))
    ...
    case .failure:
      state.bindings.alert = .init(
        ...
        primaryButton: .init(title: L10n.actionDismiss, role: .cancel) { [weak self] in
          self?.actionsSubject.send(.dismiss(shouldLeaveRoom: false))
        },
        secondaryButton: .init(title: L10n.actionTryAgain) { [weak self] in
          Task { await self?.leaveRoom(showLoading: true) }
        })
    }
  }

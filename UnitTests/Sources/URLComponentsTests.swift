...
struct URLComponentsTests {
  ...
  func addFragmentQueryItems() throws {
    ...
    let fragmentQueryItems: [URLQueryItem] = [
      .init(name: "first", value: "1"), .init(name: "second", value: "2"),
    ]
  ...
  func appendFragmentQueryItem() throws {
    ...
    #expect(
      components.url?.absoluteString
        == "https://test.matrix.org#/random/data?first=1&mr%20in%20between=hello&second=2")
  }
  ...
  func changeFragmentQueryItemValue() throws {
    ...
    #expect(
      components.url?.absoluteString == "https://test.matrix.org#/random/data?first=last&second=2")
  }

  @Test
  func elementCallParameters() throws {
    let url = try #require(
      URL(string: "https://call.element.io/room#/callName?appPrompt=true&confineToRoom=false"))
    var components = try #require(URLComponents(url: url, resolvingAgainstBaseURL: true))
    ...
    #expect(
      components.url?.absoluteString
        == "https://call.element.io/room#/callName?skipLobby=true&appPrompt=false&confineToRoom=true"
    )
  }
}

...
struct KnockRequestsListScreenViewModelTests {
  ...
  func loadingState() async throws {
    ...
    let viewModel = KnockRequestsListScreenViewModel(
    ...
    let deferred = deferFulfillment(context.$viewState) { state in
      !state.shouldDisplayRequests && state.isKnockableRoom && state.canAccept && !state.canBan
        && !state.canDecline && state.isLoading && !state.shouldDisplayEmptyView
    }
  ...
  func emptyState() async throws {
    let roomProxyMock = JoinedRoomProxyMock(
      .init(knockRequestsState: .loaded([]), joinRule: .knock))
    let viewModel = KnockRequestsListScreenViewModel(
      roomProxy: roomProxyMock,
      mediaProvider: MediaProviderMock(),
      userIndicatorController: UserIndicatorControllerMock())
  ...
  func loadedState() async throws {
    let roomProxyMock = JoinedRoomProxyMock(
      .init(
        ...
        knockRequestsState: .loaded([
          KnockRequestProxyMock(.init(eventID: "1", userID: "@alice:matrix.org")),
    ...
    var deferred = deferFulfillment(context.$viewState) { state in
      state.shouldDisplayRequests && state.isKnockableRoom && state.canAccept && state.canBan
        && state.canDecline && !state.isLoading && !state.shouldDisplayEmptyView
        && state.displayedRequests.count == 4 && state.handledEventIDs.isEmpty
        && state.shouldDisplayAcceptAllButton
    }
    try await deferred.fulfill()

    deferred = deferFulfillment(context.$viewState) { state in
      state.shouldDisplayRequests && state.handledEventIDs == ["1"] && !state.shouldDisplayEmptyView
        && state.displayedRequests.count == 3 && state.shouldDisplayAcceptAllButton
    }
    ...
    deferred = deferFulfillment(context.$viewState) { state in
      state.shouldDisplayRequests && state.handledEventIDs == ["1", "2"]
        && !state.shouldDisplayEmptyView && state.displayedRequests.count == 2
        && state.shouldDisplayAcceptAllButton
    }
    ...
    deferred = deferFulfillment(context.$viewState) { state in
      state.shouldDisplayRequests && state.handledEventIDs == ["1", "2", "3"]
        && !state.shouldDisplayEmptyView && state.displayedRequests.count == 1
        && !state.shouldDisplayAcceptAllButton
    }
  ...
  func acceptAll() async throws {
    ...
    deferred = deferFulfillment(context.$viewState) { state in
      !state.shouldDisplayRequests && state.handledEventIDs == ["1", "2", "3", "4"]
        && !state.isLoading && state.shouldDisplayEmptyView
    }
  ...
  func loadedStateBecomesEmptyIfTheJoinRuleIsNotKnocking() async throws {
    ...
    let deferred = deferFulfillment(context.$viewState) { state in
      !state.shouldDisplayRequests && state.shouldDisplayEmptyView && !state.isLoading
  ...
  func loadedStateBecomesEmptyIfPermissionsAreRemoved() async throws {
    ...
    let deferred = deferFulfillment(context.$viewState) { state in
      !state.shouldDisplayRequests && state.shouldDisplayEmptyView && !state.canAccept
        && !state.canBan && !state.canDecline && !state.isLoading
    }
    try await deferred.fulfill()
  }
}

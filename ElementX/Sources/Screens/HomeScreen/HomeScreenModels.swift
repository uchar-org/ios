...
struct HomeScreenViewState: BindableState {
  ...
  var shouldShowEmptyFilterState: Bool {
    !bindings.isSearchFieldFocused
      && (bindings.filtersState.isFiltering || selectedSpaceFilter != nil) && visibleRooms.isEmpty
  }
...
struct HomeScreenRoom: Identifiable, Equatable {
  ...
  static func placeholder() -> HomeScreenRoom {
    HomeScreenRoom(
      ...
      type: .placeholder,
      badges: .init(
        isDotShown: false, isMentionShown: false, isMuteShown: false, callBadgeType: .none),
...
extension HomeScreenRoom {
  init(
    ...
    let isDotShown =
      ...
      case .current:
        summary.hasUnreadMessages || summary.hasUnreadMentions || summary.hasUnreadNotifications
          || summary.isMarkedUnread || isUnseenInvite
      case .hide, .show:
        (!summary.isMuted && (summary.hasUnreadNotifications || summary.hasUnreadMentions))
          || summary.isMarkedUnread || isUnseenInvite
      }
...
extension RoomSummary {
  fileprivate var homeScreenLastMessageState: HomeScreenRoom.LastMessageState? {
    if isTombstoned {
      nil
    } else {

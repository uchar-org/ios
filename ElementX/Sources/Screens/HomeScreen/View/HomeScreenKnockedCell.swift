  ...
  static var previews: some View {
    ScrollView {
      VStack(spacing: 0) {
        ...
        HomeScreenKnockedCell(
          room: .roomKnocked(alias: "#footest:somewhere.org", avatarURL: .mockMXCAvatar),
          context: makeViewModel().context)

        HomeScreenKnockedCell(
          room: .roomKnocked(alias: "#footest:somewhere.org"),
          context: makeViewModel().context
        )
...
extension HomeScreenRoom {
  ...
  fileprivate static func roomKnocked(alias: String? = nil, avatarURL: URL? = nil) -> HomeScreenRoom
  {
    let inviter = RoomMemberProxyMock()

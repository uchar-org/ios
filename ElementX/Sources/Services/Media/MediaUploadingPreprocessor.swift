...
struct MediaUploadingPreprocessor {
    ...
    private func stripLocationFromImage(at url: URL, type: UTType)
        ...
        let destination =
            CGImageDestinationCreateWithData(data as CFMutableData, type.identifier as CFString, count, nil)
    ...
    private func generateThumbnailForImage(at url: URL) throws(MediaUploadingPreprocessorError)
        ...
        return .init(url: thumbnailURL, height: thumbnail.size.height, width: thumbnail.size.width,
    ...
    private func resizeImage(withSource imageSource: CGImageSource, maxPixelSize: CGFloat, destination destinationURL: URL,
                             type: UTType) throws(MediaUploadingPreprocessorError) {
        ...
        guard
            let scaledImage = CGImageSourceCreateThumbnailAtIndex(imageSource, 0, options as NSDictionary),

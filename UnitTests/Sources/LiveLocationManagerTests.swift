...
final class LiveLocationManagerTests {
  ...
  func startLiveLocationWithNoExistingLocalSession() async throws {
    ...
    let result = await manager.startLiveLocation(
      roomID: "!room:matrix.org", duration: .seconds(300))
  ...
  func startLiveLocationWithExistingSessionStopsItFirst() async throws {
    ...
    let roomProxy = makeRoomProxy(roomID: "!room:matrix.org")
    clientProxy.roomForIdentifierClosure = { _ in .joined(roomProxy) }
    appSettings.liveLocationSharingSessionsByRoomID["!room:matrix.org"] = LiveLocationSession(
      eventID: "$old_event:matrix.org", expirationDate: Date().addingTimeInterval(300))
    ...
    let result = await manager.startLiveLocation(
      roomID: "!room:matrix.org", duration: .seconds(600))
  ...
  func startLiveLocationDoesNotStopSessionForOtherRoom() async {
    ...
    appSettings.liveLocationSharingSessionsByRoomID["!room2:matrix.org"] = LiveLocationSession(
      eventID: "$event:matrix.org", expirationDate: Date().addingTimeInterval(300))
  ...
  func startLiveLocationWhenStartShareFails() async {
    setUp()
    let roomProxy = makeRoomProxy(roomID: "!room:matrix.org")
    roomProxy.startLiveLocationShareDurationReturnValue = .failure(
      .sdkError(RoomProxyMockError.generic))
    clientProxy.roomForIdentifierClosure = { _ in .joined(roomProxy) }
  ...
  func startLiveLocationStoresTimeoutDate() async throws {
    ...
    let storedSession = try #require(
      appSettings.liveLocationSharingSessionsByRoomID["!room:matrix.org"])
  ...
  func stopLiveLocationDoesNotRemoveOtherSessions() async {
    ...
    let roomProxy = makeRoomProxy(roomID: "!room1:matrix.org")
    clientProxy.roomForIdentifierClosure = { _ in .joined(roomProxy) }
    appSettings.liveLocationSharingSessionsByRoomID["!room1:matrix.org"] = LiveLocationSession(
      eventID: "$event:matrix.org", expirationDate: Date().addingTimeInterval(300))
    appSettings.liveLocationSharingSessionsByRoomID["!room2:matrix.org"] = LiveLocationSession(
      eventID: "$event:matrix.org", expirationDate: Date().addingTimeInterval(300))
  ...
  func beaconInfoUpdateFromAnotherDeviceRemovesActiveSession() async throws {
    ...
    let deferred = deferFulfillment(appSettings.$liveLocationSharingSessionsByRoomID) {
      $0["!room:matrix.org"] == nil
    }
    beaconInfoSubject.send(
      LiveLocationOwnInfoUpdate(
        roomID: "!room:matrix.org", eventID: "$external_event:matrix.org", isLive: true))
  ...
  private func setUp(accuracyAuthorization: CLAccuracyAuthorization = .fullAccuracy) {
    ...
    manager = LiveLocationManager(
      clientProxy: clientProxy, appSettings: appSettings, locationManager: locationManagerMock)
  }
  ...
  private func simulateBeaconEcho(roomID: String, eventID: String) async throws {
    ...
    beaconInfoSubject.send(
      LiveLocationOwnInfoUpdate(roomID: roomID, eventID: eventID, isLive: true))
    try await deferred.fulfill()
  }
}

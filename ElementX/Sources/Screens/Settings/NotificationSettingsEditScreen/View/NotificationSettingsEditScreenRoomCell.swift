...
struct NotificationSettingsEditScreenRoomCell: View {
  ...
  var roomDetailsLabel: ListRowDetails<EmptyView>? {
    ...
    return .label(
      title: context.viewState.strings.string(for: mode),
      icon: EmptyView())
  }
}
...
struct NotificationSettingsEditScreenRoomCell_Previews: PreviewProvider, TestablePreview {
  static var previews: some View {
    ...
    let viewModel = NotificationSettingsEditScreenViewModel(
      chatType: .groupChat,
      userSession: userSession)

    let rooms: [NotificationSettingsEditScreenRoom] = summaryProvider.roomListPublisher.value
      .compactMap { summary -> NotificationSettingsEditScreenRoom? in
        NotificationSettingsEditScreenRoom(
          id: UUID().uuidString,
          roomId: summary.id,
          name: summary.name,

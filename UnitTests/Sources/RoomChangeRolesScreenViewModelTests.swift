...
struct RoomChangeRolesScreenViewModelTests {
  ...
  mutating func initialStateModerators() {
    ...
    #expect(
      context.viewState.membersWithRole.first { $0.id == RoomMemberProxyMock.mockModerator.userID }
        != nil)
    #expect(!context.viewState.hasChanges)
    #expect(!context.viewState.isSearching)
  }
  ...
  mutating func toggleUserOn() throws {
    ...
    let firstUser = try #require(
      context.viewState.users.first { !context.viewState.isMemberSelected($0) },
  ...
  mutating func toggleUserOff() throws {
    try toggleUserOn()
    let firstUser = try #require(
      context.viewState.membersToPromote.first,
      "There should be a regular user available to promote.")
  ...
  mutating func toggleModeratorOff() throws {
    ...
    let existingModerator = try #require(
      context.viewState.membersWithRole.first { $0.role == .moderator },
  ...
  mutating func toggleModeratorOn() throws {
    ...
    let demotedMember = try #require(
      context.viewState.membersToDemote.first,
      "There should be a member with the role before we begin.")
  ...
  mutating func saveModeratorChanges() async throws {
    ...
    #expect(
      roomProxy.updatePowerLevelsForUsersReceivedUpdates?.contains {
        $0.userID == existingModerator.id && $0.powerLevel == 0
      } == true)
    #expect(
      roomProxy.updatePowerLevelsForUsersReceivedUpdates?.contains {
        $0.userID == firstUser.id && $0.powerLevel == 50
      } == true)
  }
  ...
  mutating func savePromotedAdministrator() async throws {
    ...
    #expect(
      roomProxy.updatePowerLevelsForUsersReceivedUpdates?.contains {
        $0.userID == firstUser.id && $0.powerLevel == 100
      } == true)
  }

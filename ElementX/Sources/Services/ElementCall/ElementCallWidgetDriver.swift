...
final class ElementCallWidgetDriver: WidgetCapabilitiesProvider, ElementCallWidgetDriverProtocol {
  ...
  func start(
    ...
    voiceOnly: Bool,
    rageshakeURL: String?,
    analyticsConfiguration: ElementCallAnalyticsConfiguration?
    ...
    do {
      urlString = try await generateWebviewUrl(
        widgetSettings: widgetSettings, room: room,
        props: .init(
          clientId: clientID,
      ...
      if widgetMessage.direction == .fromWidget {
        ...
        case .mediaState:
          guard let audioEnabled = widgetMessage.data.audioEnabled,
            let videoEnabled = widgetMessage.data.videoEnabled
          else {
          ...
          actionsSubject.send(
            .mediaStateChanged(audioEnabled: audioEnabled, videoEnabled: videoEnabled))
        }
      }
    } catch {

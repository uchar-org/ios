...
extension AttributedString {
  ...
  var formattedComponents: [AttributedStringBuilderComponent] {
    ...
    for run in runs[\.blockquote, \.codeBlock] {
      ...
      let componentType: AttributedStringBuilderComponent.ComponentType =
        switch (isBlockquote, isCodeBlock) {
        case (true, _):
          .blockquote
        case (false, true):
          .codeBlock
      ...
      components.append(
        AttributedStringBuilderComponent(
          id: String(attributedString.characters),
  ...
  func replacingFontWithPresentationIntent() -> AttributedString {
    ...
    for run in newValue.runs {
      ...
      newValue[run.range].inlinePresentationIntent =
        font.fontDescriptor.symbolicTraits.contains(.traitBold) ? .stronglyEmphasized : nil

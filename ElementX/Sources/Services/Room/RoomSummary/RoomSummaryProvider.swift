...
class RoomSummaryProvider: RoomSummaryProviderProtocol {
  ...
  init(
    ...
    serialDispatchQueue = DispatchQueue(
      label: "io.element.elementx.room_summary_provider", qos: .default)
  ...
  func setRoomList(_ roomList: RoomList) {
    ...
    do {
      listUpdatesSubscriptionResult = roomList.entriesWithDynamicAdapters(
        pageSize: UInt32(roomListPageSize),
      ...
      let stateUpdatesSubscriptionResult = try roomList.loadingState(
      ...
      stateSubject.send(
        RoomSummaryProviderState(roomListState: stateUpdatesSubscriptionResult.state))
    } catch {
  ...
  private func processDiff(_ diff: RoomListEntriesUpdate, on currentItems: [RoomSummary])
    -> [RoomSummary]
  {
  ...
  private func fetchRoomDetails(from room: Room) -> (
    roomInfo: RoomInfo?, latestEvent: LatestEventValue?
  ) {
    class FetchResult {
  ...
  private func buildRoomSummary(from room: Room) -> RoomSummary {
    ...
    if let latestRoomMessage = roomDetails.latestEvent {
      ...
      case .local(let timestamp, let senderID, let profile, let content, let state):
        ...
        attributedLastMessage = eventStringBuilder.buildAttributedString(
          for: content, sender: sender, isOutgoing: true)
      ...
      case .remote(let timestamp, let senderID, let isOwn, let profile, let content):
        let sender = TimelineItemSender(senderID: senderID, senderProfile: profile)
        attributedLastMessage = eventStringBuilder.buildAttributedString(
          for: content, sender: sender, isOutgoing: isOwn)
        lastMessageDate = Date(timeIntervalSince1970: TimeInterval(timestamp / 1000))
      case .remoteInvite(let timestamp, let senderID, let profile):
        ...
        if let senderID {
          ...
          let invitedYouString = eventStringBuilder.stateEventStringBuilder.buildInvitedYouString(
            senderDisplayName)
  ...
  private func buildDiff(from diff: RoomListEntriesUpdate, on rooms: [RoomSummary])
    -> CollectionDifference<RoomSummary>?
  {
    ...
    case .reset(let values):
      ...
      for (index, value) in values.enumerated() {
        changes.append(
          .insert(offset: index, element: buildRoomSummary(from: value), associatedWith: nil))
      }

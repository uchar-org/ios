...
class ClassicAppMediaLoader: MediaLoaderProtocol {
  ...
  private func loadMedia(source: MediaSourceProxy, width: UInt? = nil, height: UInt? = nil)
    async throws -> Data
  {
    ...
    guard let httpResponse = response as? HTTPURLResponse, httpResponse.statusCode == 200 else {
      MXLog.error(
        "Unexpected response fetching ClassicAppAccount media: \(source.url?.absoluteString ?? "nil")"
      )
  ...
  private func mediaURLRequest(from mxcURL: URL, width: UInt?, height: UInt?) -> URLRequest? {
    ...
    let endpoint =
      isThumbnail ? "_matrix/client/v1/media/thumbnail" : "_matrix/client/v1/media/download"

    var components = URLComponents(
      url: classicAppAccount.homeserverURL

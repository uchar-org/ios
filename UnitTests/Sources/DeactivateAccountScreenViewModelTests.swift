...
struct DeactivateAccountScreenViewModelTests {
  ...
  init() {
    ...
    viewModel = DeactivateAccountScreenViewModel(
      clientProxy: clientProxy, userIndicatorController: UserIndicatorControllerMock())
  }
  ...
  mutating func validateDeactivate(erasingData shouldErase: Bool) async throws {
    ...
    clientProxy.deactivateAccountPasswordEraseDataClosure = {
      [weak clientProxy] password, eraseData in
      ...
      if clientProxy.deactivateAccountPasswordEraseDataCallsCount == 1 {
        ...
        #expect(
          ...
          "The password should match the user's input on the second call.")
        #expect(eraseData == shouldErase, "The erase parameter is unexpected.")
        return .success(())
      }
    }
    ...
    let deferredState = deferFulfillment(context.observe(\.viewState.bindings.alertInfo)) {
      $0 != nil
    }
    context.send(viewAction: .deactivate)
    try await deferredState.fulfill()

    let confirmationAction = try #require(
    ...
    #expect(
      clientProxy.deactivateAccountPasswordEraseDataReceivedArguments?.password == enteredPassword)
    #expect(
      clientProxy.deactivateAccountPasswordEraseDataReceivedArguments?.eraseData == shouldErase)
  }
}

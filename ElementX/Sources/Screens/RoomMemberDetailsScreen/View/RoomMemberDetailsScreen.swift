...
struct RoomMemberDetailsScreen: View {
  ...
  var body: some View {
    ...
    .alert(
      item: $context.ignoreUserAlert, actions: blockUserAlertActions, message: blockUserAlertMessage
    )
  ...
  var verificationSection: some View {
    Section {
      ListRow(
        label: .default(title: L10n.commonVerifyUser, icon: \.lock),
        kind: .button {
  ...
  private var blockUserSection: some View {
    if let memberDetails = context.viewState.memberDetails {
      let title =
        memberDetails.isIgnored
        ? L10n.screenRoomMemberDetailsUnblockUser : L10n.screenRoomMemberDetailsBlockUser
      let action: RoomMemberDetailsScreenViewAction =
        memberDetails.isIgnored ? .showUnignoreAlert : .showIgnoreAlert
      let accessibilityIdentifier =
        memberDetails.isIgnored
        ? A11yIdentifiers.roomMemberDetailsScreen.unignore
        : A11yIdentifiers.roomMemberDetailsScreen.ignore
  ...
  @ViewBuilder
  private func blockUserAlertActions(
    _ item: RoomMemberDetailsScreenViewStateBindings.IgnoreUserAlertItem
  ) -> some View {
  ...
  private func blockUserAlertMessage(
    _ item: RoomMemberDetailsScreenViewStateBindings.IgnoreUserAlertItem
  ) -> some View {
...
struct RoomMemberDetailsScreen_Previews: PreviewProvider, TestablePreview {
  ...
  static var previews: some View {
    RoomMemberDetailsScreen(context: verifiedUserViewModel.context)
      .snapshotPreferences(
        expect: verifiedUserViewModel.context.$viewState.map { state in
    ...
    RoomMemberDetailsScreen(context: verificationViolationUserViewModel.context)
      .snapshotPreferences(
        expect: verificationViolationUserViewModel.context.$viewState.map { state in
    ...
    RoomMemberDetailsScreen(context: otherUserViewModel.context)
      .snapshotPreferences(
        expect: otherUserViewModel.context.$viewState.map { state in
    ...
    RoomMemberDetailsScreen(context: accountOwnerViewModel.context)
      .snapshotPreferences(
        expect: accountOwnerViewModel.context.$viewState.map { state in
    ...
    RoomMemberDetailsScreen(context: ignoredUserViewModel.context)
      .snapshotPreferences(
        expect: ignoredUserViewModel.context.$viewState.map { state in
  ...
  static func makeViewModel(member: RoomMemberProxyMock) -> RoomMemberDetailsScreenViewModel {
    ...
    return RoomMemberDetailsScreenViewModel(

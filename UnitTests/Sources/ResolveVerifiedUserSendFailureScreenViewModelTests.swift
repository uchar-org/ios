...
struct ResolveVerifiedUserSendFailureScreenViewModelTests {
  ...
  private func makeViewModel(with failure: TimelineItemSendFailure.VerifiedUser)
  ...
  private func verifyResolving(
    viewModel: ResolveVerifiedUserSendFailureScreenViewModel, userIDs: [String],
    assertStrings: Bool = true
  ) async throws {
    ...
    while remainingUserIDs.count > 1 {
      ...
      let deferredFailure = deferFailure(viewModel.actionsPublisher, timeout: .seconds(1)) {
        $0 == .dismiss
      }
  ...
  private func verifyDisplayName(
    context: ResolveVerifiedUserSendFailureScreenViewModel.Context, from remainingUserIDs: [String]
  ) throws {
    let userID = try #require(remainingUserIDs.first, "There should be a user ID to check.")
    let displayName = try #require(
      roomProxy.membersPublisher.value.first { $0.userID == userID }?.displayName,

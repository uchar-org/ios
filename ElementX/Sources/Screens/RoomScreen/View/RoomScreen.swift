...
struct RoomScreen: View {
  ...
  init(
    context: RoomScreenViewModelType.Context,
      ...
      .topBanners(
        [
          TopBannerLayer(verticalBanners: [
            TopBannerItem(
              pinnedItemsBanner,
              isVisible: context.viewState.shouldShowPinnedEventsBanner && !isVoiceOverEnabled),
            TopBannerItem(
              liveLocationBanner,
              isVisible: context.viewState.isSharingLiveLocation && !isVoiceOverEnabled),
          ]),
      ...
      .safeAreaInset(edge: .bottom, spacing: 0) {
        VStack(spacing: 0) {
          RoomScreenFooterView(
            details: context.viewState.footerDetails,
          ...
          composer
            ...
            .environment(
              \.shouldAutomaticallyLoadImages, !timelineContext.viewState.hideTimelineMedia)
        }
      }
  ...
  private var knockRequestsBanner: some View {
    KnockRequestsBannerView(
      requests: context.viewState.displayedKnockRequests,
...
struct RoomScreen_Previews: PreviewProvider, TestablePreview {
  ...
  static var previews: some View {
    ElementNavigationStack {
      RoomScreen(
        context: viewModels.room.context,
    ...
    ElementNavigationStack {
      RoomScreen(
        context: readOnlyViewModels.room.context,
        timelineContext: readOnlyViewModels.timeline.context,
        composerToolbar: ComposerToolbar(context: composerViewModel.context))
    }
    .previewDisplayName("Read-only")
    .snapshotPreferences(
      expect: readOnlyViewModels.room.context.$viewState.map { !$0.canSendMessage })

    ElementNavigationStack {
      RoomScreen(
        context: tombstonedViewModels.room.context,

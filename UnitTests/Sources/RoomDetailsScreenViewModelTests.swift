...
struct RoomDetailsScreenViewModelTests {
  ...
  init() {
    ...
    notificationSettingsProxyMock = NotificationSettingsProxyMock(
  ...
  mutating func leaveRoomTappedWhenPublic() async throws {
    ...
    roomProxyMock = JoinedRoomProxyMock(
      .init(name: "Test", members: mockedMembers, joinRule: .public))
    viewModel = RoomDetailsScreenViewModel(
      roomProxy: roomProxyMock,
      userSession: UserSessionMock(.init()),
      analyticsService: ServiceLocator.shared.analytics,
      ...
      attributedStringBuilder: AttributedStringBuilder(mentionBuilder: MentionBuilder()),
      appSettings: ServiceLocator.shared.settings)
    let deferred = deferFulfillment(context.observe(\.viewState.bindings.leaveRoomAlertItem)) {
      $0 != nil
    }
  ...
  mutating func leaveRoomTappedWhenRoomNotPublic() async throws {
    ...
    #expect(
      context.viewState.bindings.leaveRoomAlertItem?.subtitle == L10n.leaveRoomAlertPrivateSubtitle)
  }
  ...
  mutating func initialDMDetailsState() async throws {
    ...
    roomProxyMock = JoinedRoomProxyMock(
      .init(name: "Test", isDirect: true, isEncrypted: true, members: mockedMembers))
    viewModel = RoomDetailsScreenViewModel(
      roomProxy: roomProxyMock,
      userSession: UserSessionMock(.init()),
      analyticsService: ServiceLocator.shared.analytics,
    ...
    let deferred = deferFulfillment(viewModel.context.observe(\.viewState.dmRecipientInfo)) {
      $0 != nil
    }
  ...
  mutating func ignoreSuccess() async throws {
    ...
    let deferredRecipient = deferFulfillment(viewModel.context.observe(\.viewState.dmRecipientInfo))
    { $0 != nil }
    ...
    let deferredProcessing = deferFulfillment(
      viewModel.context.observe(\.viewState.isProcessingIgnoreRequest),
  ...
  mutating func cannotEditRoom() async {
    let mockedMembers: [RoomMemberProxyMock] = [.mockMe, .mockBob, .mockAlice]
    roomProxyMock = JoinedRoomProxyMock(
      .init(name: "Test", isDirect: false, members: mockedMembers))
    viewModel = RoomDetailsScreenViewModel(
      roomProxy: roomProxyMock,
      userSession: UserSessionMock(.init()),
      analyticsService: ServiceLocator.shared.analytics,
  ...
  mutating func notificationLoadingSettingsFailure() async throws {
    notificationSettingsProxyMock.getNotificationSettingsRoomIdIsEncryptedIsOneToOneThrowableError =
    ...
    var deferred = deferFulfillment(context.observe(\.viewState.notificationSettingsState)) {
      $0.isError
    }
    ...
    deferred = deferFulfillment(context.observe(\.viewState.notificationSettingsState)) {
      $0.isError
    }

    try await deferred.fulfill()

    let expectedAlertInfo = AlertInfo(
      id: RoomDetailsScreenErrorType.alert,
  ...
  func notificationDefaultMode() async throws {
    notificationSettingsProxyMock.getNotificationSettingsRoomIdIsEncryptedIsOneToOneReturnValue =
      RoomNotificationSettingsProxyMock(with: .init(mode: .allMessages, isDefault: true))

    let deferred = deferFulfillment(context.observe(\.viewState.notificationSettingsState)) {
      $0.isLoaded
    }
  ...
  func notificationRoomNotMuted() async throws {
    notificationSettingsProxyMock.getNotificationSettingsRoomIdIsEncryptedIsOneToOneReturnValue =
      RoomNotificationSettingsProxyMock(
        with: .init(mode: .mentionsAndKeywordsOnly, isDefault: false))
  ...
  func muteTapped() async throws {
    ...
    try await confirmation("muteTapped") { confirm in
      notificationSettingsProxyMock.setNotificationModeRoomIdModeClosure = {
        [weak notificationSettingsProxyMock] _, mode in
        notificationSettingsProxyMock?
          .getNotificationSettingsRoomIdIsEncryptedIsOneToOneReturnValue =
          RoomNotificationSettingsProxyMock(with: .init(mode: mode, isDefault: false))
        confirm()
      }

      let deferred = deferFulfillment(
        context.observe(\.viewState.isProcessingMuteToggleAction),
        transitionValues: [false, true, false])
      context.send(viewAction: .processToggleMuteNotifications)
      try await deferred.fulfill()
    }
    ...
    let deferred = deferFulfillment(context.observe(\.viewState.notificationSettingsState)) {
      state in
      switch state {
      case .loaded(let settings): settings.mode == .mute
      default: false
      }
    }
  ...
  func unmuteTapped() async throws {
    ...
    try await confirmation("unmuteTapped") { confirm in
      notificationSettingsProxyMock.unmuteRoomRoomIdIsEncryptedIsOneToOneClosure = {
        [weak notificationSettingsProxyMock] _, _, _ in
        notificationSettingsProxyMock?
          .getNotificationSettingsRoomIdIsEncryptedIsOneToOneReturnValue =
          RoomNotificationSettingsProxyMock(with: .init(mode: .allMessages, isDefault: false))
        confirm()
    ...
    let deferred = deferFulfillment(context.observe(\.viewState.notificationSettingsState)) {
      state in
      switch state {
      case .loaded(let settings): settings.mode == .allMessages
      default: false
      }
    }
  ...
  mutating func knockRequestsCounter() async throws {
    ...
    roomProxyMock = JoinedRoomProxyMock(
      .init(
        name: "Test", isDirect: false, knockRequestsState: .loaded(mockedRequests), joinRule: .knock
      ))
    viewModel = RoomDetailsScreenViewModel(
      roomProxy: roomProxyMock,
      userSession: UserSessionMock(.init()),
  ...
  mutating func knockRequestsCounterIsLoading() async throws {
    ServiceLocator.shared.settings.knockingEnabled = true
    roomProxyMock = JoinedRoomProxyMock(
      .init(name: "Test", isDirect: false, knockRequestsState: .loading, joinRule: .knock))
    viewModel = RoomDetailsScreenViewModel(
      roomProxy: roomProxyMock,
      userSession: UserSessionMock(.init()),
      analyticsService: ServiceLocator.shared.analytics,
  ...
  mutating func knockRequestsCounterIsNotShownIfNoPermissions() async throws {
    ...
    let deferred = deferFulfillment(context.observe(\.viewState)) { state in
      state.knockRequestsCount == 2 && state.dmRecipientInfo == nil && !state.canSeeKnockingRequests
  ...
  mutating func knockRequestsCounterIsNotShownIfDM() async throws {
    ...
    roomProxyMock = JoinedRoomProxyMock(
      .init(
        name: "Test", isDirect: true, members: mockedMembers,
        knockRequestsState: .loaded(mockedRequests), joinRule: .knock))
    viewModel = RoomDetailsScreenViewModel(
      roomProxy: roomProxyMock,
      userSession: UserSessionMock(.init()),
      analyticsService: ServiceLocator.shared.analytics,
    ...
    let deferred = deferFulfillment(context.observe(\.viewState)) { state in
      state.knockRequestsCount == 2 && !state.canSeeKnockingRequests && state.dmRecipientInfo != nil
        && state.canInviteUsers
    }
  ...
  mutating func historySharingPillDisplayed() async throws {
    ...
    let deferredShared = deferFulfillment(
      context.observe(\.viewState),
      message: "The pill should be shown for rooms with shared history visibility"
    ) { state in

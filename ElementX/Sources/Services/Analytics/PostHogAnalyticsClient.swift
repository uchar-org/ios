...
class PostHogAnalyticsClient: AnalyticsClientProtocol {
  ...
  func start(analyticsConfiguration: AnalyticsConfiguration) {
    ...
    guard let configuration = PostHogConfig.standard(analyticsConfiguration: analyticsConfiguration)
    else { return }
    ...
    superProperties = AnalyticsEvent.SuperProperties(
      appPlatform: .EXI, cryptoSDK: .Rust, cryptoSDKVersion: nil)
    postHog?.optIn()
  }
  ...
  func capture(_ event: AnalyticsEventProtocol) {
    ...
    postHog?.capture(
      event.eventName, properties: attachSuperProperties(to: event.properties),
      userProperties: pendingUserProperties?.properties.compactMapValues { $0 })
    pendingUserProperties = nil
  }
  ...
  func screen(_ event: AnalyticsScreenProtocol) {
    guard isRunning else { return }
    postHog?.screen(
      event.screenName.rawValue, properties: attachSuperProperties(to: event.properties))
  }
  ...
  func updateUserProperties(_ userProperties: AnalyticsEvent.UserProperties) {
    ...
    self.pendingUserProperties = AnalyticsEvent.UserProperties(
      URLPreviewsEnabled: nil,
      allChatsActiveFilter: userProperties.allChatsActiveFilter
        ?? pendingUserProperties.allChatsActiveFilter,
      ftueUseCaseSelection: userProperties.ftueUseCaseSelection
        ?? pendingUserProperties.ftueUseCaseSelection,
      numFavouriteRooms: userProperties.numFavouriteRooms
        ?? pendingUserProperties.numFavouriteRooms,
  ...
  func updateSuperProperties(_ updatedProperties: AnalyticsEvent.SuperProperties) {
    ...
    superProperties = AnalyticsEvent.SuperProperties(
      appPlatform: updatedProperties.appPlatform ?? currentProperties.appPlatform,
      cryptoSDK: updatedProperties.cryptoSDK ?? currentProperties.cryptoSDK,
      cryptoSDKVersion: updatedProperties.cryptoSDKVersion ?? currentProperties.cryptoSDKVersion)
  }

...
typealias UserProfileScreenViewModelType = StateStoreViewModelV2<
  UserProfileScreenViewState, UserProfileScreenViewAction
>
...
class UserProfileScreenViewModel: UserProfileScreenViewModelType, UserProfileScreenViewModelProtocol
{
  ...
  init(
    ...
    let initialViewState = UserProfileScreenViewState(
      userID: userID,
  ...
  private func loadProfile() async {
    ...
    async let identityResult = userSession.clientProxy.userIdentity(
      for: state.userID, fallBackToServer: true)

    switch await profileResult {
    case .success(let userProfile):
      state.userProfile = userProfile
      state.permalink = (try? matrixToUserPermalink(userId: state.userID)).flatMap(
        URL.init(string:))
    ...
    if case .success(.some(let identity)) = await identityResult {
  ...
  private func openDirectChat() {
    ...
    case .success(let roomID):
      if let roomID {
        ...
        Task {
          let isUnknown =
            if case .success(let identity) = await userSession.clientProxy.userIdentity(
              for: userProfile.userID, fallBackToServer: false)
            {
              identity == nil
            } else {
              true
  ...
  private func createDirectChat() async {
    ...
    switch await userSession.clientProxy.createDirectRoom(
      with: userProfile.userID, expectedRoomName: userProfile.displayName)
    {
    case .success(let roomID):
  ...
  private func showLoadingIndicator(allowsInteraction: Bool) {
    userIndicatorController.submitIndicator(
      UserIndicator(
        ...
        type: .modal(
          progress: .indeterminate, interactiveDismissDisabled: false,

...
typealias LocationSharingScreenViewModelType = StateStoreViewModelV2<
  LocationSharingScreenViewState, LocationSharingScreenViewAction
>

class LocationSharingScreenViewModel: LocationSharingScreenViewModelType,
  LocationSharingScreenViewModelProtocol
{
  ...
  override func process(viewAction: LocationSharingScreenViewAction) {
    ...
    case .selectLocation:
      ...
      Task {
        await sendLocation(
          .init(coordinate: coordinate, uncertainty: uncertainty),
          isUserLocation: state.isSharingUserLocation)
      }
    case .userDidPan:
    ...
    liveLocationService.liveLocationsPublisher
      .sink { [weak self] liveLocationsShares in
          ...
          let geoURI = liveLocation.geoURI
        {
          ...
          context.send(
            viewAction: .setMapCenter(.init(latitude: geoURI.latitude, longitude: geoURI.longitude))
          )
        }
      }
  ...
  private func updateUserProfiles(members: [RoomMemberProxyProtocol]) {
    ...
    case .picker:
      let ownUser =
        members.first { $0.userID == roomProxy.ownUserID }.map(UserProfileProxy.init)
        ?? .init(userID: roomProxy.ownUserID)
      state.userProfiles = [ownUser.userID: ownUser]
    case .viewStatic(let location):
      let sender =
        members.first { $0.userID == location.sender.id }.map(UserProfileProxy.init)
        ?? .init(sender: location.sender)
  ...
  private func showLiveLocationFlow() {
    if liveLocationManager.hasDisplayedLiveLocationDisclaimer {
      ...
      state.bindings.alertInfo = .init(
        alertID: .liveLocationDisclaimer,
  ...
  private func showLiveLocationDurationPicker() {
    ...
    let durationButtons: [AlertInfo<LocationSharingViewAlert>.AlertButton] = durations.compactMap {
      duration in
    ...
    state.bindings.alertInfo = .init(
      alertID: .liveLocationDurationSelection,
  ...
  private func startLiveLocationSharingInRoom(duration: Duration) async {
    ...
    let result = await liveLocationManager.startLiveLocation(
      roomID: roomProxy.id,
      duration: duration)
  ...
  private func showMissingAlwaysAuthorizedAlert() {
    state.bindings.alertInfo = .init(
      alertID: .missingAlwaysAuthorization,
      primaryButton: .init(title: L10n.actionNotNow, role: .cancel, action: nil),
      secondaryButton: .init(title: L10n.commonSettings) { [weak self] in
        self?.actionsSubject.send(.openSystemSettings)
      })
  }

  private func sendLocation(_ geoURI: GeoURI, isUserLocation: Bool) async {
    guard
      case .success = await timelineController.sendLocation(
        body: geoURI.bodyMessage,
        geoURI: geoURI,
        description: nil,
        zoomLevel: 15,
  ...
  static func mock(
    ...
    let interactionMode: LocationSharingInteractionMode =
      ...
      case .viewLive, .viewLiveEmpty:
        .viewLive(
          sender: .init(id: senderID, displayName: "Me"),
          initialLiveLocationShare: LiveLocationShare(
            userID: senderID,
    ...
    let liveLocationShares: [LiveLocationShare] =
      if type == .viewLive {
        [
          LiveLocationShare(
            userID: RoomMemberProxyMock.mockMe.userID,
            geoURI: .init(latitude: 41.9027835, longitude: 12.4963655),
            timestamp: .mock,
            timeoutDate: .distantFuture),
          LiveLocationShare(
            ...
            timeoutDate: .distantFuture),
          LiveLocationShare(
            userID: RoomMemberProxyMock.mockBob.userID,
    ...
    let roomProxy = JoinedRoomProxyMock(
      .init(members: .allMembers, ownUserID: RoomMemberProxyMock.mockMe.userID))
    roomProxy.makeLiveLocationServiceReturnValue = liveLocationServiceMock

    return LocationSharingScreenViewModel(
      interactionMode: interactionMode,

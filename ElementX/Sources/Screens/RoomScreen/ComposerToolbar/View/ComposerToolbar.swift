...
struct ComposerToolbar: View {
  ...
  private var suggestionView: some View {
    CompletionSuggestionView(
      ...
      showBackgroundShadow: !context.composerExpanded
    ) { suggestion in
      ...
      if !context.composerFormattingEnabled {
        if context.viewState.isUploading {
          ProgressView()
            ...
            .scaledPadding(
              .vertical, trailingButtonVerticalPadding, relativeTo: .compound.headingLG)
          ...
          voiceMessageRecordingButton(
            mode: context.viewState.isVoiceMessageModeActivated ? .recording : .idle
          )
  ...
  private var sendButton: some View {
    SendButton(mode: context.viewState.sendButtonMode, action: sendMessage)
      ...
      .animation(
        .linear(duration: 0.1).disabledDuringTests(), value: context.viewState.sendButtonDisabled
      )
  ...
  private var messageComposer: some View {
    MessageComposer(
      plainComposerText: $context.plainComposerText,
  ...
  private var composerView: WysiwygComposerView {
    WysiwygComposerView(
      ...
      keyCommands: context.viewState.keyCommands
    ) { provider in
  ...
  private func voiceMessagePreviewComposer(
    audioPlayerState: AudioPlayerState, waveform: WaveformSource
  ) -> some View {
...
struct ComposerToolbar_Previews: PreviewProvider, TestablePreview {
  ...
  static let focusedViewModel = ComposerToolbarViewModel.mock(
    focused: true, message: "Hello, World!")
  static let editingViewModel = ComposerToolbarViewModel.mock(
    message: "Hello, Wrold!", mockMode: .editing)
  static let multiLineViewModel = ComposerToolbarViewModel.mock(
    message: "Hello, World! This is a loooong message that wraps onto multiple lines.")
  static let voiceMessageRecordingViewModel = ComposerToolbarViewModel.mock(
    mockMode: .recordVoiceMessage)
  static let voiceMessagePreviewViewModel = ComposerToolbarViewModel.mock(
    mockMode: .previewVoiceMessage(isUploading: false))
  static let voiceMessageUploadingViewModel = ComposerToolbarViewModel.mock(
    mockMode: .previewVoiceMessage(isUploading: true))
  static let replyLoadingViewModel = ComposerToolbarViewModel.mock(
    mockMode: .reply(isLoading: true))
  static let replyLoadedViewModel = ComposerToolbarViewModel.mock(
    mockMode: .reply(isLoading: false))

...
struct SecurityAndPrivacyScreen: View {
  ...
  private var roomAccessSection: some View {
    Section {
      ListRow(
        label: .default(
          title: L10n.screenSecurityAndPrivacyRoomAccessAnyoneOptionTitle,
          description: L10n.screenSecurityAndPrivacyRoomAccessAnyoneOptionDescription,
          icon: \.public),
        kind: .selection(isSelected: context.desiredSettings.accessType == .anyone) {
      ...
      if context.viewState.isSpaceMembersOptionAvailable {
        ListRow(
          label: .default(
            title: L10n.screenSecurityAndPrivacyRoomAccessSpaceMembersOptionTitle,
      ...
      } else if context.viewState.isKnockingEnabled,
        context.viewState.currentSettings.accessType != .askToJoin
      {
        askToJoinOption
      }
      ...
      ListRow(
        label: .default(
          title: L10n.screenSecurityAndPrivacyRoomAccessInviteOnlyOptionTitle,
          description: L10n.screenSecurityAndPrivacyRoomAccessInviteOnlyOptionDescription,
          icon: \.lock),
        kind: .selection(isSelected: context.desiredSettings.accessType == .inviteOnly) {
  ...
  private var askToJoinOption: some View {
    ListRow(
      label: .default(
        title: L10n.screenSecurityAndPrivacyAskToJoinOptionTitle,
        description: L10n.screenSecurityAndPrivacyAskToJoinOptionDescription,
        icon: \.userAdd),
      kind: .selection(isSelected: context.desiredSettings.accessType == .askToJoin) {
        context.desiredSettings.accessType = .askToJoin
      }
    )
  ...
  private var askToJoinWithSpaceMembersOption: some View {
    ListRow(
      label: .default(
        ...
        icon: \.userAdd),
      kind: .selection(isSelected: context.desiredSettings.accessType.isAskToJoinWithSpaceMembers) {
  ...
  private var encryptionSection: some View {
    ...
    Section {
      ListRow(
        label: .plain(title: L10n.screenSecurityAndPrivacyEncryptionToggleTitle),
        kind: .toggle(binding)
      )
  ...
  private var historySection: some View {
    Section {
      ForEach(context.viewState.availableVisibilityOptions, id: \.self) { option in
        ...
        case .invited:
          ListRow(
            label: .plain(title: L10n.screenSecurityAndPrivacyRoomHistorySinceInviteOptionTitle),
            kind: .selection(isSelected: context.desiredSettings.historyVisibility == .invited) {
              context.desiredSettings.historyVisibility = .invited
            })
        case .shared:
          ListRow(
            label: .plain(title: L10n.screenSecurityAndPrivacyRoomHistorySinceSelectingOptionTitle),
            kind: .selection(isSelected: context.desiredSettings.historyVisibility == .shared) {
              context.desiredSettings.historyVisibility = .shared
            })
        case .worldReadable:
          ListRow(
            label: .plain(title: L10n.screenSecurityAndPrivacyRoomHistoryAnyoneOptionTitle),
            kind: .selection(
              isSelected: context.desiredSettings.historyVisibility == .worldReadable
            ) { context.desiredSettings.historyVisibility = .worldReadable })
        }
      }
    } header: {
  ...
  private func addressSection(canonicalAlias: String) -> some View {
    Section {
      ListRow(
        label: .plain(title: canonicalAlias),
  ...
  private var roomDirectoryVisibilityRow: some View {
    ...
    ListRow(
      label: .plain(
        title: L10n.screenSecurityAndPrivacyRoomDirectoryVisibilityToggleTitle,
...
struct SecurityAndPrivacyScreen_Previews: PreviewProvider, TestablePreview {
  ...
  static var previews: some View {
    ...
    .snapshotPreferences(
      expect: publicViewModel.context.$viewState.map { state in
    ...
    .snapshotPreferences(
      expect: singleSpaceMembersViewModel.context.$viewState.map { state in
        state.currentSettings.isVisibileInRoomDirectory == true
      }
    )
    ...
    .snapshotPreferences(
      expect: multipleSpacesMembersViewModel.context.$viewState.map { state in
        state.currentSettings.isVisibileInRoomDirectory == true
      }
    )
    ...
    .snapshotPreferences(
      expect: askToJoinViewModel.context.$viewState.map { state in
        state.currentSettings.isVisibileInRoomDirectory == true
      }
    )
    ...
    .snapshotPreferences(
      expect: singleAskToJoinSpaceMembersViewModel.context.$viewState.map { state in
        state.currentSettings.isVisibileInRoomDirectory == true
      }
    )
    ...
    .snapshotPreferences(
      expect: multipleAskToJoinSpacesMembersViewModel.context.$viewState.map { state in
        state.currentSettings.isVisibileInRoomDirectory == true
      }
    )
    ...
    .snapshotPreferences(
      expect: publicSpaceViewModel.context.$viewState.map { state in
        state.currentSettings.isVisibileInRoomDirectory == true
      }
    )

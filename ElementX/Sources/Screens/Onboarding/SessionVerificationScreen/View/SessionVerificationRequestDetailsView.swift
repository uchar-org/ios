...
struct SessionVerificationRequestDetailsView: View {
  ...
  private var userRequestDetails: some View {
    HStack(spacing: 12) {
      LoadableAvatarImage(
        url: details.senderProfile.avatarURL,
    ...
    VStack(spacing: 24) {
      VStack(alignment: .leading, spacing: 12) {
        HStack(spacing: 16) {
          ...
          let displayName =
            isUserVerification ? details.senderProfile.displayName : details.deviceDisplayName
...
struct SessionVerificationRequestDetailsView_Previews: PreviewProvider, TestablePreview {
  static let details = SessionVerificationRequestDetails(

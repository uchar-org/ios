...
struct AdvancedSettingsScreenViewState: BindableState {
  init(
    timelineMediaVisibility: TimelineMediaVisibility, hideInviteAvatars: Bool,
    isWaitingTimelineMediaVisibility: Bool = false, isWaitingHideInviteAvatars: Bool = false,
    ...
    var footerString = AttributedString(
      L10n.screenAdvancedSettingsLiveLocationSectionFooter(linkPlaceholder))
...
struct AdvancedSettingsScreenViewStateBindings {
  ...
  subscript<Setting>(
    dynamicMember keyPath: ReferenceWritableKeyPath<AdvancedSettingsProtocol, Setting>
  ) -> Setting {

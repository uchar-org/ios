...
final class ElementCallServiceTests {
  ...
  init() {
    ...
    service = ElementCallService(
      callProvider: callProvider, timeProvider: TimeProvider(clock: testClock, now: dateProvider))
  }
  ...
  func incomingCall() async {
    ...
    if let args = callProvider.reportNewIncomingCallWithUpdateCompletionReceivedArguments {
      ...
      Issue.record(
        "Expected reportNewIncomingCallWithUpdateCompletionReceivedArguments to be captured")
    }
  }
...
private class PKPushPayloadMock: PKPushPayload {
  ...
  override init() {
    ...
    dict[ElementCallServiceNotificationKey.expirationDate.rawValue] = Date(
  ...
  func updatingExpiration(_ from: Date, lifetime: TimeInterval) -> Self {
    dict[ElementCallServiceNotificationKey.expirationDate.rawValue] = from.addingTimeInterval(
      lifetime)
    return self
  }

...
typealias RoomThreadListScreenViewModelType = StateStoreViewModelV2<
  RoomThreadListScreenViewState, RoomThreadListScreenViewAction
>

class RoomThreadListScreenViewModel: RoomThreadListScreenViewModelType,
  RoomThreadListScreenViewModelProtocol
{
  private let threadListServiceProxy: RoomThreadListServiceProxyProtocol

  private var isOldestItemVisible = false

  private let actionsSubject: PassthroughSubject<RoomThreadListScreenViewModelAction, Never> =
    .init()
  ...
  init(
    threadListServiceProxy: RoomThreadListServiceProxyProtocol, mediaProvider: MediaProviderProtocol
  ) {
  ...
  override func process(viewAction: RoomThreadListScreenViewAction) {
    ...
    case .oldestItemDidAppear:
      ...
      Task {
        await paginateIfNecessary(
          paginationState: threadListServiceProxy.paginationStatePublisher.value)
      }

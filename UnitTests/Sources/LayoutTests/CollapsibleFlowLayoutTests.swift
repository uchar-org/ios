...
struct CollapsibleFlowLayoutTests {
  ...
  func flowLayoutWithExpandAndCollapse() {
    ...
    var flowLayout = CollapsibleReactionLayout(
      itemSpacing: 5, rowSpacing: 5, rowsBeforeCollapsible: 2)

    var placedViews: [CGRect] = []
    let placedViewsCallback = { rect in
      placedViews.append(rect)
    }
    let subviews = createReactionLayoutSubviews(
      with: Array(repeating: CGSize(width: 100, height: 50), count: 6),
      placedPositionCallback: placedViewsCallback)
    let subviewsMock = LayoutSubviewsMock(subviews: subviews)
    var a: () = ()
    var size = flowLayout.sizeThatFits(
      proposal: ProposedViewSize(containerSize), subviews: subviewsMock, cache: &a)
    ...
    flowLayout.placeSubviews(
      in: CGRect(origin: .zero, size: size), proposal: ProposedViewSize(containerSize),
      subviews: subviewsMock, cache: &a)
  ...
  func flowLayoutWithExpandButtonAndAddMoreIsHidden() {
    let containerSize = CGSize(width: 250, height: 400)
    let flowLayout = CollapsibleReactionLayout(
      itemSpacing: 5, rowSpacing: 5, rowsBeforeCollapsible: 2)
    ...
    let subviews = createReactionLayoutSubviews(
      with: Array(repeating: CGSize(width: 100, height: 50), count: 3),
      placedPositionCallback: placedViewsCallback)
    let subviewsMock = LayoutSubviewsMock(subviews: subviews)
    var a: () = ()
    let size = flowLayout.sizeThatFits(
      proposal: ProposedViewSize(containerSize), subviews: subviewsMock, cache: &a)
  ...
  func heightIsCorrectGivenASmallerAddButton() {
    ...
    let subviews = createReactionLayoutSubviews(
      with: Array(repeating: CGSize(width: 100, height: 50), count: 2),
  ...
  func flowLayoutEmptyState() {
    ...
    let subviews = createReactionLayoutSubviews(
      with: [], placedPositionCallback: placedViewsCallback)
    let subviewsMock = LayoutSubviewsMock(subviews: subviews)
    var a: () = ()
    let size = flowLayout.sizeThatFits(
      proposal: ProposedViewSize(containerSize), subviews: subviewsMock, cache: &a)
  ...
  func createReactionLayoutSubviews(
    with sizes: [CGSize],
    expandCollapseSize: CGSize = CGSize(width: 100, height: 50),
    addMoreSize: CGSize = CGSize(width: 100, height: 50),
    placedPositionCallback: @escaping (CGRect) -> Void
  ) -> [LayoutSubviewMock] {
    sizes.map { size in
      LayoutSubviewMock(
        size: size,
        layoutValues: [
          String(describing: ReactionLayoutItemType.self): ReactionLayoutItem.reaction
        ],
      ...
      LayoutSubviewMock(
        ...
        layoutValues: [
          String(describing: ReactionLayoutItemType.self): ReactionLayoutItem.expandCollapse
        ],
        placedPositionCallback: placedPositionCallback),
      LayoutSubviewMock(
        size: addMoreSize,

...
final class ClassicAppManager: ClassicAppManagerProtocol {
  ...
  init?(
    classicAppGroupIdentifier: String? = InfoPlistReader.main.classicAppGroupIdentifier,
    classicAppKeychainServiceIdentifier: String? = InfoPlistReader.main
      .classicAppKeychainServiceIdentifier,
    classicAppKeychainAccessGroupIdentifier: String? = InfoPlistReader.main
      .classicAppKeychainAccessGroupIdentifier
  ) {
    guard let classicAppGroupIdentifier, let classicAppKeychainServiceIdentifier,
      let classicAppKeychainAccessGroupIdentifier
    else {
    ...
    keychain = Keychain(
      service: classicAppKeychainServiceIdentifier,
      accessGroup: classicAppKeychainAccessGroupIdentifier)
  }
  ...
  func loadAccounts() throws -> [ClassicAppAccount] {
    ...
    guard
      let url = FileManager.default.containerURL(
        forSecurityApplicationGroupIdentifier: classicAppGroupIdentifier)
    else {
    ...
    guard let accountIV = try keychain.getData(KeychainKeys.accountIV.rawValue),
      let accountAESKey = try keychain.getData(KeychainKeys.accountAESKey.rawValue)
    else {
      throw ClassicAppManagerError.missingAccountKeys
    }

    guard let cryptoStorePassphrase = try keychain.getData(KeychainKeys.cryptoSDKStoreKey.rawValue)
    else {
      throw ClassicAppManagerError.missingCryptoStorePassphrase
    }

    let accountManager = ClassicAppAccountManager(
      cacheFolder: url,
  ...
  func availableSecrets(for account: ClassicAppAccount) async throws
    -> ClassicAppAccount.AvailableSecrets
  {
    switch try await databaseContainsSecretsBundle(
      databasePath: account.cryptoStoreURL.path(percentEncoded: false),
  ...
  func secretsBundle(for account: ClassicAppAccount) async throws -> SecretsBundleWithUserId {
    ...
    return try await SecretsBundleWithUserId.fromDatabase(
      databasePath: account.cryptoStoreURL.path(percentEncoded: false),
      passphrase: account.cryptoStorePassphrase,

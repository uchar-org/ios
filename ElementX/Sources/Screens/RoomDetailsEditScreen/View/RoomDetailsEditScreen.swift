...
struct RoomDetailsEditScreen: View {
  ...
  private var avatar: some View {
    Button {
      ...
      OverridableAvatarImage(
        overrideURL: context.viewState.localMedia?.thumbnailURL,
  ...
  private var nameSection: some View {
    Section {
      if context.viewState.canEditName {
        ListRow(
          label: .plain(title: L10n.commonRoomNamePlaceholder),
  ...
  private var topicSection: some View {
    Section {
      if context.viewState.canEditTopic {
        ListRow(
          label: .plain(
            title: isSpace ? L10n.commonSpaceTopicPlaceholder : L10n.commonTopicPlaceholder),
...
struct RoomDetailsEditScreen_Previews: PreviewProvider, TestablePreview {
  ...
  static var previews: some View {
    ...
    .snapshotPreferences(
      expect: editableViewModel.context.$viewState.map { $0.canEditTopic == true }
    )
    ...
    .snapshotPreferences(
      expect: editableSpaceViewModel.context.$viewState.map { $0.canEditTopic == true }
    )
  ...
  static func makeViewModel(readOnly: Bool, isSpace: Bool = false) -> RoomDetailsEditScreenViewModel
  {
    ...
    return RoomDetailsEditScreenViewModel(
      roomProxy: roomProxy,

...
struct RoomMembersListMemberCell_Previews: PreviewProvider, TestablePreview {
  static let members: [RoomMemberListScreenEntry] = [
    ...
    .init(
      member: .init(
        withProxy: RoomMemberProxyMock(
          with: .init(
            userID: "@nodisplayname:matrix.org",
            membership: .join))),
      verificationState: .notVerified),
    .init(
      member: .init(
  ...
  static let bannedMembers: [RoomMemberListScreenEntry] = [
    ...
    .init(
      member: .init(
        withProxy: RoomMemberProxyMock(
          with: .init(
            userID: "@badavatar:matrix.org",
            avatarURL: .mockMXCUserAvatar,
            membership: .ban))),
      verificationState: .verificationViolation),
  ]

  static let viewModel = RoomMembersListScreenViewModel(
    ...
    VStack(spacing: 0) {
      Section("Invited/Joined") {
        ForEach(members, id: \.member.id) { entry in
          RoomMembersListScreenMemberCell(
            listEntry: entry, isLast: members.last == entry, context: viewModel.context)
        }
      }
      ...
      Section("Banned") {
        ForEach(bannedMembers, id: \.member.id) { entry in
          RoomMembersListScreenMemberCell(
            listEntry: entry, isLast: bannedMembers.last == entry, context: viewModel.context)
        }
      }
    }
  }

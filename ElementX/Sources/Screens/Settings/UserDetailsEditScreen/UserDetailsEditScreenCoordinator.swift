...
final class UserDetailsEditScreenCoordinator: CoordinatorProtocol {
  ...
  private let actionsSubject: PassthroughSubject<RoomDetailsEditScreenCoordinatorAction, Never> =
    .init()
  ...
  init(parameters: UserDetailsEditScreenCoordinatorParameters) {
    ...
    viewModel = UserDetailsEditScreenViewModel(
    ...
    viewModel.actions
      .sink { [weak self] action in
        ...
        case .displayFilePicker:
          displayMediaPickerWithMode(
            .init(source: .documents(types: [.image]), selectionType: .single))
        }
      }

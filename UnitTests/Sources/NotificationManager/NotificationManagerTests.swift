...
final class NotificationManagerTests {
  ...
  init() {
    ...
    notificationManager = NotificationManager(
      notificationCenter: notificationCenter, appSettings: appSettings)
  ...
  func whenRegistered_pusherIsCalledWithCorrectValues() async throws {
    ...
    let defaultPayload = APNSPayload(
      aps: APSInfo(
        mutableContent: 1,
        alert: APSAlert(
          locKey: "Notification",
  ...
  func whenStart_notificationCategoriesAreSet() {
    let replyAction = UNTextInputNotificationAction(
      identifier: NotificationConstants.Action.inlineReply,
      title: L10n.actionQuickReply,
      options: [])
    let messageCategory = UNNotificationCategory(
      identifier: NotificationConstants.Category.message,
      actions: [replyAction],
      intentIdentifiers: [],
      options: [])

    let inviteCategory = UNNotificationCategory(
      identifier: NotificationConstants.Category.invite,
      actions: [],
      intentIdentifiers: [],
      options: [])
    #expect(
      notificationCenter.setNotificationCategoriesReceivedCategories == [
  ...
  func whenStart_requestAuthorizationCalledWithCorrectParams() async {
    ...
    #expect(
      notificationCenter.requestAuthorizationOptionsReceivedOptions == [.alert, .sound, .badge])
  }
  ...
  func whenStartAndAuthorizationGranted_delegateCalled() async {
    ...
    await waitForConfirmation(
      "registerForRemoteNotifications delegate function should be called", timeout: .seconds(10)
    ) { confirm in
  ...
  func whenWillPresentNotificationsDelegateNotSet_CorrectPresentationOptionsReturned() async throws
  {
    ...
    let options = await notificationManager.userNotificationCenter(
  ...
  func whenNotificationCenterReceivedResponseInLineReply_delegateIsCalled() async throws {
    ...
    notificationManager.delegate = self
    let response = try UNTextInputNotificationResponse.with(
      userInfo: [AnyHashable: Any](), actionIdentifier: NotificationConstants.Action.inlineReply)
    await notificationManager.userNotificationCenter(
      UNUserNotificationCenter.current(), didReceive: response)
  ...
  func whenNotificationCenterReceivedResponseWithActionIdentifier_delegateIsCalled() async throws {
    ...
    notificationManager.delegate = self
    let response = try UNTextInputNotificationResponse.with(
      userInfo: [AnyHashable: Any](), actionIdentifier: UNNotificationDefaultActionIdentifier)
    await notificationManager.userNotificationCenter(
      UNUserNotificationCenter.current(), didReceive: response)
...
extension NotificationManagerTests: @MainActor NotificationManagerDelegate {
  ...
  func handleInlineReply(
    _ service: ElementX.NotificationManagerProtocol, content: UNNotificationContent,
    replyText: String
  ) async {

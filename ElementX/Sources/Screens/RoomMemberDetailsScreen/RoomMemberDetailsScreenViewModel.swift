...
typealias RoomMemberDetailsScreenViewModelType = StateStoreViewModel<
  RoomMemberDetailsScreenViewState, RoomMemberDetailsScreenViewAction
>

class RoomMemberDetailsScreenViewModel: RoomMemberDetailsScreenViewModelType,
  RoomMemberDetailsScreenViewModelProtocol
{
  ...
  private var actionsSubject: PassthroughSubject<RoomMemberDetailsScreenViewModelAction, Never> =
    .init()
  ...
  private func loadMember() async {
    ...
    if case .success(.some(let identity)) = await userSession.clientProxy.userIdentity(
      for: state.userID, fallBackToServer: true)
    {
  ...
  private func displayFullScreenAvatar(_ url: URL) async {
      ...
      case .success(let file) = await userSession.mediaProvider.loadFileFromSource(mediaSource)
    {
      state.bindings.mediaPreviewItem = MediaPreviewItem(
        file: file, title: roomMemberProxy.displayName)
    }
  }
  ...
  private func openDirectChat() {
    ...
    userIndicatorController.submitIndicator(
      UserIndicator(
        id: loadingIndicatorIdentifier,
        type: .modal(
          progress: .indeterminate, interactiveDismissDisabled: true, allowsInteraction: false),
    ...
    case .success(let roomID):
      if let roomID {
        ...
        Task {
          let identity = await self.userSession.clientProxy.userIdentity(
            for: roomMemberProxy.userID, fallBackToServer: false)
          let user: UserProfileProxy = .init(
            userID: roomMemberProxy.userID, displayName: roomMemberProxy.displayName,
            avatarURL: roomMemberProxy.avatarURL)
          let isUnknown =
            if case .success(let identity) = identity {
              identity == nil
            } else {
              true
            }
        ...
        state.bindings.inviteConfirmationUser = .init(
          user: .init(
            userID: roomMemberProxy.userID, displayName: roomMemberProxy.displayName,
            avatarURL: roomMemberProxy.avatarURL), isUnknown: false)
      }
    case .failure:
  ...
  private func createDirectChat() async {
    ...
    switch await userSession.clientProxy.createDirectRoom(
      with: roomMemberProxy.userID, expectedRoomName: roomMemberProxy.displayName)
    {
    case .success(let roomID):

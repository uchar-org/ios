...
struct TypingIndicatorView: View {
  ...
  var content: some View {
    ...
    case 1:
      ...
      Text(firstMember).bold() + Text(L10n.screenRoomTypingNotificationSingularIos)
    case 2:
      let firstMember = typingMembers.members[0]
    ...
    case 3...:
      ...
      Text(
        L10n.tr(
          "Localizable", "screen_room_typing_many_members_first_component_ios", firstMember,
          lastMember)
      ).bold()
        + Text(L10n.tr("Localizable", "common_many_members", typingMembers.members.count - 2))
        .bold() + Text(L10n.screenRoomTypingNotificationPluralIos)
    default:
      if didShowTextOnce {
...
struct TypingIndicatorView_Previews: PreviewProvider, TestablePreview {
  static var previews: some View {
    VStack(alignment: .leading, spacing: 16.0) {
      ...
      TypingIndicatorView(
        typingMembers: TypingMembersObservableObject(members: ["Alice", "Bob", "Charlie"]))
      TypingIndicatorView(
        typingMembers: TypingMembersObservableObject(members: ["Alice", "Bob", "Charlie", "Dan"]))
      TypingIndicatorView(
        typingMembers: TypingMembersObservableObject(members: [
          "Alice", "Bob", "Charlie", "Dan", "Frannie",
        ]))
      TypingIndicatorView(
        typingMembers: TypingMembersObservableObject(members: [
          "Alice with her very long display name", "Bob", "Charlie", "Dan", "Frannie",
        ]))
    }
    .frame(width: 300)
  }

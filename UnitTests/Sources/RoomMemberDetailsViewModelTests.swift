...
struct RoomMemberDetailsViewModelTests {
  ...
  mutating func ignoreSuccess() async throws {
    ...
    let memberDetails = try #require(
      context.viewState.memberDetails,
  ...
  mutating func inviteConfirmationFetchesIdentity() async throws {
    ...
    let deferred = deferFulfillment(
      viewModel.context.$viewState.compactMap(\.bindings.inviteConfirmationUser)
    ) {
      !$0.isUnknown
    }
  ...
  private mutating func setup(
    roomMemberProxyMock: RoomMemberProxyMock, clientProxy: ClientProxyMock? = nil
  ) {
    ...
    let userSession =
      clientProxy != nil
      ? UserSessionMock(.init(clientProxy: clientProxy!)) : UserSessionMock(.init())

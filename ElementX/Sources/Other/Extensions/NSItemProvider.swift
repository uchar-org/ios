...
extension NSItemProvider {
  ...
  func storeData(withinAppGroupContainer: Bool = false) async -> URL? {
    ...
    if contentType.type.identifier == UTType.image.identifier {
      return await generateURLForUIImage(
        contentType, withinAppGroupContainer: withinAppGroupContainer)
    } else {
      return await generateURLForGenericData(
        contentType, withinAppGroupContainer: withinAppGroupContainer)
    }
  }

  private func generateURLForUIImage(
    _ contentType: PreferredContentType, withinAppGroupContainer: Bool
  ) async -> URL? {
    guard
      let uiImage = try? await loadItem(forTypeIdentifier: contentType.type.identifier) as? UIImage
    else {
    ...
    let filename =
      if let suggestedName = suggestedName as NSString?,
        ...
        let filename = (suggestedName.deletingPathExtension as NSString).appendingPathExtension(
    ...
    do {
      return try FileManager.default.writeDataToTemporaryDirectory(
        data: pngData,
        fileName: filename,
  ...
  private func generateURLForGenericData(
    _ contentType: PreferredContentType, withinAppGroupContainer: Bool
  ) async -> URL? {
    ...
    let shareData: Data? = await withCheckedContinuation { continuation in
      _ = loadDataRepresentation(for: contentType.type) { data, error in
        if let error {
          MXLog.error(
            "Failed processing NSItemProvider: \(providerDescription) with error: \(error)")
  ...
  var preferredContentType: PreferredContentType? {
    ...
    guard let preferredContentType = supportedContentTypes.first,
      let fileExtension = preferredFileExtension(for: preferredContentType)
    else {
      return nil
    }
  ...
  private func isMimeTypeSupported(_ mimeType: String?) -> Bool {
    ...
    return mimeType.hasPrefix("application/") || mimeType.hasPrefix("audio/")
      || mimeType.hasPrefix("image/") || mimeType.hasPrefix("video/")
  }
}

extension NSString {
  fileprivate var hasPathExtension: Bool {

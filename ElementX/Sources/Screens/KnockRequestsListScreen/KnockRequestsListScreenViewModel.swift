...
typealias KnockRequestsListScreenViewModelType = StateStoreViewModel<
  KnockRequestsListScreenViewState, KnockRequestsListScreenViewAction
>

class KnockRequestsListScreenViewModel: KnockRequestsListScreenViewModelType,
  KnockRequestsListScreenViewModelProtocol
{
  private let roomProxy: JoinedRoomProxyProtocol
  private let userIndicatorController: UserIndicatorControllerProtocol

  private let actionsSubject: PassthroughSubject<KnockRequestsListScreenViewModelAction, Never> =
    .init()
  ...
  override func process(viewAction: KnockRequestsListScreenViewAction) {
    ...
    case .declineRequest(let eventID):
      ...
      state.bindings.alertInfo = .init(
        ...
        primaryButton: .init(
          title: L10n.screenKnockRequestsListDeclineAlertConfirmButtonTitle,
          role: .destructive
        ) { [weak self] in self?.decline(request: request) },
        secondaryButton: .init(title: L10n.actionCancel, role: .cancel, action: nil))
    case .ban(let eventID):
      ...
      state.bindings.alertInfo = .init(
        ...
        primaryButton: .init(
          title: L10n.screenKnockRequestsListBanAlertConfirmButtonTitle,
          role: .destructive
        ) { [weak self] in self?.declineAndBan(request: request) },
        secondaryButton: .init(title: L10n.actionCancel, role: .cancel, action: nil))
    }
  }
    ...
    Task {
      ...
      case .failure:
        ...
        state.bindings.alertInfo = .init(
          ...
          primaryButton: .init(title: L10n.actionYesTryAgain) { [weak self] in
    ...
    Task {
      ...
      case .failure:
        ...
        state.bindings.alertInfo = .init(
          ...
          message: L10n.screenKnockRequestsListDeclineFailedAlertDescription,
          primaryButton: .init(title: L10n.actionYesTryAgain) { [weak self] in
            self?.declineAndBan(request: request)
          },
          secondaryButton: .init(title: L10n.actionCancel, role: .cancel, action: nil))
      }
    }

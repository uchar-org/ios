...
typealias QRCodeLoginScreenViewModelType = StateStoreViewModel<
  QRCodeLoginScreenViewState, QRCodeLoginScreenViewAction
>
...
class QRCodeLoginScreenViewModel: QRCodeLoginScreenViewModelType, QRCodeLoginScreenViewModelProtocol
{
  ...
  init(
    ...
    super.init(
      initialViewState: .init(
        state: initialViewState, mode: mode, canSignInManually: canSignInManually))
    setupSubscriptions()
  ...
  private func listenToDisplayQRProgress(
    progressPublisher: LinkNewDeviceService.LinkMobileProgressPublisher
  ) {
    state.bindings.qrResult = nil
  ...
  private func handleError(_ error: QRCodeLoginError) {
    ...
    case .providerNotAllowed(let scannedProvider, let allowedProviders):
      state.state = .scan(
        .scanFailed(
          .notAllowed(scannedProvider: scannedProvider, allowedProviders: allowedProviders)))
  ...
  fileprivate init(
    state: QRCodeLoginState, mode: QRCodeLoginScreenMode, canSignInManually: Bool,

          ...
          case .nextPreview:
            Task { [weak self] in
              ...
              do {
                ...
                try client.send(
                  .accessibilityAudit(.nextPreviewReady(name: previewsWrapper.previewName)))
              } catch {
...
@Observable final class PreviewsWrapper {
  ...
  func updateCurrentIndex() async {
    ...
    let preferenceReadingView = newPreview.content.onPreferenceChange(
      SnapshotFulfillmentPreferenceKey.self
    ) { fulfillmentSource = $0?.source }

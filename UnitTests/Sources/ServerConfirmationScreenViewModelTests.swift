...
final class ServerConfirmationScreenViewModelTests {
  ...
  func confirmLoginAfterConfiguration() async throws {
    ...
    setupViewModel(authenticationFlow: .login)
    guard
      case .success = await service.configure(for: viewModel.state.homeserverAddress, flow: .login)
    else {
  ...
  func confirmRegisterAfterConfiguration() async throws {
    ...
    setupViewModel(authenticationFlow: .register)
    guard
      case .success = await service.configure(
        for: viewModel.state.homeserverAddress, flow: .register)
    else {
      Issue.record("The configuration should succeed.")
      return
    }
  ...
  func registrationNotSupportedAlert() async throws {
    ...
    setupViewModel(
      authenticationFlow: .register, supportsOIDC: false, supportsOIDCCreatePrompt: false)
    #expect(service.homeserver.value.loginMode == .unknown)
    #expect(
      clientFactory
        .makeClientHomeserverAddressSessionDirectoriesPassphraseClientSessionDelegateAppSettingsAppHooksCallsCount
  ...
  func loginNotSupportedAlert() async throws {
    ...
    setupViewModel(
      authenticationFlow: .login, supportsOIDC: false, supportsOIDCCreatePrompt: false,
  ...
  func elementProRequired() async throws {
    ...
    setupViewModel(
      authenticationFlow: .login, supportsOIDC: false, supportsOIDCCreatePrompt: false,
      supportsPasswordLogin: false, requiresElementPro: true)
    #expect(service.homeserver.value.loginMode == .unknown)
    #expect(
      clientFactory
        .makeClientHomeserverAddressSessionDirectoriesPassphraseClientSessionDelegateAppSettingsAppHooksCallsCount
  ...
  func pickerAfterConfiguration() async throws {
    ...
    setupViewModel(authenticationFlow: .login, restrictedFlow: true)
    guard
      case .success = await service.configure(for: appSettings.accountProviders[0], flow: .login)
    else {
      Issue.record("The configuration should succeed.")
      return
    }
  ...
  private func setupViewModel(
    ...
    if restrictedFlow {
      appSettings.override(
        accountProviders: ["matrix.org", "beta.matrix.org"],
    ...
    let configuration = AuthenticationClientFactoryMock.Configuration(homeserverClients: [
      "matrix.org": client
    ])
    ...
    viewModel = ServerConfirmationScreenViewModel(
...
extension ServerConfirmationScreenViewState {
  fileprivate var homeserverAddress: String {
    switch mode {
...
extension ServerConfirmationScreenViewModelAction {
  fileprivate var isContinueWithOIDC: Bool {
    switch self {
  ...
  fileprivate var isContinueWithPassword: Bool {
    switch self {

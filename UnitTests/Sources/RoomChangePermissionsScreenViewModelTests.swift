...
struct RoomChangePermissionsScreenViewModelTests {
  ...
  mutating func changeSetting() throws {
    ...
    let index = try #require(
      context.settings[.roomDetails]?.firstIndex { $0.keyPath == \.roomAvatar },
    ...
    let setting = RoomPermissionsSetting(
      title: "",
  ...
  mutating func settingsCantBeChanged() throws {
    ...
    index = try #require(
      context.settings[.messagesAndContent]?.firstIndex { $0.keyPath == \.eventsDefault },
  ...
  mutating func save() async throws {
    ...
    let index = try #require(
      context.settings[.roomDetails]?.firstIndex { $0.keyPath == \.roomAvatar },
      "There should be a setting for the room avatar.")
    context.settings[.roomDetails]?[index] = RoomPermissionsSetting(
      title: "",
      value: RoomRole.user.powerLevelValue,
      ownPowerLevel: RoomRole.creator.powerLevel,
      keyPath: \.roomAvatar)
    ...
    #expect(
      context.settings[.roomDetails]?[index].availableValues.map(\.tag)
        == RoomPermissionsSetting.allValues.map(\.tag))
    #expect(context.viewState.hasChanges)
    ...
    #expect(
      roomProxy.applyPowerLevelChangesReceivedChanges == .init(roomAvatar: 0),
  ...
  private mutating func setup(
    isSpace: Bool, ownPowerLevel: RoomPowerLevel = RoomRole.creator.powerLevel
  ) {
    roomProxy = JoinedRoomProxyMock(.init(isSpace: isSpace))
    viewModel = RoomChangePermissionsScreenViewModel(
      currentPermissions: .init(powerLevels: .mock),

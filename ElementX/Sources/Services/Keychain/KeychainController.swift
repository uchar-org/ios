...
final class KeychainController: KeychainControllerProtocol {
  ...
  init(service: KeychainControllerService, accessGroup: String) {
    restorationTokenKeychain = Keychain(
      service: service.restorationTokenID, accessGroup: accessGroup)
  ...
  func retrieveSessionFromKeychain(userId: String) throws -> Session {
    ...
    guard let session = restorationTokenForUsername(userId)?.session else {
      throw ClientError.Generic(
        msg: "Failed to find RestorationToken in the Keychain.", details: nil)
    }
    return session
  }
